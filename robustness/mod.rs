...
impl RetryExecutor {
        ...
        E: std::error::Error + Send + Sync + 'static,
    {
        ...
        Err(SongbirdError::Operation(
            format!("Operation failed after {} retries", self.config.max_retries)
        ))
    }
...
impl RobustnessManager {
        ...
        E: std::error::Error + Send + Sync + 'static,
    {
        ...
        if !self.rate_limiter.is_allowed().await {
            return Err(SongbirdError::RateLimit("Rate limit exceeded".to_string()));
        }
        
        // Check circuit breaker
        let mut circuit_breaker = self.get_circuit_breaker(service_id).await;
        if !circuit_breaker.can_execute() {
            return Err(SongbirdError::CircuitBreaker(
                format!("Circuit breaker open for service: {}", service_id)
            ));
        }

    ...
    pub async fn export_results_json(&self) -> Result<String> {
        if let Some(ref results) = *self.results.read().await {
            serde_json::to_string_pretty(results).map_err(|e| {
                crate::errors::SongbirdError::ExecutionFailed {
                    message: format!("Failed to serialize benchmark results: {e}"),
                }
            })
        } else {
            Err(crate::errors::SongbirdError::ExecutionFailed {
                message: "No benchmark results available".to_string(),
            })
        }
    }
}

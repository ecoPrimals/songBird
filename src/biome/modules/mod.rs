//! Biome Modules
//!
//! This module organizes the biome functionality into focused sub-modules:
...
pub use lifecycle::{
    HealthMonitoringConfig, ServiceLifecycleManager, ServiceRegistration, ServiceRegistry,
...
impl BiomeCoordinator {
    ...
    pub fn new(config: BiomeCoordinatorConfig) -> Self {
        let orchestrator_manager = OrchestratorManager::new(config.orchestrator_config.clone());
    ...
    pub async fn deploy_biome(
        ...
        let deployment_result = BiomeDeploymentResult {
    ...
    pub fn get_deployment_status(&self, team_id: &str) -> Option<BiomeDeploymentInfo> {
    ...
    pub fn list_deployments(&self) -> Vec<BiomeDeploymentInfo> {
        // Deployments are tracked through orchestrator manager
        ...
        for orchestrator_id in orchestrator_ids {
            if let Some(orchestrator) = self.orchestrator_manager.get_orchestrator(&orchestrator_id)
            {
                ...
                deployments.push(BiomeDeploymentInfo {
                    team_id: orchestrator_id.clone(), // Using orchestrator ID as team ID
    ...
    pub fn get_orchestrator_count(&self) -> usize {
        self.orchestrator_manager.orchestrator_count()
    }

    /// Get service registry
    pub fn get_service_registry(&self) -> &ServiceRegistry {
    ...
    pub async fn perform_cleanup(
        ...
        let result = CleanupResult {
            cleaned_volumes: 0, // Storage handled by universal primal adapter
            cleaned_orchestrators,
            cleaned_services,
        };
    ...
    pub fn get_health_status(&self) -> BiomeCoordinatorHealth {
        BiomeCoordinatorHealth {
            orchestrator_count: self.get_orchestrator_count(),
            service_count: self.lifecycle_manager.list_services().len(),
...
pub fn create_default_biome_coordinator() -> BiomeCoordinator {
    BiomeCoordinator::new(BiomeCoordinatorConfig::default())
}

// Helper function for creating biome coordinator with custom config
pub fn create_biome_coordinator(config: BiomeCoordinatorConfig) -> BiomeCoordinator {
...
mod tests {
    ...
    async fn test_cleanup_operations() {
        let mut coordinator = create_default_biome_coordinator();
        let result = coordinator.perform_cleanup().await;

        assert!(result.is_ok());
        let cleanup_result = result.expect("Failed to perform cleanup in test");

...
pub async fn handle_gaming_command(args: GamingArgs) -> Result<()> {
    match args.command {
        GamingCommand::Scan { interface, duration, continuous, filter } => {
            scan_for_games(interface, duration, continuous, filter).await
        }
        GamingCommand::Host { auto, name, encrypt, private } => {
        ...
        GamingCommand::Browse => execute_browse().await.map_err(|e| SongbirdError::Gaming { 
            message: format!("Browse failed: {}", e), protocol: None,
        }),
        GamingCommand::Diagnostics => execute_diagnostics().await.map_err(|e| SongbirdError::Gaming { 
            message: format!("Diagnostics failed: {}", e), protocol: None,
        }),
        GamingCommand::Configure => execute_configure().await.map_err(|e| SongbirdError::Gaming { 
            message: format!("Configure failed: {}", e), protocol: None,
        }),
        GamingCommand::OneTouch { name, family_safe, parental_controls, guests } => {
            execute_one_touch(name, family_safe, parental_controls, guests)
                .await.map_err(|e| SongbirdError::Gaming { 
                    message: format!("One-touch setup failed: {}", e), protocol: None,
                })
        }
        GamingCommand::ZeroTouch { endpoint, token } => {
            execute_zero_touch(endpoint, token).await.map_err(|e| SongbirdError::Gaming { 
                message: format!("Zero-touch setup failed: {}", e), protocol: None,
            })
        }
        GamingCommand::FamilySafe { family_name } => {
            execute_family_safe(family_name).await.map_err(|e| SongbirdError::Gaming { 
                message: format!("Family-safe setup failed: {}", e), protocol: None,
            })
        }
        GamingCommand::QuickStart { auto_detect, game, family_safe, name } => {
            execute_quick_start(auto_detect, game, family_safe, name)
                .await.map_err(|e| SongbirdError::Gaming { 
                    message: format!("Quick start failed: {}", e), protocol: None,
                })
        }
    }
}
...
pub async fn execute_gaming(command: GamingCommand) -> CliResult<()> {
    ...
    handle_gaming_command(args).await.map_err(|e| crate::cli::CliError::ExecutionError(e.to_string()))
}

...
pub async fn handle_basic_iot_command(command: BasicIoTCommands) -> Result<()> {
    match command {
        BasicIoTCommands::Discover {
            device_type,
            detailed,
        } => discover_devices(&device_type, detailed).await,
        BasicIoTCommands::Connect {
            address,
            device_type,
            name,
        ...
        BasicIoTCommands::Command { device, action } => send_device_command(&device, &action).await,
    }
}
...
async fn discover_devices(device_type: &str, detailed: bool) -> Result<()> {
    println!(
        "{}",
        "🔍 SongBird Universal Device Discovery"
            .bright_cyan()
            .bold()
    );
    println!("{}", "====================================".bright_cyan());
...
async fn connect_device(address: &str, device_type: &str, name: &str) -> Result<()> {
    println!(
        "{}",
        "🔗 SongBird Universal Device Connection"
            .bright_cyan()
            .bold()
    );
    println!("{}", "======================================".bright_cyan());
...
async fn list_connected_devices(device_type_filter: Option<&str>) -> Result<()> {
    ...
    for (name, device_type, address, status) in filtered_devices {
        ...
        println!(
            "   Status: {}",
            match *status {
                "Connected" | "Ready" | "Streaming" => status.green(),
                _ => status.yellow(),
            }
        );
        println!();
...
async fn send_device_command(device: &str, action: &str) -> Result<()> {

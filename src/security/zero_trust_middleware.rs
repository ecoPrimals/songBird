    ...
    fn extract_credentials(&self, headers: &HeaderMap) -> Result<Credentials> {
        if let Some(auth_header) = headers.get("authorization") {
            ...
            if auth_str.starts_with("Bearer ") {
                let token = auth_str.strip_prefix("Bearer ").ok_or_else(|| SongbirdError::Security { message: "Malformed Bearer token".to_string(), context: Some("authentication".to_string()) })?;
                return Ok(Credentials::Token(token.to_string()));
            }

            if auth_str.starts_with("Basic ") {
                let encoded = auth_str.strip_prefix("Basic ").ok_or_else(|| SongbirdError::Security { message: "Malformed Basic auth".to_string(), context: Some("authentication".to_string()) })?;

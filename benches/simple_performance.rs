...
use songbird_core::performance::zero_cost_optimizations::{LockFreeCounter, SafeStringInterner};
use std::sync::{Arc, Mutex};
use std::thread;

// ===== LOCK-FREE COUNTER VS MUTEX BENCHMARKS =====

fn bench_counter_single_thread(c: &mut Criterion) {
...
fn bench_counter_multi_thread(c: &mut Criterion) {
...
fn bench_string_operations(c: &mut Criterion) {
    ...
    let test_strings: Vec<String> = (0..1000)
        .map(|i| format!("test_string_{}", i % 100)) // Some duplicates
        .collect();
...
fn bench_arithmetic_patterns(c: &mut Criterion) {
    ...
    group.bench_function("Checked arithmetic operations", |b| {
        b.iter(|| {
            let mut accumulator = 0u64;
            for i in 0..100000 {
                // Rust's checked arithmetic - safe and fast

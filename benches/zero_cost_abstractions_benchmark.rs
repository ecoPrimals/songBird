...
use songbird_primal_sdk::{
    HighPerformancePrimalSDK, LightweightPrimalSDK, OptimizedPrimalPool, PoolStats,
    PrimalConnection, StandardPrimalSDK,
};
use songbird_types::{
    performance::{
        const_hash, ConstBuffer, PerformanceConfig, ProductionConfig, StackString, StackVec,
    },
...
fn bench_primal_connection_creation(c: &mut Criterion) {
    ...
    let endpoint = &format!(
        "https://security.primal.example.com:{}",
...
fn bench_connection_pool_performance(c: &mut Criterion) {
    ...
    for pool_size in [4, 16, 64].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("heap_pool", pool_size), pool_size, |b, &size| {
            b.iter(|| {
                let mut pool = Vec::with_capacity(size);
                for i in 0..size {
                    let conn =
                        (Uuid::new_v4(), CanonicalPrimalType::Gaming, format!("endpoint_{}", i));

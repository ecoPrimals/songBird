...
//!
//! ## Scenarios Covered:
//! - Configuration loading and validation (TOML parsing + validation)
...
use songbird_core::performance::cache::ConfigCache;
use songbird_core::robustness::circuit_breaker::CircuitBreaker;
...
"#;
    ...
    group.bench_function("TOML Parse + Deserialize", |b| {
        b.iter(|| {
            let config: Result<SongbirdConfig, _> = toml::from_str(config_toml);
            black_box(config);
        })
    });
    ...
    group.bench_function("Config Validation", |b| {
        let config: SongbirdConfig = toml::from_str(config_toml).unwrap();
        b.iter(|| {
...
fn bench_primal_discovery(c: &mut Criterion) {
    ...
    group.bench_function("Primal Registration", |b| {
        b.iter(|| {
            ...
            for i in 0..100 {
                let primal_type = format!("primal_{}", i);
        ...
        b.iter(|| {
            let handles: Vec<_> = (0..4)
                .map(|thread_id| {
                    ...
                    thread::spawn(move || {
                        for i in 0..25 {
                            let primal_type = format!("primal_{}", thread_id * 25 + i);
...
fn bench_config_cache(c: &mut Criterion) {
    ...
    group.bench_function("Cache Misses", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            for i in 0..1000 {
                let key = format!("unique_key_{}", i); // Ensure cache misses
                let result = cache.get(&key).await;
                black_box(result);
            }
        })
        ...
        b.to_async(&rt).iter(|| async {
            let handles: Vec<_> = (0..4)
                .map(|thread_id| {
                    ...
                    tokio::spawn(async move {
                        for i in 0..250 {
                            let key = format!("config_key_{}", (thread_id * 250 + i) % 100);
                            let result = cache.get(&key).await;
                            black_box(result);
                        }
                    })
...
fn bench_large_config_processing(c: &mut Criterion) {
    ...
    let generate_large_config = || {
        ...
        for i in 0..1000 {
            let primal_type = format!("service_{}", i);
            ...
            for i in 0..100 {
                match i % 5 {
                    0 => {
                        ...
                        let primal_type = format!("service_{}", i % 20);
                        let result = config.get_primal_config(&primal_type);
                        black_box(result);
                    }
                    1 => {

...
//!
//! ## Benchmarks Included: //! - Safe String Interning vs HashMap<String, usize>
...
use songbird_orchestrator: :core::performance::zero_cost_optimizations::{
    // FixedCircularBuffer, LockFreeCounter, PerformanceMeasurement, SafeBufferPool, SafeStringInterner,
};
...
fn bench_string_interning() {
    ...
    let test_strings: Vec<String> = (0..1000)
      ...
      ;
    }", i % 100)) // Some duplicates for realistic interning
        .collect();
  ...
  ;
    ...
    group.bench_function("HashMap<String, usize>", |b| {
        ...
        b.iter(|||| {
            ...
            let mut map = HashMap: :with_capacity(1000); // Pre-allocate for better performance
            let mut next_id = 0;
            for s in &test_strings { if !map.contains_key(s) {
    ...
    group.bench_function("String: :clone() baseline", |b| {
        ...
        b.iter(|||| {
            ...
            for s in &test_strings { black_box(s.clone());
...
fn bench_circular_buffers() {
    ...
    group.bench_function("FixedCircularBuffer<u32, 1024>", |b| {
        ...
        b.iter(|||| {
            ...
            for &item in &test_data { black_box(buffer.push(item));
...
fn bench_buffer_pools() {
    ...
    group.bench_function("SafeBufferPool<u8> reuse", |b||| {
        ...
        // Warmup the pool
        for _ in 0..10 { let _ = pool.get_buffer(BUFFER_SIZE);
  ;
       ...
       ;
        ...
        b.iter(|| {
            ...
            for _ in 0..1000 { let buffer = pool.get_buffer(BUFFER_SIZE);
                // Simulate some work;
        black_box(&buffer);
    ...
    group.bench_function("Vec: :with_capacity() new", |b| {
        ...
        b.iter(|||| {
            ...
            for _ in 0..1000 { let buffer: Vec<u8> = Vec::with_capacity(BUFFER_SIZE);
                black_box(&buffer);
  ;
    ...
    group.bench_function("vec![0u8; 4096] zeroed", |b| {
        ...
        b.iter(|||| {
            ...
            for _ in 0..1000 { let buffer = vec![0u8; BUFFER_SIZE];
                black_box(&buffer);
...
fn bench_memory_patterns() {
    ...
    group.bench_function("Sequential Access", |b| {
        ...
        b.iter(|||| {
            ...
            let mut sum = 0u64;
            for &value in &data { sum += value as u64;
    ...
    group.bench_function("Random Access", |b||| {
        ...
        b.iter(|| { 
            let mut sum = 0u64;
            for &idx in &indices { sum += data[idx] as u64;
  ;
...
fn bench_zero_allocation_patterns() {
    ...
    group.bench_function("Stack-allocated config processing", |b| {
        ...
        b.iter(|||| {
            ...
            for i in 0..10000 { // Simulate config processing with no heap allocation
   ...
   ;
    ...
    group.bench_function("String formatting (allocating)", |b| {
        ...
        b.iter(|||| {
            ...
            for i in 0..10000 { let formatted = format!("config_value_{  
    ...
    group.bench_function("Pure arithmetic (zero-allocation)", |b| {
        ...
        b.iter(|||| {
            ...
            for i in 0..10000 { accumulator = accumulator
...
fn bench_realistic_workloads() {
    ...
    group.bench_function("Config Update Cycle", |b||| {
        ...
        // Warmup
        for i in 0..100 { interner.intern(&format!("config_key_{  ;
       ...
       ;
        ...
        b.iter(|||| {
            ...
            for i in 0..100 { // Intern configuration keys
                let key_id = interner.intern(&format!("config_key_{  
    ...
    group.bench_function("Message Processing Pipeline", |b||| {
        ...
        b.iter(|| { 
            ...
            for i in 0..1000u64 { let pipeline = i % 4;

...
//! 
//! Target: Test actual observability API and address unused field warnings
//! Focus: MetricsCollector, ObservabilityConfig, unused fields coverage
...
mod observability_test_utils {
    ...
    pub async fn create_test_orchestrator() -> Arc<Orchestrator> {
        ...
        Arc::new(Orchestrator::new(config).await.unwrap())
    }
}
...
mod metrics_collector_tests {
    ...
    async fn test_metrics_collection_multiple_times() {
        ...
        let last_time = collector.last_collection_time().unwrap();
    ...
    #[tokio::test]
    async fn test_unused_fields_coverage() {
        let config = create_test_observability_config();
        let collector = MetricsCollector::new(config).unwrap();
        
        // Test increment methods (addresses unused field warnings)
    ...
    async fn test_metrics_snapshot_consistency() {
        ...
        let snapshot1 = collector.get_current_snapshot().await.unwrap();
        let snapshot2 = collector.get_current_metrics().await.unwrap();
    ...
    async fn test_concurrent_metrics_collection() {
        ...
        for i in 0..5 {
            ...
            let handle = tokio::spawn(async move {
                ...
                // Collect metrics
                collector_clone.collect_all_metrics().await.unwrap()
            });
            handles.push(handle);
        }

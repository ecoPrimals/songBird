...
fn test_songbird_config_security_fields() -> Result<()> {
    ...
    assert!(config.security.encryption_enabled || !config.security.encryption_enabled); // Boolean field exists
    assert!(config.security.tls_enabled || !config.security.tls_enabled); // Boolean field exists
...
async fn test_network_config_creation() -> Result<()> {
    let network_config = NetworkConfig {
        ...
        gaming_port_range: PortRange { start: 7000, end: 8000 },
        discovery_port: 8001,
        health_port: 8002,
        dashboard_port: 8003,
        ..Default::default()
    ...
    assert_eq!(network_config.bind_address, "127.0.0.1".parse::<IpAddr>().unwrap());
...
fn test_config_file_operations() -> Result<()> {
    ...
    if save_result.is_ok() {
        ...
        assert_eq!(loaded_config.network.orchestrator_port, config.network.orchestrator_port);
...
fn test_config_custom_fields() -> Result<()> {
    ...
    config.custom.insert("custom_key".to_string(), serde_json::Value::String("custom_value".to_string()));
    config.custom.insert("custom_number".to_string(), serde_json::Value::Number(42.into()));
    config.custom.insert("custom_bool".to_string(), serde_json::Value::Bool(true));
...
fn test_config_stun_servers() -> Result<()> {
    ...
    assert!(config.network.stun_servers.len() >= 0);
...
fn test_config_monitoring_settings() -> Result<()> {
    ...
    // Valid log levels
    let valid_log_levels = vec!["error", "warn", "info", "debug", "trace"];
...
fn test_config_security_settings() -> Result<()> {
    ...
    assert!(config.environment.enable_encryption || !config.environment.enable_encryption);
    assert!(config.environment.require_tls || !config.environment.require_tls);
...
fn test_config_cors_settings() -> Result<()> {
    ...
    // CORS should have default settings
    assert!(cors.enabled || !cors.enabled);
...
fn test_config_federation_settings() -> Result<()> {
    ...
    assert!(config.network.federation_endpoints.len() >= 0);
...
fn test_config_deserialization() -> Result<()> {
    ...
    assert_eq!(deserialized_config.network.orchestrator_port, config.network.orchestrator_port);
    assert_eq!(deserialized_config.environment.bind_port, config.environment.bind_port);
...
fn test_config_clone_behavior() -> Result<()> {
    ...
    assert_eq!(cloned_config.network.orchestrator_port, config.network.orchestrator_port);
    assert_eq!(cloned_config.environment.bind_port, config.environment.bind_port);
    assert_eq!(cloned_config.environment.log_level, config.environment.log_level);
...
async fn test_gaming_network_config() -> Result<()> {
    let gaming_config = GamingNetworkConfig {
        starcraft_port: 6112,
        aoe2_port: 2300,
        cnc_port_range: PortRange { start: 1234, end: 1240 },

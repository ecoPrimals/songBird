...
impl TestRunner {
            ...
            TestSuite::All => {
                ...
                for suite in suites {
                    ...
                    if !is_successful && self.config.fail_fast {
                        println!("❌ Test suite {:?} failed, stopping execution (fail-fast enabled)", suite);
                        break;
                    }
                }
            }
    ...
    fn print_summary(&self, results: &HashMap<TestSuite, TestResults>, total_time: Duration) {
        ...
        for (suite, result) in results {
            ...
            let status = if result.is_successful() { "✅" } else { "❌" };
            println!("{} {:?}: {} passed, {} failed ({:.1}% success)", 
                status, suite, result.passed, result.failed, result.success_rate());
        }

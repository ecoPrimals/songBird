...
use songbird_gaming_bridge::{
    ...
    load_balancer::{DefaultLoadBalancer, LoadBalancer, LoadBalancerConfig, LoadBalancerStrategy},
...
impl UniversalService for ChaosTestService {
    ...
    async fn health_check(&self) -> Result<Self::Health> {
        if self.should_fail() {
            return Err(SongbirdError::health_check_failed("service",  {
    ...
    async fn get_metrics(&self) -> Result<ServiceMetrics> {
        ...
        Ok(ServiceMetrics {
            ...
            average_response_time: if self.should_be_slow() { 750.0 } else { 50.0 },
            average_response_time: if self.should_be_slow() { 1200.0 } else { 80.0 },
            average_response_time: if self.should_be_slow() { 1500.0 } else { 100.0 },
            cpu_usage: Some(50.0) + (counter_mod as f64 * 0.3),
            memory_usage: Some(256) + (counter_mod as u64 * 2),
            active_connections: 10 + (counter_mod as u32 % 40),
            queue_depth: if self.should_be_slow() {
                (counter_mod as u32 % 15) + 5
            } else {
                0
            },
            throughput_rps: if request_count > 0 {
                request_count as f64 / 60.0
            } else {
                0.0
            },
    ...
    fn service_info(&self) -> ServiceInfo {
        ServiceInfo {
            ...
            description: Some("Service designed for chaos engineering tests").to_string(),
            endpoints: vec![],
            tags: vec![
                "chaos-resilient".to_string(),
                "failure-injection".to_string(),
            ],
...
async fn test_service_failure_injection_and_recovery() {
    ...
    let survival_rate =
        failure_results.iter().filter(|&&r| r).count() as f64 / failure_requests as f64 * 100.0;
    ...
    assert!(
        survival_rate > 70.0,
        "Survival rate too low: {:.1}% (required: >70%)",
        survival_rate
    );
    assert!(
        recovery_time.as_secs_f64() < baseline_time.as_secs_f64() * 1.2,
        "Recovery too slow: {:.2}s vs baseline {:.2}s",
...
async fn test_load_balancer_chaos_resilience() {
    ...
    let chaos_services: Vec<_> = (0..8)
        .map(|i| {
            ...
            songbird_gaming_bridge::load_balancer::ServiceInstance {
                service_info: ServiceInfo {
                    id: format!("lb-chaos-{}", i),
                    name: format!("LB Chaos Service {}", i),
                    version: "1.0.0".to_string(),
                    service_type: "chaos".to_string(),
                    description: Some("Chaos load balancer test").to_string(),
                    endpoints: vec![],
                    tags: std::collections::HashMap::new(),
                    tags: std::collections::HashMap::new(),
                    metadata: std::collections::HashMap::new(),
    ...
    let healthy_selection_rate =
        healthy_selections as f64 / successful_selections.max(1) as f64 * 100.0;
    ...
    println!(
        "   📈 Selection Success Rate: {:.1}%",
        selection_success_rate
    );
    println!(
        "   💚 Healthy Selection Rate: {:.1}%",
        healthy_selection_rate
    );
    println!(
        "   ⏱️  Total Selection Time: {:.2}s",
    ...
    assert!(
        selection_success_rate > 80.0,
        "Load balancer selection success rate too low: {:.1}%",
        selection_success_rate
    );
    assert!(
        healthy_selection_rate > 90.0,
        "Healthy selection rate too low: {:.1}%",
        healthy_selection_rate
    );
    assert!(
        stats_result.is_ok(),
        "Load balancer stats collection failed under chaos"
    );
...
async fn test_cascading_failure_prevention() {
    ...
    println!(
        "   ⏱️  Total Test Time: {:.2}s",
        total_cascade_time.as_secs_f64()
    );
    ...
    assert!(
        success_rate > 60.0,
        "Success rate during cascade too low: {:.1}% (required: >60%)",
        success_rate
    );
    assert!(
        recovery_success_rate > 95.0,
        "Recovery success rate too low: {:.1}% (required: >95%)",
...
async fn test_network_partition_simulation() {
    ...
    for i in 0..partition_a_services {
        let service_info = ServiceInfo {
            ...
            description: Some("Service in partition A").to_string(),
            endpoints: vec![],
            tags: std::collections::HashMap::new(),
            tags: {
    ...
    for i in 0..partition_b_services {
        let service_info = ServiceInfo {
            ...
            version: "1.0.0".to_string(),
            service_type: "partition-test".to_string(),
            description: Some("Service in partition B").to_string(),
            endpoints: vec![],
            tags: std::collections::HashMap::new(),
            tags: {
                let mut tags = std::collections::HashMap::new();
    ...
    let all_services = discovery.list_all().await.expect("Test assertion failed");
    let partition_a_query = discovery
        .discover(
            ServiceQuery::new()
                .with_service_type("partition-test")
                .with_tag("partition"),
        )
        .await
        .expect("Test assertion failed")
        .into_iter()
        .filter(|s| s.tags.get("partition").map(|v| v == "a").unwrap_or(false))
        .collect::<Vec<_>>();
    let partition_b_query = discovery
        .discover(
            ServiceQuery::new()
                .with_service_type("partition-test")
                .with_tag("partition"),
        ...
        .expect("Test assertion failed")
        .into_iter()
        .filter(|s| s.tags.get("partition").map(|v| v == "b").unwrap_or(false))
        .collect::<Vec<_>>();
    ...
    for i in 0..partition_b_services {
        ...
        discovery
            .update_health(&service_id, ServiceHealthStatus::Unhealthy)
            .await
            .expect("Test assertion failed");
    }
    ...
    for _ in 0..partition_test_iterations {
        ...
        if let Ok(healthy_services) = discovery.discover(ServiceQuery::new()).await {
            ...
            if healthy_count >= partition_a_services {
                // Should at least find partition A
    ...
    let discovery_success_rate =
        discovery_success_count as f64 / partition_test_iterations as f64 * 100.0;
    ...
    for i in 0..partition_b_services {
        let service_id = format!("partition-b-{}", i);
        discovery
            .update_health(&service_id, ServiceHealthStatus::Healthy)
            .await
            .expect("Test assertion failed");
    }
    ...
    let recovered_services = discovery.list_all().await.expect("Test assertion failed");
    let post_recovery_partition_a = discovery
        .discover(
            ServiceQuery::new()
                .with_service_type("partition-test")
                .with_tag("partition"),
        ...
        .filter(|s| s.tags.get("partition").map(|v| v == "a").unwrap_or(false))
        .collect::<Vec<_>>();
    let post_recovery_partition_b = discovery
        .discover(
            ServiceQuery::new()
                .with_service_type("partition-test")
                .with_tag("partition"),
    ...
    println!(
        "   💔 Discovery Success During Partition: {:.1}%",
        discovery_success_rate
    );
    println!(
        "   ⏱️  Partition Test Time: {:.2}s",
        partition_time.as_secs_f64()
    );
    println!("   🔄 Recovered Services: {}", recovered_services.len());
    println!(
        "   ✅ Post-Recovery A/B: {}/{}",
        post_recovery_partition_a.len(),
        post_recovery_partition_b.len()
    );
    ...
    assert_eq!(
        ...
        "Initial service registration failed"
    );
    assert!(
        discovery_success_rate > 90.0,
        "Discovery success rate during partition too low: {:.1}%",
        discovery_success_rate
    );
    assert_eq!(
        recovered_services.len(),
        partition_a_services + partition_b_services,
        "Service recovery incomplete"
    );
    assert_eq!(
        post_recovery_partition_a.len(),
        partition_a_services,
        "Partition A recovery failed"
    );
    assert_eq!(
        post_recovery_partition_b.len(),
        partition_b_services,
        "Partition B recovery failed"
    );
...
async fn test_resource_exhaustion_resilience() {
    ...
    let successful_under_pressure = resource_results
        .iter()
        .filter(|(_, success)| *success)
        .count();
    let success_rate_under_pressure =
        successful_under_pressure as f64 / resource_test_requests as f64 * 100.0;
    let avg_response_time = resource_results
        .iter()
        .map(|(time, _)| time.as_millis())
        .sum::<u128>() as f64
        / resource_test_requests as f64;
    ...
    let recovery_successes = recovery_results
        .iter()
        .filter(|(_, success)| *success)
        .count();
    let recovery_success_rate = recovery_successes as f64 / recovery_requests as f64 * 100.0;
    let recovery_avg_time = recovery_results
        .iter()
        .map(|(time, _)| time.as_millis())
        .sum::<u128>() as f64
        / recovery_requests as f64;
    ...
    println!(
        "   📈 Success Rate Under Pressure: {:.1}%",
        success_rate_under_pressure
    );
    println!(
        "   ⏱️  Avg Response Time Under Pressure: {:.2}ms",
        avg_response_time
    );
    println!("   🔄 Recovery Success Rate: {:.1}%", recovery_success_rate);
    println!(
        "   ⚡ Recovery Avg Response Time: {:.2}ms",
        recovery_avg_time
    );
    ...
    assert!(
        success_rate_under_pressure > 75.0,
        "Success rate under pressure too low: {:.1}%",
        success_rate_under_pressure
    );
    assert!(
        recovery_success_rate > 95.0,
    ...
    assert!(
        recovery_avg_time < avg_response_time,

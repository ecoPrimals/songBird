...
use songbird_discovery::{
    production::adapters::ProductionDiscoveryBackend, DiscoveryConfig, ServiceDiscovery,
    ServiceInfo,
};
...
async fn test_concurrent_service_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let all_registered =
        wait_for_all_services_registered(&discovery, 5, Duration::from_millis(200)).await;
...
async fn test_service_metadata_propagation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(discovered_service.metadata.get("region"), Some(&"us-east-1".to_string()));
    assert_eq!(discovered_service.metadata.get("environment"), Some(&"production".to_string()));

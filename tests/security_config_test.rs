...
use songbird_orchestrator::config::{CorsConfig, NetworkConfig, OrchestratorConfig, DefaultServiceConfig};
use songbird_orchestrator::config::constants::logging;
...
fn test_secure_cors_defaults() {
    ...
    assert!(!cors_config.enabled, "CORS should be disabled by default");
    
    // Origins should be empty, requiring explicit configuration
    assert!(cors_config.allowed_origins.is_empty(), "Allowed origins should be empty by default");
    ...
    assert!(cors_config.allowed_headers.contains(&"Content-Type".to_string()));
    assert!(cors_config.allowed_headers.contains(&"Authorization".to_string()));
...
fn test_environment_aware_tls() {
    ...
    assert!(!dev_config.enable_tls, "TLS should be disabled in development");
    ...
    assert!(prod_config.enable_tls, "TLS should be enabled in production");
    
    // Test staging environment (TLS enabled)
    std::env::set_var("SONGBIRD_ENVIRONMENT", "staging");
    let staging_config = NetworkConfig::default();
    assert!(staging_config.enable_tls, "TLS should be enabled in staging");
...
fn test_secure_defaults_integration() {
    ...
    // Network security
    assert!(!config.network.cors.enabled, "CORS should be disabled by default");
    assert!(config.network.cors.allowed_origins.is_empty(), "CORS origins should be empty");
    
    // Security configuration
    assert!(!config.security.enable_auth, "Authentication should be disabled by default");
    assert!(!config.security.enable_authz, "Authorization should be disabled by default");
    assert!(config.security.api_key.is_none(), "API key should be None by default");
    assert!(!config.security.rate_limiting.enabled, "Rate limiting should be disabled by default");
    assert!(!config.security.audit_logging.enabled, "Audit logging should be disabled by default");
...
fn test_no_hardcoded_credentials() {
    ...
    assert!(config.security.api_key.is_none(), "No default API key should be present");
...
fn test_production_security_checklist() {
    ...
    assert!(config.network.enable_tls, "TLS should be enabled in production");
    
    // CORS should still be disabled by default (requires explicit configuration)
    assert!(!config.network.cors.enabled, "CORS should require explicit configuration");
...
mod security_validation_tests {
    ...
    fn test_security_environment_variables() {
        ...
        let environment = std::env::var("SONGBIRD_ENVIRONMENT").unwrap();
        let tls_enabled = std::env::var("SONGBIRD_TLS_ENABLED").unwrap();
        let auth_enabled = std::env::var("SONGBIRD_AUTH_ENABLED").unwrap();

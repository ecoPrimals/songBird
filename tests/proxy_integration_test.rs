...
use songbird_orchestrator::proxy::{ConnectionProxy, ProxyConfig, ProxyRequest, LoadBalancingStrategy};
use songbird_orchestrator::traits::service::{ServiceInfo, ServiceEndpoint};
use axum::http::{Method, Uri, HeaderMap, StatusCode};
...
async fn test_proxy_creation_and_basic_functionality() {
    ...
    let service = ServiceInfo {
        ...
        description: "Test service for proxy integration tests".to_string(),
        endpoints: vec![
            ServiceEndpoint {
                path: "http://httpbin.org/json".to_string(), // Use httpbin for testing
                method: "GET".to_string(),
                description: "JSON endpoint for testing".to_string(),
                parameters: vec![],
    ...
    proxy.update_services(services).await.expect("Failed to update services");
    ...
    match proxy.route_request("test-service", proxy_request).await {
        Ok(response) => {
            ...
            println!("✅ Proxy request successful: {} bytes received", response.body.len());
        }
        Err(e) => {
...
async fn test_proxy_load_balancing() {
    ...
    let services = vec![
        ServiceInfo {
            ...
            version: "1.0.0".to_string(),
            service_type: "web".to_string(),
            description: "First load balanced service".to_string(),
            endpoints: vec![
                ServiceEndpoint {
                    path: "http://httpbin.org/uuid".to_string(),
                    method: "GET".to_string(),
                    description: "UUID endpoint".to_string(),
                    parameters: vec![],
                    response_schema: None,
                }
            ],
        ...
        ServiceInfo {
            ...
            version: "1.0.0".to_string(),
            service_type: "web".to_string(),
            description: "Second load balanced service".to_string(),
            endpoints: vec![
                ServiceEndpoint {
                    path: "http://httpbin.org/ip".to_string(),
                    method: "GET".to_string(),
                    description: "IP endpoint".to_string(),
                    parameters: vec![],
                    response_schema: None,
                }
            ],
...
async fn test_proxy_error_handling() {
    ...
    let service = ServiceInfo {
        ...
        version: "1.0.0".to_string(),
        service_type: "web".to_string(),
        description: "Service with invalid endpoint for testing error handling".to_string(),
        endpoints: vec![
            ServiceEndpoint {
                path: "http://invalid-host-that-does-not-exist.com".to_string(),
                method: "GET".to_string(),
                description: "Invalid endpoint for testing".to_string(),
                parameters: vec![],
                response_schema: None,
            }
        ],
    ...
    proxy.update_services(vec![service]).await.expect("Failed to update services");

...
use songbird_gaming_bridge::api::{
    create_router, ApiResponse, ApiState, BroadcastMessageRequest, RegisterServiceRequest,
    SendMessageRequest,
};
use songbird_gaming_bridge::communication::{WebSocketCommunication, WebSocketConfig};
use songbird_gaming_bridge::traits::communication::MessageType;
use songbird_gaming_bridge::{Orchestrator, OrchestratorConfig};
use std::sync::Arc;
...
async fn create_test_router() -> (Router, Arc<Orchestrator>, Arc<WebSocketCommunication>) {
    ...
    let orchestrator = Arc::new(Orchestrator::new(config).await.unwrap_or_default());
...
async fn make_request(
    ...
    let response = app.oneshot(request).await.unwrap_or_default();
...
fn validate_api_response_format(body: &str) {
    ...
    assert!(
        ...
        "timestamp field should be string"
    );

    // Validate timestamp format
    let timestamp_str = json["timestamp"].as_str().unwrap_or_default();
...
mod basic_health_tests {
    ...
    async fn test_health_endpoint() {
        ...
        assert!(json["success"].as_bool().unwrap_or_default());
    ...
    async fn test_system_info_endpoint() {
        ...
        let endpoints = system_info["api_endpoints"].as_array().unwrap_or_default();
...
mod service_management_tests {
    ...
    async fn test_list_services_empty() {
        ...
        let json: Value = serde_json::from_str(&body).unwrap_or_default();
        assert!(json["success"].as_bool().unwrap_or_default());
        assert!(json["data"].as_array().unwrap_or_default().is_empty());
    }
    ...
    async fn test_register_service() {
        ...
        let request_body = serde_json::to_string(&service_request).unwrap_or_default();
        ...
        let service_id = json["data"].as_str().unwrap_or_default();
    ...
    async fn test_get_service_not_found() {
        ...
        let (status, body) =
            make_request(app, Method::GET, &format!("/services/{}", fake_id), None).await;
        ...
        assert!(json["error"].as_str().unwrap_or_default().contains("not found"));
    }
    ...
    async fn test_service_lifecycle_operations() {
        ...
        let (status, body) = make_request(
            app,
            Method::POST,
            &format!("/services/{}/start", service_id),
            None,
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        ...
        let json: Value = serde_json::from_str(&body).unwrap_or_default();
        assert!(json["success"].as_bool().unwrap_or_default());
        assert!(json["data"].as_str().unwrap_or_default().contains("started"));
    }
}
...
mod communication_tests {
    ...
    async fn test_send_message() {
        ...
        let message_request = SendMessageRequest {
            target_service_id: "test-service".to_string(),
            message_type: MessageType::Request,
            topic: Some("test.topic".to_string()),
            body: json!({"test": "data", "value": 42}),
            headers: Some([("source".to_string(), "test".to_string())].into()),
            ttl: Some(30000),
        };

        let request_body = serde_json::to_string(&message_request).unwrap_or_default();
        let (status, body) =
            make_request(app, Method::POST, "/communication/send", Some(request_body)).await;
        ...
        if json["success"].as_bool().unwrap_or(false) {
            ...
            let message_id = json["data"].as_str().unwrap_or_default();
    ...
    async fn test_broadcast_message() {
        ...
        let broadcast_request = BroadcastMessageRequest {
            ...
            body: json!({"broadcast": true, "timestamp": Utc::now()}),
            headers: Some([("type".to_string(), "test".to_string())].into()),
            ttl: Some(60000),
        };

        let request_body = serde_json::to_string(&broadcast_request).unwrap_or_default();
        let (status, body) = make_request(
            app,
            Method::POST,
    ...
    async fn test_communication_stats() {
        ...
        assert!(
            stats["active_connections"].is_number(),
            "active_connections should be a number"
        );
        assert!(
            stats["messages_sent"].is_number(),
            "messages_sent should be a number"
        );
        assert!(
            stats["messages_received"].is_number(),
            "messages_received should be a number"
        );
        assert!(
            stats["bytes_sent"].is_number(),
            "bytes_sent should be a number"
        );
        assert!(
            stats["bytes_received"].is_number(),
            "bytes_received should be a number"
        );
        assert!(
            stats["failed_connections"].is_number(),
            "failed_connections should be a number"
        );
        assert!(
            stats["last_activity"].is_string(),
            "last_activity should be a timestamp string"
        );

        // Verify values are non-negative
        assert!(stats["active_connections"].as_u64().unwrap_or_default() >= 0);
        assert!(stats["messages_sent"].as_u64().unwrap_or_default() >= 0);
        assert!(stats["messages_received"].as_u64().unwrap_or_default() >= 0);
        assert!(stats["bytes_sent"].as_u64().unwrap_or_default() >= 0);
        assert!(stats["bytes_received"].as_u64().unwrap_or_default() >= 0);
        assert!(stats["failed_connections"].as_u64().unwrap_or_default() >= 0);

        // Verify timestamp format
        let timestamp_str = stats["last_activity"].as_str().unwrap_or_default();
        let _timestamp: DateTime<Utc> = timestamp_str.parse().unwrap_or_default();
    }
    ...
    async fn test_get_connections() {
        ...
        let (status, body) =
            make_request(app, Method::GET, "/communication/connections", None).await;
...
mod metrics_monitoring_tests {
    ...
    async fn test_all_service_metrics() {
        ...
        let map = metrics_map.as_object().unwrap_or_default();
...
mod dashboard_tests {
    ...
    async fn test_dashboard_data() {
        ...
        let timestamp_str = dashboard["timestamp"].as_str().unwrap_or_default();
        let timestamp: DateTime<Utc> = timestamp_str.parse().unwrap_or_default();
        assert!(timestamp <= Utc::now());
...
mod error_handling_tests {
    ...
    async fn test_empty_request_body() {
        ...
        let (status, _body) =
            make_request(app, Method::POST, "/services", Some("".to_string())).await;

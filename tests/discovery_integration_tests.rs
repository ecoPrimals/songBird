...
use songbird_gaming_bridge::{
    ...
    discovery::{MonitoringConfig, NetworkConfig, NodeType, SongbirdDiscoveryConfig, TrustConfig},
...
async fn test_orchestrator_with_songbird_discovery() {
    ...
    let orchestrator =
        Orchestrator::new_with_discovery(config, DiscoveryBackend::Songbird(songbird_config))
            .await
    ...
    let test_service = ServiceInfo {
        ...
        description: Some("Service for testing integration").to_string(),
        endpoints: vec![],
    ...
    assert!(orchestrator
        .service_exists("integration-test-service")
        .await
        .expect("Test assertion failed"));

    // Test service discovery
    let discovered_services = orchestrator
        .discover_services(ServiceQuery::new().with_service_type("test"))
        .await
        .expect("Test assertion failed");
    assert_eq!(
    ...
    // Test health updates
    orchestrator
        .update_service_health_in_discovery(
...
async fn test_multi_orchestrator_federation() {
    ...
    let mit_orchestrator = Orchestrator::new_with_federation(config1, Some("MIT".to_string()))
        .await
        .expect("Test assertion failed");

    // Create second orchestrator (Harvard)
    let config2 = OrchestratorConfig::default();
    let harvard_orchestrator =
        Orchestrator::new_with_federation(config2, Some("Harvard".to_string()))
            .await
            .expect("Test assertion failed");
    ...
    let mit_service = ServiceInfo {
        ...
        description: Some("Genomics analysis service").to_string(),
        endpoints: vec![],
        tags: std::collections::HashMap::new(),
        tags: {
            let mut tags = HashMap::new();
    ...
    let harvard_service = ServiceInfo {
        ...
        description: Some("Protein folding service").to_string(),
        endpoints: vec![],
        tags: std::collections::HashMap::new(),
        tags: {
            let mut tags = HashMap::new();
    ...
    mit_orchestrator
        .discovery()
        .register(mit_service)
        .await
        .expect("Test assertion failed");
    harvard_orchestrator
        .discovery()
        .register(harvard_service)
        .await
        .expect("Test assertion failed");
    ...
    let mit_genomics = mit_orchestrator
        .discover_services(ServiceQuery::new().with_service_type("genomics"))
        .await
        .expect("Test assertion failed");
    assert_eq!(
        mit_genomics.len(),
        1,
        "MIT should find its genomics service"
    );

    let harvard_proteomics = harvard_orchestrator
        .discover_services(ServiceQuery::new().with_service_type("proteomics"))
        .await
        .expect("Test assertion failed");
    assert_eq!(
        harvard_proteomics.len(),
        1,
        "Harvard should find its proteomics service"
    );
    ...
    if let Some(mit_songbird) = mit_orchestrator.songbird_discovery() {
        ...
        println!(
            "   📊 Federation stats: {} nodes, {} services",
...
async fn test_discovery_service_resilience() {
    ...
    for i in 0..10 {
        let service = ServiceInfo {
            ...
            description: Some("Service for testing resilience").to_string(),
            endpoints: vec![],
            tags: std::collections::HashMap::new(),
            tags: {
                let mut tags = HashMap::new();
    ...
    let successful_discoveries = results
        .into_iter()
        .filter_map(|r| r.ok())
    ...
    let final_services = orchestrator
        .discover_services(ServiceQuery::new().with_service_type("resilience"))
        .await
        .expect("Test assertion failed");
    assert_eq!(
        final_services.len(),
        10,
        "All services should still be discoverable"
    );
...
async fn test_discovery_performance_metrics() {
    ...
    for i in 0..service_count {
        let service = ServiceInfo {
            ...
            .to_string(),
            description: Some("Service for performance testing").to_string(),
            endpoints: vec![],
            tags: std::collections::HashMap::new(),
            tags: {
                let mut tags = HashMap::new();
    ...
    let web_services = orchestrator
        .discover_services(ServiceQuery::new().with_service_type("web"))
        .await
        .expect("Test assertion failed");
    ...
    let batch_services = orchestrator
        .discover_services(ServiceQuery::new().with_tag("batch".to_string()))
        .await
        .expect("Test assertion failed");
    ...
    println!(
        "      Registration: {} services in {:.2}ms ({:.0} services/sec)",
        service_count,
        registration_time.as_millis(),
        service_count as f64 / registration_time.as_secs_f64()
    );
    println!(
        "      List All: {} services in {:.2}ms",
        all_services.len(),
        list_time.as_millis()
    );
    println!(
        "      Type Query: {} services in {:.2}ms",
        web_services.len(),
        query_time.as_millis()
    );
    println!(
        "      Tag Query: {} services in {:.2}ms",
        batch_services.len(),
        tag_query_time.as_millis()
    );
    ...
    assert_eq!(
        all_services.len(),
        service_count,
        "Should find all registered services"
    );
    assert_eq!(
        web_services.len(),
        service_count / 5,
        "Should find correct number of web services"
    );
    assert!(
    ...
    assert!(
        ...
        "Query operation should be fast"
    );
...
async fn test_discovery_watch_functionality() {
    ...
    let test_service = ServiceInfo {
        ...
        description: Some("Service for testing watch functionality").to_string(),
        endpoints: vec![],
        tags: std::collections::HashMap::new(),
        tags: HashMap::new(),
    ...
    let discovered = orchestrator
        .discover_services(ServiceQuery::new().with_service_type("watchable"))
        .await
        .expect("Test assertion failed");

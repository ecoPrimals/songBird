...
async fn test_capability_based_routing() {
    ...
    let storage_providers: Vec<_> =
        services.iter().filter(|(_, caps)| caps.contains(&"storage")).collect();
...
async fn test_health_monitoring_workflow() {
    ...
    let healthy_services: Vec<_> =
        service_health.iter().filter(|(_, &h)| h).map(|(name, _)| *name).collect();
...
async fn test_metrics_collection_workflow() {
    ...
    // Calculate metrics
    let error_rate =
        (*metrics.get("errors").unwrap() as f64 / *metrics.get("requests").unwrap() as f64) * 100.0;

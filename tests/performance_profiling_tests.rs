...
async fn test_run_full_benchmark_suite() -> Result<()> {
    ...
    println!("  Load Balancer Performance: {:.2}ms avg", results.load_balancer_avg_latency_ms);
    println!("  Cache Performance: {:.2}ms avg", results.cache_avg_latency_ms);
    println!("  Memory Optimizer: {:.2}ms avg", results.memory_optimizer_avg_latency_ms);
    println!("  Batch Processing: {:.2}ms avg", results.batch_processing_avg_latency_ms);

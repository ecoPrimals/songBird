...
- **IPC**: JSON-RPC 2.0 + tarpc 0.37 dual-protocol inter-primal communication; G64 cephalization dual-socket: JSON-RPC on `songbird.sock`, tarpc binary on `songbird.tarpc.sock`; **G65 Protocol Negotiation**: primary UDS detects `PROTOCOLS: tarpc,jsonrpc\n` and negotiates protocol at connection time (Phase 3 cephalization, sourDough `d3d125f` reference pattern); **G66 Transport Abstraction**: `TransportEndpoint::platform_default()` / `from_env_or_default()` + `TransportListener` for silicon-agnostic IPC; **G68 Platform Substrate**: `PlatformAccess::apply()` for all permissions, `platform_link()` for symlinks — zero raw `PermissionsExt` in business logic, all `#[cfg(unix)]` confined to substrate+transport modules; **Vertebrate Evolution (Wave 157a)**: `CanonicalTransport` trait abstracts 9 transport crates (STUN/QUIC/TLS/IGD/OnionRelay/TURN/Tor/LineageRelay/Federation) behind shared lifecycle (start/shutdown/health/endpoints); gossip methods (`mesh.capabilities_announce`, `mesh.capabilities_revoke`) delegated to swarmVine; `capability_registry.toml` self-audit complete; **riboCipher Tier 2 (Wave 157d)**: federation port `:7700` accepts `0xED` mito framing with full `IpcServiceHandler` dispatch — closes the Tier 2 encode/decode chain (bearDog `RiboCipherHandler` + biomeOS client pool `[0xED,0x01]` + songBird acceptance); BTSP Phase 2 handshake on UDS accept when `FAMILY_ID` set (length-prefix + NDJSON wire formats; first-line auto-detect); BTSP Phase 3 `btsp.negotiate` encrypted framing (ChaCha20-Poly1305, HKDF session keys) on all 3 transport paths; domain symlink via `platform_link()` for capability discovery

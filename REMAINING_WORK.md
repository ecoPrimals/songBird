...
**Last Deep Debt Audit**: Wave 147f (July 17, 2026) — **P0 PROXY_PATH resolved**: drawbridge routing verified for footPrint composition (`/footprint/*` → `:8090`, env-configured via `SONGBIRD_DRAWBRIDGE_ROUTES` + `SONGBIRD_PROXY_ROUTES`). **GAP-037 resolved**: `/jsonrpc` HTTP endpoint added to drawbridge — enables esotericWebb and NDJSON JSON-RPC consumers to call songBird methods via `POST /jsonrpc` (auto-forwards to local IPC socket, returns JSON-RPC response as HTTP 200). **Discovery schemas confirmed**: `discovery.topology`, `discovery.health`, `discovery.query`, `discovery.bonds` methods implemented and wired through both universal-ipc and orchestrator dispatch (4 new `DiscoveryMethod` variants, 4 handler implementations each side). Response schemas: topology → `{gates, gate_count, self_node_id}`; health → `{alive, mesh_active, peer_count/registered_services}`; query → `{services, total_count, filter}`; bonds → `{bonds, bond_count}`. Previous: Wave 143b (July 16, 2026) — `IpcStream` migration batch 2 complete: 9 additional crates migrated from platform-specific `PlatformStream`/`IpcStream` type aliases to unified `songbird_types::IpcStream`. Eliminated `CryptoStream` enum + `pin-project` dependency from `songbird-tls` (type alias to `IpcStream`). Eliminated 30+ `#[cfg(unix)]`/`#[cfg(windows)]` `connect_platform` function pairs. Clippy-driven `&PathBuf` → `&Path` parameter evolution (4 sites). `tokio::io::split` adoption for non-owned stream splits. Total `IpcStream` coverage: 12 crates migrated (crypto-provider, lineage-relay, network-federation, sovereign-onion, tls, universal × 2, http-client × 2, orchestrator × 3). Previous: Wave 142b (July 16, 2026) — `drawbridge.rs` smart refactored: 813L→498L production via `drawbridge_auth.rs` extraction (auth gate, CIDR trust, external proxy allowlist, percent-decode helpers). Phase 2 "abstraction over gating" audit completed: identified 35 trait-backend candidates (IPC transport × 15+ crates, `ProcessBackend`, `SysMetricsBackend`, `NetworkBackend`), 25 inherently platform-specific (keep `#[cfg]`), 20 stubs/fallbacks. Priority: `IpcTransport` trait to deduplicate 15+ `connect_ipc` copies. Previous: Wave 141a (July 15, 2026) — Cross-platform `#[cfg]` gate evolution: eliminated **all** Windows cross-compile warnings across 8 crates (was 30+ warnings). Key changes: `session_protocol.rs` split into unix-only BTSP handlers + cross-platform NDJSON/encrypted handlers; `parse_capabilities_result`, `list_biomeos_sock_paths`, `storage_socket_from_endpoint`, `btsp_validation` module all gated to `#[cfg(any(unix, test))]`; platform-conditional constants (`SOCKET_RESCAN_INTERVAL_SECS`, `ROUTE_GET_TARGET_V4`, `KB_TO_BYTES`) properly gated; `clippy::items_after_statements` fix in `job_manager.rs`; `anyhow::Context` import gated per-platform in `binding.rs`. Result: **0 warnings** on both `cargo clippy --workspace --all-targets -- -D warnings` (Linux) and `cargo check --target x86_64-pc-windows-gnu` (Windows). Previous: Wave 140a — tideGlass drawbridge bonds + HTTPS proxy `OnceLock` optimization. Previous: Wave 139b (July 14, 2026) — NCBI/PubChem science bonds (`SCIENCE_BONDS` constant, 6 APIs), `compute.gpu` + `access.remote` capabilities for northGate, deep debt: unused var/import fixes from `#[cfg]` gates. Previous: Wave 139a (July 14, 2026) — Windows cross-compile (`x86_64-pc-windows-gnu`): 15 files across 7 crates — `#[cfg(unix)]`/`#[cfg(windows)]` IPC abstraction, `tokio-rustls` ring-only (no aws-lc-sys), virtual_relay gated, platform modules gated, test expectations updated. Previous: Wave 137d (July 13, 2026) — DRAWBRIDGE-CAP: Drawbridge routes now advertise as capabilities in `capabilities.list`. New `capabilities_list_with_runtime()` merges runtime-discovered capabilities (from `SONGBIRD_PROXY_ROUTES` and `SONGBIRD_DRAWBRIDGE_ROUTES`) into the Wire Standard L3 envelope. `capability.call` gains a drawbridge proxy-router fallback — when no UDS provider is registered but the capability has a proxy route, calls forward via HTTP. `capability.resolve` also returns drawbridge-served capabilities (with `TransportEndpoint::Tcp`). Unblocks `capability.call` for drawbridge services (e.g. `jupyter`, `inference`). FP-API-CADDY: Caddy config snippet drafted for production GIS proxy (10 HTTPS hosts, TLS natively handled). Previous: Wave 137c (July 13, 2026) — FP-API COMPLETE: Drawbridge external proxy now supports HTTPS outbound via `tokio-rustls` + system CA certificates (`rustls-native-certs`). Resolved the last blocker for footPrint→drawbridge migration: `proxy_to_external_tls()` establishes TLS connections to allowlisted GIS services (Nominatim, USGS EPQS, FEMA NFHL, Overpass, ArcGIS, etc.). Fixed duplicate Content-Length header bug in proxy forwarding. Drawbridge startup wired into `commands/server.rs` (was only in unused `bin_interface/server.rs`). systemd unit updated with env vars. footPrint `PROXY_PATH` migrated from `/api/proxy` to `/ext` (drawbridge route). Vite dev proxy wired. E2E verified: `curl → drawbridge:7780 → nominatim.openstreetmap.org (HTTPS)` returns real geocoding data. Non-allowlisted domains correctly rejected (403). POST with body verified (Overpass API). Previous: Wave 137b (July 12, 2026) — UDS-HTTP-PROTOCOL (CRITICAL): `peer.connect` now performs full federation probe (riboCipher + health.ping) and registers discovered peers in `BeaconMesh` via `add_endpoint`. Closes the gap where TCP connect succeeded but `mesh.peers` stayed empty. New `node_id` and `register_mesh` params, `probe_peer_full` promoted to public API, `PeerHandler::with_mesh()` constructor wires mesh handler. Previous: Wave 137a (July 12, 2026) — FLOCKGATE-MESH resolved: `parse_wg_conf()` and `parse_wg_dump()` in `mesh_seed.rs` were using `DEFAULT_HTTP_PORT` (8080) instead of `DEFAULT_MESH_PEER_PORT` (7700) for auto-discovered WG overlay peers — root cause of "0 mesh peers from flockGate"; `mesh-peers.toml` updated to 7700; songBird immediately connected to 4 overlay peers (golgi, east-gate, spore-gate, iron-gate). FP-API compat: drawbridge external proxy now supports `?url=<encoded_url>` query-string mode alongside path-based routing, enabling footPrint Express→drawbridge migration without frontend changes; `validate_url()` method validates full URLs against domain allowlist; `percent_decode()` helper for query parameter decoding. Previous: Wave 137 (July 11, 2026) — Production stub evolution: CLI `network.rs` fake `fastrand` metrics replaced with real TCP RTT probes against `SONGBIRD_PEERS` (real latency, jitter, loss); `gaming_demo.rs` binary removed (simulation theater); dead `config/environment.rs` (595L deprecated shim, past Q2 2026 removal deadline) deleted; dep diet: `fastrand` + `rand` removed from songbird-cli. Previous: Wave 136b (July 11, 2026) — Drawbridge hardening: EXP-06 auth-gate (`eb4d0be`) — bearer token auth, CIDR trusted peers, public path bypass, per-route `!public` flag, `_sb_token` query param for browser access; external proxy allowlist (`87b7779`) — `ExternalProxyAllowlist` replaces Express `/api/proxy` for footPrint composition, domain-validated forward proxying with `_external_proxy` capability, service→base_url mapping, 403 for non-allowlisted services. Clippy fixes: `format_push_string`→`write!`, `clone()`→`clone_from()`, `map().unwrap_or()`→`map_or()`, unused bindings, collapsible_if, `unnecessary_map_or`→`is_some_and()`. Previous: Wave 134f (July 10, 2026) — P2 bug fix: `http.request` path-handling — `songbird-http-client` was sending absolute-form URIs for HTTP/1.1 direct connections (causing 404 at drawbridge); fixed to use `uri.path_and_query()` origin-form; drawbridge also hardened with defensive absolute-form→path normalization. Previous: Wave 131b (July 4, 2026) — LAN peering critical path: `UdpPeerConnector` LAN bypass (same-subnet detection via `/proc/net/fib_trie`, TCP probe on federation port, honest "failed" state); `HolePunchCoordinator` LAN fast-path (skip signaling for private IPs); federation port convergence (DEFAULT_MESH_PEER_PORT=7700 as server default, 0.0.0.0 bind). Security: `bluetooth_pure.rs` fake credentials eliminated (fail-closed errors); `discovery_bridge.rs` auto-accept removed (reject without security provider). Health honesty: `SubsystemStatus` struct — callers pass real state, "degraded" reported honestly. Dep diet: `tarpc` "full"→["tokio1","serde-transport","serde-transport-bincode","tcp"]; `hyper` "full"→["http1","client","server"]. Hardcoding: `192.0.2.10:8000` removed from builders.rs (env-derived). unreachable!(): connection.rs unknown tier→error+drop; trust.rs None→Err. Previous: Wave 131 (July 4, 2026) — Allocation elimination: `registry.rs` `to_string()` per-capability-lookup removed (zero-alloc `iter().any()`); `stun_handler/config.rs` returns `&'static [String]` (stops per-call `Vec::clone()`); `federation.rs` + `consent/rules.rs` + `protocol_api.rs` same pattern. Typed dispatch: `GraphMethod`, `CoordinationMethod`, `LegacyMethod`, `HttpMethod::Put/Delete` added to `JsonRpcMethod`; `mesh.discover_remotes/mirror/publish` wired into universal-ipc dispatch (was gap); orchestrator legacy string dispatch eliminated (all routing via exhaustive enum match). Dependency diet: `songbird-types` tokio `"full"` → `["net", "sync", "time"]`. File refactoring: `mesh_handler/mod.rs` 850→697L via `discovery_federation.rs` extraction. Previous: Wave 128 (June 28, 2026) — WG auto-init validation and hardening. multi-tier discovery (1: `wg show` with CAP_NET_ADMIN, 2: WG config file parsing `/etc/wireguard/wg0.conf`, 3: user-accessible `~/.config/songbird/mesh-peers.toml`); `parse_wg_conf()` INI parser for non-root environments; `config_dir()` path resolver added to `songbird-types`; orphaned doc comment fixed; +6 new tests (WG conf parsing, mesh-peers.toml format); deployed `mesh-peers.toml` on flockGate for immediate zero-config init. Previous: Wave 124 (June 23, 2026) — Zero-config mesh auto-initialization: `discover_wireguard_peers()` reads WG peer allowed-IPs from `wg show all dump` (tier-3 after env vars and persisted state); WG-discovered peers auto-registered as overlay endpoints (priority 0); `mesh.status` returns `initialized: false` / `"awaiting_init"` instead of hard error when mesh not yet seeded (probers get structured status, not failure); +4 new tests for WG dump parsing. Previous: Wave 123 (June 22, 2026) — Topology-aware routing Phase 1: `BeaconMesh::record_overlay_connection()` for WireGuard latency tracking; overlay-aware health probing (probe WG peers alongside WAN bootstrap, record with correct EndpointType); path-optimal `find_peer_with_capability` (min-cost selection among multiple holders); targeted remote dispatch (cached-holder fast-path before fan-out); `topology_graph` module (merge local edges + peer gossip into global adjacency, partition detection); `beacon.rs` refactored (858→347L via test extraction). Previous: Wave 121 (June 21, 2026) — zero clippy --all-targets warnings, zero `"literal".to_string()` in production (2,020→0), `keys.rs` (801→555L), all external deps justified, zero production mocks. Previous: Wave 111 (June 13, 2026) — riboCipher transport signal detection shipped (Stream 7); `connection.rs` (815→472L) smart-refactored via `session_protocol.rs` extraction; `hardcoded_elimination.rs` (872→689L) via `hardcoded_replace.rs` extraction; BTSP max frame magic number evolved to `BTSP_MAX_FRAME_SIZE` constant; ad-hoc URL construction centralized to `jsonrpc_endpoint_url()` helper; stale "DEEP DEBT: polling loop" comment removed from `lineage-relay/relay.rs` (implementation was already event-driven).
...
**Current Wave**: 147f — Upstream demand delivery: P0 PROXY_PATH (footPrint composition wiring verified), GAP-037 `/jsonrpc` HTTP→IPC bridge (esotericWebb unblocked), `discovery.topology/health/query/bonds` schemas confirmed and implemented. Previous: 143b — `IpcStream` migration batch 2: 9 crates migrated, `CryptoStream`+`pin-project` eliminated from `songbird-tls`, 30+ platform-specific `connect_platform` functions consolidated. Previous: 142b — Phase 2 "abstraction over gating": `drawbridge.rs` smart refactor (813→498L production), platform cfg audit (35 trait candidates identified, `IpcTransport` priority), deep debt cleanup. Previous: 141a — Cross-platform reference implementation polish: zero Windows warnings, platform-clean `#[cfg]` gating across all 31 crates. Previous: 140a — tideGlass drawbridge bonds (4 pharmacogenomics APIs) + HTTPS proxy `OnceLock` optimization + bond registry parity (20 total bonds: 11 GIS + 6 science + 3 tideGlass unique). Previous: 139a/b — Windows cross-compile + NCBI/PubChem bonds + northGate capabilities. Previous: 137d — DRAWBRIDGE-CAP (runtime capabilities advertising in `capabilities.list` + `capability.call` drawbridge fallback routing) + FP-API-CADDY (Caddy production proxy snippet). Previous: 137c — FP-API COMPLETE (HTTPS outbound proxy via tokio-rustls, drawbridge wired into server command, footPrint client migrated to `/ext` route, E2E verified). Previous: 137b — UDS-HTTP-PROTOCOL fix (peer.connect → mesh registration) + FLOCKGATE-MESH fix (WG port mismatch 8080→7700) + FP-API `?url=` compat layer. Previous: 137 — Deep debt evolution (CLI stub→real probes, dead code removal, dep diet). Previous: 136b — Drawbridge auth-gate + external proxy allowlist. Previous: 113 — **Stream 6 Divergence: Mesh Partition Tolerance + Version Negotiation**: (1) MESH-PARTITION-TOLERANCE: `PeerMetadata` struct tracks per-peer version + cross-gate reachability reports. `mesh.capabilities_announce` now includes `reachable_peers` and `version` fields. `partition_status_for()` computes partition state per peer. `mesh.status` surfaces `partition_warnings` array. `mesh.health_check` emits `partition_detected: true` + `partitions[]` when cross-gate gossip reveals asymmetric reachability. (2) PEER-VERSION-MISMATCH: `probe_peer_full()` extracts version from `health.ping` response. `spawn_peer_health_loop` records peer versions on every successful probe. `mesh.status` reports `version_skew` array. `mesh.peers` includes `version` and `version_mismatch` per peer. (3) +8 new tests covering partition detection, version tracking, gossip storage, and version skew reporting. Previous: 112 — Deep Debt Evolution. Previous: 111 — FEDERATION-STATUS-WIRE + FEDERATION-RECONNECT. Previous: 110 — HEALTH-SB-01 + federation.status root cause. Previous: 107 — Topology-aware cross-gate routing (M1). Previous: 106 — Mesh persistence + federation port fix. Previous: 104 — env-var race conditions eliminated. Previous: 103 — Flaky test root-cause fix, mDNS wired. Previous: 102 — capability-neutral naming, P0 JWT hardening. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  
...
| **Edition** | Rust 2024 |
| **Build** | Zero errors, zero warnings, all 31 crates compile clean (~10s incremental); Windows cross-compile zero warnings (July 15, 2026) |
| **Clippy Pedantic** | 31/31 crates clean — zero warnings (`clippy::pedantic + nursery`, `-D warnings`, July 15 verified) |
...
### Phase 2: Abstraction Over Gating (Wave 142b assessment)

**Status**: Assessed. petalTongue `petal-tongue-platform` is the ecosystem reference.
...
| `genesis/src/physical_channels/bluetooth_pure.rs:138–181` | Returns fail-closed errors (Wave 131b) | Real BLE GATT credential exchange when security provider wired |

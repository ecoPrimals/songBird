...
**Date**: April 15, 2026  
**Version**: v0.2.1  
**Last Deep Debt Audit**: Wave 160 (Apr 15, 2026)  
**Current Wave**: 160 — BTSP NDJSON auto-detect in server accept loop (primalSpring Phase 45c): wired `is_btsp_client_hello()` + `perform_server_handshake_ndjson()` into both Unix and TCP accept loops in `bin_interface/server.rs`; added `btsp.session.*` semantic mappings to `security_rpc_client/rpc.rs`; refactored monolithic `handle_json_rpc_connection` into `handle_connection`/`handle_json_rpc_lines`/`dispatch_json_rpc_line`. All tests pass, 0 clippy warnings, 0 build warnings  

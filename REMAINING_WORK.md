...
**Current Wave**: 165 — Dependency cleanup and hardcoded elimination: `serde_yaml` → `serde_yaml_ng` (maintained fork), `hostname` consolidated to `gethostname` across 5 crates, `futures` 0.3 → `futures-util` in songbird-universal (13 tests converted to tokio::test), `url` 2.0 → 2.5 aligned, hardcoded `"0.0.0.0"` → constants in udp_discovery and stun server, bare `#[allow()]` given reasons, 7 zero-caller deprecated items removed (MockSquirrel, MockToadStool, 3 module aliases, discover_beardog_binary, discover_security_provider_socket_with). All tests pass (497), 0 clippy warnings.  

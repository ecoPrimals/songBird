...
**Current Wave**: 112 — **Deep Debt Evolution**: (1) Hardcoded elimination: `service_locator.rs` — removed 2× hardcoded `/run/user/1000` fallback → `dirs::runtime_dir()` + `songbird_types::constants::BIOMEOS_SYSTEM_RUNTIME_DIR` derivation; `"127.0.0.1"` → `songbird_types::constants::LOCALHOST`. (2) Production stubs evolved: `HealthProbe::NoOp` → 5 real probe variants (TcpConnect, UnixSocket, FilesystemAccess, Custom, JsonRpcHealth) with proper timing and real I/O; `check_gaming_services`/`check_federation_services` → actual TCP reachability probes (no longer env-only stubs). (3) Capability-based naming: `BearDogVerifier` → `SecurityProviderVerifier` (type alias retained for backward compat). (4) Dependency hoisting: `sha2`, `hmac`, `hex`, `base64`, `gethostname`, `hyper-util` — 6 crates consolidated to `[workspace.dependencies]` (13 crate-local specs eliminated). (5) Test coverage: +12 new probe tests (TCP connect healthy/unhealthy, UDS healthy/unhealthy, filesystem access, custom ok/fail, checker aggregation, timing measurement). Previous: 111 — FEDERATION-STATUS-WIRE + FEDERATION-RECONNECT. Previous: 110 — HEALTH-SB-01 + federation.status root cause. Previous: 107 — Topology-aware cross-gate routing (M1). Previous: 106 — Mesh persistence + federation port fix. Previous: 104 — env-var race conditions eliminated. Previous: 103 — Flaky test root-cause fix, mDNS wired. Previous: 102 — capability-neutral naming, P0 JWT hardening. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  
...
| Metric | Value |
|--------|-------|
| **Tests** | 8,910+ lib tests passed, 0 failures, 0 flaky (Wave 112 verified June 12, 2026) |

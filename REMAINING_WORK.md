...
**Current Wave**: 155i — **WINDOWS PLATFORM GATE P0 FIX. DEEP DEBT EXECUTION.** TCP IPC fallback on Windows (port 9901, configurable via `SONGBIRD_IPC_PORT`). bearDog TCP pattern adopted. All `--listen` and `--socket` paths now functional cross-platform. Emergency tunnel sovereignty-gated. File-size compliance enforced (0 files >800L). Test suite green (8,937 passing).  

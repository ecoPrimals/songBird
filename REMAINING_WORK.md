...
**Last Deep Debt Audit**: Wave 157g (August 10, 2026) — **G72 DEPENDENCY PANDEMIC TIER 1+2.** Tier 1: Tokio `["full"]` excised from 16 production `[dependencies]` across 16 crates → bare `{ workspace = true }` (workspace floor: rt-multi-thread, net, io-util, macros, sync, time, fs, signal, process). Eliminates `io-std` + `parking_lot` feature bloat. Dead deps removed: `futures-util` (songbird-config, songbird-network-federation), `uuid` (songbird-onion-relay), `thiserror` (songbird-canonical), `anyhow` (songbird-nfc, songbird-registry, songbird-cli), `tokio` from songbird-canonical production deps (zero production usage). **Tier 2**: `tokio::sync` → `std::sync` conversion: 51 sites across 3 zero-risk crates (songbird-quic 13, songbird-registry 36, songbird-genesis 2). Pattern: `.lock().unwrap_or_else(std::sync::PoisonError::into_inner)`. 730 total sites identified, 117 KEEP (held across .await). Remaining 679 sites deferred (mixed KEEP/CONVERT on same struct fields requires structural refactoring). **File refactoring**: `http_server.rs` (854L) → 4 focused modules (mod.rs + federation_dispatch + tls_server + port_binding). STUN error type fix. **BTSP DRY**: shared `btsp_rpc.rs` consolidates 3 identical tunnel RPC stubs. **Previous**: Wave 157e MESHRELAY GOSSIP TRANSPORT + TEST HARDENING. `gossip.relay` + `gossip.inject` methods added — `MeshRelay` transport variant enables cross-gate gossip propagation through songBird's `:7700` federation mesh when swarmVine's direct TCP 7800 is unreachable. `GossipMethod` enum + dispatch module + 4 unit tests. **Test suite hardening**: `dispatch_hits_each_json_rpc_arm` (was hanging indefinitely due to live-service UDS reads with no timeout + real IGD/STUN/Tor network IO) — fixed via `dispatched!` macro (100ms timeout fence) for all real-network-IO calls; production fix: 5s read timeout in `SecurityRelayAuthority::call_security_rpc()`. Suite: 47s → 0.71s. **Previous (157d)**: Federation port `:7700` accepts `0xED` riboCipher framing with full `IpcServiceHandler` dispatch (Tier 2 chain closing). `TransportRegistry`. Gossip excision to swarmVine. PID P2. **Previous**: Wave 157a vertebrate evolution (CanonicalTransport trait + 9 adapters + self-audit). G68 platform substrate. G66 transport abstraction. G65 protocol negotiation. tarpc 0.37 + dual-socket.
...
**Current Wave**: 157g — **G72 DEPENDENCY PANDEMIC TIER 1+2 + DEEP DEBT REFACTORING + MESHRELAY GOSSIP + TEST HARDENING.** `gossip.relay` + `gossip.inject` enable cross-gate gossip propagation through `:7700` federation mesh (MeshRelay transport variant). swarmVine can now relay gossip through songBird when direct TCP 7800 is unreachable. `GossipMethod` enum in type system. Full dispatch + introspection + capability_registry wiring. Dispatch test suite hardened: `dispatched!` timeout macro eliminates live-service hangs (47s→0.71s); production read timeout added to `SecurityRelayAuthority` UDS path. **Previous (157d)**: riboCipher Tier 2 chain closed. `TransportRegistry`. Gossip excision to swarmVine. PID P2. **Previous**: 157a vertebrate evolution (CanonicalTransport trait + 9 adapters + RPC self-audit). G68 platform substrate. G66 transport abstraction. G65 protocol negotiation. tarpc 0.37 + dual-socket (C1+C2).  
...
- [x] `anyhow` → typed errors in `songbird-config` (Wave 157i): `capability_port_config` and `discovery_helpers` evolved from `anyhow::Result` → `SongbirdResult<T>`. Lock poisoning replaced with `unwrap_or_else(PoisonError::into_inner)`. Infallible methods (register, clear, build, has_capability, list_capabilities) simplified to return directly without Result. `anyhow` dependency removed from songbird-config entirely. **Remaining**: `songbird-http-client` still exposes `anyhow::Result` in public APIs (larger surface, lower priority)
- [ ] **`rustls-rustcrypto` elimination (Tower Atomic collab with bearDog)**:
  ...
  - Phase 2 BLOCKED on bearDog: bearDog completes ACME cert provisioning using songBird's challenge responder, then provides TLS cert signing via `crypto.tls.*` delegation
  - Phase 3: `songbird-tls` client mode replaces `outbound_tls_connector()` in drawbridge — delegates crypto to bearDog instead of `rustls-rustcrypto`
  - Phase 4: Remove `rustls-rustcrypto` from `songbird-universal-ipc/Cargo.toml` and `songbird-cli/Cargo.toml`; remove `rustls_rustcrypto::provider().install_default()` from CLI tower command
  - Net result: Drop `rustls-rustcrypto 0.0.2-alpha` (DO NOT USE IN PRODUCTION), all transitive RustCrypto deps, and `rustls-webpki 0.102` ghost lock
- [x] **G72 Dependency Pandemic Tier 1** (Wave 157g): Tokio `["full"]` excised from 16 production `[dependencies]` → bare workspace inheritance. 7 dead deps removed (futures-util ×2, uuid, thiserror, anyhow ×3, tokio from canonical). `tokio::sync` Tier 2 audit scoped (40+ sites). See `infra/wateringHole/specs/DEPENDENCY_PANDEMIC_SPEC.md`.
- [ ] **G72 Tier 2** (IN PROGRESS): `tokio::sync` → `std::sync` where lock is never held across `.await`. **Done**: songbird-quic (13), songbird-genesis (2), songbird-registry (36), songbird-observability (34), songbird-network-federation (40), songbird-orchestrator (43), songbird-universal (28), songbird-onion-relay (13). **Total: 209 sites converted across 8 crates.** Remaining deferred: files holding guards across `.await` (consent_management, discovery/registry, capability_router) need structural field-level separation before conversion.
...
- [ ] `bincode` 1.x (RUSTSEC-2025-0141): transitive via tarpc 0.37/tokio-serde 0.9 — not directly actionable, blocked on tarpc upstream codec migration (PR #529 open). `bincode` is only used as the serialization format for `tarpc` RPC transport (songbird-orchestrator, songbird-universal). Zero direct production usage outside tarpc protocol
...
| ~~`orchestrator/src/bin_interface/server.rs:252–255`~~ | ~~Windows: no IPC server spawned~~ | **RESOLVED** Wave 155i: TCP IPC fallback on non-Unix (shared handler, port 9901) |
| ~~`orchestrator/src/app/core/mod.rs:466–483`~~ | ~~Non-Unix: IPC returns hard error~~ | **RESOLVED** Wave 155i: `#[cfg(not(unix))]` falls through to TCP IPC when `listen_addr` provided |

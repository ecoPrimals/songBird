...
**Last Deep Debt Audit**: Wave 156p (August 6, 2026) — **G65 CEPHALIZATION: Protocol Negotiation.** Implemented G65 single-socket protocol negotiation on primary UDS (`songbird.sock`). Client sends `PROTOCOLS: tarpc,jsonrpc\n`, server responds with `PROTOCOL: tarpc\n`. Backward-compatible: legacy clients without negotiation line continue as JSON-RPC. New `protocol_negotiation` module in `songbird-universal` with full type system (`IpcProtocol`, `NegotiationRequest`, `NegotiationResponse`, `NegotiationError`), client-side `negotiate_client()`, server-side `negotiate_server_from_line()`, and `select_protocol()`. 18 unit + 5 integration tests. Follows sourDough `d3d125f` reference implementation pattern. **Wave 156h** (August 5): tarpc 0.37 upgrade + dual-socket (C1+C2). **Wave 156d** (August 4): GIS bond expansion + content distribution validation. **Wave 155n** (August 3): `mesh.connectivity_check` and `mesh.throughput` JSON-RPC methods (20 total mesh methods). **Wave 155i**: TCP shared ServiceRegistry fix, PID path portability, riboCipher noise reduction, Windows platform gate evolution.
...
**Current Wave**: 156p — **G65 CEPHALIZATION: Protocol Negotiation.** songBird now fully G65 — primary UDS listener detects `PROTOCOLS:` requests and negotiates tarpc or jsonrpc. `protocol_negotiation` module provides client/server negotiation types, wire format parsing, and `negotiate_client()` for other primals to negotiate with songBird. Combined with C2 dual-socket (`.tarpc.sock`), songBird supports all three cephalization phases: Phase 1 (JSON-RPC), Phase 2 (dual-socket tarpc), Phase 3 (G65 negotiation). 14,860+ tests passing, 23 new G65 tests. **Previous**: 156h — tarpc 0.37 + dual-socket (C1+C2). 156d — ironGate downstream readiness. 155n — mesh methods.  

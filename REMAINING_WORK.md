...
**Current Wave**: 150x — **CALLER IDENTITY + UDS HARDENING + CRYPTO COMPOSITION.** P1 Caller Identity (4 findings): `SO_PEERCRED` extraction on ALL production UDS accepts (not just bin_interface), uid-based authorization in `MethodGate` (same-uid = trusted, different-uid requires token), `from_unix_stream()` constructor on `CallerContext`. P1 UDS Hardening (5 findings): directory guard on all bind sites (stale directory → remove), symlink rejection before bind (anti-hijack), `chmod 0o600` post-bind (owner-only access), concurrency semaphore (256 max, `SONGBIRD_MAX_UDS_CONNECTIONS`), `let-else` accept pattern. Applied to both orchestrator `UnixSocketServer` and universal-ipc platform layer. Crypto Composition: `local-crypto-fallback` feature flags gate inline crypto, bearDog UDS delegation for cold-path. 0 clippy warnings, 0 unsafe, 0 production mocks.  

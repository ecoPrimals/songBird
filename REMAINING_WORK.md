...
**Last Deep Debt Audit**: Wave 157a (August 9, 2026) — **VERTEBRATE EVOLUTION.** Created `CanonicalTransport` trait in songbird-types with lifecycle methods (start/shutdown/health/endpoints/is_ready) and `TransportHealth` type. Implemented trait adapters across all 9 transport crates: STUN, QUIC, TLS, IGD, OnionRelay, TURN, Tor, LineageRelay, Federation. Excised `mesh.capabilities_announce` and `mesh.capabilities_revoke` as swarmVine delegation targets — songBird retains fallback handler, dispatch logs delegation intent. Self-audit: updated `capability_registry.toml` to document 6 previously-unlisted mesh methods and formally declare gossip methods as `owner = "swarmVine"`. **Previous**: G68 platform substrate (Aug 7). G66 transport abstraction. G65 protocol negotiation. tarpc 0.37 + dual-socket. GIS bond expansion. mesh.connectivity_check + mesh.throughput.
...
**Current Wave**: 157a — **VERTEBRATE EVOLUTION.** Shared `CanonicalTransport` trait unifies 9 transport crates behind common lifecycle interface. `mesh.capabilities_announce`/`revoke` formally delegated to swarmVine. RPC surface self-audit complete — capability_registry.toml now matches actual code surface. **Previous**: G68 platform substrate. G66 transport abstraction. G65 protocol negotiation. tarpc 0.37 + dual-socket (C1+C2).  

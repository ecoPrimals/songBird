...
**Last Deep Debt Audit**: Wave 156d (August 4, 2026) — **GIS Bond Expansion + Content Distribution Validation**: Added `arcgis` (services.arcgis.com) and `usgs_eq` (earthquake.usgs.gov) to external proxy bonds (13 GIS bonds total, 22 ALL_BONDS). Validates Phase 2 footPrint deployment readiness on ironGate — drawbridge external allowlist covers all declared GIS sources. Inter-gate `content.get` routing path verified: `capability.call(content, get, routing=any)` → remote dispatch → `discovery.content_peers` → first-responder-serves with priority-sorted peers. **Wave 155n** (August 3): `mesh.connectivity_check` and `mesh.throughput` JSON-RPC methods added (20 total mesh methods). Active E2E cross-gate validation with riboCipher acceptance, path classification, and sustained throughput measurement (target >800 MB/s on 10G). **Wave 155i**: TCP IPC handler now uses the shared `ServiceRegistry` (was creating isolated instance → `services: 0` on blueGate). PID file paths evolved to platform-aware (`songbird_runtime_dir()` → `%PROGRAMDATA%\songbird` on Windows). riboCipher probe noise reduced (legacy unsignalled connection log downgraded error→debug; TCP IPC handler now strips riboCipher prefix before JSON parse). Windows cross-compile warnings fixed (unused imports gated). `#[cfg(not(unix))]` platform gate evolved to TCP IPC fallback (port 9901). Virtual relay evolved to TCP-based relay on non-Unix. Emergency tunnel tier (cloudflared) gated to opt-in via `emergency_tunnel_enabled` config (sovereignty-first default). `service_tests.rs` (1,018L) and `mesh_handler/tests.rs` (998L) refactored into thematic module directories. `anyhow` eliminated from `SongbirdResult` response APIs → typed `SongbirdError::ResponseExtraction`. Clippy `--all-features` fixed (wildcard imports, `#[must_use]`). Doc warning resolved. Hardcoded IPs evolved to canonical constants. **Previous**: Wave 155d — Tower Atomic Health Facade.
...
**Current Wave**: 156d — **IRONGATE DOWNSTREAM READINESS.** GIS bond expansion (arcgis + usgs_eq). Drawbridge external proxy validated for all footPrint sources (USGS, FEMA, OSM, ArcGIS). Inter-gate content.get dispatch path audited and confirmed operational: `capability.call` → cached peer lookup → priority-sorted remote dispatch → `routing: "local"` on remote hop (prevents relay loops). `discovery.content_peers` + `ContentAnnouncementStore` validates "mesh broadcast → first-responder-serves" pattern. songBird is Phase 5-ready for live E2E mesh validation once nestGate registers content capability on remote gates. **Previous**: 155n — Inter-gate mesh validation (`mesh.connectivity_check` + `mesh.throughput`). 155i — Windows transport maturity + TCP registration fix. Test suite green (14,840+ passing).  

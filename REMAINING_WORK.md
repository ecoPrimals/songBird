...
**Date**: April 30, 2026  
**Version**: v0.2.1  
**Last Deep Debt Audit**: Wave 179 (Apr 30, 2026)  
**Current Wave**: 179 — Coverage expansion: +92 tests across 15+ files in 8 crates (songbird-orchestrator, songbird-cli, songbird-config, songbird-http-client, songbird-compute-bridge, songbird-network-federation, songbird-discovery, songbird-process-env). Tested server/mod.rs, tarpc_server.rs, CLI commands (status, tower, discovery, resources), config mDNS/DNS-SD parsers, agnostic primal config, remote probes JSON parsing, federation security helpers, broadcaster message assembly, HTTPS connection logic. Deep debt audit: 0 files >800L, 0 unsafe, 0 hardcoded prod values, cargo deny fully passing. 0 clippy warnings, 7,784 lib tests pass.  
...
| Metric | Value |
|--------|-------|
| **Tests** | 7,784 lib passed, 0 failures, 22 ignored |
...
Phase 2 relay fixed (Waves 132/153/156/158/160/162/164/167/168/169): `perform_server_handshake` + `perform_server_handshake_ndjson` wired into UDS accept, startup resilience (graceful crypto fallback), NDJSON handshake timeouts, Neural API timeout fix. Wave 158: all 6 wire-protocol mismatches between Songbird's relay and BearDog's `btsp.server.*` methods resolved. Wave 160: BTSP auto-detection wired into server accept loops (first-line peek). Wave 162: `stream.shutdown()` removed. Wave 164: `read_to_end()` → JSON-aware chunked reads. Wave 167: error frames for all failure modes, env fallbacks. Wave 168: two root causes fixed — (1) `SecurityRpcClient::new()` → `new_direct()` (was wrapping BearDog calls in Neural API `capability.call` — BearDog returned "Method not found"); (2) `resolve_family_seed()` now base64-encodes the raw env string (BearDog base64-decodes the param internally). Wave 169: remaining `::new()` → `::new_direct()` in `bin_interface/server.rs` (IPC + TCP server paths missed by W168). **Remaining**: Phase 3 encrypted framing (ChaCha20-Poly1305 / HMAC-plain actual stream encryption via `btsp.server.export_keys`), multi-frame sessions, and E2E integration test with live BearDog + primalSpring.
...
**Note (Apr 30, 2026)**: 73.29% measured via llvm-cov `--workspace --lib` (Apr 30 2026). Wave 179 added 92 tests across 15+ files in 8 crates targeting low-coverage pure-logic modules. Wave 171 added 271 tests (71.28% → 73.41%); Wave 174 added 18; Wave 177 added 9. Target 90% via ongoing coverage expansion of I/O-heavy modules with mock infrastructure.

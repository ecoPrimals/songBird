...
**Last Deep Debt Audit**: Wave 77 (June 4, 2026) — **Retry hardening**: exponential backoff (120s × 2^attempt), queue depth cap (50 entries), stale capability TTL expiry (10 min), max 5 retries (up from 3). `PeerCapabilityEntry` tracks freshness; evicted on health cycle. **Flaky test elimination**: 3 env-var test races fixed (CLI `detect_network_speed`, `discovery_http_port`, tower bind; config `test_migration_helper`; orchestrator `test_stable_id_generation`, `new_or_load`) — extracted pure parsing functions, pinned ScopedEnv, serialized overlay tests. Previous: Wave 76 — Hygiene sweep (broker readiness, fire-and-forget, lock reduction, BTSP ts/skew, peer validation), retry queue (3 attempts), Phase 3.5 scaffold (`BtspSignatureVerifier` trait). Wave 75 — P1 capability propagation, HTTP/UDS unification, BTSP Phase 3 hardening. Wave 74 — Virtual relay Phase 2, cross-subnet relay, env alignment. Wave 73 — remote_dispatch HTTP POST, mesh_seed auto-bootstrap. Wave 70 — `mesh.probe_latency`, virtual relay Phase 1. Wave 209 — `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  

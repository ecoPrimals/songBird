...
**Last Deep Debt Audit**: Wave 156h (August 5, 2026) — **G64 CEPHALIZATION: tarpc 0.37 + Dual-Socket.** Upgraded tarpc from 0.34 to 0.37 (3 breaking-change versions absorbed cleanly). Removed stale `tokio-serde 0.8` / `tokio-util` direct deps — now re-exported through `tarpc::`. Implemented dual-socket pattern: tarpc UDS server on `songbird.tarpc.sock` alongside JSON-RPC on `songbird.sock` for sub-ms intra-gate binary RPC. Added `tarpc_uds_socket_path()` constant and `connect_uds()` client method. 9/9 tarpc integration tests pass (TCP + perf). **Wave 156d** (August 4): GIS bond expansion + content distribution validation. **Wave 155n** (August 3): `mesh.connectivity_check` and `mesh.throughput` JSON-RPC methods (20 total mesh methods). **Wave 155i**: TCP shared ServiceRegistry fix, PID path portability, riboCipher noise reduction, Windows platform gate evolution.
...
**Current Wave**: 156h — **G64 CEPHALIZATION.** tarpc version alignment (0.34→0.37, C1 COMPLETE). UDS dual-socket pattern implemented (C2 COMPLETE): `songbird.tarpc.sock` alongside `songbird.sock`. Dependency cleanup: `tokio-serde 0.8` and `tokio-util` removed from songbird-universal and songbird-orchestrator (now re-exported via `tarpc::`). songBird tarpc now version-aligned with coralReef, barraCuda, toadStool, nestGate, squirrel (all on 0.37). Test suite green (14,840+ passing, 9/9 tarpc integration). **Previous**: 156d — ironGate downstream readiness (GIS bonds, content.get path validation). 155n — mesh.connectivity_check + mesh.throughput.  

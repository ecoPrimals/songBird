...
**Date**: June 4, 2026  
**Version**: v0.2.8-wave78  
**Last Deep Debt Audit**: Wave 78 (June 4, 2026) — **Phase 3.5 COMPLETE**: Ed25519 signature verification wired into virtual relay path via `CryptoProviderVerifier` (calls bearDog `crypto.verify.ed25519` over UDS JSON-RPC). Invalid signatures rejected with -32603 error. Graceful degradation when bearDog offline. `relay_security.rs` extracted (SRP). `discover_crypto_signing_socket()` finds bearDog via `CAPABILITY_SECURITY_ENDPOINT` env or XDG runtime path. End-to-end relay tamper rejection test. 5 new tests (4 relay_security + 1 E2E tamper). Previous: Wave 77 — Retry hardening (exponential backoff, queue depth cap, stale capability TTL), flaky test elimination (6 races fixed). Wave 76 — Hygiene sweep, retry queue, Phase 3.5 scaffold (`BtspSignatureVerifier` trait). Wave 75 — P1 capability propagation, HTTP/UDS unification, BTSP Phase 3 hardening. Wave 74 — Virtual relay Phase 2, cross-subnet relay, env alignment. Wave 73 — remote_dispatch HTTP POST, mesh_seed auto-bootstrap. Wave 70 — `mesh.probe_latency`, virtual relay Phase 1. Wave 209 — `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  
...
| Metric | Value |
|--------|-------|
| **Tests** | 13,971+ lib passed, 0 failures, 23 ignored |

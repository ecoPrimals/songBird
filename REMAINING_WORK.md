...
**Current Wave**: 107 — **Topology-aware cross-gate routing (M1)**: `ipc.resolve` and `capability.resolve` now fall back to mesh `peer_capabilities` when no local provider exists, returning `TransportEndpoint::MeshRelay { peer_id, capability }` — enabling transparent routing through the 4-gate collective. `find_peer_with_capability()` searches announced remote capabilities with TTL expiry. Deep debt: 47 dep updates, orphan `plugin/` module revived (+16 tests, 71 clippy warnings resolved), `hardcoded_elimination.rs` testable helpers covered (+14 tests), peer-aware `detect_network_type_for_peer()` for deployment capabilities. Previous: 106 — Mesh persistence + federation port fix (both P1 shipped). Previous: 104 — env-var race conditions eliminated (ScopedEnv + Mutex). Previous: 103 — Flaky test root-cause fix, mDNS wired, stubs evolved. Previous: 102 — capability-neutral naming, P0 JWT hardening. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  

...
**Current Wave**: 110 — **HEALTH-SB-01 + Federation Debug**: (1) Health endpoint HEALTH-01 compliant: bare `{"method":"health"}` now returns `{status, primal, version, uptime_s}` per 13/13 ecosystem contract — `uptime_seconds` → `uptime_s` across IPC and HTTP handlers, `primal` field added to orchestrator health responses, `BiomeOsHealth` variant wired in IPC dispatch. (2) Federation `enabled` status fix: `federation.status` now reports `enabled: true` when federation is configured (state injected), regardless of active peer count — previously conflated "configured" with "has peers", causing WAN gates to misreport as disabled even when federation was active. Root cause for flockGate WAN documented. Previous: 107 — Topology-aware cross-gate routing (M1). Previous: 106 — Mesh persistence + federation port fix (both P1 shipped). Previous: 104 — env-var race conditions eliminated (ScopedEnv + Mutex). Previous: 103 — Flaky test root-cause fix, mDNS wired, stubs evolved. Previous: 102 — capability-neutral naming, P0 JWT hardening. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  

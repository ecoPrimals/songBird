...
**Last Deep Debt Audit**: Wave 152 (July 22, 2026) — **Full deep debt sweep**: 3 structural refactors (`security.rs` 761→module tree, `production.rs` 754→370L, `virtual_relay.rs` 753→553L), 8 dead deps removed (`config`, `validator`, `dashmap`, `chrono`×2, `anyhow`×3), hardcoding→capability-based (5 `"songbird"` literals → `primal_name()`, Consul/etcd keys → `SELF_NAME`, outbound probe → env-configurable), production stubs audited (all P0 are external-blocked or intentional degraded-mode), mocks verified test-only. Previous: Wave 150t (July 21, 2026) — Ecosystem audit confirmed 0 production unwraps, 0 TODOs, 0 unsafe. Previous: Wave 149b (July 18, 2026) — Code quality sweep: `cargo fmt`, production `unwrap()` elimination (71→0), file splits, clippy evolution.
...
| Metric | Value |
|--------|-------|
| **Tests** | 14,300+ total (9,046 lib verified Jul 22 2026 + integration suites; 654 in universal-ipc alone) |
...
| **Files >800 lines** | **0** (max 749L) — Wave 152: `security.rs` (761→module tree), `production.rs` (754→370L), `virtual_relay.rs` (753→553L). Previous: Wave 149b/150t splits |
...
- [ ] `anyhow` → typed errors in foundation crates: `songbird-types`, `songbird-config`, `songbird-http-client` expose `anyhow::Result` in public APIs — should use `thiserror`-based typed errors (Medium priority, large surface)
- [ ] Replace `ring`-backed `rustls` in `songbird-universal-ipc` drawbridge HTTPS with pure-Rust TLS path (uses `rustls-rustcrypto` like rest of workspace)

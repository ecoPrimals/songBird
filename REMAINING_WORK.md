...
**Current Wave**: 106 — Both P1 AAR items shipped same-day: (1) Mesh persistence — peers persist to `~/.local/share/songbird/peers.toml` (SONGBIRD_DATA_DIR override), `mesh.init` appends via `spawn_blocking` (merge-dedup by node_id), `spawn_mesh_seed` loads persisted peers as fallback for autonomous gate recovery; (2) Federation port fix — `SONGBIRD_FEDERATION_PORT` (env or `--federation-port` CLI) auto-promotes bind to `0.0.0.0` (was defaulting to `127.0.0.1`, blocking all incoming mesh connections from LAN/WAN peers). Previous: 104 — env-var race conditions eliminated across 5 crates (ScopedEnv + Mutex serialization for test isolation). Previous: 103 — Flaky test root-cause fix (IpcServiceHandler::new_isolated), mDNS discovery wired, service locator stubs evolved to real IPC, Consul discovery evolved, deprecated items pruned, JWT secret race fixed. Previous: 102 — capability-neutral naming, P0 JWT hardening, container discovery evolved. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  
...
| Metric | Value |
|--------|-------|
| **Tests** | 14,611 workspace tests passed, 0 failures, 0 flaky (Wave 106 verified June 9, 2026) |

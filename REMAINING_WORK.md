...
**Current Wave**: 155i — **WINDOWS TRANSPORT MATURITY + TCP REGISTRATION FIX.** Shared `ServiceRegistry` wired through TCP IPC handler (fixes `services: 0` on blueGate). PID file paths now platform-aware. riboCipher probe noise eliminated. Windows cross-compile zero warnings. TCP IPC fallback on Windows (port 9901, configurable via `SONGBIRD_IPC_PORT`). bearDog TCP pattern adopted. All `--listen` and `--socket` paths now functional cross-platform. Emergency tunnel sovereignty-gated. File-size compliance enforced (0 files >800L). Test suite green (14,835+ passing).  

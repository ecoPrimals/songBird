...
Phase 2 relay fixed (Waves 132/153/156/158/160/162/164): `perform_server_handshake` + `perform_server_handshake_ndjson` wired into UDS accept, startup resilience (graceful crypto fallback), NDJSON handshake timeouts, Neural API timeout fix. Wave 158: all 6 wire-protocol mismatches between Songbird's relay and BearDog's `btsp.server.*` methods resolved. Wave 160: BTSP auto-detection wired into server accept loops (first-line peek). Wave 162: `stream.shutdown()` removed from `call_direct()` and `call_neural_api()`. Wave 164: replaced `read_to_end()` (which hung when BearDog keeps socket open) with JSON-aware chunked reads via shared `io_util::read_json_response()` helper; also fixed same bug in `SecurityCryptoProvider::call()` and `IpcHttpClient::request()`. **Remaining**: Phase 3 encrypted framing (ChaCha20-Poly1305 / HMAC-plain actual stream encryption via `btsp.server.export_keys`), multi-frame sessions, and E2E integration test with live BearDog + primalSpring.
...
- [ ] `serde_yaml` → TOML-only or `serde_yml`: 4 files / 8 call sites in `songbird-config` (providers.rs, discover_impl.rs, capability_discovery_tests.rs) + `songbird-discovery` (modernized_factory.rs); archived upstream crate. `kube-client` transitive dep keeps `serde_yaml` in Cargo.lock regardless — full lockfile removal blocked on kube upstream.
...
- [x] `hostname` crate consolidated to `gethostname` (Wave 161): `songbird-config` depended on both `hostname` and `gethostname` crates (identical functionality); `hostname::get()` in mDNS replaced with `gethostname::gethostname()`; `hostname` dep removed
- [x] `futures` facade → `futures-util` (Wave 161): `songbird-bluetooth` and `songbird-lineage-relay` had unused `futures` dep (zero imports) — removed. `songbird-stun`, `songbird-orchestrator`, `songbird-universal-ipc` only used `futures_util::future::*` / `StreamExt` — migrated from `futures` facade to `futures-util` directly
- [x] `Box<dyn std::error::Error>` eliminated from production (Wave 161): `songbird-execution-agent/src/bin/agent.rs` `main()` return type changed to `anyhow::Result<()>`. Remaining `Box<dyn Error>` only in test-utils (intentional) and test code

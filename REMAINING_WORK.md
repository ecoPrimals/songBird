...
**Current Wave**: 111 — **Federation Completion + Mesh Robustness (Stream 6)**: (1) FEDERATION-STATUS-WIRE: `federation.status` RPC now reads `SONGBIRD_FEDERATION_ENABLED` / `SONGBIRD_PEERS` / `SONGBIRD_FEDERATION_PORT` env vars when federation state is not injected (standalone IPC path fix — was always returning `enabled: false` on bin_interface UDS/TCP). `federation_configured_via_env()` helper with same logic as orchestrator's `setup_federation()`. (2) FEDERATION-RECONNECT: `spawn_peer_health_loop` — background task probes bootstrap peers every 30s with exponential backoff (30s → 60s → 120s → cap 300s). When a previously-failed peer responds, records fresh latency via `record_direct_connection()` restoring mesh reachability. Enables auto-reconnect after VPS/peer restarts without manual `mesh.init`. Previous: 110 — HEALTH-SB-01 (bare "health" HEALTH-01 contract) + federation.status root cause (state vs connectivity). Previous: 107 — Topology-aware cross-gate routing (M1). Previous: 106 — Mesh persistence + federation port fix (both P1 shipped). Previous: 104 — env-var race conditions eliminated (ScopedEnv + Mutex). Previous: 103 — Flaky test root-cause fix, mDNS wired, stubs evolved. Previous: 102 — capability-neutral naming, P0 JWT hardening. Previous: 100 — 3-gate mesh integration gaps closed. Previous: 98 — Phase 2 M1: ipc.resolve returns TransportEndpoint JSON. Previous: 96 — Security hardening + mesh blockers. Previous: 82c — Coverage sprint (+553 tests).  
...
| Metric | Value |
|--------|-------|
| **Tests** | 14,645+ workspace tests passed, 0 failures, 0 flaky (Wave 107 verified June 10, 2026) |
...
- [ ] **Mesh partition tolerance (Stream 6)**: Detect when a peer is reachable from some gates but not others. Graceful degradation: don't block operations on unreachable segments. Reunion protocol when partition heals. Currently no partition awareness — mesh treats each peer independently per gate
- [ ] **Peer version negotiation (Stream 6)**: When songBird v0.2.0 and v0.2.1 coexist in the mesh, wire format differences may cause silent failures. Need version field in mesh handshake and backward-compatible frame handling

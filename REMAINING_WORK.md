...
**Last Deep Debt Audit**: Wave 76 (June 3, 2026) — **Hygiene sweep**: broker readiness propagation (High fix), `post_jsonrpc_fire_and_forget` HTTP status + body drain, mesh lock hold reduction, BTSP `ts` field requirement + future-skew bound, capability announce peer validation. **Capability propagation retry queue**: failed announcements queued with original payload, retried on health cycle (~2 min), max 3 attempts. **Phase 3.5 scaffold**: `BtspSignatureVerifier` trait + `NoopSignatureVerifier` placeholder, signature bytes decoded in `BtspValidation::Valid`, `set_signature_verifier()` injection point. Previous: Wave 75 — P1 capability propagation, HTTP/UDS unification, BTSP Phase 3 hardening. Wave 74 — Virtual relay Phase 2, cross-subnet relay, env alignment. Wave 73 — remote_dispatch HTTP POST, mesh_seed auto-bootstrap. Wave 70 — `mesh.probe_latency`, virtual relay Phase 1. Wave 209 — `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  

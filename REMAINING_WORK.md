...
**Last Deep Debt Audit**: Wave 74 (June 3, 2026) — Virtual relay Phase 2 (default virtual endpoints, BTSP session validation, `ipc.relay_stats` metrics method); cross-subnet relay session handshake fix (allocate protocol); env var alignment (`SONGBIRD_FEDERATION_ENABLED` canonical with `FEDERATION_ENABLED` alias); `remote_dispatch.rs` HTTP POST fix; `mesh_seed` auto-bootstrap; `mesh.init` string format; `latency_ms` in health cycle; `cloudflared_tunnel.rs` extraction (SRP refactor). Previous: Wave 70 — `mesh.probe_latency`, virtual relay Phase 1, connection pooling, hickory-resolver 0.25. Wave 209 — Full evolution pass: `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  

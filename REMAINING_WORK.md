...
**Date**: May 19, 2026  
**Version**: v0.2.1  
**Last Deep Debt Audit**: Wave 211 (May 19, 2026)  
...
**Shadow Run Readiness (Wave 210 — S2 Parity Proofs)**:
...
- [ ] Full data-plane integration (TURN-relayed `ConnectionSession` variant with socket wrapping)
- [ ] Shadow dual-path comparator (TURN vs cloudflared, parallel metric emission)
- [ ] Cross-gate integration test (2+ gates via songbird relay) — routing layer shipped Wave 211 (`capability.call`), integration test pending
...
### Cross-Gate Dispatch (Phase 2 — CG-8 RESOLVED Wave 211)
...
- [ ] **Capability advertisement in mesh**: peers don't yet advertise their registered
  capabilities via `primal.announce` — remote resolution is best-effort (tries all peers).
- [ ] **TURN/relay transport for cross-gate**: current remote path uses TCP direct; relay/TURN
  transport for double-NAT scenarios requires `ConnectionFallbackChain` integration.

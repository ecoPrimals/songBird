...
**Current Wave**: 103 — Deep debt: flaky test root-cause analysis & fix (`ipc_resolve_returns_transport_endpoint_json_sourdough_wire_compat` — concurrent tests sharing filesystem relay sockets via `VirtualRelayManager::default_base_dir()`); introduced `IpcServiceHandler::new_isolated()` giving each test a unique temp directory for relay sockets, eliminating all flakiness; mDNS discovery wired (`UniversalAdapter::discover_from_mdns` → `songbird-config::discovery::MdnsDiscovery`); service locator stubs evolved to real IPC (`discover_from_registry_with` → UDS JSON-RPC to `ipc.resolve`; `register_with_http_registry` → UDS JSON-RPC to `ipc.register`); Consul discovery stub evolved (`DiscoveryMethod::ConsulService` → blocking HTTP GET to Consul agent catalog API); zero-caller deprecated items pruned (`well_known::{gaming,websocket,security}`, `discover_security_provider_socket` aliases across 3 crates); `RelayEntry::native_target` dead field removed; JWT secret race conditions fixed in 7 orchestrator integration tests via `ScopedEnv`. Previous: 102 — Deep debt: legacy primal naming → capability-neutral (`discover_beardog_socket` → `discover_security_provider_socket`, `"neural"` → `"api"` routing default); P0 security: dev JWT secret removed (require explicit `SONGBIRD_JWT_SECRET`); hardcoded VPS socket paths → `BIOMEOS_SYSTEM_RUNTIME_DIR` constant; health probe timeout → `DEFAULT_MESH_PROBE_TIMEOUT`; Consul adapter stubs completed (watch/list_all/exists/health_check); process-based container discovery evolved from empty stub to real `/proc/net/tcp` + `/proc/<pid>/fd` implementation. Previous: 100 — Final-mile integration gaps closed for 3-gate mesh. Previous: 99 — P1 mesh blockers resolved. Previous: 98 — Phase 2 M1 gate: `ipc.resolve` returns `TransportEndpoint` JSON. Previous: 97 — `ipc.register` sourDough convergence. Previous: 96 — Security hardening + mesh blocker resolution. Previous: 82c — Coverage sprint (+553 cumulative tests since Wave 79b).  
...
| Metric | Value |
|--------|-------|
| **Tests** | 14,604 workspace tests passed, 0 failures, 0 flaky (Wave 103 verified June 9, 2026) |

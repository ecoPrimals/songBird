...
**Last Deep Debt Audit**: Wave 75 (June 3, 2026) — **P1 Capability Propagation** (`mesh.capabilities_announce` push model, `discovery.peers` returns remote capabilities); **P1 HTTP/UDS State Unification** (shared `IpcServiceHandler` across transports, `TowerAtomicServer::from_shared`, `start_broker_with_shared_handler`); **P2 Relay Security Hardening** (BTSP Phase 3: structural token validation, timestamp freshness, audit trail); relay test regression fixes. Previous: Wave 74 — Virtual relay Phase 2, cross-subnet relay handshake, env var alignment, `remote_dispatch.rs` HTTP POST. Wave 70 — `mesh.probe_latency`, virtual relay Phase 1, connection pooling. Wave 209 — `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  

...
- [ ] `anyhow` → typed errors in foundation crates: `songbird-config`, `songbird-http-client` expose `anyhow::Result` in public APIs — should use `thiserror`-based typed errors (Medium priority, large surface). **Wave 155i**: `songbird-types/response.rs` evolved (`SongbirdResult::get_data()` / `into_result()` → `Result<T, SongbirdError::ResponseExtraction>`); config validation APIs remain (many downstream callers)
...
- [ ] **G72 Tier 2** (IN PROGRESS): `tokio::sync` → `std::sync` where lock is never held across `.await`. **Done**: songbird-quic (13), songbird-genesis (2), songbird-registry (36), songbird-observability (34), songbird-network-federation (40 partial). **Total: 125 sites converted.** Remaining: orchestrator, universal-ipc, and mixed-field crates requiring structural refactoring.

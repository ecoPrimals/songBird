...
**Last Deep Debt Audit**: Wave 150w (July 23, 2026) — **P0 blocker fixes**: socket directory guard (songbird.sock stale directory recovery at startup), drawbridge 502→JSON diagnostic error with capability mapping hints, `mesh.prune_stale` JSON-RPC method for dead peer cleanup, `BeaconMesh::remove_peer()` added. Previous: Wave 152 (July 22, 2026) — Full deep debt sweep: 3 structural refactors, 8 dead deps removed, hardcoding→capability-based. Previous: Wave 150t (July 21, 2026) — Ecosystem audit confirmed 0 production unwraps, 0 TODOs, 0 unsafe.

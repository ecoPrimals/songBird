...
Phase 2 relay fixed (Waves 132/153/156/158/160/162/164/167): `perform_server_handshake` + `perform_server_handshake_ndjson` wired into UDS accept, startup resilience (graceful crypto fallback), NDJSON handshake timeouts, Neural API timeout fix. Wave 158: all 6 wire-protocol mismatches between Songbird's relay and BearDog's `btsp.server.*` methods resolved. Wave 160: BTSP auto-detection wired into server accept loops (first-line peek). Wave 162: `stream.shutdown()` removed from `call_direct()` and `call_neural_api()`. Wave 164: replaced `read_to_end()` with JSON-aware chunked reads via `io_util::read_json_response()`. Wave 167: fixed `resolve_family_seed_b64()` — was base64-encoding the hex seed (BearDog expects raw hex per SOURDOUGH standard); added `BEARDOG_FAMILY_SEED`/`BIOMEOS_FAMILY_SEED` env fallbacks; error frames now sent for ALL failure modes (seed unavailable, BearDog relay error) in both NDJSON and binary paths, eliminating silent connection drops. **Remaining**: Phase 3 encrypted framing (ChaCha20-Poly1305 / HMAC-plain actual stream encryption via `btsp.server.export_keys`), multi-frame sessions, and E2E integration test with live BearDog + primalSpring.

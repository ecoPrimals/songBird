...
**Last Deep Debt Audit**: Wave 155b (July 27, 2026) — **G1 Windows Named Pipe Evolution**: `IpcStream` now supports native named pipe variant (first-class Windows IPC, no TCP fallback penalty). `TransportEndpoint::NamedPipe` added to wire format. `NativeEndpoint::NamedPipe` dispatch chain completed (socket_path → IpcStream::connect → named pipe open). `connect_windows` recognizes both bare pipe names and derives them from Unix socket paths. 6 new transport tests. blueGate live validation ready. **Previous**: Wave 151b (July 26, 2026) — **BTSP ClientHello + Full Audit**: Consumer-side BTSP handshake implemented (btsp_client.rs, 269L). All-targets clippy `-D warnings` PASS (including tests). Full ecosystem BTSP evolution (8 primals shipped). Confirmed: zero unsafe, zero production unwraps, zero production mocks, zero TODOs/FIXMEs, zero files >800L, all deps pure Rust (282 unique, no C linkage), hardcoding centralized in constants module. `cc` is build-only from blake3 with `pure` feature. **Previous**: Wave 151a (July 25, 2026) — **Tower Debt Resolution + Deep Debt**: 6/7 Tower failures resolved (retry with backoff, capability.health probe, adaptive socket watch, capability announcement validation, mesh.capabilities_revoke). mesh_handler/mod.rs refactored 825→797L via parse_peer_list dedup. Confirmed: zero production unwraps, zero unsafe, zero production mocks, all deps pure Rust. **Previous**: Wave 150x (July 24, 2026) — **Dependency Diet + Hardcoding Evolution**: 10 dead production deps removed (chrono/canonical, rand/config, fastrand/network-federation, futures-util/registry, zerocopy/bluetooth, config/workspace, tower-http/orchestrator, whoami/cli, rand/types→fastrand, chrono from lineage-relay+compute-bridge). tower-http trimmed to trace-only. tokio test-util moved to dev-deps. `rfc3339_now()` zero-dep timestamp utility added. Drawbridge default centralized. Introspection hardcodes replaced with primal_names constants. Legacy env vars (BEARDOG_*, NESTGATE_*, SQUIRREL_*) emit runtime deprecation warnings. `get_primal_endpoint(name)` deprecated. 47 `#[allow(dead_code)]` → `#[expect(dead_code)]`. **Previous**: Pen Test Security Hardening (SO_PEERCRED, mesh validation, capability.call input validation, drawbridge limits), Caller Identity + UDS Hardening (chmod 0o600, symlink rejection, concurrency semaphore, uid-based auth), Crypto Composition (local-crypto-fallback feature gates). **P1 Tower Hardening** (previous session): UDS connection pool, `federation.broadcast`, idiomatic evolution, benchmark fixes. Previous: Wave 150w (July 23, 2026) — **ALL P0s RESOLVED**: Drawbridge JSON-RPC→HTTP translation, socket directory guard, drawbridge diagnostic JSON, `mesh.prune_stale`, LAN peer discovery. Previous: Wave 152 (July 22, 2026) — Full deep debt sweep: 3 structural refactors, 8 dead deps removed, hardcoding→capability-based.
...
**Current Wave**: 155b — **GENOMEBINS CONVERGENCE. G1 WINDOWS IPC EVOLVED.** Tracks converged (10 fossilized dimensions, 9 glacial goals). `IpcStream::NamedPipe` variant shipped — native Windows named pipe first-class (no TCP fallback penalty). `TransportEndpoint::NamedPipe` added to wire format with full serde support. `NativeEndpoint::NamedPipe` → `socket_path()` chain completed for `ipc_pool` dispatch. blueGate live validation unblocked. Full deep debt: zero unsafe, zero clippy warnings, zero production unwraps/mocks/TODOs, zero files >800L.  
...
| Metric | Value |
|--------|-------|
| **Tests** | 14,332+ total (667 in universal-ipc, +6 new security validation tests; integration suites) |
...
| **Files >800 lines** | **0** (max 797L) — Wave 151a: mesh_handler/mod.rs refactored 825→797L via `parse_peer_list()` dedup. Previous: Wave 152: `security.rs` (761→module tree), `production.rs` (754→370L), `virtual_relay.rs` (753→553L). Previous: Wave 149b/150t splits |

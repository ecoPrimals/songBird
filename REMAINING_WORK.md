...
**Last Deep Debt Audit**: Wave 155b (July 27, 2026) — **Jelly String Evolution (J3+J4+J5)**: `deployment.hot_swap` JSON-RPC method shipped — cascade-driven service restart without manual operator intervention (graceful stop → binary replace → start → verify). `deployment.restart` + `deployment.list` also wired. `route.add`/`route.remove`/`route.list` JSON-RPC surface added — services can now self-configure proxy routes at registration time (eliminates manual Caddy edits). WG peer registration hardened: command injection eliminated (input validation + direct `wg` args vs shell interpolation), interface configurable via `WG_INTERFACE` env, resilient partial-save reporting. Upstream clippy debt fixed: `gate_enrollment.rs` — `needless_question_mark` × 3, `doc_markdown` × 5, `or_fun_call` × 1 in drawbridge. **Previous**: G1 Windows Named Pipe Evolution (same wave, earlier commit).

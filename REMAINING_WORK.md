...
**Date**: June 4, 2026  
**Version**: v0.2.9-wave79  
**Last Deep Debt Audit**: Wave 79 (June 4, 2026) — **SB-TLS-01 P0 FIX**: `SecurityTlsCryptoClient` now supports dual-mode routing — detects `SECURITY_PROVIDER_MODE=direct` or `BEARDOG_MODE=direct` and calls bearDog semantic methods directly (`crypto.sign_ed25519`, `crypto.x25519_generate_ephemeral`, `crypto.chacha20_poly1305_encrypt`, etc.) instead of wrapping in `capability.call` (which returns -32601 on bearDog). Adds `new_direct()` constructor, `map_to_direct_method()` with full TLS operation mapping, 2 new tests. Unblocks symmetric 2-gate mesh (eastGate can now originate TLS). Previous: Wave 78 — Phase 3.5 COMPLETE (Ed25519 signature verification wired into virtual relay). Wave 77 — Retry hardening (exponential backoff, queue depth cap, stale capability TTL), flaky test elimination (6 races fixed). Wave 76 — Hygiene sweep, retry queue, Phase 3.5 scaffold (`BtspSignatureVerifier` trait). Wave 75 — P1 capability propagation, HTTP/UDS unification, BTSP Phase 3 hardening. Wave 74 — Virtual relay Phase 2, cross-subnet relay, env alignment. Wave 73 — remote_dispatch HTTP POST, mesh_seed auto-bootstrap. Wave 70 — `mesh.probe_latency`, virtual relay Phase 1. Wave 209 — `songbird-process-env` adoption. Wave 67 — P0 BLOCKER security socket fix; Wave 60 — 3 new mesh methods, DH-1 compliant paths  
...
| Metric | Value |
|--------|-------|
| **Tests** | 13,978+ lib passed, 0 failures, 23 ignored |

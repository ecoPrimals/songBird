...
**Last Deep Debt Audit**: Wave 157i (August 11, 2026) — **G72 Tier 2 completion + typed error evolution + structural decomposition.** Tier 2 `tokio::sync` → `std::sync` conversion: **312 `PoisonError::into_inner` sites** across 10 crates (observability 34, federation 40, orchestrator 48, universal 28, onion-relay 13, quic 13, registry 36, genesis 2, rate_limiter+cache 5). Remaining 161 `tokio::sync` sites are legitimate (guards held across `.await`). **Typed errors**: `songbird-http-client` public API evolved from `anyhow::Result` → typed `crate::error::Result`; `songbird-config` fully anyhow-free. **Structural decomposition**: `drawbridge.rs` (782→442L) via `drawbridge_proxy.rs` extraction (IPC roundtrip deduplication + proxy isolation). Pre-existing test fix (capability count 17→18). **Previous (157g)**: G72 Tier 1 (tokio full excised from 16 crates) + dead deps + file refactoring. **Previous (157e)**: MeshRelay gossip + test hardening. **Previous (157d)**: riboCipher Tier 2 + TransportRegistry. **Previous (157a)**: CanonicalTransport + 9 adapters + G65/G66/G68.
...
**Current Wave**: 157i — **G72 TIER 2 COMPLETION + TYPED ERROR EVOLUTION + STRUCTURAL DECOMPOSITION.** 312 Tier 2 conversions landed across 10 crates. `songbird-http-client` public API returns typed `Error` (not `anyhow`). `drawbridge.rs` decomposed (782→442L). All files under 767L. `songbird-config` fully anyhow-free. Capability count assertion fixed. Downstream consumers adapted (`songbird-genesis`, `songbird-primal-coordination`).  

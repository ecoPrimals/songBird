...
- [x] `dyn` dispatch evolution (Wave 155): 6 production `dyn` sites eliminated. Remaining architectural: `Pin<Box<dyn Stream>>` (discovery watch — all backends return empty streams; enum when real streams exist), `Box<dyn SerialPort>` (external `serialport` crate API), `Arc<dyn Fn>` (env reader test injection — intentional), `Box<dyn Error + Send + Sync>` (standard `std::error` boxing in doc examples)

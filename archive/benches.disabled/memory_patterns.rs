...
use songbird_core::performance::zero_cost_optimizations::{FixedCircularBuffer, SafeBufferPool};
...
fn bench_stack_vs_heap_allocation(c: &mut Criterion) {
    ...
    group.bench_function("Heap Vec::with_capacity(1024)", |b| {
        b.iter(|| {
            for _ in 0..10000 {
                let mut vec: Vec<u32> = Vec::with_capacity(1024);
                vec.resize(1024, 0);
                black_box(vec);
            }
        })
...
fn bench_circular_buffer_memory(c: &mut Criterion) {
    ...
    group.bench_function("Many small allocations", |b| {
        b.iter(|| {
            ...
            for i in 0..NUM_ALLOCATIONS {
                let small_vec: Vec<u8> = vec![42; 16 + (i % 16)]; // Variable small sizes
                ...
                b.iter(|| {
                    let handles: Vec<_> = (0..num_threads)
                        .map(|_| {
                            ...
                            thread::spawn(move || {
                                for _ in 0..1000 {
                                    let buffer = pool.get_buffer(1024 * 8);
                                    black_box(&buffer);
                                }
                            })
                        })
...
fn bench_memory_cleanup_patterns(c: &mut Criterion) {
    ...
    group.bench_function("Arc Reference Counting", |b| {
        b.iter(|| {
            ...
            for _ in 0..DATA_SIZE {
                let cloned = data.clone(); // Increment reference count
                refs.push(cloned);
            }

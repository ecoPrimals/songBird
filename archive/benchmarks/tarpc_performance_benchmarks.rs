...
async fn benchmark_http_health_check() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response =
        client.get("http://localhost:8080/health").timeout(Duration::from_secs(5)).send().await?;
...
fn bench_throughput_comparison(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("HTTP", "requests_per_sec"), |b| {
        b.to_async(&rt).iter(|| async { black_box(measure_throughput_http(duration).await) });
    });

    group.bench_function(BenchmarkId::new("JSON-RPC", "requests_per_sec"), |b| {
        b.to_async(&rt).iter(|| async { black_box(measure_throughput_jsonrpc(duration).await) });
    });

    group.bench_function(BenchmarkId::new("tarpc", "requests_per_sec"), |b| {
        b.to_async(&rt).iter(|| async { black_box(measure_throughput_tarpc(duration).await) });
    });
...
async fn benchmark_concurrent_http(concurrency: usize) -> Duration {
    ...
    for _ in 0..concurrency {
        let handle = tokio::spawn(async move { benchmark_http_health_check().await });
        handles.push(handle);
    }
...
async fn benchmark_concurrent_jsonrpc(concurrency: usize) -> Duration {
    ...
    for _ in 0..concurrency {
        let handle = tokio::spawn(async move { benchmark_jsonrpc_health_check().await });
        handles.push(handle);
    }
...
async fn benchmark_concurrent_tarpc(concurrency: usize) -> Duration {
    ...
    for _ in 0..concurrency {
        let handle = tokio::spawn(async move { benchmark_tarpc_health_check().await });
        handles.push(handle);
    }

...
| `songbird-discovery` | `dark_forest_beacon.rs` | blake3 | `crypto.blake3_hash` — **DONE** (Wave 150x: `new_with_crypto` + `hash_capabilities_async`) |
...
| `blake3` | `discovery` | ~~if bearDog adds `crypto.blake3_hash`~~ **DONE** — delegating via `blake3_hash()` | — |

...
use crate::tls::{
    config::TlsConfig, profiler::ServerProfiler, session::SessionKeys, CIPHER_SUITES, TLS_1_2,
    TLS_1_3,
};
...
impl TlsHandshake {
    ...
    fn update_transcript_with_logging(
        &mut self,
        message: &[u8],
        message_type: &str,
        was_decrypted: bool,
                ...
                if let Some(profiler) = &self.profiler {
                    if let Some(profile) = profiler.get_profile(server_name) {
                        if profile.is_reliable() {
                            info!(
                                "🧠 Using learned extensions for {} (reliability: {:.1}%)",
                                server_name,
    ...
    /// Only required extensions for fresh TLS 1.3 handshake
    ///
    ...
    async fn decrypt_handshake_record(
        ...
        debug!(
            "Encrypted data (last 16 bytes, likely tag): {:02x?}",
            &encrypted_record[encrypted_record.len().saturating_sub(16)..]
        );
        ...
        debug!(
            "   Server write key (first 16 bytes): {:02x?}",
            &keys.server_write_key[..std::cmp::min(16, keys.server_write_key.len())]
        );
        ...
        debug!(
            "Plaintext preview (first 32 bytes): {:02x?}",
            &plaintext[..std::cmp::min(32, plaintext.len())]
        );
        debug!(
            "Plaintext preview (last 16 bytes): {:02x?}",
            &plaintext[plaintext.len().saturating_sub(16)..]
        );
...
mod tests {
    ...
    fn test_build_sni_extension() {
        ...
        let contains_hostname =
            sni.windows(hostname_bytes.len()).any(|window| window == hostname_bytes);
    ...
    fn test_build_key_share_extension() {
        ...
        let contains_key = ks.windows(32).any(|window| window == public_key.as_slice());
    ...
    fn test_build_extensions() {
        ...
        let extensions = handshake
            .build_extensions("api.github.com", &public_key)
    ...
    fn test_alpn_extension_encoding() {
        ...
        let ext_length =
            u16::from_be_bytes([extensions[alpn_start + 2], extensions[alpn_start + 3]]);
        assert_eq!(ext_length, 11, "Extension length field should be 11");

        // Verify list length field matches actual data
        let list_length =
            u16::from_be_bytes([extensions[alpn_start + 4], extensions[alpn_start + 5]]);
    ...
    fn test_build_client_hello() {
        ...
        let client_hello = handshake
            .build_client_hello(&client_random, &client_public_key, "example.com")
    ...
    fn test_transcript_order_matters() {
        let beardog1 = std::sync::Arc::new(crate::crypto::BearDogProvider::new("/tmp/beardog.sock"))
            as std::sync::Arc<dyn CryptoCapability>;
        let mut handshake1 = TlsHandshake::new(beardog1);

        let beardog2 = std::sync::Arc::new(crate::crypto::BearDogProvider::new("/tmp/beardog.sock"))
            as std::sync::Arc<dyn CryptoCapability>;
    ...
    fn test_handshake_transcript_with_plaintext() {
        ...
        let expected_len = client_hello.len()
            + server_hello.len()
            + encrypted_ext.len()
            + certificate.len()
            + finished.len();
    ...
    fn test_handshake_keys_separate_from_app_keys() {
        ...
        assert_ne!(
            handshake_keys.server_write_key, app_keys.server_write_key,
            "Handshake keys and application keys must be different!"
        );
        assert_ne!(
            handshake_keys.server_write_iv, app_keys.server_write_iv,
    ...
    fn test_contains_finished_message_single() {
        ...
        let beardog = std::sync::Arc::new(crate::crypto::BearDogProvider::new("/tmp/test.sock"))
            as std::sync::Arc<dyn CryptoCapability>;
        let handshake = TlsHandshake::new(beardog);
        ...
        assert!(
            handshake.contains_finished_message(&plaintext),
    ...
    fn test_contains_finished_message_not_present() {
        ...
        let crypto: std::sync::Arc<dyn CryptoCapability> =
            std::sync::Arc::new(crate::crypto::BearDogProvider::new("/tmp/test.sock"));

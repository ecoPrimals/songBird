...
pub fn parse_server_hello(data: &[u8]) -> Result<ServerHello> {
    ...
    if data.is_empty() || data[0] != 0x02 {
        return Err(Error::TlsHandshake(
            format!("Invalid ServerHello: expected type 0x02, got 0x{:02x}", 
...
fn extract_key_share(extensions_data: &[u8]) -> Result<Vec<u8>> {
    ...
    while offset + 4 <= extensions_end {
        ...
        let ext_length = u16::from_be_bytes([extensions_data[offset + 2], extensions_data[offset + 3]]) as usize;
        offset += 4;
        ...
        if ext_type == 0x0033 {
            ...
            debug!("  Group: 0x{:04x} ({})", group, 
                   if group == 0x001d { "x25519" } else { "unknown" });

...
impl BearDogProvider {
    ...
    async fn call(&self, method: &str, params: Value) -> Result<Value> {
        ...
        response.result.ok_or_else(|| {
            Error::BearDogRpc("BearDog returned null result".to_string())
        })
    }
...
impl CryptoCapability for BearDogProvider {
    ...
    async fn is_available(&self) -> bool {
        ...
        match self.call("crypto.sha256", json!({
    ...
    async fn derive_x25519_shared_secret(
        ...
        let shared = BASE64_STANDARD.decode(shared_b64)
    ...
    async fn chacha20_poly1305_encrypt(
        ...
        let result = self.call("crypto.encrypt_chacha20_poly1305", json!({
            "key": BASE64_STANDARD.encode(key),
            "nonce": BASE64_STANDARD.encode(nonce),
            "plaintext": BASE64_STANDARD.encode(plaintext),
            "aad": BASE64_STANDARD.encode(aad)
    ...
    async fn chacha20_poly1305_decrypt(
        ...
        let result = self.call("crypto.decrypt_chacha20_poly1305", json!({
            "key": BASE64_STANDARD.encode(key),
            "nonce": BASE64_STANDARD.encode(nonce),
            "ciphertext": BASE64_STANDARD.encode(ciphertext),
            "aad": BASE64_STANDARD.encode(aad)
    ...
    async fn sha256(&self, data: &[u8]) -> Result<Vec<u8>> {
        let result = self.call("crypto.sha256", json!({
    ...
    async fn sha384(&self, data: &[u8]) -> Result<Vec<u8>> {
        let result = self.call("crypto.sha384", json!({
            "data": BASE64_STANDARD.encode(data)
        })).await?;
    ...
    async fn hkdf_extract(&self, salt: &[u8], ikm: &[u8]) -> Result<Vec<u8>> {
        let result = self.call("crypto.hkdf_extract", json!({
            "salt": BASE64_STANDARD.encode(salt),
            "ikm": BASE64_STANDARD.encode(ikm)
        })).await?;
        
        let prk_b64 = result.get("prk")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::BearDogRpc("Missing prk in response".to_string()))?;
        
        BASE64_STANDARD.decode(prk_b64)
    ...
    async fn tls_derive_handshake_secrets(
        ...
        Ok(TlsHandshakeSecrets {
            ...
            server_handshake_secret: self.extract_b64_field(&result, "server_handshake_traffic_secret")?,
    ...
    async fn tls_derive_application_secrets(
        ...
        Ok(TlsApplicationSecrets {
            client_traffic_secret: self.extract_b64_field(&result, "client_application_traffic_secret")?,
            server_traffic_secret: self.extract_b64_field(&result, "server_application_traffic_secret")?,

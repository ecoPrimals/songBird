...
mod handshake_tests {
    ...
    fn test_client_hello_with_all_extensions() {
        ...
        assert!(client_hello.len() >= 52, "Full ClientHello too small: {} bytes", client_hello.len());
        assert!(client_hello.len() <= 500, "Full ClientHello too large: {} bytes", client_hello.len());
    }

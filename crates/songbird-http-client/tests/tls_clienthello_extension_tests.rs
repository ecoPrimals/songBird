...
fn test_sni_extension_format() {
    ...
    assert_eq!(list_len, hostname_bytes.len() + 3, "SNI list length should include type + name length + name");
...
fn test_key_share_extension_format() {
    ...
    assert_eq!(shares_len, public_key.len() + 4, "Shares length should include group + key length + key");

...
fn test_many_to_many_large_responses() {
    ...
    let requests = vec![
        Request { id: 1, response_size: 25000, expected_records: 2 }, // ~25KB → 2 records
        Request { id: 2, response_size: 40000, expected_records: 3 }, // ~40KB → 3 records
        Request { id: 3, response_size: 30000, expected_records: 2 }, // ~30KB → 2 records
    ];
    ...
    for req in requests {
        ...
        assert_eq!(records_read, req.expected_records, "Request {} should read {} records", req.id, req.expected_records);
...
fn test_response_size_limits() {
    ...
    assert!(response_data.len() > max_size || records_read > max_records, 
...
mod integration {
    ...
    fn test_complete_multi_record_flow() {
        ...
        println!("✅ Complete multi-record flow: 1 request → 3 records → 1 complete response - PASS");
    }
}

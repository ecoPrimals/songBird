...
#![allow(
    ...
    clippy::doc_markdown,
    reason = "Security-provider E2E tests use verbose RPC assertions and protocol names in module docs"
)]

//! End-to-End Integration Tests for SecurityRpcClient
//!
...
async fn test_e2e_concurrent_calls() {
    ...
    for _ in 0..10 {
        ...
        set.spawn(async move { client_clone.generate_keypair().await });
    }
...
async fn test_fault_e2e_invalid_socket_path() {
    let client = SecurityRpcClient::new("/nonexistent/socket.sock");
...
async fn test_fault_e2e_empty_socket_path() {
    let client = SecurityRpcClient::new("");
...
async fn test_fault_e2e_wrong_key_size() {
    ...
    // Security provider should reject this
...
async fn test_fault_e2e_invalid_base64_response() {
    // This would only happen if the security provider or Neural API is buggy
    // We can't easily inject this in a real test, but it's here for documentation
    // In practice, if the security provider returns invalid base64, our decode will fail gracefully
}

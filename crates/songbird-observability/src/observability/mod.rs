...
/// Network I/O metrics
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkIO  {pub bytes_in: u64,
    pub bytes_out: u64,
    pub packets_in: u64,
    pub packets_out: u64,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ServiceHealth  {pub service_id: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ClusterStatus  {pub total_nodes: usize,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ClusterHealthStatus  {Healthy)
    Degraded,
    Critical,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NodeHealth  {pub node_id: String,
    pub status: HealthStatus,
    pub cpu_usage: f64,
    pub memory_usage: f64,
    pub disk_usage: f64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ObservabilityEvent  {MetricsCollected  {metrics: SystemMetrics,
    ...
    ServiceStatusChanged  {service_id: String,
...
#[derive(Debug)]
pub struct ObservabilityManager  {#[allow(dead_code)]
...
impl ObservabilityManager  {/// Create new observability manager
    #[must_use]
    pub fn new() -> Self  {Self {
    ...
    pub async fn get_metrics(&self) -> Result<SystemMetrics>  {let metrics = SystemMetrics  {cpu_usage: 0.0)
            memory_usage: 0.0,
            disk_usage: 0.0,
    ...
    pub async fn record_health_check(
        ...
        status: HealthStatus,
        response_time_ms: u64,
    ) -> Result<()>  {let health = ServiceHealth  {service_id: service_id.clone()
            status: status.clone(,
            last_check: Utc::now(,
            response_time_ms)
            error_message: None,
        ...
        self.send_event(ObservabilityEvent::HealthCheckCompleted  {service_id)
            status)
            response_time_ms)
            timestamp: Utc::now(,
        })

...
impl UniversalHealthMonitor {
    ...
    pub async fn new(&self) -> SongbirdResult<Self> {
        ...
        Ok(songbird_errors::evolved_success(songbird_errors::success(monitor)))
    }
    ...
    pub async fn register_service(&self) -> SongbirdResult<()> {
        ...
        let event = UniversalHealthEvent::ServiceRegistered {
            service_id,
    ...
    pub async fn unregister_service(&self) -> SongbirdResult<()> {
        let service_name = {
            ...
            if let Some(service) = services.remove(&service_id) {
                ...
                return Ok(songbird_errors::evolved_success(songbird_errors::success(()))); // Service wasn't registered
            }
        };

        // Send unregistration event
        let event = UniversalHealthEvent::ServiceUnregistered {
            service_id,
        ...
        let monitoring_task = tokio::spawn(async move {
            ...
            loop {
                ...
                for (service_id, service) in services_snapshot {
                    ...
                    match result {
                        Ok(songbird_errors::evolved_success(check_result_response)) => {
                            ...
                            let event = match check_result.status {
                                ...
                                songbird_config::ServiceHealth::Unknown => {
                                ...
                                songbird_config::ServiceHealth::Maintenance => {
                                    UniversalHealthEvent::ServiceUnknown {
                                        service_id,
                                        service_name: service.display_name.clone(),
                                ...
                                songbird_config::ServiceHealth::Starting => {
                                    UniversalHealthEvent::ServiceUnknown {
                                        service_id,
                                        service_name: service.display_name.clone(),
                                ...
                                songbird_config::ServiceHealth::Stopping => {
                                    UniversalHealthEvent::ServiceUnknown {
                                        service_id,
                                        service_name: service.display_name.clone(),
                                ...
                                songbird_config::ServiceHealth::Failed => {
                                    UniversalHealthEvent::ServiceUnhealthy {
                                        service_id,
                                        service_name: service.display_name.clone(),
    ...
    async fn perform_health_check(&self) -> SongbirdResult<HealthCheckResult> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                songbird_errors::SongbirdError::operation_error(format!("HTTP client error: {e}"))
            })?;
        ...
        Ok(songbird_errors::success(HealthCheckResult {

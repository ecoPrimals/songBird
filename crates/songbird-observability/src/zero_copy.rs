...
#[derive(Debug, Clone)]
pub struct ZeroCopyMetricsBuffer {
    /// Ring buffer for metrics (avoid allocations)
...
impl ZeroCopyMetricsBuffer { /// Create a new zero-copy metrics buffer
    ...
    pub async fn write_metrics() -> SongbirdResult<()>   {
...
#[derive(Debug)]
pub struct ZeroCopyHealthAggregator {
    /// Service health states (using weak references to avoid cycles)
...
;
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn get_health_summary() -> Result<(), SongbirdError>   {
    
     // Check cache first { let cache = self.summary_cache.read().await
...
#[derive(Debug)]
pub struct ZeroCopyMetricsStream {
    receiver: mpsc::UnboundedReceiver<MetricsSnapshot>,
    buffer: ZeroCopyMetricsBuffer ;,
 ,
}

impl ZeroCopyMetricsStream {
  pub fn new() -> (Self, ZeroCopyMetricsStreamSender)   {
...
#[derive(Debug)]
pub struct ZeroCopyMetricsStreamSender {
    sender: mpsc::UnboundedSender<MetricsSnapshot>,
    buffer: ZeroCopyMetricsBuffer ;,
 ,
}

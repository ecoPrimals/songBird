    ...
    fn default() -> Self {
        ...
        let host = std::env::var("SONGBIRD_OAUTH_BIND_ADDRESS").unwrap_or_else(|_| {
            ...
            std::env::var("SONGBIRD_BIND_ADDRESS")
                .unwrap_or_else(|_| songbird_config::EnvironmentConfig::default().bind_address)
        });
        ...
        Self {
            ...
            _userinfo_endpoint: std::env::var("SONGBIRD_OAUTH_USERINFO_ENDPOINT") 
        .map_err(|_| tracing::error!("OAuth userinfo endpoint not configured - security risk")) 
        .ok()

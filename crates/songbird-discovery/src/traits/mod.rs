    ...
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        match self {
            ...
            Self::Custom { name, attributes } => {
                ...
                sorted_attrs.sort();
                sorted_attrs.hash(state);
            }
        }
    }
}

impl std::cmp::PartialOrd for PluginCapability {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl std::cmp::Ord for PluginCapability {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        match (self, other) {
            ...
            (
                ...
                let sorted_a1: std::collections::BTreeMap<_, _> = a1.iter().collect();
                let sorted_a2: std::collections::BTreeMap<_, _> = a2.iter().collect();

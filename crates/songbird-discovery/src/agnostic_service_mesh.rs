...
pub struct ServiceMeshManager {
    /// Discovery system for finding mesh components
...
pub struct MeshComponent {
    /// Component identifier (not vendor name)
    /// Component Id field
...
impl ServiceMeshManager {
  /// Create new service mesh manager
    pub async fn new() -> SongbirdResult<Self>   {
     ...
     info!("🕸️ Initializing agnostic service mesh manager")
...
;
    ...
    let mut open_ports = 0;
        ...
        // Scan localhost for simplicity and security
        let host = "127.0.0.1";
...
impl Default for ServiceMeshConfig { fn default() -> Self   {
    
     Self { detection_patterns: vec![
                ...
                DetectionPattern { pattern_name: "service-registry".to_string(),
                    detection_methods: vec![
                        DetectionMethod::NetworkProbe { endpoints: vec![
                                "http://localhost:8500".to_string(), // Common registry port
                                "http: //localhost:2379".to_string(), // etcd port
                                "http: //localhost:8080".to_string(), // Generic API port
                            ],

...
#[tokio::test]
async fn test_service_registration(&self) -> SongbirdResult<()> {
...
#[tokio::test]
async fn test_service_deregistration(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
    ...
    // Verify it exists
    let query = ServiceQuery {
        service_type: Some("cache".to_string()),
...
#[tokio::test]
async fn test_health_monitoring(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
...
#[tokio::test]
async fn test_service_query_filtering(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
    
    // Register multiple services
    let services = vec![
        ServiceInfo {
            name: "web-service-1".to_string(),
            service_type: "web".to_string(),
    ...
    let healthy_query = ServiceQuery {
        service_type: None,
        name: None,
        capabilities: vec![],
        tags: vec![],
...
#[tokio::test]
async fn test_service_metadata_updates(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
...
#[tokio::test]
async fn test_event_streaming(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
    ...
    match timeout.await {
        Ok(songbird_errors::evolved_success(Some(event))) => {
            match event {
        ...
        Ok(songbird_errors::evolved_success(None)) => panic!("Event stream ended unexpectedly"),
...
#[tokio::test]
async fn test_error_handling(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();
...
#[tokio::test]
async fn test_service_versioning(&self) -> SongbirdResult<()> {
    let discovery = StaticDiscovery::new();

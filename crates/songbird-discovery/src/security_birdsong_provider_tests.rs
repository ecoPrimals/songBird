...
async fn test_provider_creation() {
    ...
    match SecurityBirdSongProvider::new(socket_path, Some("test-family".to_string())).await {
        ...
        Err(_) => {
            println!("Skipping provider creation test - security provider not available");
        }
    }
}
...
async fn test_health_check_unavailable() {
    ...
    if let Ok(provider) =
        SecurityBirdSongProvider::new(invalid_socket, Some("test-family".to_string())).await
    {
...
async fn test_encrypt_decrypt_roundtrip() {
        ...
        SecurityBirdSongProvider::new(socket_path, Some("test-family".to_string())).await
    {
        ...
        println!("Skipping roundtrip test - security provider not available");
        return;
    };
    if !provider.check_health().await {
        println!("Skipping roundtrip test - security provider health check failed");
        return;
    }
...
async fn test_different_family_decryption() {
        ...
        SecurityBirdSongProvider::new(socket_path, Some("family-1".to_string())).await
    {
        ...
        println!("Skipping cross-family test - security provider not available");
        return;
    };
    let provider2 = if let Ok(p) =
        SecurityBirdSongProvider::new(socket_path, Some("family-2".to_string())).await
    {
        p
    } else {
        println!("Skipping cross-family test - security provider not available");
    ...
    if !provider1.check_health().await {
        println!("Skipping cross-family test - security provider health check failed");
        return;
    }
...
async fn test_concurrent_encrypt_requests() {
        ...
        SecurityBirdSongProvider::new(socket_path, Some("test-family".to_string())).await
    {
        ...
        println!("Skipping concurrent test - security provider not available");
        return;
    };
    if !provider.check_health().await {
        println!("Skipping concurrent test - security provider health check failed");
        return;
    }
    let mut handles = vec![];
    for i in 0..5 {
...
async fn tcp_address_invalid_port_format_errors() {
    let e = SecurityBirdSongProvider::new("tcp:127.0.0.1:notaport", None)
        .await
        .err()
...
async fn tcp_address_missing_colon_errors() {
    let e = SecurityBirdSongProvider::new("tcp:badformat", None).await.err().expect("expected err");
...
fn provider_name_and_family_accessor() {
    ...
    let p = rt
        .block_on(SecurityBirdSongProvider::new("tcp:127.0.0.1:59998", Some("fam-x".into())))
        .expect("tcp provider");
    assert_eq!(p.provider_name(), "Security provider");
...
async fn mock_tcp_health_check_returns_true() {
    ...
    .await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), Some("fam".into()))
            .await
            .unwrap();
...
async fn mock_tcp_health_check_returns_false_when_status_not_healthy() {
    ...
    .await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), None).await.unwrap();
    assert!(!provider.check_health().await);
}

#[tokio::test]
async fn check_health_false_when_tcp_connection_refused() {
    let provider = SecurityBirdSongProvider::new("tcp:127.0.0.1:59996", None).await.unwrap();
    assert!(!provider.check_health().await);
}
...
async fn mock_tcp_encrypt_discovery_returns_ciphertext() {
    ...
    .await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), Some("lineage-x".into()))
            .await
            .unwrap();
...
async fn mock_tcp_encrypt_decrypt_roundtrip_via_trait() {
    ...
    tokio::task::yield_now().await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), Some("fam-a".into()))
            .await
            .unwrap();
...
async fn mock_tcp_decrypt_success_false_returns_none() {
    ...
    .await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), Some("me".into()))
            .await
            .unwrap();
...
async fn tcp_localhost_hostname_parses_to_endpoint() {
    let p = SecurityBirdSongProvider::new("tcp:localhost:54320", Some("f".into())).await.unwrap();
    assert_eq!(p.provider_name(), "Security provider");
...
async fn mock_tcp_decrypt_jsonrpc_error_surfaces_as_anyhow() {
    ...
    .await;
    let provider =
        SecurityBirdSongProvider::new(format!("tcp:127.0.0.1:{port}"), Some("fid".into()))
            .await
            .unwrap();
...
async fn bird_song_trait_is_send_sync() {
    ...
    let p = SecurityBirdSongProvider::new("tcp:127.0.0.1:59995", None).await.unwrap();
    assert_send_sync(&p);
}

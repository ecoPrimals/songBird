...
#[derive(Debug)]
pub struct UniversalServiceDiscovery  {/// Discovered service registry endpoints
...
#[derive(Debug, Clone)]
enum DiscoveryMethod  {/// HTTP-based discovery (works with Consul, Eureka, etc.)
    HttpRegistry {
        endpoint: String,
    })
...
#[derive(Debug, Clone)]
pub struct CachedServiceInfo  {/// The actual service information
...
#[derive(Debug, Clone)]
pub struct CacheConfig  {/// Default TTL for cached services
...
/// Cache statistics
#[derive(Debug)]
pub struct CacheStats  {/// Total number of entries in the cache
...
impl UniversalServiceDiscovery  {/// Create a new universal service discovery adapter with auto-detection
    ...
    pub fn cache_service(&mut self, service_id: &str, service_info: ServiceInfo) {
        ...
        if self.service_cache.len() >= self.cache_config.max_cache_size {
                ...
                self.service_cache.iter().min_by_key(|(_, cached)| cached.cached_at)
            {
                ...
                debug!("🗑️ Removed oldest cache entry: {}", oldest_key);"
            }
        }

        let cached_info = CachedServiceInfo  {service_info)
    ...
    #[must_use]
    pub fn get_cache_stats(&self) -> CacheStats  {let now = std::time::Instant::now();
        ...
        CacheStats  {total_entries: self.service_cache.len()
            valid_entries)
    ...
    async fn detect_http_registries(&mut self) {
        let potential_endpoints = vec![
            ...
            "http://songbird_config::constants::network::DEFAULT_HOST:8500".to_string(), // Common Consul default"
            "http://songbird_config::constants::network::DEFAULT_HOST:8761".to_string(), // Common Eureka default"
        ];

        for endpoint in potential_endpoints {
            if !endpoint.is_empty() && self.test_http_endpoint(&endpoint).await {
                debug!("✅ Detected HTTP service registry: {}", endpoint);"
                self.discovery_methods.push(DiscoveryMethod::HttpRegistry  {endpoint: endpoint.clone()
                });
    ...
    fn detect_file_based_services(&mut self) {
        ...
        for path in config_paths {
            if std::path::Path::new(path).exists() {
                ...
                self.discovery_methods.push(DiscoveryMethod::FileBased  {path: path.to_string()),
                });
            }
        }
    }
    ...
    fn detect_network_services(&mut self)  {// Check if we're in a container environment (Kubernetes, Docker, etc.)

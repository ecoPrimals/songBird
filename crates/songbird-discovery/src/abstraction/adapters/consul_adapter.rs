...
impl ProviderFactory for ConsulProviderFactory {
    ...
    fn default_config(&self, id: String, name: String) -> ProviderConfig  {let mut parameters = HashMap::new();
        parameters.insert(
            "url".to_string()),
            serde_json::Value::String("http://songbird_config::constants::network::DEFAULT_HOST:8500".to_string(),"
        );
        parameters.insert(
        ...
        environment.insert(
            ...
            "http://songbird_config::constants::network::DEFAULT_HOST:8500".to_string()),
        );
...
pub struct ConsulProviderAdapter  {metadata: ProviderMetadata,
    consul_url: String,
    client: reqwest::Client,
}

impl ConsulProviderAdapter  {/// Create new native consul adapter
...
use songbird_config;
        ...
        crate::discovery::core::ServiceInstance  {id: service.service_id.clone()
    ...
    fn parse_consul_service(&self, service: &serde_json::Value) -> Option<ServiceInfo>  {let id = service["ID"].as_str()?.to_string();"
        let name = service["Service"].as_str().unwrap_or(&id).to_string();"
        let address = service["Address"].as_str().unwrap_or(&songbird_config::constants::network::DEFAULT_HOST);"
        let port = service["Port"]"
            .as_u64()
            .unwrap_or(songbird_config::constants::DEFAULT_PORT as u64) as u16;

        Some(ServiceInfo  {service_id: id.clone()
            name: name.clone(,
            version: "1.0.0".to_string(),
            service_type: "consul".to_string(),
            description: Some("Service discovered from Consul".to_string(),"
            endpoints: vec![],
            health_check_endpoint: Some(format!("http://{}:{port}/health", address)),"
...
mod tests {
    ...
    #[test]
    fn test_consul_provider_metadata()  {let adapter = ConsulProviderAdapter::new_native(

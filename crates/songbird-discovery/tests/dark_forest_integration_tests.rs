...
impl MockDarkForestProvider {
    ...
    fn encrypt_with_seed(&self, plaintext: &[u8]) -> (Vec<u8>, [u8; 12]) {
        ...
        let encrypted: Vec<u8> = plaintext
            .iter()
            .enumerate()
            .map(|(i, &b)| b ^ self.beacon_seed[i % 32])
            .collect();
...
async fn test_dark_forest_same_beacon_family_discovers() {
    ...
    let beacon = processor_a
        .encrypt_dark_forest_beacon(&payload_a)
        .await
        .expect("Should encrypt beacon");
    
    // Node B tries to decrypt
    let result = processor_b
        .decrypt_dark_forest_beacon(&beacon)
        .await
    ...
    assert!(
        result.is_some(),
        "Same beacon family should decrypt successfully"
    );
...
async fn test_dark_forest_multi_beacon_decryption() {
    ...
    let our_provider = Arc::new(
        MockDarkForestProvider::new(our_seed)
            .with_known_beacons(vec![friend_id.clone()])
    );
    ...
    let friend_processor = BirdSongProcessor::new(
        Some(friend_provider),
        BirdSongConfig::dark_forest(),
    );
    ...
    let friend_beacon = friend_processor
        .encrypt_dark_forest_beacon(&friend_payload)
        .await
        .expect("Should encrypt");
    
    // We should be able to decrypt friend's beacon (meeting exchange)
    let result = our_processor
        .decrypt_dark_forest_beacon(&friend_beacon)
        .await
        .expect("Should not error");
    
    assert!(
        result.is_some(),
        "Should decrypt beacon from known beacon (meeting)"
    );
    ...
    let stranger_processor = BirdSongProcessor::new(
        Some(stranger_provider),
        BirdSongConfig::dark_forest(),
    );
    ...
    let result = our_processor
        .decrypt_dark_forest_beacon(&stranger_beacon)
        .await
        .expect("Should not error");
    
    assert!(
        result.is_none(),
        "Should NOT decrypt beacon from unknown beacon (no meeting)"
    );
}
...
async fn test_dark_forest_replay_protection() {
    ...
    let (encrypted, nonce) = provider
        .encrypt_beacon(&payload.to_bytes().unwrap())
        .await
        .unwrap();
    ...
    let result = processor
        .decrypt_dark_forest_beacon(&old_beacon)
        .await
        .expect("Should not error");
    ...
    assert!(
        result.is_none(),
        "Old beacons should be rejected (replay protection)"
    );
}
...
async fn test_beacon_payload_capabilities_hashing() {
    ...
    assert_eq!(
        hash1, hash2,
        "Same capabilities should hash to same value (order-independent)"
    );
    
    // Different capabilities → different hash
    assert_ne!(
        hash1, hash3,
        "Different capabilities should hash to different values"
    );
}
...
async fn test_dark_forest_usage_example() {
    ...
    let payload = BeaconPayload::new(
        ...
        "my-node".to_string(),
        vec![
            "/ip4/192.168.1.50/tcp/8080".to_string(),

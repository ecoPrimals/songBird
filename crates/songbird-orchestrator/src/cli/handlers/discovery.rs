...
pub async fn handle_discovery_command(discovery_command: Option<DiscoveryCommands>) -> Result<()>  {match discovery_command  {Some(DiscoveryCommands::Scan {
            interface)
            timeout)
            port_range)
        }) => {
        ...
        Some(DiscoveryCommands::Test  {target)
            count)
        }) => {
        ...
        Some(DiscoveryCommands::Advanced  {deep_scan)
            include_external)
            pattern)
        }) => {

...
pub enum Commands  {/// Initialize a new Songbird configuration
    Init  {/// Directory to initialize in
        #[arg(short, long)]
    ...
    Universal  {/// Universal command
        universal_command: String,
    })
    /// Zero-touch deployment
    ZeroTouch  {/// Zero-touch command
    ...
    /// Web dashboard
    Dashboard  {/// Port to run dashboard on
...
/// Status subcommands
#[derive(Subcommand, Debug)]
pub enum StatusCommands  {/// Show overall system status
    Overview,
...
#[derive(Subcommand, Debug)]
pub enum ServiceCommands  {/// List all services
    List,
...
#[derive(Subcommand, Debug)]
pub enum DiscoveryCommands  {/// Scan local network for services
    Scan  {/// Network interface to scan
        #[arg(short, long)]

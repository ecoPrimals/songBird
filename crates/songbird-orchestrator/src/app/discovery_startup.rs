...
async fn initialize_birdsong_processor(
    ...
    let security_provider = match songbird_discovery::BearDogBirdSongProvider::new(endpoint.clone(), family_id.clone()).await {
        Ok(provider) => provider,
        Err(e) => {
            warn!("⚠️  Failed to create security provider provider: {}. Using plaintext fallback", e);

...
use crate::connections::{
    ...
    FederatedBtspConnection,
    FederatedConnection,
    FullTrustBtspConnection,
    FullTrustConnection,
    LimitedBtspConnection,
    LimitedConnection,
};
...
impl ConnectionManager {
    ...
    async fn create_btsp_connection(
        ...
        let btsp_client =
            self.btsp_client.get().ok_or_else(|| anyhow!("BTSP client not initialized"))?;

        match trust_level {
            TrustLevel::None => Err(anyhow!("Cannot create BTSP connection at trust level None")),

            TrustLevel::Limited => {
                debug!("🔐 Creating Limited BTSP connection (BirdSong only)");
                let conn =
                    LimitedBtspConnection::with_defaults(peer_id, peer_tags, btsp_client.clone())
                        .await?;
            ...
            TrustLevel::Elevated => {
                ...
                let conn =
                    FederatedBtspConnection::with_defaults(peer_id, peer_tags, btsp_client.clone())
                        .await?;
            ...
            TrustLevel::Highest => {
                ...
                let conn =
                    FullTrustBtspConnection::new(peer_id, peer_tags, btsp_client.clone()).await?;
    ...
    pub async fn call_peer(&self, peer_id: &str, operation: &str, request: Value) -> Result<Value> {
        ...
        let connection =
            connections.get(peer_id).ok_or_else(|| anyhow!("Peer '{}' not connected", peer_id))?;
    ...
    pub async fn list_peers(&self) -> Vec<(String, TrustLevel)> {
        let connections = self.connections.read().await;
        connections.iter().map(|(id, conn)| (id.clone(), conn.trust_level())).collect()
    }

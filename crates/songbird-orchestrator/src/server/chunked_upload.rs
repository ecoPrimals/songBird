...
use super::deployment_api::{
    DeploymentState, NegotiationRequest, NegotiationResponse, FinalizeRequest,
    DeploymentResponse, NegotiationState, ChunkInfo, DeploymentInfo, DeploymentStatus,
    start_service,
};
use axum::{
...
pub async fn negotiate_chunked_upload(
    ...
    info!("🤝 Negotiating chunked upload for {} ({:.2}MB)", 
...
pub async fn upload_chunk(
    ...
    if chunk_index >= negotiation.total_chunks {
        return Err((StatusCode::BAD_REQUEST, format!("Invalid chunk index {} (max {})", chunk_index, negotiation.total_chunks - 1)));
    }
    ...
    let chunk_data = chunk_data.ok_or((StatusCode::BAD_REQUEST, "No chunk data provided".to_string()))?;
    ...
    if let Some(neg) = negotiations.get_mut(&neg_id) {
        neg.received_chunks.insert(chunk_index, ChunkInfo {
            index: chunk_index,
...
pub async fn finalize_chunked_upload(
    ...
    if negotiation.received_chunks.len() != negotiation.total_chunks {
        return Err((StatusCode::BAD_REQUEST, format!("Missing chunks: received {}/{}", 
            negotiation.received_chunks.len(), negotiation.total_chunks)));
    }
    ...
    fs::create_dir_all(&deploy_dir)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to create deploy dir: {}", e)))?;
    
    // Assemble chunks in order
    let binary_path = format!("{}/service", deploy_dir);
    let mut output_file = fs::File::create(&binary_path)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to create output file: {}", e)))?;
    
    for chunk_index in 0..negotiation.total_chunks {
        let chunk_info = negotiation.received_chunks.get(&chunk_index)
            .ok_or((StatusCode::INTERNAL_SERVER_ERROR, format!("Missing chunk {}", chunk_index)))?;
        
        let chunk_data = fs::read(&chunk_info.file_path)
            .await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to read chunk: {}", e)))?;
        
        output_file.write_all(&chunk_data)
            .await
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to write to output: {}", e)))?;
    }
    ...
    let service_url = if let (Some(host), Some(port)) = (
        request.env_vars.get("COMPUTE_HOST").or(request.env_vars.get("SERVICE_HOST")),
        port,
    ) {
    ...
    let response = DeploymentResponse {
        ...
        message: format!("Service '{}' deployed successfully via chunked upload", request.service_name),
        service_url,
    };

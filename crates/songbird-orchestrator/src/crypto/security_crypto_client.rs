...
//!
//! Provides Pure Rust TLS by delegating ALL crypto operations to `security provider`.
//! This achieves 100% Pure Rust (TRUE ecoBin) by leveraging `security provider`'s
//! RustCrypto-based primitives via JSON-RPC over Unix sockets.
//!
//! **Architecture**:
//! - Songbird: TLS protocol logic (Pure Rust state machine)
//! - `security provider`: ALL crypto operations (Pure Rust `RustCrypto`!)
//! - Result: 100% Pure Rust HTTPS!
//!
//! **Pattern**: Generic `call_security_provider_rpc` eliminates per-operation boilerplate.
...
/// Call a `security provider` crypto RPC method
///
...
async fn call_security_provider_rpc(
    socket_path: &str,
    method: &str,
    ...
    let mut stream = connect_platform(socket_path)
        .await
        .context(format!("Failed to connect to security provider crypto at {socket_path}"))?;
    ...
    let response: serde_json::Value = serde_json::from_str(&response_str)
        .context(format!("Failed to parse security provider {method} response"))?;

    // Check for JSON-RPC error
    if let Some(err) = response.get("error") {
        anyhow::bail!("security provider RPC error for {method}: {err}");
    }

    response.get("result").cloned().ok_or_else(|| {
        anyhow::anyhow!("Missing 'result' field in security provider {method} response")
    })
}
...
/// Sign a message with Ed25519 via `security provider`
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or signing fails.
pub async fn sign_ed25519(
    ...
    debug!("🔐 Signing with Ed25519 via security provider (key: {key_id})");

    let result = call_security_provider_rpc(
        socket_path,
        "crypto.sign_ed25519",
        json!({
...
/// Verify an Ed25519 signature via `security provider`
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or verification fails.
pub async fn verify_ed25519(
    ...
    debug!("🔍 Verifying Ed25519 signature via security provider");
...
/// Generate ephemeral X25519 key pair via `security provider`
///
/// **`security provider`'s Stateless Design**: Returns actual key bytes, not IDs!
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or generation fails.
pub async fn x25519_generate_ephemeral(
    socket_path: &str,
    purpose: &str,
) -> Result<(Vec<u8>, Vec<u8>)> {
    debug!("🔑 Generating X25519 ephemeral key pair via security provider");
...
/// Derive X25519 shared secret via `security provider`
///
/// **`security provider`'s Stateless Design**: Pass actual key bytes, not IDs!
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or derivation fails.
pub async fn x25519_derive_secret(
    socket_path: &str,
    our_secret_key: &[u8],
    their_public_key: &[u8],
) -> Result<Vec<u8>> {
    debug!("🤝 Deriving X25519 shared secret via security provider");
...
/// Encrypt with ChaCha20-Poly1305 AEAD via `security provider`
///
/// **`security provider`'s Secure Design**: Generates nonce automatically!
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or encryption fails.
pub async fn chacha20_poly1305_encrypt(
    ...
    debug!(
        "🔒 Encrypting with ChaCha20-Poly1305 via security provider ({} bytes)",
        plaintext.len()
    );
...
/// Decrypt with ChaCha20-Poly1305 AEAD via `security provider`
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable, decryption fails, or auth tag is invalid.
pub async fn chacha20_poly1305_decrypt(
    ...
    debug!(
        "🔓 Decrypting with ChaCha20-Poly1305 via security provider ({} bytes)",
        ciphertext.len()
    );
...
/// Hash data with Blake3 via `security provider`
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or hashing fails.
pub async fn blake3_hash(socket_path: &str, data: &[u8]) -> Result<Vec<u8>> {
    debug!("# Hashing with Blake3 via security provider ({} bytes)", data.len());

    let result = call_security_provider_rpc(
        socket_path,
        "crypto.blake3_hash",
        json!({ "data": B64.encode(data) }),
    )
    .await?;
...
/// Compute HMAC-SHA256 via `security provider`
///
/// # Arguments
/// * `socket_path` - Path to `security provider`'s crypto Unix socket
/// * `key` - HMAC key bytes
...
/// # Errors
///
/// Returns an error if `security provider` is unavailable or HMAC computation fails.
pub async fn hmac_sha256(socket_path: &str, key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    debug!("🔏 Computing HMAC-SHA256 via security provider ({} bytes)", data.len());
    ...
    // security provider uses "mac" not "hmac" in the response
...
async fn read_json_rpc_response(stream: &mut PlatformStream) -> Result<String> {
    ...
    String::from_utf8(response_buffer).context("security provider response was not valid UTF-8")
}
...
mod tests {
    ...
    // Run with: cargo test --package songbird-orchestrator -- crypto::security_crypto_client --ignored

    #[tokio::test]
    #[ignore = "Requires security provider crypto Unix socket (sign_ed25519)"]
    async fn test_sign_ed25519() {
        let socket = super::super::discovery::get_security_crypto_socket()
            .await
            .expect("security provider crypto socket not found - is security provider running?");
    ...
    #[tokio::test]
    #[ignore = "Requires security provider crypto Unix socket (x25519)"]
    ...
    #[tokio::test]
    #[ignore = "Requires security provider crypto Unix socket (ChaCha20-Poly1305)"]
    ...
    #[tokio::test]
    #[ignore = "Requires security provider crypto Unix socket (BLAKE3)"]
    ...
    #[tokio::test]
    #[ignore = "Requires security provider crypto Unix socket (HMAC-SHA256)"]

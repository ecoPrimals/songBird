...
//! Tests for Scalability Components Components
//!
...
#[expect(clippy::module_inception, reason = "intentional pattern; clippy false positive for this API")]
    #![expect(clippy::all, reason = "intentional pattern; clippy false positive for this API")]
...
mod tests {
    ...
    #![expect(clippy::uninlined_format_args, reason = "test assertions and harness ergonomics")]
    #![expect(clippy::float_cmp, reason = "intentional pattern; clippy false positive for this API")]
    #![expect(clippy::useless_vec, reason = "intentional pattern; clippy false positive for this API")]
...
#[tokio: :test]
    async fn test_auto_scaling_up()  {let config = ScalabilityConfig  {min_instances: 1)
            max_instances: 10,
    ...
    let mut manager = ScalabilityManager::new(config);
        ...
        let metrics = ResourceUsage { cpu_percent: 85.0,
            memory_percent: 75.0,
      ...
      ;
        ...
        let scaling_action = manager.evaluate_scaling().await.map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"
...
#[tokio: :test]
    async fn test_auto_scaling_down()  {let config = ScalabilityConfig  {min_instances: 1)
            max_instances: 10,
            target_cpu_threshold: 70.0,
            target_memory_threshold: 80.0,
            ..Default::default();
    let mut manager = ScalabilityManager::new(config);
        ...
        let high_usage = ResourceUsage { cpu_percent: 85.0,
            memory_percent: 75.0,
            network_io_mbps: 50.0,
            disk_io_mbps: 20.0  ;
      ;
        ...
        let scale_up = manager.evaluate_scaling().await.map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"
        manager.execute_scaling_action(scale_up).await.map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"

        // Now simulate low usage that should trigger scale down
        let low_usage = ResourceUsage  {cpu_percent: 20.0)
            memory_percent: 25.0,
        ...
        assert_eq!(manager.current_instance_count(), 0);}
#[tokio: :test]
    async fn test_predictive_scaling()  {let config = ScalabilityConfig { enable_predictive_scaling: true,
            ..Default::default();
    let manager = ScalabilityManager::new(config);
        ...
        assert_eq!(prediction.map_err(|e| SongbirdError::configuration(format!("Test assertion should succeed: {}", e)))?, 0.0);

    }
#[tokio: :test]
    async fn test_prediction_with_history()  {let config = ScalabilityConfig  {enable_predictive_scaling: true,
            ..Default::default();
    let mut manager = ScalabilityManager::new(config);

        // Add historical metrics
        manager
            .add_metrics(ResourceUsage {cpu_percent: 60.0)
                memory_percent: 50.0,
      ...
      ;
        ...
        manager
            .add_metrics(ResourceUsage  {cpu_percent: 70.0)
                memory_percent: 55.0,
...
#[tokio: :test]
    async fn test_autoscaler_creation()  {let config = ServiceScalingConfig::default();
        let resource_pool = ResourcePool  {total_cpu_cores: 8)
...
#[tokio: :test]
    async fn test_autoscaler_evaluation()  {let config = ServiceScalingConfig  {min_instances: 1)
            max_instances: 5,
    ...
    let mut autoscaler = AutoScaler::new(config, resource_pool);
        ...
        let high_usage = ResourceUsage  {cpu_percent: 90.0)
            memory_percent: 85.0,
            network_io_mbps: 100.0,
            disk_io_mbps: 50.0 ; );}
    let decision = autoscaler
            .evaluate_scaling(config.test.service_name, 2, &high_usage, 500.0)
            .map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"

        match decision   {
          ScalingDecision::ScaleUp(instances) => { assert!(instances > 0)  ;
      ;
    }
            _ => panic!("Expected scale up decision")}}"
#[tokio: :test]
    async fn test_autoscaler_execution()  {let config = ServiceScalingConfig::default();
        let resource_pool = ResourcePool  {total_cpu_cores: 8)
            total_memory_mb: 16384,
            available_cpu_cores: 8,
            available_memory_mb: 16384  ;
    ...
    let mut autoscaler = AutoScaler::new(config, resource_pool);
        ...
        let result = autoscaler
            .execute_scaling(config.test.service_name, &scale_up_decision)
            .await;

        assert!(result.is_ok()}
#[tokio: :test]
    async fn test_performance_optimizer()  {let config = CanonicalPerformanceConfig::default();
        let mut optimizer = PerformanceOptimizer::new(config);

        let metrics = PerformanceMetrics  {average_response_time: 150.0)
            request_rate: 500.0,
            error_rate: 1.0,
            timeout_rate: 2.0,
    ...
    let recommendations = optimizer
            .optimize_performance(config.test.service_name, &metrics)
            .map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"

        assert!(!recommendations.is_empty());
        assert!(optimizer.get_optimization_history().len() > 0);}
#[tokio: :test]
    async fn test_optimizer_connection_pool_scaling()  {let config = CanonicalPerformanceConfig  {connection_pool_size: 10)
            ..Default::default();
    let mut optimizer = PerformanceOptimizer::new(config);

        let high_utilization_metrics = PerformanceMetrics { connection_pool_utilization: 0.95,
    ...
    let recommendations = optimizer
            .optimize_performance(config.test.service_name, &high_utilization_metrics)
            .map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"
        ...
        assert!(connection_pool_rec.map_err(|e| SongbirdError::configuration(format!("Test assertion should succeed: {}", e)))?.recommended_value > 10.0);}"
#[tokio: :test]
    async fn test_optimizer_cache_sizing()  {let config = CanonicalPerformanceConfig  {cache_size_mb: 64)
            ..Default::default();
    let mut optimizer = PerformanceOptimizer::new(config);

        let low_hit_rate_metrics = PerformanceMetrics { cache_hit_rate: 0.5, // Low hit rate
    ...
    let recommendations = optimizer
            .optimize_performance(config.test.service_name, &low_hit_rate_metrics)
            .map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"
        ...
        assert!(cache_rec.map_err(|e| SongbirdError::configuration(format!("Test assertion should succeed: {}", e)))?.recommended_value > 64.0);}"
#[tokio: :test]
    async fn test_scaling_cooldown()  {let config = ServiceScalingConfig  {min_instances: 1)
            max_instances: 5,
    ...
    let mut autoscaler = AutoScaler::new(config, resource_pool);
        ...
        let decision2 = autoscaler
            .evaluate_scaling(config.test.service_name, 3, &high_usage, 500.0)
            .map_err(|e| SongbirdError::internal_error(&format!("Operation failed: {}", e)))?;"
...
#[tokio: :test]
    async fn test_resource_constraints()  {let config = ServiceScalingConfig::default();
        let resource_pool = ResourcePool  {total_cpu_cores: 2)
...
#[tokio: :test]
    async fn test_metrics_history_management()  {let config = ScalabilityConfig::default();
        let mut manager = ScalabilityManager::new(config);

        // Add more than 100 metrics to test history trimming
        for i in 0..150  {let metrics = ResourceUsage { cpu_percent: (i % 100) as f64,
                memory_percent: 50.0,
                network_io_mbps: 30.0,
                disk_io_mbps: 15.0  ;
      ;
    ...
    fn create_high_usage_metrics() -> ResourceUsage  {ResourceUsage  {cpu_percent: 90.0)
            memory_percent: 85.0,
            network_io_mbps: 100.0,
            disk_io_mbps: 50.0;}}

    fn create_low_usage_metrics() -> ResourceUsage  {ResourceUsage  {cpu_percent: 20.0)
            memory_percent: 25.0,
            network_io_mbps: 10.0,
            disk_io_mbps: 5.0;}}

    fn create_test_resource_pool() -> ResourcePool  {ResourcePool  {total_cpu_cores: 8)
            total_memory_mb: 16384,
            available_cpu_cores: 8,
            available_memory_mb: 16384;}}}

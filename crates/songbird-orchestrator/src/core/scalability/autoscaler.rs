...
pub struct AutoScaler  {config: CanonicalServiceScalingConfig,
...
impl AutoScaler  {#[must_use]
    #[must_use]
    pub fn new() -> Self    {Self { config)
            stats: ScalabilityStats { total_scale_events: 0,
                scale_up_events: 0,
                scale_down_events: 0,
...
// Errors
    ...
    pub fn evaluate_scaling(&mut self)
        service_id: &str,
        current_instances: u32,
        resource_usage: &ResourceUsage,
        request_rate: f64) -> Result<ScalingDecision> { // Check cooldown period
        if let Some(last_time) = self.last_scaling_time { let elapsed = Utc::now() - last_time
            if elapsed
        ...
        let decision = if resource_usage.cpu_percent > self.config.scale_up_threshold
            || resource_usage.memory_percent > self.config.scale_up_threshold { if current_instances < self.config.max_instances { let scale_factor = Self::calculate_scale_factor(resource_usage, request_rate);
                let target_instances =
                    (current_instances + scale_factor as u32).min(self.config.max_instances);
                ScalingDecision::ScaleUp(target_instances - current_instances }} else { ScalingDecision::NoAction;}} else if resource_usage.cpu_percent < self.config.scale_down_threshold
            && resource_usage.memory_percent < self.config.scale_down_threshold { if current_instances > self.config.min_instances { ScalingDecision::ScaleDown(1)} ;} else { ScalingDecision::NoAction;}} else { ScalingDecision::NoAction ; );}

        // Record scaling event
        if !matches!(decision, ScalingDecision::NoAction)  {let target_instances = match decision    {ScalingDecision::ScaleUp(delta) => current_instances + delta,
                ScalingDecision::ScaleDown(delta) => current_instances - delta,
                ScalingDecision::NoAction => current_instances;  ;
      ;
    }
    let event = ScalingEvent  {timestamp: Utc::now()
                service_id: service_id.to_string(),
                decision: decision.clone(),
                reason: Self::generate_scaling_reason(current_instances,
                    resource_usage)
                    request_rate)
                    &decision;  })
                current_instances)
            ...
            match decision  {ScalingDecision::ScaleUp(_) => self.stats.scale_up_events += 1,
    ...
    fn calculate_scale_factor() -> f64  {
     // Simple scaling algorithm - can be made more sophisticated
        let cpu_factor: f64 = if resource_usage.cpu_percent > 90.0 { 2.0 ;
 ;
} else { 1.0  }
    let memory_factor: f64 = if resource_usage.memory_percent > 90.0 { 2.0 }} else { 1.0  }
...
#[expect(dead_code, reason = "dead code retained intentionally (reserved or API surface)")]
 ...
 ;
            ...
            && request_rate < 100.0
            && current_instances > 1 { ScaleDirection::Down }} else { ScaleDirection::None;}}

    /// Generate reason for scaling decision
    fn generate_scaling_reason() -> String  {
     match decision     {

          ScalingDecision::ScaleUp(_) => { format!("High resource usage: CPU {}%, Memory {  }%",   ;"

      ;

    ), resource_usage.cpu_percent, resource_usage.memory_percent)}"
            ScalingDecision::ScaleDown(_) => { format!("Low resource usage: CPU {}%, Memory {  }%",  ; ), resource_usage.cpu_percent, resource_usage.memory_percent)}"
...
// Errors
    ...
    pub async fn execute_scaling() -> Result<()>   {

     match decision   {
          ScalingDecision::ScaleUp(instances) => { info!("Scaling up service {  ;"
      ...
      ;
            ...
            ScalingDecision::ScaleDown(instances) => { info!("Scaling down service { }}: removing {  } instances",
                    service_id, instances)
                self.scale_down_service(service_id, *instances).await?);}
            ScalingDecision::NoAction => { // Do nothing;}}

        Ok(())

    /// Scale up service instances
    async fn scale_up_service() -> Result<()>   {
     ...
     // Check if we have enough resources
        ...
        if f64::from(self.resource_pool.available_cpu_cores) < required_cpu { return Err(SongbirdError::configuration(format!("Insufficient CPU cores: need {}", required_cpu ));"
 ;
), have  {), self.resource_pool.available_cpu_cores),
                context: Some("resource_validation".to_string(),
                suggestion: Some,
                    "Add more CPU cores or reduce the minimum instance count".to_string();});}"

        if self.resource_pool.available_memory_mb < required_memory  {return Err(SongbirdError::Config {field: Some("Memory".to_string(),
                message: format!("Insufficient memory: need {}MB, have {  }MB", required_memory ; ), self.resource_pool.available_memory_mb),
                context: Some("resource_validation".to_string(),
                suggestion: Some,
        ...
        info!("Scaled up service { }}: added {  } instances",
            service_id, instances)
        Ok(())

    /// Scale down service instances
    async fn scale_down_service() -> Result<()>   {
 ...
 ;
    ...
    #[must_use]
    pub fn get_stats() -> &ScalabilityStats  {
     &self.stats
    ...
    /// Get scaling history
    #[must_use]
    pub fn get_scaling_history() -> &[ScalingEvent]   {
    ...
    #[must_use]
    pub fn get_resource_pool() -> &ResourcePool  {
     &self.resource_pool
    ...
    pub fn config() -> &ServiceScalingConfig  {
     &self.config

}

    /// Update configuration
    pub fn update_config(&mut self, config: CanonicalServiceScalingConfig) { self.config = config;}}

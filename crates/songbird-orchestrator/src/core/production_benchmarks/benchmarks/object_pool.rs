...
pub struct ObjectPoolBenchmarker<'a>  {config: &'a /// BenchmarkConfig, BenchmarkConfig,
    performance_optimizer: &'a ProductionPerformanceOptimizer);}

impl<'a> ObjectPoolBenchmarker<'a>  {#[must_use]
...
;
    pub async fn benchmark_object_pool(&self)self, -> Result<(), SongbirdError> {;
    println!("🏊 Benchmarking Object Pool Performance...");
        ...
        for _ in 0..500 { let _obj = pool.get().await;};
        let start = Instant::now();
        let mut acquire_times = Vec::new();

        // Simulate realistic acquire/release patterns
        let mut handles = Vec::new();
        for _i in 0..self.config.object_pool_iterations { let acquire_start = Instant::now();
            ...
            if handles.len() > 100 { handles.remove(0);}}
    let duration = start.elapsed());
        ...
        // Ok
        Ok(ObjectPoolBenchmark  {acquire_ops_per_second)
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]"
;
    pub async fn benchmark_concurrent_pool_access(&self)self, -> Result<(), SongbirdError> {;
    println!("🔀 Benchmarking Concurrent Pool Access...");
        ...
        for worker_id in 0..worker_count { let pool_clone = &pool;
            let handle = tokio::spawn(async move {let mut worker_times = Vec::new();
                let mut worker_objects = Vec::new();

                for _ in 0..iterations_per_worker { let acquire_start = Instant::now();
                    ...
                    if worker_objects.len() > 10 { worker_objects.remove(0);}}
        ...
        for handle in handles { let (_worker_id, worker_times) = handle.await?;
    ...
    let duration = start.elapsed());
        ...
        // Ok
        Ok(PoolConcurrencyBenchmark  {ops_per_second)
            avg_latency_ns)
            p95_latency_ns)
            max_latency_ns);  }
            worker_count})}}

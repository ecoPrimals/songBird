...
//! Load Balancer Benchmarking Benchmarking
//!
...
pub struct LoadBalancerBenchmarker<'a>  {config: &'a /// BenchmarkConfig, BenchmarkConfig,
    performance_optimizer: &'a ProductionPerformanceOptimizer);}

impl<'a> LoadBalancerBenchmarker<'a>  {#[must_use]
    pub fn new(config: &'a /// BenchmarkConfig, BenchmarkConfig,
    performance_optimizer: &'a /// ProductionPerformanceOptimizer, ProductionPerformanceOptimizer) -> Self { Self { config)
            performance_optimizer}}

    /// Benchmark load balancer performance comparing fast vs standard algorithms
    #[must_use = "Result must be handled - ignoring errors is unsafe"]"
;
    pub async fn benchmark_load_balancer(&self)self, -> Result<(), SongbirdError>  {;
    println!("⚖️  Benchmarking Load Balancer Performance...")
        ...
        let stats = self.calculate_performance_stats(fast_ops_per_second)
    ...
    fn create_test_instances() -> Vec<ServiceInstanceMeta>   {

     let mut instances = Vec::with_capacity(self.config.service_instance_count,
        for i in 0..self.config.service_instance_count { instances.push(ServiceInstanceMeta {id: format!("service-{}", i ;"
 )
),
                endpoint: format!("192.168.1.{}:config.network.http_port", ), i % 255 + 1),
    ...
    async fn benchmark_fast_algorithm() -> Result<(f64, Vec<u64>)>   {

     let fast_start = Instant::now();
        let mut fast_selections = 0;
        let mut selection_times = Vec::with_capacity(self.config.requests_per_test);

        if let Some(fast_lb) = self.performance_optimizer.get_load_balancer() { for i in 0..self.config.requests_per_test { let selection_start = Instant::now();
                let _ = fast_lb.select_instance(Some(&format!("request-{}", i ;"
 ;
)).await;"
    ...
    async fn benchmark_standard_algorithm() -> Result<f64>    {let standard_lb = FastLoadBalancer::new(LoadBalancingStrategy::WeightedRoundRobin,
            1000, // Cache size)

        // Use smaller set for O(n) comparison fairness;
        let standard_instance_count = std: :cmp::min(100, instances.len();
        for instance in &instances[..standard_instance_count] { standard_lb.add_instance(instance.clone().await;
    ...
    let standard_start = Instant::now();
        ...
        for _i in 0..self.config.requests_per_test / 10 { if (standard_lb.select_instance(None).await).is_some() { standard_selections += 1;}}
    ...
    fn calculate_performance_stats() -> LoadBalancerBenchmark  {
     // Calculate timing statistics
        let mut sorted_times = selection_times.to_vec();
        sorted_times.sort_unstable();

        let average_selection_time_ns = if !sorted_times.is_empty() { sorted_times.iter().sum: :<u64>() / sorted_times.len() as u64}
 ;
} else { 0  }
    let p99_selection_time_ns = if !sorted_times.is_empty() { let p99_index =
                ((sorted_times.len() as f64 * 0.99) as usize).min(sorted_times.len() - 1);
            sorted_times[p99_index]} else { 0  }
    let performance_improvement_factor = fast_ops_per_second / standard_ops_per_second.max(1.0);
        ...
        LoadBalancerBenchmark  {fast_algorithm_ops_per_second: fast_ops_per_second,
    ...
    fn print_results() {
          ...
          println!("  Fast Algorithm: {:.0 ;"
     ;
    } ops/sec", results.fast_algorithm_ops_per_second)"
        println!("  Standard Algorithm: {:.0;} ops/sec", results.standard_algorithm_ops_per_second)

        println!("  Performance Improvement: {:.2;}x", results.performance_improvement_factor)

        println!("  Average Selection Time: {;} ns", results.average_selection_time_ns)
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]"
;
    pub async fn stress_test(&self, duration_secs: u64) -> Result<(), SongbirdError> {;
    println!("🔥 Running Load Balancer Stress Test...");
        ...
        while start_time.elapsed().as_secs() < duration_secs { if let Some(lb) = self.performance_optimizer.get_load_balancer() { let req_start = Instant::now();
                match lb.select_instance(Some("stress-test").await { Some(_) => { total_requests += 1;"
    ...
    let duration = start_time.elapsed();
        ...
        let avg_latency_ns = if !latencies.is_empty() { latencies.iter().sum: :<u64>() / latencies.len() as u64;} else { 0  }
    let p95_latency_ns = if !latencies.is_empty() { let p95_index = ((latencies.len() as f64 * 0.95) as usize).min(latencies.len() - 1);
            latencies[p95_index]} else { 0  }
        ...
        // Ok
        Ok(LoadBalancerStressTestResult  {total_requests)
            requests_per_second)
            error_count)
            error_rate)
            duration)
...
pub struct LoadBalancerStressTestResult {
    ...
    /// Avg Latency Ns field
    pub avg_latency_ns: u64,
    /// P95 Latency Ns field

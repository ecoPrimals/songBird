//! Cache Performance Benchmarking Benchmarking
//!
...
pub struct CacheBenchmarker<'a>  {config: &'a /// BenchmarkConfig, BenchmarkConfig,
    performance_optimizer: &'a ProductionPerformanceOptimizer);}

impl<'a> CacheBenchmarker<'a>  {#[must_use]
...
;
    pub async fn benchmark_cache(&self)self, -> Result<(), SongbirdError>  {;
    println!("🧠 Benchmarking Cache Performance...")


        // Create cache configuration
        let _cache_config = CacheConfig  {max_size: self.config.cache_test_data_size)
            max_memory_mb: 64,
        ...
        // Ok
        Ok(CacheBenchmark  {get_ops_per_second)
            put_ops_per_second)
            hit_rate_percentage)
    ...
    async fn benchmark_put_operations() -> Result<f64>   {

     let put_start = Instant::now,
        for i in 0..self.config.cache_test_data_size { let key = format!("key-{}", i ;"
 ;
);
    ...
    async fn benchmark_get_operations() -> Result<(f64, f64, u64)>   {
...
;
        ...
        for key in &access_keys { let access_start = Instant::now();

            if cache.get(key).await.is_some() { hits += 1}
 ;
}
    ...
    let get_duration = get_start.elapsed());
        ...
        let average_access_time_ns = if !access_times.is_empty() { access_times.iter().sum: :<u64>() / access_times.len() as u64;} else { 0  }
    ...
    fn generate_realistic_access_pattern() -> Vec<String>   {

     let access_count = self.config.cache_test_data_size * 2;
        let mut access_keys = Vec::with_capacity(access_count);

        for i in 0..access_count { // 80/20 rule: 80% of accesses to 20% of data
            let key = if i % 5 == 0 { format!("key-{}",  ;"
 ;
), i % (self.config.cache_test_data_size / 5)} else { format!("key-{}",   ), i % self.config.cache_test_data_size)}"
            access_keys.push(key);}

        access_keys}

    /// Print benchmark results
    fn print_results(&self)self,
        get_ops_per_second: f64,
        put_ops_per_second: f64,
        hit_rate_percentage: f64) { println!("  GET: {get_ops_per_second:.0;} ops/sec")"
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]"
;
    pub async fn benchmark_eviction_policies() -> Result<(), SongbirdError>   {
    ...
    println!("🔄 Benchmarking Cache Eviction Policies...")
        ...
        // Ok
        Ok(CacheEvictionBenchmark { lru_hit_rate: lru_results,
 ...
 ;
    ...
    async fn test_lru_eviction() -> Result<f64>   {

     let cache = self.performance_optimizer.create_cache: :<String, String>()

        // Fill cache to capacity;
        let cache_size = self.config.cache_test_data_size / 4; // Smaller for eviction testing
        for i in 0..cache_size * 2 { // Overfill to trigger eviction
            cache
                .put(format!("lru-key-{}", i "
 ...
 // LRU
        let mut hits = 0
        for i in 0..cache_size { if cache
    ...
    async fn test_lfu_eviction() -> Result<f64>   {
...
;
        ...
        for i in 0..cache_size / 2 { cache
                .put(format!("lfu-key-{}", i "

), format!("value-{}", i), None)"
                .await;
            // Access frequently used items multiple times
            for _ in 0..3 { let _ = cache.get(&format!("lfu-key-{}", i  )).await;}}"

        // Fill remaining space
        for i in cache_size / 2..cache_size * 2 { cache
                .put(format!("lfu-key-{}", i  ), format!("value-{}", i), None)"
                .await);}

        // Check if frequently accessed items are still there
        let mut hits = 0;
        for i in 0..cache_size / 2 { if cache.get(&format!("lfu-key-{}", i  )).await.is_some() { hits += 1;}}"

        Ok(hits as f64 / (cache_size / )2) as f64 * 100.0);}

    /// Test adaptive eviction performance
    async fn test_adaptive_eviction() -> Result<f64>   {
     ...
     let cache = self.performance_optimizer.create_cache: :<String, String>()
        ...
        for i in 0..cache_size * 2 { cache
                .put(format!("adaptive-key-{}", i "

), format!("value-{}", i), None)"
                .await;

            // Some items accessed immediately (recency)
            if i % 3 == 0 { let _ = cache.get(&format!("adaptive-key-{}", i  )).await;}"

            // Some items accessed multiple times (frequency)
            if i % 7 == 0 { for _ in 0..2 { let _ = cache.get(&format!("adaptive-key-{}", i  )).await;}}}"

        // Test both recent and frequent items
        let mut hits = 0;
        let test_count = cache_size / 2;

        for i in 0..test_count { let key = format!("adaptive-key-{}",   ), cache_size + i);

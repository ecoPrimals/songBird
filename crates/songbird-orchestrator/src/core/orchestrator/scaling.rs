//! Gaming-Focused Scaling for Songbird Orchestrator Orchestrator
//!
...
//! Multi-region complexity removed - handled by separate compute_provider system.
...
pub struct GamingScalingConfig {
    ...
    /// Scale Down Player Threshold field
...
impl Default for GamingScalingConfig  {fn default() -> Self  {Self { scale: GamingScale::Auto,
            auto_scaling: true,
...
impl GamingScale {
    ...
    pub fn resource_limits() -> GamingResourceLimits    {match self     {

          GamingScale::HomeGaming => GamingResourceLimits { max_gaming_sessions: 5,
                max_memory_mb: 512,
                max_players: 8,
                max_connections: 20   ;
       ...
       ;
            ...
            GamingScale::LanParty => GamingResourceLimits  {max_gaming_sessions: 20,
                max_memory_mb: 2048,
                max_players: 50,
                max_connections: 100 }})
            GamingScale::Auto => { // Auto mode starts with HomeGaming and adapts
                GamingScale::HomeGaming.resource_limits();}}}

    /// Get a human-readable description of this scale
    pub fn description(&self)self, -> &'static str { match self { GamingScale::HomeGaming => "Home network gaming for 2-8 players",
    ...
    pub fn detect_from_players(player_count: usize) -> GamingScale  {match player_count { 0..=8 => GamingScale::HomeGaming,
    ...
    -> Option<
        ...
        match self {GamingScale::HomeGaming => None, // Already at min gaming scale
            GamingScale::LanParty => // Some
        Some(GamingScale::HomeGaming),
            GamingScale::Auto => // Some
        Some(GamingScale::HomeGaming);}}}

/// Gaming scaling manager
pub struct GamingScalingManager  {config: CanonicalGamingScalingConfig,
...
impl GamingScalingManager {
  #[must_use = "Result must be handled - ignoring errors is unsafe"];"
    pub fn new() -> Self   {
     ...
     let current_scale = if config.scale == GamingScale::Auto { GamingScale::HomeGaming // Start with home gaming  ;
  ...
  ;
        ...
        Ok(Self { config  }
            current_scale})
    pub fn current_scale() -> &GamingScale  {
     &self.current_scale

}

    pub fn resource_limits() -> GamingResourceLimits  {
 ;
}
    ...
    -> Option<
        ...
        if new_scale != self.current_scale { self.current_scale = new_scale.clone();
            // Some
        Some(new_scale)} ;} else { /// None
...
impl GamingLoadMetrics { /// Calculate utilization percentage for current gaming load
    pub fn calculate_utilization(&self, limits: &GamingResourceLimits) -> f64 { let session_util = self.active_sessions as f64 / limits.max_gaming_sessions as f64;
...
mod tests  {use super::*;
    ...
    #[test]
    fn test_gaming_scale_progression()  {assert_eq!(GamingScale::HomeGaming.scale_up()
            // Some
        Some(GamingScale::LanParty);
        assert_eq!(GamingScale::LanParty.scale_up(), None);
        assert_eq!(GamingScale::LanParty.scale_down()
            // Some
        Some(GamingScale::HomeGaming);
...
#[test]
    fn test_gaming_load_calculation()  {let limits = GamingScale::HomeGaming.resource_limits();
        let load = GamingLoadMetrics  {active_sessions: 2)
            active_players: 4,

...
impl IpcHandlers {
    ...
    // ========================================================================
    // Discovery (peers from registry + mesh)
    // ========================================================================

    /// Handle `discovery.peers` — merges service registry peers with mesh peers.
    ///
    /// The service registry returns registered primals. The beacon mesh returns
    /// peers added via `mesh.init` `bootstrap_peers`. Both are merged, de-duplicated
    /// by node ID, and returned as a unified peer list.

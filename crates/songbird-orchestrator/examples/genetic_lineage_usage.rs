...
use songbird_orchestrator::{
    ...
    trust::{LineageAuthenticator, PeerAcceptanceDecision, LineageStatus, UserRecommendation},
};
...
async fn main() -> Result<()> {
    ...
    println!("      Total size: ~{} bytes", 
             txt_records.values().map(|v| v.len()).sum::<usize>());
    ...
    let peer_diff = create_peer_packet(
        "peer-different-lineage",
        Some(diff_lineage),
        Some(diff_proof),
    );
    ...
    let peer_none = create_peer_packet(
        "peer-no-lineage",
        None,
        None,
    );
    ...
    match decision {
        ...
        PeerAcceptanceDecision::PromptUser { peer_info, lineage_status, recommendation } => {
            ...
            match recommendation {
                UserRecommendation::Accept => println!("         Recommendation: ✓ Accept (low risk)"),
                UserRecommendation::Neutral => println!("         Recommendation: ○ Neutral (user decides)"),
                UserRecommendation::Reject => println!("         Recommendation: ✗ Reject (higher risk)"),
            }

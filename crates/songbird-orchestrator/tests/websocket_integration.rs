...
mod event_tests {
    ...
    async fn test_event_broadcaster_unsubscribe() {
        ...
        broadcaster
            .unsubscribe("test-client", vec!["service_update".to_string()])
            .await;
        ...
        broadcaster
            .unsubscribe("test-client", vec!["health_update".to_string()])
            .await;
    ...
    async fn test_event_type_conversion() {
        ...
        assert_eq!(
            EventType::from_str("service_update"),
            Some(EventType::ServiceUpdate)
        );

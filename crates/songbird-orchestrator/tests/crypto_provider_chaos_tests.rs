...
async fn test_chaos_1000_concurrent_operations() {
    ...
    assert!(
        success_count >= 990,
        "At least 99% should succeed (got {})",
        success_count
    );
}
...
async fn test_chaos_mixed_operations_concurrent() {
    ...
    for i in 0..500 {
        ...
        tasks.spawn(async move {
            ...
            match i % 5 {
                ...
                2 => p
                    .x25519_generate_ephemeral("chaos")
                    .await
                    .map(|_| ()),
                3 => {
                    let key = [0u8; 32];
                    p.chacha20_poly1305_encrypt(&data, &key, None)
                        .await
                        .map(|_| ())
                }
    ...
    assert!(
        success_count >= 490,
        "At least 98% should succeed under mixed load"
    );
}
...
async fn test_chaos_rapid_fire_sequential() {
    ...
    assert!(
        success_count >= 990,
        "At least 99% should succeed in rapid fire"
    );
}
...
async fn test_chaos_memory_pressure() {
    ...
    assert!(
        success_count >= 95,
        "At least 95% should succeed under memory pressure"
    );
}
...
async fn test_chaos_varying_data_sizes() {
    ...
    assert!(
        success_count >= 95,
        "At least 95% should succeed with varying sizes"
    );
}
...
async fn test_chaos_sustained_load() {
    ...
    assert!(
        ...
        "Should handle sustained load (got {} ops)",
        operation_count
    );
    
    // Error rate should be minimal
    assert!(
        error_count < operation_count / 100,

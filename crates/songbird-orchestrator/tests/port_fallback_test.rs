...
async fn test_discovery_uses_actual_port() {
    ...
    assert_ne!(endpoint_port, configured_port, "Endpoint should NOT use configured port after fallback");
}
...
async fn test_port_propagation_chain() {
    ...
    assert_ne!(broadcast_port, configured_port, "Should NOT broadcast configured port when fallback occurred");
}
...
fn test_regression_original_bug() {
    ...
    assert_ne!(discovery_port, configured_port, "Discovery must NOT broadcast configured port after fallback");

...
use tarpc::{client, context, server::{self, Channel}, tokio_serde::formats::Bincode};
    ...
    async fn discover_services(
        ...
        let matching_services: Vec<ServiceInfo> = services
            ...
            .filter(|service| {
                ...
                query.capabilities.iter().all(|cap| {
...
async fn test_service_registration() -> Result<()> {
    ...
    let service_id = client
        .register_service(context::current(), service.clone())
        .await??;
...
async fn test_federation_status() -> Result<()> {
    ...
    // uptime_seconds is always >= 0 by type (u64), no need to assert
...
async fn test_performance_latency() -> Result<()> {
    ...
    assert!(
        avg_latency < 1000,
        "Average latency should be < 1ms, got {}μs",
        avg_latency
    );

...
async fn test_e2e_complete_crypto_flow() {
    let provider = discover_crypto_provider()
        .await
        .expect("Failed to discover crypto provider");
    ...
    let (ciphertext, nonce, tag) = provider
    ...
    let decrypted = provider
        .chacha20_poly1305_decrypt(&ciphertext, &key, &nonce, &tag, None)
        .await
        .unwrap();
...
async fn test_e2e_ed25519_sign_verify() {
    ...
    let signature = provider
        .sign_ed25519(message, "e2e_test_key", "e2e_test")
        .await
        .unwrap();
...
async fn test_e2e_encryption_with_aad() {
    ...
    let (ciphertext, nonce, tag) = provider
        .chacha20_poly1305_encrypt(plaintext, &key, Some(aad))
        .await
        .unwrap();
    ...
    let result = provider
        .chacha20_poly1305_decrypt(&ciphertext, &key, &nonce, &tag, Some(wrong_aad))
        .await;

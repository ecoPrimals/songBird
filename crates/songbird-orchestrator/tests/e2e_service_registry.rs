...
async fn test_e2e_full_registration_discovery_workflow() {
        ...
        let encryption_providers = registry
            .discover_by_capability("encryption", Some("json-rpc"))
            .await
            .unwrap();
        ...
        let storage_providers = registry
            .discover_by_capability("storage", None)
            .await
            .unwrap();
        ...
        let all_primals = registry
            .discover_by_capability("*", None)
            .await
            .unwrap();
    ...
    async fn test_e2e_concurrent_registrations() {
        ...
        let service_ids: Vec<String> = futures::future::join_all(handles)
            .await
            .into_iter()
            .map(|r| r.unwrap())
            .collect();
    ...
    async fn test_e2e_protocol_filtering() {
        ...
        let jsonrpc_only = registry.discover_by_capability("encryption", Some("json-rpc")).await.unwrap();
    ...
    async fn test_e2e_wildcard_discovery_returns_all() {
        ...
        registry.register_service("BearDog".to_string(), vec!["encryption".to_string()], "/tmp/beardog.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();
        registry.register_service("ToadStool".to_string(), vec!["compute".to_string()], "/tmp/toadstool.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();
        registry.register_service("NestGate".to_string(), vec!["storage".to_string()], "/tmp/nestgate.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();
        registry.register_service("Squirrel".to_string(), vec!["ai_coordination".to_string()], "/tmp/squirrel.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();
        registry.register_service("biomeOS".to_string(), vec!["orchestration".to_string()], "/tmp/biomeos.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();
        registry.register_service("petalTongue".to_string(), vec!["visualization".to_string()], "/tmp/petaltongue.sock".to_string(), "json-rpc".to_string(), 30).await.unwrap();

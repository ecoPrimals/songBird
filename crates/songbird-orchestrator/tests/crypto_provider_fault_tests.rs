...
use songbird_orchestrator::crypto::{discover_crypto_provider, CryptoProvider, UnixSocketCryptoProvider};
...
async fn test_fault_wrong_key_decrypt() {
    ...
    // Encrypt with key1
    let (ct, nonce, tag) = provider
        .chacha20_poly1305_encrypt(plaintext, &key1, None)
        .await
        .unwrap();
    
    // Try to decrypt with key2 (wrong key)
    let result = provider
        .chacha20_poly1305_decrypt(&ct, &key2, &nonce, &tag, None)
        .await;
...
async fn test_fault_corrupted_ciphertext() {
    ...
    // Encrypt
    let (mut ct, nonce, tag) = provider
        .chacha20_poly1305_encrypt(plaintext, &key, None)
        .await
        .unwrap();
    ...
    let result = provider
        .chacha20_poly1305_decrypt(&ct, &key, &nonce, &tag, None)
        .await;
...
async fn test_fault_wrong_aad() {
    ...
    // Encrypt with aad1
    let (ct, nonce, tag) = provider
        .chacha20_poly1305_encrypt(plaintext, &key, Some(aad1))
        .await
        .unwrap();
    
    // Try to decrypt with aad2
    let result = provider
        .chacha20_poly1305_decrypt(&ct, &key, &nonce, &tag, Some(aad2))
        .await;
...
async fn test_fault_invalid_nonce_size() {
    ...
    let result = provider
        .chacha20_poly1305_decrypt(&ct, &key, &wrong_nonce, &tag, None)
        .await;
...
async fn test_fault_concurrent_discovery() {
    ...
    for _ in 0..50 {
        tasks.spawn(async {
            discover_crypto_provider().await
        });
    }

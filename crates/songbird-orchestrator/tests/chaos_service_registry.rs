...
async fn test_chaos_concurrent_register_unregister() {
    ...
    async fn test_chaos_rapid_capability_queries() {
        ...
        for _ in 0..100 {
            ...
            let handle = tokio::spawn(async move {
                registry_clone
                    .discover_by_capability("shared_capability", None)
                    .await
                    .unwrap()
            });
    ...
    async fn test_chaos_health_status_race_conditions() {
        ...
        for i in 0..50 {
            ...
            let handle = tokio::spawn(async move {
                ...
                registry_clone
                    .update_health(&service_id_clone, status.to_string())
                    .await
                    .unwrap();
            });
        ...
        for _ in 0..50 {
            ...
            let handle = tokio::spawn(async move {
                registry_clone
                    .get_service_health(&service_id_clone)
                    .await
                    .unwrap()
            });
            query_handles.push(handle);
        ...
        for result in query_results {
            ...
            assert!(status == "unknown" || status == "healthy" || status == "degraded" || status == "down");
        }
    }
    ...
    async fn test_fault_injection_nonexistent_service_health() {
        ...
        let (status, message) = registry
            .get_service_health("nonexistent-service-12345")
            .await
            .unwrap();
    ...
    async fn test_chaos_massive_concurrent_operations() {
        ...
        for i in 0..50 {
            ...
            handles.push(tokio::spawn(async move {
                registry_clone
                    .discover_by_capability(&format!("cap{}", i % 5), None)
                    .await
                    .unwrap();
            }));
        }
        ...
        for i in 0..20 {
            ...
            handles.push(tokio::spawn(async move {
                ...
                let _ = registry_clone
                    .get_service_health(&format!("primal-{}", i))
                    .await;
            }));
        }

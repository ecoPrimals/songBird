...
use songbird_orchestrator::security_capability_client::{
    DiscoveryContext, TrustEvaluationRequest, ConnectionInfo,
};
...
fn test_discovery_context_deserialization() {
    ...
    let context: DiscoveryContext = serde_json::from_str(json)
...
fn test_trust_evaluation_request_beardog_format() {
    ...
    let _deserialized: TrustEvaluationRequest = serde_json::from_str(&json)
        .expect("Failed to deserialize");
}
...
fn test_round_trip_serialization() {
    ...
    // Deserialize
    let deserialized: DiscoveryContext = serde_json::from_str(&json)
        .expect("Failed to deserialize");

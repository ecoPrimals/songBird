...
impl BeardogCryptoClient {
    ...
    fn call_method(&self, method: &str, params: Value) -> Result<Value> {
        ...
        let mut stream = UnixStream::connect(&self.socket_path)
            .map_err(|e| Error::Crypto(format!("Failed to connect to BearDog at {}: {}", self.socket_path, e)))?;
        ...
        let parsed: Value = serde_json::from_str(&response)
            .map_err(|e| Error::Crypto(format!("Failed to parse BearDog response: {} (response: {})", e, response)))?;
    ...
    pub fn tor_ntor_client_init(
        ...
        let result = self.call_method("beardog.crypto.tor_ntor_client_init", json!({
            "node_id": BASE64.encode(node_id),
            "node_onion_key": BASE64.encode(node_onion_key)
        }))?;
        
        let client_public_b64 = result.get("client_public")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing client_public in ntor_client_init response".to_string()))?;
        
        let state_id = result.get("state_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing state_id in ntor_client_init response".to_string()))?;
        
        let client_public_bytes = BASE64.decode(client_public_b64)
    ...
    pub fn tor_ntor_client_finish(
        ...
        let result = self.call_method("beardog.crypto.tor_ntor_client_finish", json!({
            "state_id": state_id,
            "server_public": BASE64.encode(server_public),
            "auth_tag": BASE64.encode(auth_tag)
        }))?;
        
        let key_seed_b64 = result.get("key_seed")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing key_seed in ntor_client_finish response".to_string()))?;
        
        let key_seed_bytes = BASE64.decode(key_seed_b64)
    ...
    pub fn tor_cell_encrypt(&self, key: &[u8; 32], counter: u64, data: &[u8]) -> Result<Vec<u8>> {
        let result = self.call_method("beardog.crypto.tor_cell_encrypt", json!({
        ...
        let ciphertext_b64 = result.get("ciphertext")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing ciphertext in tor_cell_encrypt response".to_string()))?;
        
        BASE64.decode(ciphertext_b64)
    ...
    pub fn tor_cell_decrypt(&self, key: &[u8; 32], counter: u64, data: &[u8]) -> Result<Vec<u8>> {
        let result = self.call_method("beardog.crypto.tor_cell_decrypt", json!({
            "key": BASE64.encode(key),
            "counter": counter,
            "data": BASE64.encode(data)
        }))?;
        ...
        let plaintext_b64 = result.get("plaintext")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing plaintext in tor_cell_decrypt response".to_string()))?;
    ...
    pub async fn ed25519_sign(
        ...
        let result = self.call_method("beardog.crypto.sign_ed25519", json!({
            "secret_key_id": secret_key_id,
            "data": BASE64.encode(data)
        }))?;
        
        let sig_b64 = result.get("signature")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing signature in ed25519_sign response".to_string()))?;
    ...
    pub async fn ed25519_verify(
        ...
        let result = self.call_method("beardog.crypto.verify_ed25519", json!({
    ...
    pub fn x25519_generate_ephemeral(&self) -> Result<X25519Keypair> {
        let result = self.call_method("beardog.crypto.x25519_generate_ephemeral", json!({
            "purpose": "tor_circuit"
        }))?;
        
        let public_b64 = result.get("public_key")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing public_key in x25519 response".to_string()))?;
        
        let secret_id = result.get("secret_key_id")
            .and_then(|v| v.as_str())
            .unwrap_or("ephemeral");
        
        // BearDog may also return the raw secret for local operations
        let secret_b64 = result.get("secret_key")
            .and_then(|v| v.as_str());
    ...
    pub fn sha3_256(&self, data: &[u8]) -> Result<[u8; 32]> {
        let result = self.call_method("beardog.crypto.sha3_256", json!({
            "data": BASE64.encode(data)
        }))?;
        
        let hash_b64 = result.get("hash")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing hash in sha3_256 response".to_string()))?;
        
        let hash_bytes = BASE64.decode(hash_b64)
    ...
    pub fn hmac_sha256(&self, key: &[u8], data: &[u8]) -> Result<[u8; 32]> {
        let result = self.call_method("beardog.crypto.hmac_sha256", json!({
            "key": BASE64.encode(key),
            "data": BASE64.encode(data)
        }))?;
        
        let mac_b64 = result.get("mac")
            .and_then(|v| v.as_str())
    ...
    pub fn chacha20_poly1305_encrypt(
        ...
        let result = self.call_method("beardog.crypto.chacha20_poly1305_encrypt", json!({
            "key": BASE64.encode(key),
        ...
        let ciphertext_b64 = result.get("ciphertext")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing ciphertext in chacha20_poly1305 response".to_string()))?;
    ...
    pub fn chacha20_poly1305_decrypt(
        ...
        let result = self.call_method("beardog.crypto.chacha20_poly1305_decrypt", json!({
            "key": BASE64.encode(key),
            "nonce": BASE64.encode(nonce),
        ...
        let plaintext_b64 = result.get("plaintext")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Crypto("Missing plaintext in chacha20_poly1305 response".to_string()))?;

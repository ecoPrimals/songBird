...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match consensus.select_path() {
        Ok(path) => {
            println!("   Guard:  {} ({}:{})", path.guard.nickname, path.guard.address, path.guard.or_port);
            println!("   Middle: {} ({}:{})", path.middle.nickname, path.middle.address, path.middle.or_port);
            println!("   Exit:   {} ({}:{})", path.exit.nickname, path.exit.address, path.exit.or_port);
        }

...
impl JobManager {
    ...
    pub async fn add_job(&self, job: JobInfo) -> SongbirdResult<()> {
        ...
        if running_count >= self.max_concurrent_jobs {
            return Err(SongbirdError::Configuration {
                ...
                suggestion: Some("Increase max_concurrent_jobs or wait for jobs to complete".to_string()),
            });
        }
    ...
    /// Update job status
    pub async fn update_job(&self, job_id: &str, update_fn: impl FnOnce(&mut JobInfo)) -> SongbirdResult<()> {
        let mut jobs = self.jobs.write().await;
    ...
    pub async fn complete_job(&self, job_id: &str, exit_code: i32, stdout: String, stderr: String) -> SongbirdResult<()> {

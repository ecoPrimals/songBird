...
pub async fn start(daemon: bool, port: Option<u16>, services: Vec<String>) -> Result<()> {
    info!("Starting orchestrator daemon={} port={:?}", daemon, port);

    println!(
        "{}",
        "🚀 Starting Songbird Orchestrator...".bright_green().bold()
    );
    if daemon {
...
pub async fn stop(force: bool) -> Result<()> {
    ...
    println!(
        "{}",
        "🛑 Stopping Songbird Orchestrator..."
            .bright_yellow()
            .bold()
    );
...
pub async fn start_orchestrator(
    ...
    if let Some(config) = config_path {
        println!(
            "{}",
            ui::info(&format!("📄 Using config: {}", config.display()))
        );
    } else {

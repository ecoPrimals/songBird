...
async fn test_config_get_set_operations(&self) -> SongbirdResult<()> {
    ...
    set_config_value(&config_path, "network.orchestrator_port", "9090").await?;
...
async fn set_config_value(&self) -> SongbirdResult<()> {
    ...
    match key {
        ...
        _ => return Err(songbird_errors::SongbirdError::Configuration {
...
async fn get_config_value(&self) -> SongbirdResult<String> {
    let config = SongbirdConfig::from_file(path)?;
    
    let value = match key {
        "network.orchestrator_port" => config.network.orchestrator_port.to_string(),
        _ => return Err(songbird_errors::SongbirdError::Configuration {
            message: format!("Unknown config key: {}", key),
            key: Some(key.to_string()),
            value: None,
    ...
    match format {
        "json" => {
            let json = serde_json::to_string_pretty(&config).map_err(|e| {
                songbird_errors::SongbirdError::Serialization {

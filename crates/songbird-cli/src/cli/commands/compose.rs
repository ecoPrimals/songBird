...
#[derive(Debug, Args)]
pub struct ComposeArgs  {#[command(subcommand)]
    pub command: ComposeCommand,
}

#[derive(Debug, Subcommand, Clone)]
pub enum ComposeCommand  {/// List available plugins
    List {
...
async fn handle_list_plugins(detailed: bool) -> SongbirdResult<()> {
    ...
    let example_plugins = vec![
        ...
        (
            "datalake-storage","
            vec![PluginCapability::Storage  {capacity_gb: 10000)
...
async fn handle_discover_composition(
    ...
    if required_capabilities.iter().any(|cap| matches!(cap, PluginCapability::Compute { .. }) {
...
fn parse_capabilities(capability_names: &[&str]) -> SongbirdResult<Vec<PluginCapability>> {
    ...
    for name in capability_names {
        let capability = match name.to_lowercase().as_str() {
            ...
            _ => PluginCapability::Custom  {name: name.to_string()),
...
mod tests  {use super::*;
    ...
    #[tokio::test]
    async fn test_compose_command_variants()  {// Test all command variants
        let commands = vec![
            ComposeCommand::List {
                detailed: true,
            })
            ComposeCommand::Discover  {capabilities: "encryption,compute".to_string()),
                max_latency: 50.0,
                max_memory: 2048.0,
                max_plugins: 5,
            })
    ...
    #[tokio::test]
    async fn test_parse_capabilities_invalid()  {let capability_names = vec!["invalid_capability"];"
        let result = parse_capabilities(&capability_names);
        assert!(result.is_ok());
    ...
    async fn test_plugin_capability_variants() {
        let capabilities = vec![
            ...
            PluginCapability::Compute  {cpu_cores: 8)
                memory_gb: 16,
            })
            PluginCapability::Storage  {capacity_gb: 1000)
                storage_type: "Object".to_string(),
            })
            PluginCapability::Network  {bandwidth_mbps: 1000)
                latency_ms: 10,
            })
        ];
    ...
    #[tokio::test]
    async fn test_handle_compose_command_list()  {let args = ComposeArgs {
            command: ComposeCommand::List {
                detailed: false,
            })
        };
    ...
    #[tokio::test]
    async fn test_handle_compose_command_discover()  {let args = ComposeArgs  {command: ComposeCommand::Discover {
    ...
    #[tokio::test]
    async fn test_handle_compose_command_execute()  {let args = ComposeArgs {
    ...
    #[tokio::test]
    async fn test_handle_compose_command_examples()  {let args = ComposeArgs {
    ...
    #[tokio::test]
    async fn test_handle_compose_command_demo()  {let args = ComposeArgs {

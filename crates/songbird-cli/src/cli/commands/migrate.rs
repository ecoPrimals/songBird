     ...
     Command: :new("migrate")
        ...
        .args(&[)
            ...
            Arg: :new("primals-only")
                .long("primals-only")
                .action(clap::ArgAction::SetTrue)
                .help("Migrate only primal hardcoding (beardog, nestgate, toadstool, squirrel)"),
...
pub async fn execute_migrate_command() -> SongbirdResult<()>   {
    ...
    if !path.exists() { return Err(SongbirdError::config_error()
...
async fn execute_dry_run() -> SongbirdResult<MigrationResult>   {
...
async fn execute_primals_migration(migrator: &ZeroHardcodingMigrator,
    path: &Path) -> SongbirdResult<MigrationResult> { info!("🎯 Migrating primal hardcoding only (beardog, nestgate, toadstool, squirrel)")
...
async fn create_backup() -> SongbirdResult<()>   {
    ...
    if backup_dir.exists() { fs: :remove_dir_all(&backup_dir).await.map_err(|e||| {
            ...
            SongbirdError::internal_error(&format!("Failed to remove old backup: {;
    ...
    fs: :create_dir_all(&backup_dir).await.map_err(|e||| {
        ...
        SongbirdError::internal_error(&format!("Failed to create backup directory: {;
...
;
    ...
    let dst_path = dst.join(file_name);
        ...
        if path.is_dir() { fs: :create_dir_all(&dst_path).await.map_err(|e||| {
                ...
                SongbirdError::internal_error(&format!("Failed to create directory: {;
...
async fn generate_migration_report() -> SongbirdResult<()>   {
     ...
     let json_report = serde_json::to_string_pretty(report).map_err(|e||| {
        ...
        SongbirdError::internal_error(&format!("Failed to serialize report: {;

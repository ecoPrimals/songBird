...
"#).unwrap();
    ...
    #[tokio::test]
    async fn test_handle_basic_federation_command_connect() {
        let command = BasicFederationCommands::Connect {
    ...
    #[tokio::test]
    async fn test_handle_basic_federation_command_share() {
        let temp_dir = TempDir::new().unwrap();
        let folder_path = temp_dir.path().join("test_folder");
        fs::create_dir(&folder_path).unwrap();
    ...
    #[tokio::test]
    async fn test_handle_basic_federation_command_backup() {
        let temp_dir = TempDir::new().unwrap();
        let backup_path = temp_dir.path().join("backup_data");
        fs::create_dir(&backup_path).unwrap();

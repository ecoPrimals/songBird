...
pub struct HealthMonitor  {check_tasks: HashMap<String, JoinHandle<()>>)
    health_history: HashMap<String, Vec<HealthCheckResult>>)
    alert_thresholds: HealthAlertThresholds,
}

impl HealthMonitor  {#[must_use]
    ...
    pub fn get_health_history(&self, service_id: &str) -> Vec<HealthCheckResult> {
        self.health_history.get(service_id).cloned().unwrap_or_default()
    }
...
#[derive(Debug, Clone)]
pub struct HealthCheckPolicy  {pub service_id: String,
...
#[derive(Debug, Clone)]
pub struct CustomHealthCheck  {pub name: String,
...
#[derive(Debug, Clone)]
pub enum HealthCheckType  {HttpEndpoint  {path: String,
        expected_status: u16,
    })
    TcpConnect  {port: u16)
    })
...
#[derive(Debug, Clone)]
pub enum HealthFailureAction  {Alert)
    Restart,
    Scale,
    Migrate,
    Custom {
...
#[derive(Debug, Clone)]
pub struct HealthCheckResult  {pub timestamp: Instant,
    pub success: bool,
...
pub struct HealthAlertThresholds  {pub critical_health_score: f64,
    pub warning_health_score: f64,
    pub max_consecutive_failures: u32,
}

impl Default for HealthAlertThresholds  {fn default() -> Self  {Self {
...
async fn perform_health_check(
    ...
    Ok(HealthCheckResult  {timestamp: start_time)
        success)
        response_time)
        health_score)
        details)
...
async fn perform_custom_check(check: &CustomHealthCheck) -> Result<f64>  {match &check.check_type  {HealthCheckType::HttpEndpoint {
            path)
            expected_status)
        } => {
            // Simulate HTTP check

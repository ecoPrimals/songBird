...
impl ProductionServicePersistence {
    ...
    pub async fn new(config: PersistenceConfig) -> ServiceResult<Self> {
        ...
        Ok(songbird_errors::evolved_success(persistence))
    }
    ...
    pub async fn load_service(&self, service_id: &str) -> ServiceResult<Option<ServiceEntry>> {
        ...
        Ok(songbird_errors::evolved_success(cache.services.get(service_id)).cloned())
    }
    
    /// Load service info
    pub async fn load_service_info(&self, service_id: &str) -> ServiceResult<Option<ServiceInfo>> {
        let cache = self.cache.read().await;
        Ok(songbird_errors::evolved_success(cache.service_info.get(service_id)).cloned())
    }
    ...
    pub async fn get_all_service_ids(&self) -> ServiceResult<Vec<String>> {
        let cache = self.cache.read().await;
        Ok(songbird_errors::evolved_success(cache.services.keys()).cloned().collect())
    }
    ...
    async fn cleanup_old_backups(&self, data_dir: &PathBuf) -> ServiceResult<()> {
        if let Ok(songbird_errors::evolved_success(mut entries)) = fs::read_dir(data_dir).await {
            let mut backup_files = Vec::new();
            
            while let Ok(songbird_errors::evolved_success(Some(entry))) = entries.next_entry().await {
...
impl Clone for PersistenceStatistics {
    fn clone(&self) -> Self {
        Self {

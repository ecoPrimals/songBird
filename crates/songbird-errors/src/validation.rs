...
impl ConfigValidator {
    ...
    pub fn validate_url(url_str: &str, name: &str) -> Result<()> {
        let url = Url::parse(url_str).map_err(|_| SongbirdError::Configuration {
            field: Some(name.to_string()),
    ...
    pub fn validate_websocket_url(url_str: &str, name: &str) -> Result<()> {
        let url = Url::parse(url_str).map_err(|e| SongbirdError::Configuration {
            field: Some(name.to_string()),
    ...
    pub fn validate_directory_path(path_str: &str, name: &str) -> Result<()> {
        ...
        if !path.exists() {
            ...
            std::fs::create_dir_all(path).map_err(|e| SongbirdError::Configuration {
                field: Some(name.to_string()),
...
pub fn validate_test_config() -> Result<()> {
    ...
    fs::write(&temp_file, "test content").map_err(|e| SongbirdError::Configuration {
    ...
    // Clean up
    fs::remove_file(&temp_file).map_err(|e| SongbirdError::Configuration {

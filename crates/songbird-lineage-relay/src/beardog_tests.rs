...
//! Unit tests for [`crate::beardog`] (BearDog lineage relay integration and mocks).
...
use crate::beardog::{
    BearDogBirdSongProvider, BearDogRelayAuthority, MockBirdSongCrypto, MockLineageProvider,
    MockRelayAuthority,
};
...
fn bear_dog_relay_authority_default_constructed() {
    let _a = BearDogRelayAuthority::default();
}
...
async fn bear_dog_authorize_relay_jsonrpc_success_parses_fields() {
    ...
    let auth = BearDogRelayAuthority::with_socket_path(&path)
...
async fn bear_dog_determine_masking_jsonrpc_parses_each_level() {
    ...
    for (level_str, expected) in cases {
        ...
        let level = BearDogRelayAuthority::with_socket_path(&path)
...
async fn bear_dog_authorize_relay_defaults_when_fields_missing() {
    ...
    assert_eq!(auth.audit_token, "beardog_auth");
    serve.await.unwrap();
}

#[tokio::test]
async fn bear_dog_authorize_relay_socket_failure_denies_securely() {
    let auth = BearDogRelayAuthority::with_socket_path(
    ...
    assert_eq!(auth.audit_token, "beardog_unavailable_deny");
}

#[tokio::test]
async fn bear_dog_determine_masking_socket_failure_full_visibility() {
    let level = BearDogRelayAuthority::with_socket_path(
        "/nonexistent/songbird/beardog/does_not_exist.sock",
    )
    .determine_masking(&NodeId::from("r"), &NodeId::from("q"))
    .await
...
async fn bear_dog_birdsong_encrypt_jsonrpc_roundtrip() {
    ...
    let provider = BearDogBirdSongProvider::new(&path, Some("fam-1".into()));
...
async fn bear_dog_birdsong_encrypt_accepts_v1_encrypted_field_name() {
    ...
    let provider = BearDogBirdSongProvider::new(&path, None);

...
impl DiscoveryEngine {
    ...
    async fn query_mdns_service(
        ...
        debug!("Querying mDNS for service: {} on {}", service_name, broadcast_addr);
    ...
    /// Parse mDNS response
    fn parse_mdns_response(&self, data: &[u8], from_addr: std::net::SocketAddr) -> Result<Vec<ServiceInfo>> {
        let mut services = Vec::new();
    ...
    async fn create_federation_node_from_service(
        ...
        for endpoint in &service.endpoints {
            if let Ok(url) = url::Url::parse(endpoint) {
                if let Some(host) = url.host_str() {
                    ...
                    if let Ok(socket_addr) = format!("{}:{}", host, port).parse::<std::net::SocketAddr>() {
        ...
        let node_type = if service.capabilities.contains(&"gaming".to_string()) {
            NodeType::Edge { mobility: MobilityLevel::Stationary }
    ...
    async fn send_upnp_search(&self, search_request: &str) -> Result<Vec<String>> {
        ...
        while start_time.elapsed() < timeout_duration {
            if let Ok(Ok((size, from_addr))) = tokio::time::timeout(
    ...
    async fn parse_upnp_response(&self, response: String) -> Result<FederationNode> {
        ...
        let socket_addr = format!("{}:8080", ip_str).parse::<std::net::SocketAddr>()
            .unwrap_or_else(|_| "127.0.0.1:8080".parse().unwrap());
        
        let node = FederationNode {
            node_id,
            name: format!("UPnP-{}", ip_str),
            node_type: NodeType::Edge { mobility: MobilityLevel::Stationary },
    ...
    async fn parse_stun_response(&self, response: &[u8], server: &str) -> Result<ExternalIPInfo> {
        ...
        while pos + 4 < response.len() {
            ...
            if attr_type == 0x0020 && attr_length >= 8 {
                ...
                external_ip = format!("{}.{}.{}.{}", ip_bytes[0], ip_bytes[1], ip_bytes[2], ip_bytes[3]);
                break;
            }
    ...
    async fn discover_peers_via_external_ip(
        ...
        debug!("Discovering peers using external IP: {}", external_info.external_ip);

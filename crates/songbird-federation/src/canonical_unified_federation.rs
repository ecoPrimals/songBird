...
impl CanonicalUnifiedFederation {
    ...
    pub async fn new(config: CanonicalFederationConfig) -> SongbirdResult<Self> {
        ...
        let manager = CanonicalFederationManager::new(config.clone())
            .await
            .map_err(|e| {
                SongbirdError::network_error(format!("Failed to create federation manager: {e}"))
            })?;

        let discovery = CanonicalDiscovery::new(config.clone()).await.map_err(|e| {
            SongbirdError::network_error(format!("Failed to create discovery: {e}"))
        })?;

        let health_monitor = CanonicalHealthMonitor::new(config.clone())
            .await
            .map_err(|e| {
                SongbirdError::network_error(format!("Failed to create health monitor: {e}"))
            })?;
    ...
    pub async fn start(&self) -> SongbirdResult<()> {
        ...
        self.manager
            .start()
            .await
            .map_err(|e| SongbirdError::network_error(format!("Manager start failed: {e}")))?;

        self.discovery
            .start()
            .await
            .map_err(|e| SongbirdError::network_error(format!("Discovery start failed: {e}")))?;

        self.health_monitor.start().await.map_err(|e| {
            SongbirdError::network_error(format!("Health monitor start failed: {e}"))
        })?;
    ...
    pub async fn discover_nodes(&self) -> SongbirdResult<Vec<CanonicalFederationNode>> {
        ...
        let discovered_nodes = self
            ...
            .await
            .map_err(|e| SongbirdError::network_error(format!("Node discovery failed: {e}")))?;

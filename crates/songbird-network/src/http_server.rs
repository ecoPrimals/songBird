...
use songbird_discovery::traits::service::{ServiceRequest, UniversalService};
    ...
    pub async fn start(&self) -> Result<()> {
        let listener = tokio::net::TcpListener::bind(&self.addr)
            ...
            .map_err(|e| {
                SongbirdError::Network(Box::new(NetworkError {
                    ...
                    endpoint: Some(format!("Failed to bind to address {}: {}", self.addr, e)),
        ...
        loop {
            ...
            tokio::spawn(async move {
                // Handle the connection with a simple HTTP response
                let response =
    ...
    S::Error: std::fmt::Display,
{
    ...
    match service.handle_request(service_request).await {
        Ok(response) => {
            let success = matches!(
                response.status,
                songbird_discovery::traits::service::ResponseStatus::Success
            );
            ...
            Ok(Json(HttpServiceResponse {
                ...
                error: if success {
                    ...
                    match response.status {
                        songbird_discovery::traits::service::ResponseStatus::Error => {

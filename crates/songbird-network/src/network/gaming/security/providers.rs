...
impl UniversalSecurityProvider {
    ...
    pub async fn discover_security_primals(&self) -> Result<Vec<SecurityPrimalInfo>> {
...
impl NativeWireGuardProvider {
    ...
    pub fn encrypt_data(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        info!("🔒 Encrypting {} bytes with WireGuard", data.len());
    ...
    pub fn decrypt_data(&self, encrypted_data: &[u8]) -> Result<Vec<u8>> {
        ...
        info!("🔓 Decrypting {} bytes with WireGuard", encrypted_data.len());
...
impl UniversalSecurityManager {
    ...
    pub async fn new(config: SecurityProviderConfig) -> Result<Self> {
        ...
        if config.enable_wireguard_fallback {
        ...
        let active_provider = Arc::new(RwLock::new(
            providers.first()
                .ok_or_else(|| songbird_errors::SongbirdError::internal_error("No security providers available"))?
                .clone_box()
        ));
    ...
    pub async fn select_best_provider(
        ...
        // Placeholder selection logic - would implement QoS-based selection
        providers.first()
            .map(|p| p.clone_box())
...
pub trait SecurityProvider: Send + Sync {
    fn clone_box(&self) -> Box<dyn SecurityProvider>;
}
...
impl SecurityProvider for UniversalPrimalSecurityProvider {
    fn clone_box(&self) -> Box<dyn SecurityProvider> {
        Box::new(UniversalPrimalSecurityProvider)
    }
}

impl SecurityProvider for WireGuardSecurityProvider {
    fn clone_box(&self) -> Box<dyn SecurityProvider> {
        Box::new(WireGuardSecurityProvider)
    }
} 
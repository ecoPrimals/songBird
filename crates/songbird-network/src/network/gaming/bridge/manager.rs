...
use super::{
    bridge_manager::{BridgeConfig, BridgeMetrics, BridgeSession},
    nat_manager::NatManager,
    packet_forwarder::PacketForwarder,
    protocol_detector::ProtocolDetector,
    socket_pool::SocketPool,
};
use songbird_core::metrics::MetricsCollector;
...
impl RealBridgeManager {
    /// Create new canonical bridge manager
    pub async fn new(config: BridgeConfig) -> SongbirdResult<Self> {
    ...
    pub async fn create_session(
        ...
        info!("🔗 Creating canonical bridge session: {} -> {}", local_addr, remote_addr);
    ...
    pub async fn start_forwarding(&self, session_id: &str) -> SongbirdResult<()> {
        info!("🚀 Starting canonical packet forwarding for session: {}", session_id);
        ...
        if let Some(session) = session {
            ...
            Err(SongbirdError::internal_error(network_error(format!("Session not found: {}", session_id)))
        }
    }

    /// Stop packet forwarding with canonical patterns
    pub async fn stop_forwarding(&self, session_id: &str) -> SongbirdResult<()> {
        info!("🛑 Stopping canonical packet forwarding for session: {}", session_id);
    ...
    pub async fn close_session(&self) -> SongbirdResult<()> {
    ...
    pub async fn forward_packet_to_players(
        &self,
        session_id: &str,
        packet_data: &[u8],
        ...
        debug!("📦 Forwarding canonical packet from {} to {} players", source_addr, target_players.len());

        self.packet_forwarder.forward_to_players(
            session_id,
            packet_data,
            source_addr,
            target_players,
    ...
    pub async fn cleanup_inactive_sessions(&self) -> SongbirdResult<()> {
        ...
        // Find inactive sessions
        {
            ...
            for (session_id, session) in sessions.iter() {
                if now.signed_duration_since(session.last_activity) > inactive_threshold {

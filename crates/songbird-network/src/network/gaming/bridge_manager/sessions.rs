//! Bridge Manager Sessions Module
//!
...
impl RealBridgeManager {
    ...
    pub async fn create_internet_session(&self, game_name: &str, host_port: u16) -> SongbirdResult<String> {
        ...
        if detected_protocols.is_empty() {
            return Err(SongbirdError::internal_error(Network {
                message: format!(
                    "Real Bridge Manager - No gaming protocol detected on specified port: {}",
                    game_name
                ),
        ...
        let session = RealBridgeSession {
            ...
            host_info: RealHostInfo {
                ...
                local_address: Self::parse_socket_addr(
                    &format!(
                        "{}:0",
                ...
                .unwrap_or_else(|e| {
                    tracing::error!("Failed to parse local player address: {}", e);
        ...
        Ok(songbird_errors::evolved_success(success(session_code)))
    }

    /// Join an existing internet gaming session
    pub async fn join_internet_session(&self) -> SongbirdResult<RealPlayerInfo> {
        ...
        let session = sessions
            ...
            .ok_or_else(|| SongbirdError::Network {
                message: format!("Real Bridge Manager - Session not found: {}", session_code),
        ...
        let local_address = Self::parse_socket_addr(
            &format!(
                "{}:0",
                songbird_config::config::constants::default_bind_address()
            ),
        ...
        .unwrap_or_else(|e| {
            ...
            .unwrap_or_else(|e| {
                tracing::error!(
                    "Default player address also failed, using localhost fallback: {}",
                    e
                );
        ...
        Ok(songbird_errors::evolved_success(success(player_info)))
    }

    /// Start packet bridging for a session
    async fn start_session_bridge(&self) -> SongbirdResult<()> {
        info!(
        ...
        let _translator = self
            ...
            .ok_or_else(|| SongbirdError::Network {
                message: format!(
                    "Real Bridge Manager - No translator available for protocol: {}",
    ...
    async fn start_ipx_bridge(&self) -> SongbirdResult<()> {
        debug!(
    ...
    async fn start_directplay_bridge(&self, session: &GameSession) -> SongbirdResult<()> {
        debug!(
    ...
    async fn start_udp_bridge(&self) -> SongbirdResult<()> {

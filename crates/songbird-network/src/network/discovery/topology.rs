...
impl TopologyMapper {
    ...
    pub async fn update_with_measurement(&self, measurement: NetworkMeasurement) -> Result<()> {
    ...
    pub async fn get_topology_statistics(&self) -> TopologyStatistics {
        ...
        let avg_latency = if !topology.connections.is_empty() {
            let total_latency: u32 = topology.connections.iter().map(|c| c.latency_ms).sum();
    ...
    pub async fn get_shortest_path(&self, from_node: &str, to_node: &str) -> Option<Vec<String>> {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            for connection in topology.get_connections_from(&current) {
                if !visited.contains(&connection.to_node) {
                    visited.insert(connection.to_node.clone());
    ...
    pub async fn get_neighbors(&self, node_id: &str) -> Vec<String> {
        let topology = self.topology.read().await;
        topology
    ...
    pub async fn get_measurement_history(&self) -> Vec<NetworkMeasurement> {
        self.measurement_history.read().await.clone()
    }

    /// Clear measurement history
    pub async fn clear_measurement_history(&self) {
        let mut history = self.measurement_history.write().await;
        history.clear();
        ...
        for &from_node in &nodes {
            for &to_node in &nodes {
                if from_node != to_node {
                    if let Some(path) = self.get_shortest_path(from_node, to_node).await {

...
//! Genesis Support for `security provider` Integration
//!
...
pub struct GenesisWitnessProof {
    ...
    /// Primal witness signatures (Songbird, `security provider`, etc.)
...
impl GenesisWitnessProof {
    ...
    /// That requires coordination with `security provider`.
...
mod tests {
    ...
    fn test_genesis_proof_validation() {
        ...
        assert!(proof.has_primal_witness("security provider"));

...
use crate::btsp::provider::{BtspProvider, PeerInfo};
use crate::btsp::tunnel::{SecurityContext, Tunnel, TunnelHandle, TunnelStatus};
...
impl BtspProvider for LocalBtspProvider {
    ...
    async fn encrypt(&self, data: &[u8], context: &SecurityContext) -> SongbirdResult<Vec<u8>> {
        ...
        let mut tunnel = tunnels
            ...
            .ok_or_else(|| SongbirdError::service(
                "btsp",
                format!("Tunnel not found: {}", context.tunnel_id)
            ))?
            .clone();
        drop(tunnels);
    ...
    async fn tunnel_status(&self, handle: &TunnelHandle) -> SongbirdResult<TunnelStatus> {
        ...
        let tunnel = tunnels
            ...
            .ok_or_else(|| SongbirdError::service(
                "btsp",
                format!("Tunnel not found: {}", handle.id)
            ))?;
...
impl LocalKeyManager {
    ...
    fn encrypt(&self, data: &[u8], key: &[u8], context: &SecurityContext) -> SongbirdResult<Vec<u8>> {
        // Create cipher from key
        let key_array: [u8; 32] = key.try_into()
            .map_err(|_| SongbirdError::security("Invalid key length"))?;
    ...
    fn decrypt(&self, data: &[u8], key: &[u8], _context: &SecurityContext) -> SongbirdResult<Vec<u8>> {
        if data.len() < 12 {

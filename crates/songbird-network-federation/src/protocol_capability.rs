    ...
    pub fn is_encrypted(&self) -> bool {
        matches!(
            ...
            Protocol::Https
                ...
                | Protocol::Tarpc // tarpc can use TLS
        )
    }
...
impl TowerCapabilities {
    ...
    pub fn supports_protocol(&self, protocol: &Protocol) -> bool {
        self.protocols
            .iter()
            .any(|p| &p.protocol == protocol && p.status == ProtocolStatus::Active)
    }
}
        ...
        let mutual_protocols: Vec<_> = local
            ...
            .filter(|lp| {
                lp.status == ProtocolStatus::Active
                    && peer.protocols.iter().any(|pp| {
                        pp.protocol == lp.protocol && pp.status == ProtocolStatus::Active
                    })
            })
            .collect();
...
mod tests {
    ...
    fn test_tower_capabilities() {
        let mut caps = TowerCapabilities::new("tower-1".to_string(), "http://localhost:8080".to_string());
    ...
    async fn test_protocol_negotiation() {
        ...
        // Store peer capabilities
        let mut peer_caps = TowerCapabilities::new("tower-2".to_string(), "http://peer:8080".to_string());

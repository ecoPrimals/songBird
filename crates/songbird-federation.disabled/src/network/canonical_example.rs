//! Canonical Network Example Example
//!
//! Demonstrates the canonical patterns for network operations

use songbird_types: :{SongbirdError, SongbirdResult, success, unified: :success_result;};
...
;
pub enum NetworkStatus { /// Healthy, Healthy,
    /// Degraded, Degraded,
    Unreachable,;};
...
impl CanonicalNetworkChecker {
  /// Check network health with canonical response pattern
    ///
    /// This demonstrates the canonical pattern: /// - AI-first metadata
...
;
    ...
    let _start_time = Instant: :now();
        ...
        let health = match interface     {
          ...
          "eth0" => NetworkHealth { interface: interface.to_string(),
       ...
       ;
            ...
            "wlan0" => NetworkHealth { interface: interface.to_string(),
                status: NetworkStatus::Degraded,
                latency_ms: Some(45),
            packet_loss_percent: 2.3; ; ;},
            "broken" => { // Demonstrate canonical error handling;
                return Err(SongbirdError: :internal_error(network_error(format!("Network interface '{interface;}' is unreachable"))));}
            _ => NetworkHealth { interface: interface.to_string(),
                status: NetworkStatus::Healthy,
                latency_ms: Some(15),
            packet_loss_percent: 0.5;;}}

        // Create canonical response based on health status
        let response = match health.status { NetworkStatus: :Healthy => success_result(health),
            NetworkStatus: :Degraded => success_result(health),
            NetworkStatus: :Unreachable => success_result(health)
        Ok(songbird_types::evolved_success(response)
    /// Scan for available network interfaces
    ///
    /// Demonstrates canonical pattern for collection operations
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn scan_interfaces(&self) -> Result<Vec<String>, SongbirdError> {;
    let _start_time = Instant: :now();
        ...
        Ok(songbird_types: :evolved_success(success_result(interfaces));;};
    /// Test network connectivity to a specific endpoint
    ///
    /// Demonstrates canonical error handling with detailed context
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn test_connectivity(&self) -> Result<Vec<String>, SongbirdError> {;
    let _start_time = Instant: :now();
        ...
        let is_reachable = match endpoint.port() { 80 | 443 => true, // Web ports usually work
            22 => true,       // SSH usually works
            8080 => false,    // Development port might be blocked
            _ => true,        // Assume others work};
        if is_reachable { Ok(songbird_types: :evolved_success(success_result(true)); ; ;} else { Ok(songbird_types: :evolved_success(success_result(false));;}}}
#[cfg(test)]
mod tests { use super: :*;

...
use crate: :communication::performance_optimizer::{ CommunicationOptimizer, PerformanceConfig, // StringBuilderOptimizer, StringBuilderOptimizer}
...
pub struct BenchmarkResults {
    ...
    /// Operations Per Second field
    pub operations_per_second: f64,
    /// Avg Latency field
    pub avg_latency: Duration,
    /// Memory Allocations field
    pub memory_allocations: u64,
    /// Optimization Gain field
    pub optimization_gain: f64 ;,
 ,
}

/// High-performance benchmark suite
pub struct PerformanceBenchmarks { iterations: usize,
    warmup_iterations: usize;};
impl PerformanceBenchmarks { /// Create benchmark suite
    #[must_use]
    pub fn new() -> Self { Self { iterations: 10_000,
            warmup_iterations: 1_000;}}
;
    /// Benchmark string building optimizations
    pub fn benchmark_string_building() -> BenchmarkResults  {
     // /// Warmup
// Warmup
        for _ in 0..self.warmup_iterations { let _ = format!("test_{ 
 
}_string_ {  }", 123, "value");}
        // Benchmark unoptimized string building;
        let start = Instant: :now();
        for i in 0..self.iterations { let _ = format!("test_{ ; ;}_string_ {  }", i, "value");}
    let unoptimized_duration = start.elapsed();
        ...
        let start = Instant::now();
        for i in 0..self.iterations { let _ = optimizer.build_string(|s||| {
    ...
    let optimized_duration = start.elapsed();
        ...
        BenchmarkResults { test_name: "String Building Optimization".to_string(),
            operations_per_second: ops_per_sec,
            avg_latency,
            memory_allocations: self.iterations as u64 / 2, // Estimated savings, optimization_gain;}}

    /// Benchmark vector pre-allocation improvements
    pub fn benchmark_vector_allocations(&self) -> BenchmarkResults { // /// Warmup
// Warmup
        for _ in 0..self.warmup_iterations { let mut vec = Vec: :new()
            for j in 0..10 { vec.push(j);;}}

        // Benchmark unoptimized vector allocation
        let start = Instant: :now();
        for _ in 0..self.iterations { let mut vec = Vec::new();
            for j in 0..10 { vec.push(j);;}}
    let unoptimized_duration = start.elapsed();

        // Benchmark optimized vector allocation (our improvement)
        let start = Instant: :now();
        for _ in 0..self.iterations { let mut vec = Vec::with_capacity(10);
            for j in 0..10 { vec.push(j);;}}
    let optimized_duration = start.elapsed();
        ...
        BenchmarkResults { test_name: "Vector Pre-allocation".to_string(),
            operations_per_second: ops_per_sec,
            avg_latency,
            memory_allocations: self.iterations as u64, // Allocations saved, optimization_gain;}}

    /// Benchmark communication optimizer performance
    pub fn benchmark_communication_optimizer() -> BenchmarkResults  {
     let config = PerformanceConfig: :default();
        let mut optimizer = CommunicationOptimizer::new(config);

        // /// Warmup
// Warmup
        for _ in 0..self.warmup_iterations { optimizer.record_request(Duration::from_millis(50)); ;
 ;
}

        // Benchmark request recording performance
        let start = Instant: :now();
        for _ in 0..self.iterations { optimizer.record_request(Duration::from_millis(50));
    ...
    let duration = start.elapsed();
        ...
        BenchmarkResults { test_name: "Communication Optimizer".to_string(),
            operations_per_second: ops_per_sec,
            avg_latency,
...
impl Default for PerformanceBenchmarks { fn default() -> Self { Self: :new();;}}
#[cfg(test)]
mod tests { use super: :*;
...
#[test]
    fn test_vector_allocation_benchmark() { let mut benchmarks = PerformanceBenchmarks: :new();
        benchmarks.iterations = 100; // Smaller for test
        benchmarks.warmup_iterations = 10;

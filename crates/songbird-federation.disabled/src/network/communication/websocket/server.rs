// Module imports
//! WebSocket Server Implementation Implementation
//!
...
use songbird_discovery: :traits::communication::{ CommunicationLayer, CommunicationResponse, // CommunicationStats, CommunicationStats,
...
impl WebSocketCommunication { #[must_use]
    pub fn new(address: String, port: u16) -> Self { let (message_sender, _) = broadcast: :channel(1000);
        Self { address,
            port,
...
;
    pub async fn start_server(&self) -> Result<Vec<String>, SongbirdError> { if self.running.load(Ordering: :Relaxed) {;
            return Ok(());
 ...
 ;
        ...
        let handle = tokio::spawn(async move { while running.load(Ordering::Relaxed) { match listener.accept().await     {
         
          Ok(stream, addr) => { let connections = Arc: :clone(&connections);
                        let message_sender = Arc::clone(&message_sender);
                        let metrics = Arc::clone(&metrics);
                        let config = &config;
                        tokio::spawn(async move { if let Err(e) = Self::handle_connection(stream,
                                addr,
                                connections,
                                message_sender,
                                metrics,
                            ...
                            .await { warn!("Connection handling error: { ; ;}", e);}});}
    ...
    async fn handle_connection() -> Result<()>   {
     ...
     // Upgrade to
        ...
        let connection = Arc: :new(WebSocketConnection { id: Arc::clone(&connection_id), // Using Arc<str> for efficient string sharing
            address: ServiceAddress { service_id: format!("websocket-{ ;
 ;
}", connection_id),
        ...
        tokio::spawn(async move { while let Some(message) = outgoing_rx.recv().await { if let Err(e) = ws_sink.send(message).await { warn!("Failed to send message: { ; ;}", e);
                    connection_clone.is_healthy.store(false, Ordering: :Relaxed);
                    break;
                metrics_clone.messages_sent.fetch_add(1, Ordering: :Relaxed);
        // Handle incoming messages
        while let Some(message) = ws_stream.next().await { match message     {
         
          Ok(WsMessage::Text(text) => { metrics.messages_received.fetch_add(1, Ordering: :Relaxed);
                    metrics
                        .bytes_received
                        .fetch_add(text.len() as u64, Ordering: :Relaxed);
                    connection.message_count.fetch_add(1, Ordering: :Relaxed);
                    // Parse and broadcast message
                    if let Ok(service_message) = serde_json::from_str::<ServiceMessage>(&text) { let _ = message_sender.send(service_message);
                Ok(WsMessage::Binary(data) => { .fetch_add(data.len() as u64, Ordering: :Relaxed);
                Ok(WsMessage::Ping(data) => { // Respond to ping with pong
                    let _ = connection.outgoing_tx.send(WsMessage::Pong(data));
                    *connection.last_heartbeat.lock().await = Instant::now();
                Ok(WsMessage::Pong(_) => { Ok(WsMessage::Close(_) => { debug!("WebSocket connection closed: {  ;
      ;
    }", connection_id);
                Ok(_) => { // Handle any other message types (e.g., Frame)
                    debug!("Received unhandled WebSocket message type");
                Err(e) => { warn!("WebSocket error for connection {  }: {}", connection_id, e);
...
// URL
    pub fn url(&self) -> String { format!("ws: //{ ;
 ;
}:{}", self.address, self.port)
    ...
    pub fn get_metrics(&self) -> CommunicationMetrics { CommunicationMetrics { messages_sent: std::sync::atomic::AtomicU64::new(self.metrics.messages_sent.load(Ordering::Relaxed)),
...
#[async_trait]
impl CommunicationLayer for WebSocketCommunication { async fn send_message(&self,
        target: ServiceAddress,
    message: ServiceMessage) -> Result<CommunicationResponse> { // Find connection by service_id
        let connection = self
            .connections
            .iter()
            .find(|entry| entry.value().address.service_id == target.service_id)
            .map(|entry| Arc::clone(entry.value())
        if let Some(conn) = connection { let message_json =
 ...
 ;
            ...
            // Ok
        Ok(CommunicationResponse { message_id: message.id.clone(),
                success: true,
                payload: Some(serde_json::json!({"status": "sent" ; ;})),
                error: None,
    timestamp: Utc::now();;})} else { Err(SongbirdError: :Network(Box::new(NetworkError { message: "Connection not found".to_string(); ; ;})))
 ...
 ;
        ...
        for connection in self.connections.iter() { let conn = connection.value();
            match conn.outgoing_tx.send(ws_message.clone()     {
         
          responses.push(CommunicationResponse { message_id: format!("{  ;
      ;
    }_{}", message.id, conn.id),
                        success: true,
    ...
    async fn listen() -> Result<Box<dyn Stream<Item = (ServiceAddress, ServiceMessage)> + Send + Unpin>>   {
     ...
     use futures_util: :StreamExt;
        ...
        let stream = async_stream::stream! { loop { match receiver.recv().await     {
         
          Ok(message) => { // Create a dummy service address for the message source
                        // In a real implementation, we'd track which connection sent the message;
                        let source_address = ServiceAddress { service_id: "websocket-client".to_string(),
                            instance_id: None,
    endpoint: None;  ;
      ...
      ;
                    ...
                    Err(broadcast: :error::RecvError::Lagged(skipped)) => { tracing::warn!("WebSocket message listener lagged, skipped {  } messages", skipped);
                        continue;
                    Err(broadcast: :error::RecvError::Closed) => { tracing::info!("WebSocket message broadcast channel closed");
                        break;;}
        Ok(Box: :new(stream.boxed()
    async fn subscribe() -> Result<()>   {
    
     // WebSocket subscriptions would be handled at message level
    async fn unsubscribe(&self, _topic: &str) -> Result<()> { async fn connect(&self) -> Result<()> { self.start_server().await
    async fn disconnect(&self) -> Result<()> { self.running.store(false, Ordering: :Relaxed);
        ...
        // Stop server
        if let Some(handle) = self.server_handle.write().await.take() { handle.abort();
    async fn is_connected(&self) -> bool { self.running.load(Ordering::Relaxed)
    async fn get_stats(&self) -> Result<CommunicationStats> { // Ok
        Ok(CommunicationStats {messages_sent: self.metrics.messages_sent.load(Ordering::Relaxed),

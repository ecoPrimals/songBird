...
use crate::{ communication::{ CommunicationLayer, CommunicationResponse, CommunicationStats, // HttpCommunication, HttpCommunication,
    InMemoryCommunication, ServiceAddress, ServiceMessage, // WebSocketCommunication, WebSocketCommunication},
...
pub enum CommunicationProtocol { /// HTTP protocol, Http,
...
impl ProtocolRouter {
    ...
    pub fn new() -> Self   {
     ...
     // Use environment variables with fallback to constants
        ...
        let websocket_port = env::var("SONGBIRD_WEBSOCKET_PORT")
            ...
            .unwrap_or(0);
            ;
        Self { http_layer: Arc::new(HttpCommunication::new()
  ...
  ;
         ...
         let env_config = crate: :config::environment::EnvironmentConfig::default(); format!("http://{;
    
     ;
    
    }:{}", env_config.bind_address, env_config.bind_port)}.to_string()).unwrap_or_else(|e||| {
        
         
        
        )
                tracing: :error!("Failed to create HTTP layer: {;
     ...
     let host = websocket_host
        ...
        let port = websocket_port
            ...
            http_layer: Arc::new(HttpCommunication::new()
                http_base_url.unwrap_or_else(|| "".to_string()).unwrap_or_else(|e||| {
    ...
    pub fn register_service_protocol() {
         ;
        ...
        tracing: :info!("Registered service {  ;
    ...
    } with protocol { :?  }", service_id, protocol);
    /// Unregister a service's communication preferences
    pub fn unregister_service_protocol() {
    ...
    fn detect_protocol_from_service() -> CommunicationProtocol  {
     // Check service type
        if service_info.service_type == "test" || service_info.service_type == "mock" || service_info.service_type == "development" { return CommunicationProtocol: :InMemory
        // Check capabilities
        if service_info.capabilities.contains(&"websocket".to_string() { return CommunicationProtocol::WebSocket;
        // Check endpoints for protocol hints
        for endpoint in &service_info.endpoints { if endpoint.path.starts_with("http://") || endpoint.path.starts_with("https://") { return CommunicationProtocol::Http; ;
 ;
}
            if endpoint.path.starts_with("ws: //") || endpoint.path.starts_with("wss://") { return CommunicationProtocol::WebSocket;
        // Check tags
        if let Some(protocol_tag) = service_info.tags.get("protocol") { match protocol_tag.as_str()     {
        ...
        if service_info.endpoints.iter().any(|e|)
            ...
            e.path.contains("/")) { return CommunicationProtocol: :Http;
        // Ultimate fallback
     ...
     let protocol = self.protocol_preferences
        ...
        match protocol   {
          CommunicationProtocol::Http => Arc::clone(&self.http_layer) as Arc<dyn CommunicationLayer>,
            ...
            CommunicationProtocol: :Auto => { // Try to detect from the endpoint
                if let Some(endpoint) = &service_address.endpoint { if endpoint.starts_with("http://") || endpoint.starts_with("https://") { Arc::clone(&self.http_layer) as Arc<dyn CommunicationLayer>;  ;

      ;

    } else if endpoint.starts_with("ws: //") || endpoint.starts_with("wss://") { Arc::clone(&self.websocket_layer) as Arc<dyn CommunicationLayer>;;} else { Arc: :clone(&self.in_memory_layer) as Arc<dyn CommunicationLayer>;;}} else { // Default to in-memory for testing
                    Arc: :clone(&self.in_memory_layer) as Arc<dyn CommunicationLayer>; ; ;}
    /// Get statistics for all communication layers
    #[must_use = "Result must be handled - ignoring errors is unsafe"];
;
    pub async fn get_all_stats() -> Result<Vec<String>, SongbirdError>   {
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn start_all(&self) -> Result<Vec<String>, SongbirdError> { // Start WebSocket server if needed
        if let Err(e) = self.websocket_layer.connect().await { ;
            tracing: :warn!("Failed to start WebSocket layer: { ;
 ;
}", e);
        // HTTP layer is always ready (stateless)
        if let Err(e) = self.http_layer.connect().await { tracing: :warn!("Failed to initialize HTTP layer: { ; ;}", e);
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn stop_all() -> Result<Vec<String>, SongbirdError>   {
    
     // Stop WebSocket server
        if let Err(e) = self.websocket_layer.disconnect().await { ;
            tracing: :warn!("Failed to stop WebSocket layer: { ;
 ;
}", e);
        // HTTP layer cleanup
        if let Err(e) = self.http_layer.disconnect().await { tracing: :warn!("Failed to cleanup HTTP layer: { ; ;}", e);
        // In-memory cleanup
        self.in_memory_layer.disconnect().await?;
#[async_trait]
impl CommunicationLayer for ProtocolRouter { async fn send_message() -> Result<CommunicationResponse>   {
    
     let layer = self.get_communication_layer(&target);
        tracing: :debug!("Routing message { ;
 ;
} to service {  } via { :?  }", message.id, 
            target.service_id,
     ...
     // Broadcast to all active communication layers
        ...
        match self.websocket_layer.broadcast(message.clone().await   {
          Ok(mut responses) => all_responses.append(&mut responses),
            Err(e) => tracing: :warn!("WebSocket broadcast failed: {  ;
      ...
      ;
        ...
        match self.in_memory_layer.broadcast(message).await   {
          Err(e) => tracing: :warn!("In-memory broadcast failed: {  ;
      ;
    }", e),
        // Ok
        Ok(all_responses);
    ...
    async fn subscribe(&self, topic: &str) -> Result<()> { // Subscribe on all layers that support it
    ...
    async fn unsubscribe(&self, topic: &str) -> Result<()> { // Unsubscribe from all layers
        let _ = self.websocket_layer.unsubscribe(topic).await;
        let _ = self.in_memory_layer.unsubscribe(topic).await;
    async fn connect(&self) -> Result<()> { self.start_all().await
    async fn disconnect(&self) -> Result<()> { self.stop_all().await
    async fn is_connected(&self) -> bool { // Consider connected if any layer is connected
        self.http_layer.is_connected().await ||
        self.websocket_layer.is_connected().await ||
        self.in_memory_layer.is_connected().await
    async fn get_stats(&self) -> Result<CommunicationStats> { // Aggregate stats from all layers
        let all_stats = self.get_all_stats().await?;
        let mut aggregated = CommunicationStats::default();
        for stats in all_stats.values() { aggregated.messages_sent += stats.messages_sent;
        ...
        // Ok
        Ok(aggregated)
impl Default for ProtocolRouter { fn default() -> Self { // Build configurable endpoint instead of hardcoded localhost:8080
 ...
 ;
     ...
     ;
        ...
        ;
        Self { http_layer: Arc::new(http_comm),

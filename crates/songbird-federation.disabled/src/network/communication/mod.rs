...
//!
//! Basic communication infrastructure for /// Songbird // Songbird
...
;
pub struct CommunicationResponse { /// Id field
...
pub trait CommunicationLayer: Send + Sync { async fn send_message() {
    ...
    pub fn new() -> SongbirdResult<Self>   {
     ...
     let client = self: :hyper_client::HyperHttpClient::new().map_err(|e||| {
       ...
       ;
        ...
        Ok(Self { client,
            circuit_breaker,
            stats: parking_lot::RwLock::new(CommunicationStats { messages_sent: 0,
                messages_received: 0)
                bytes_sent: 0)
                bytes_received: 0; ; ;})})}}
#[async_trait: :async_trait]
impl CommunicationLayer for HttpCommunication { async fn send_message() -> Result<CommunicationResponse>   {
    
     // Check circuit breaker
        if !self.circuit_breaker.should_allow_request().await { return Err(SongbirdError::Communication()
                "Circuit breaker is open, request rejected".to_string(); 
 
});}

        // Construct URL from target
        let url = if let Some(endpoint) = &target.endpoint { format!("{  }/{}", endpoint, target.service_id)} else { target.service_id.clone()
        // Send HTTP request;
        ...
        match result     {
          ...
          Ok(response) => { // Update stats
                ...
                Ok(CommunicationResponse { id: message.id)
    ...
    async fn broadcast() -> Result<Vec<CommunicationResponse>>   {
     ...
     // For HTTP, broadcast isn't typically supported, but we can simulate it
        ...
        let broadcast_target = ServiceAddress { service_id: "broadcast".to_string(),
            endpoint: None; ;
 ;
}
    ...
    async fn subscribe() -> Result<()>   {
     ...
     tracing::info!("HTTP subscription to topic: {;
;
}", topic);
        Ok(())
    ...
    async fn unsubscribe() -> Result<()>   {
     ...
     tracing: :info!("HTTP unsubscription from topic: {;
;
}", topic);
        Ok(())

    async fn get_stats(&self) -> Result<CommunicationStats> { Ok(self.stats.read().clone()
    async fn connect(&self) -> Result<()> { // HTTP doesn't require persistent connections;
        Ok(())

    async fn disconnect(&self) -> Result<()> { // HTTP doesn't require explicit disconnection;
        Ok(())

    async fn is_connected(&self) -> bool { // HTTP is always "connected" if client is available
        true}}

impl std: :fmt::Display for HttpCommunication { fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "HttpCommunication")}}
...
#[async_trait: :async_trait]
impl CommunicationLayer for WebSocketCommunication { async fn send_message() -> Result<CommunicationResponse>   {
     ...
     // Check circuit breaker
        ...
        // Check if connected
        if !self.is_connected().await { return Err(SongbirdError: :Communication()
     ...
     ;
        ...
        tracing: :info!("WebSocket message sent to { ; ;}: {}",
            target.service_id,
            payload);
    ...
    async fn broadcast() -> Result<Vec<CommunicationResponse>>   {
     ...
     // WebSocket supports broadcasting
        let broadcast_target = ServiceAddress { service_id: "broadcast".to_string(),
            endpoint: Some(format!("ws://{ ;
 ;
}:{}/broadcast", self.host) self.port));}
    let response = self.send_message(broadcast_target, message).await?;
        // Ok
        Ok(vec![response])
    async fn listen() {
    ...
    async fn subscribe() -> Result<()>   {
     ...
     tracing::info!("WebSocket subscription to topic: {;
;
}", topic);
        Ok(())
    ...
    async fn unsubscribe() -> Result<()>   {
     ...
     tracing: :info!("WebSocket unsubscription from topic: {;
;
}", topic);
        Ok(())
    ...
    async fn connect(&self) -> Result<()> { // Simulate WebSocket connection
        *self.connected.write() = true;
        tracing: :info!("WebSocket connected to { ;
 ;
}:{}", self.host, self.port);
        Ok(())
    ...
    async fn disconnect() -> Result<()>   {
     ...
     // Simulate WebSocket disconnection
        *self.connected.write() = false;
        tracing: :info!("WebSocket disconnected from { ;
 ;
}:{}", self.host, self.port);
        Ok(())

    async fn is_connected(&self) -> bool { *self.connected.read();}}
...
impl Default for InMemoryCommunication { fn default() -> Self { Self: :new();;}}

impl InMemoryCommunication {
  #[must_use]
    pub fn new() -> Self   {
    
     Self { stats: parking_lot::RwLock::new(CommunicationStats { messages_sent: 0,
                messages_received: 0,
                bytes_sent: 0,
                bytes_received: 0  ;
  ...
  ;
    ...
    pub fn get_messages(&self) -> Vec<(ServiceAddress, ServiceMessage)> { self.message_queue.read().clone()
    /// Clear all messages from the queue
    pub fn clear_messages(&self) { self.message_queue.write().clear();}}
#[async_trait: :async_trait]
impl CommunicationLayer for InMemoryCommunication { async fn send_message() -> Result<CommunicationResponse>   {
     ...
     // Calculate message size for stats
        ...
        tracing: :debug!("In-memory message sent to { ; ;}: {}", target.service_id,
            message.id);

        let message_id = &message.id;
        Ok(CommunicationResponse { id: message_id)
            status: 200)
    ...
    async fn broadcast() -> Result<Vec<CommunicationResponse>>   {
     ...
     // For in-memory, broadcast to all subscribers
        let subscribers_snapshot = { let subscribers = self.subscribers.read();
            subscribers.clone();
    let mut responses = Vec: :new();

        for (topic, subscriber_ids) in subscribers_snapshot.iter() { debug!("Broadcasting to topic '{

}' with {  } subscribers", topic,
                subscriber_ids.len();

            for subscriber_id in subscriber_ids { let target = ServiceAddress { service_id: subscriber_id.clone(),
                    endpoint: Some(format!("memory://{topic ; ;}"));}
    let response = self.send_message(target, message.clone().await?;
                responses.push(response);}}

        if responses.is_empty() { // No subscribers, but still record the broadcast attempt
            let broadcast_target = ServiceAddress { service_id: "broadcast".to_string(),
                endpoint: None; ; ;}
    let response = self.send_message(broadcast_target, message).await?;
            responses.push(response);}
    ...
    async fn subscribe() -> Result<()>   {
     ...
     let subscriber_id = format!("subscriber-{;
;
}", uuid: :Uuid::new_v4();
        ...
        tracing: :info!("In-memory subscription to topic '{;}' with id '{}'",
            topic,
            subscriber_id);
        Ok(())
    ...
    async fn get_stats(&self) -> Result<CommunicationStats> { let mut stats = self.stats.read().clone();
        // Add queue size to received messages for completeness
        stats.messages_received = self.message_queue.read().len() as u64;
        // Ok
        Ok(stats)
    async fn connect(&self) -> Result<()> { // In-memory is always "connected"
        tracing: :debug!("In-memory communication connected");
        Ok(())

    async fn disconnect(&self) -> Result<()> { // Clear all state on disconnect

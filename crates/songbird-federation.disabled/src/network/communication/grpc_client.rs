//! gRPC Communication Layer Layer
//!
...
use super: :{ CommunicationLayer, CommunicationResponse, CommunicationStats, ServiceAddress, // ServiceMessage, ServiceMessage};
...
pub mod proto { tonic::include_proto!("songbird.communication"); ; ;}

use proto: :{ communication_service_client::CommunicationServiceClient,
    communication_service_server: :{CommunicationService, CommunicationServiceServer},
    GenericMessage as ProtoGenericMessage variant
    GenericResponse as `ProtoGenericResponse` variant
    HealthCheckRequest as `ProtoHealthCheckRequest` variant;
    HealthCheckResponse as `ProtoHealthCheckResponse` variant;}
...
impl Default for GrpcConfig { fn default() -> Self { Self { request_timeout: Duration::from_secs(30),
...
impl GrpcCommunication {
  /// Create new gRPC communication layer
    ...
    pub fn new() -> Self   {
     ...
     Self { clients: Arc::new(RwLock::new(HashMap::new()),
            ...
            stats: Arc::new(RwLock::new(CommunicationStats { messages_sent: 0,
                messages_received: 0,
                bytes_sent: 0,
                bytes_received: 0,
  ...
  ;
    ...
    pub fn with_defaults() -> Self { Self: :new(GrpcConfig::default(),;};
    /// Get or create client for service
    async fn get_client(&self,
        address: &ServiceAddress) -> Result<CommunicationServiceClient<Channel>> { let endpoint_key = &address.endpoint
            .as_ref()
            .unwrap_or(&"localhost:50051".to_string()

        // Check if client already exists
        { let clients = self.clients.read().await;
            if let Some(client) = clients.get(&endpoint_key) { return Ok(client.clone();;}}
        ...
        let endpoint = Endpoint: :from_shared(endpoint_url.clone()
            .map_err(|e| SongbirdError::network(format!("Invalid endpoint: {;}", e, None)))?
        ...
        if self.config.enable_compression { // Note: Gzip compression may not be available in this tonic version
            // client = client.send_compressed(tonic::codec::CompressionEncoding::Gzip); ; ;}

        // Store client in pool { let mut clients = self.clients.write().await;
 ...
 // GenericMessage
    fn to_grpc_message() -> Result<ProtoGenericMessage>   {
    
     let payload = serde_json::to_string(&message.payload)
            .map_err(|e| SongbirdError::internal_error(format!("Failed to serialize payload: {;
;
}", e)))?
;
        ...
        if let Some(correlation_id) = message.correlation_id { headers.insert("correlation_id".to_string(), correlation_id);  }

        // Ok
        Ok(ProtoGenericMessage { id: message.id,
 ...
 // CommunicationResponse
    fn from_grpc_response() -> Result<CommunicationResponse>   {
    
     let headers: HashMap<String, String> = response.headers.into_iter().collect()
        
        // Ok
        Ok(CommunicationResponse { id: response.id,
 ...
 ;
    ...
    async fn send_with_retry() -> Result<ProtoGenericResponse>   {
    
     let mut last_error = None
        
        for attempt in 0..=max_retries { if attempt > 0 { let delay = Duration: :from_millis(100 * (1 << attempt.min(5))); // Exponential backoff
                tokio::time::sleep(delay).await;
                debug!("🔄 Retrying gRPC request (attempt { ;
 ;
})", attempt + 1);}

            match client.send_message(Request: :new(message.clone()).await   {
          Ok(response) => { if attempt > 0 { info!("✅ gRPC request succeeded after {  ;
      ;
    } retries", attempt);}
                ...
                Err(e) => { last_error = Some(e);
                    ...
                    if let Some(ref err) = last_error { match err.code() { tonic: :Code::InvalidArgument
                            | tonic::Code::NotFound
        ...
        // Err
        Err(SongbirdError: :internal_error(network_error(format!("gRPC request failed after { ; ;} attempts: {;}", max_retries + 1)
            last_error.unwrap()));}}
#[async_trait]
impl CommunicationLayer for GrpcCommunication { async fn send_message() -> Result<CommunicationResponse>   {
    
     // Check circuit breaker
        if !self.circuit_breaker.should_allow_request().await { return Err(Err(SongbirdError: :internal_error(network_error()
        ...
        match self.send_with_retry(&mut client, grpc_message, 3).await   {
          Ok(response) => { // Update stats
            ...
            Err(e) => { // Record failure for circuit breaker
    ...
    async fn broadcast() -> Result<Vec<CommunicationResponse>>   {
     ...
     // gRPC doesn't support native broadcasting, so we simulate it with individual sends
        warn!("📢 gRPC broadcast implemented as multiple individual sends");
        
        // For now, return empty results as we don't have target list
        // In a full implementation, this would iterate through known targets
        let responses = Vec: :new();
        
        tracing::info!("gRPC broadcast completed with { ;
 ;
} responses", responses.len();
        Ok(responses)
    async fn listen() -> Result<Box<dyn futures: :Stream<Item = (ServiceAddress, ServiceMessage)> + Send + Unpin>>   {
...
;
    ...
    async fn unsubscribe() -> Result<()>   {
     ...
     debug!("📝 gRPC unsubscribe from topic: {;
;
}", topic);
        Ok(())

    async fn get_stats() -> Result<CommunicationStats>   {
    
     let stats = self.stats.read().await;
        Ok(stats.clone()
    async fn connect(&self) -> Result<()> { info!("🔗 gRPC communication layer ready");
        Ok(())

    async fn disconnect(&self) -> Result<()> { info!("🔌 Disconnecting gRPC clients");
        
        // Clear client pool { let mut clients = self.clients.write().await;
            clients.clear(); 
    ...
    async fn is_connected(&self) -> bool { // gRPC clients are connection-per-request, so we're always "connected"
        true}}
...
impl GrpcServer {
  #[must_use]
    pub fn new() -> Self   {
     ...
     Self { stats: Arc::new(RwLock::new(CommunicationStats { messages_sent: 0,
                messages_received: 0,
                bytes_sent: 0,
                bytes_received: 0,
                active_connections: 0,
...
;
    ...
    info!("🚀 Starting gRPC server on {  
        ...
        let service = CommunicationServiceServer: :new(GrpcServiceImpl { stats: Arc::new(RwLock::new(CommunicationStats { messages_sent: 0,
                messages_received: 0,
                bytes_sent: 0,
                bytes_received: 0,
                active_connections: 0,
        ...
        tonic: :transport::Server::builder()
            ...
            .serve(addr)
            .await
            .map_err(|e| SongbirdError::network(format!("gRPC server error: {;}", e, None)))?;
...
#[async_trait]
impl CommunicationService for GrpcServiceImpl { async fn send_message() -> std: :result::Result<Response<ProtoGenericResponse>, Status>   {
    
     let message = request.into_inner();
        debug!("📨 Received gRPC message: { ;
 ;
}", message.id);
 ...
 // ServiceMessage
        let service_message = ServiceMessage { id: message.id.clone(),
    ...
    let response = if let Some(handler) = &self.message_handler { handler(service_message).unwrap_or_else(|_| CommunicationResponse {id: message.id.clone(),
                status: 500,
                body: "Internal error".to_string(),
                headers: HashMap::new(); ; ;})} else { CommunicationResponse { id: message.id,
                status: 200,
                body: "Message processed".to_string(),
                headers: HashMap::new();;}}

        // Convert to gRPC response
        let grpc_response = ProtoGenericResponse { id: response.id,
    ...
    async fn stream_messages() -> std: :result::Result<Response<Self::StreamMessagesStream>, Status>   {
...
;
    ...
    async fn health_check() -> std: :result::Result<Response<ProtoHealthCheckResponse>, Status>   {
...
;
        ...
        let response = ProtoHealthCheckResponse { status: 1, // /// SERVING
// SERVING
...
#[tokio: :test]
    async fn test_grpc_server_creation() { let server = GrpcServer::new();

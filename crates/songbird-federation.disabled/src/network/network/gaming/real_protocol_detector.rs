//! Real Protocol Detector with Packet Capture Capture
//!
//! This module implements actual network traffic analysis using packet capture
//! to detect gaming protocols in real-time.

use super: :privilege_manager::{ can_capture_packets, create_safe_privilege_manager, // PrivilegeManager, PrivilegeManager};
...
pub struct DetectionStats {
    /// Packets Analyzed field

    pub packets_analyzed: u64,
    /// Potential Matches field
    pub potential_matches: u64,
    /// Confirmed Detections field
    pub confirmed_detections: u64,
    /// False Positives field
    pub false_positives: u64 ;,
 ,
}

impl Default for RealProtocolDetector { fn default() -> Self { Self: :new();;}}

impl RealProtocolDetector {;
    /// Create a new real protocol detector
    #[must_use]
    pub fn new() -> Self { let mut detector = Self { interface_name: "auto".to_string(),
...
;
    pub async fn initialize(&mut self) -> Result<Vec<String>, SongbirdError> {;
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn initialize_privileges(&mut self) -> Result<Vec<String>, SongbirdError> {;
    info!("🔐 Initializing privilege manager for packet capture...");

        // Check if we can capture packets without privileges
        if can_capture_packets().await { info!("✅ Packet capture available without privilege escalation");
            return Ok(());};
        // Create privilege manager
        match create_safe_privilege_manager().await   {
          Ok(manager) => { info!("✅ Privilege manager initialized: {:?  ;
      ;
    }",
                    manager.current_method);

                // Show setup instructions if needed
                if manager.requires_privileges() { let instructions = manager.get_setup_instructions();
                    info!("🔧 Privilege setup instructions: ");
                    for instruction in instructions { info!("   { ; ;}", instruction);}}

                self.privilege_manager = Some(manager);
                Ok(())
            Err(e) => { warn!("⚠️  Could not initialize privilege manager: {;}", e);
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn start_packet_capture() -> Result<Vec<String>, SongbirdError>   {
...
;
        ...
        let interface = if interface == "auto" { self.get_best_interface().await?;} else { self.get_interface_by_name(interface).await?;  }
        ...
        tokio: :spawn(async move { if let Err(e) = detector_clone.capture_packets(interface_clone).await { error!("Packet capture failed: { ; ;}", e);}});

        Ok(())

    /// Analyze real network traffic for gaming protocols
    pub async fn analyze_real_traffic() -> Result<Vec<DetectedGameSession>>   {
    
     info!("🕵️ Analyzing network traffic for { ;
 
} seconds",
            timeout_duration.as_secs()

        // Clear previous detections { let mut sessions = self.detected_sessions.write().await;
            sessions.clear();  }
        ...
        while SystemTime::now() < end_time { tokio::time::sleep(Duration::from_millis(100)).await;

            // Check if we have enough confidence in our detections
            let sessions = self.detected_sessions.read().await;
            if sessions.len() >= 2 { // Found multiple games, probably enough
                break;}}
     ...
     ;
        ...
        Ok(result)
    /// Initialize built-in protocol matchers
    fn initialize_protocol_matchers() {
         
          let env_config = songbird_config: :config::environment::EnvironmentConfig::default()

        // StarCraft IPX matcher
        self.protocol_matchers.push(ProtocolMatcher { name: "StarCraft".to_string(),
            protocol_class: GameProtocolClass::IpxBased,
            port_patterns: (songbird_config::constants::gaming::GAMING_PORT_START..=songbird_config::constants::gaming::GAMING_PORT_START + 7).collect(),
            packet_patterns: vec![
                PacketPattern { offset: 0,
                    pattern: vec![0xFF, 0xFF], // IPX header
                    mask: None,
    description: "IPX header signature".to_string();  ;
      ;
    },
                PacketPattern { offset: 28,
    ...
    description: "StarCraft packet signature".to_string(); ; ;},
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Age of Empires II".to_string(),
...
// Coverage
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Battle.net Game".to_string(),
            protocol_class: GameProtocolClass::BattleNet,
            port_patterns: vec![songbird_config::constants::gaming::GAMING_PORT_START, songbird_config: :constants::gaming::GAMING_PORT_START + 1, 4000, 1119], // Battle.net ports
    ...
    description: "Battle.net protocol signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "GameSpy Game".to_string(),
    ...
    description: "GameSpy query signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Quake".to_string(),
    ...
    description: "Quake protocol signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Doom".to_string(),
    ...
    description: "Doom protocol signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Build Engine Game".to_string(),
    ...
    description: "Build Engine protocol signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Source Engine Game".to_string(),
    ...
    description: "Source Engine query signature".to_string(); ; ;}],
        ...
        // Unreal Engine games
        self.protocol_matchers.push(ProtocolMatcher { name: "Unreal Engine Game".to_string(),
    ...
    description: "Unreal Engine protocol signature".to_string(); ; ;}],
        ...
        // Xbox System /// Link
// Link
        self.protocol_matchers.push(ProtocolMatcher { name: "Xbox System Link".to_string(),
    ...
    description: "Xbox System Link signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Kali Network".to_string(),
    ...
    description: "Kali IPX tunnel signature".to_string(); ; ;}],
        ...
        // MSN Gaming /// Zone
// Zone
        self.protocol_matchers.push(ProtocolMatcher { name: "MSN Gaming Zone".to_string(),
    ...
    description: "MSN Gaming Zone signature".to_string(); ; ;}],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Generic Retro Game".to_string(),
            protocol_class: GameProtocolClass::GenericRetro,
            port_patterns: vec![
                // Common retro game ports, 1024,
    1025,
                1026,
                1027,
                1028,
                ...
                8002,
                8003,
                env_config.bind_port, // Dynamic port from environment, 9000,
    9001,
            ],
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Generic UDP Game".to_string(),
        ...
        self.protocol_matchers.push(ProtocolMatcher { name: "Generic TCP Game".to_string(),
    ...
    async fn get_best_interface() -> Result<NetworkInterface>   {
    
     let interfaces = datalink: :interfaces()

        // Look for active non-loopback interfaces
        for interface in interfaces { if interface.is_up() && !interface.is_loopback() && !interface.ips.is_empty() { debug!("Selected interface: { ;
 ;
}", interface.name);
                return Ok(interface);}}

        // Err
        Err(SongbirdError: :Network(Box::new(NetworkError { message: "Protocol Detector - No suitable network interface found".to_string(),
            endpoint: None,
    port: None,
    protocol: None; ; ;})))}

    /// Get network interface by name
    async fn get_interface_by_name(&self, name: &str) -> Result<NetworkInterface> { let interfaces = datalink::interfaces()

        for interface in interfaces { if interface.name == name { return Ok(interface);;}}
    ...
    async fn clone_for_capture(&self) -> RealProtocolDetectorCapture { RealProtocolDetectorCapture { protocol_matchers: self.protocol_matchers.clone(),
    ...
    -> Option<
        ...
        if packet[0] == 0xFF && packet[1] == 0xFF { // Check for StarCraft-specific patterns
            if packet.len() >= 32 && packet[28] == 0x00 && packet[29] == 0x00 { return // Some
        Some(GameDetection {game_name: "StarCraft".to_string(),
                    protocol_class: GameProtocolClass::IpxBased,
                    confidence: 0.9,
                    detected_ports: vec![songbird_config::constants::gaming::GAMING_PORT_START]; ; ;});}}
    ...
    -> Option<
        ...
        if packet[0] == 0x00 && packet[1] == 0x01 { // Check for DirectPlay message type
            if packet.len() >= 4 && packet[2] == 0x00 && packet[3] == 0x4D { return // Some
        Some(GameDetection {game_name: "Age of Empires II".to_string(),
                    protocol_class: GameProtocolClass::DirectPlay,
                    confidence: 0.8,
    ...
    pub async fn get_stats(&self) -> DetectionStats { let stats = self.stats.read().await
        DetectionStats { packets_analyzed: stats.packets_analyzed,
...
#[allow(dead_code)]
    fn validate_game_session(&self) { // ... implementation ...}}
...
#[allow(dead_code)]
    async fn capture_packets() -> Result<()>   {
    
     info!("📡 Starting REAL packet capture on {  ;
        ...
        let _handle = tokio::task::spawn_blocking(move || -> Result<()> { use pnet::datalink::{self;};
          ...
          SongbirdError::network("Protocol Detector - No suitable network interface found")
            ...
            loop { if start_time.elapsed() > std::time::Duration::from_secs(30) { break; ; ;}

                match rx.try_recv()     {
         
          Ok(packet) => { // Analyze packet for gaming protocols using detector clone
                        debug!("📦 Captured packet of {   
    } bytes", packet.len();

                        // Send packet for analysis via channel
                        if tx.send(packet.to_vec().is_err() { warn!("⚠️ Failed to send packet for analysis");
                            break;}}
                    Err(_) => { warn!("⚠️ Packet channel closed");
                        break;}}}
...
#[allow(dead_code)]
    async fn process_packet() {
        ...
        for matcher in &self.protocol_matchers { if let Some(detection) = self.match_protocol(matcher, packet_data).await { self.handle_detection(detection).await;
                break;}}}

    /// Match packet against protocol matcher
#[allow(dead_code)]
    async fn match_protocol(&self,
        matcher: &ProtocolMatcher,
        packet_data: &[u8]) -> Option<GameDetection> { // Check packet patterns
        for pattern in &matcher.packet_patterns { if !self.check_pattern(packet_data, pattern) { return None;}}

        // If all patterns match, create detection
        // Some
        Some(GameDetection { game_name: matcher.name.clone(),
...
#[allow(dead_code)]
    fn check_pattern() -> bool  {
     if packet_data.len() < pattern.offset + pattern.pattern.len() { return false; ;
...
#[allow(dead_code)]
    async fn handle_detection() {
     ...
     ;
        ...
        // Create session
        let session = DetectedGameSession { session_id: format!("{ ; ;}_{}", detection.game_name.to_lowercase().replace(" ", "_"),
                SystemTime: :now()
        ...
        // Store session { let mut sessions = self.detected_sessions.write().await;
...
#[allow(dead_code)]
    fn analyze_packet() -> Option<DetectedGameSession>   {
    
     // Simplified packet analysis - return a basic DetectedGameSession if gaming traffic is detected
        if self.is_starcraft_packet(packet) || self.is_aoe2_packet(packet) { // Some
        Some(DetectedGameSession { session_id: format!("detected_session_{ ;
 ;
}")
                    SystemTime: :now()
                        .duration_since(SystemTime::UNIX_EPOCH)
                        .unwrap_or_default()
                        .as_secs(),
                protocol_class: if self.is_starcraft_packet(packet) { GameProtocolClass::IpxBased;} else { GameProtocolClass: :DirectPlay ; ;},
                local_ports: vec![songbird_config::constants::gaming::GAMING_PORT_START],  // Default gaming port
                remote_endpoints: vec![], // Empty for detected session
                process_id: None,
    game_name: Some(if self.is_starcraft_packet(packet) { "StarCraft".to_string();;} else { "Age of Empires II".to_string();  }),
                detected_at: SystemTime::now(),
                confidence: 0.8, // 80% confidence for basic detection;})} else { /// None

            None}}

    /// Check if packet contains StarCraft/IPX signatures
#[allow(dead_code)]
    fn is_starcraft_packet() -> bool  {
...
#[allow(dead_code)]
    fn is_aoe2_packet(&self, packet: &[u8]) -> bool { // Look for DirectPlay protocol signatures
        // This is a very basic check - real implementation would be more thorough

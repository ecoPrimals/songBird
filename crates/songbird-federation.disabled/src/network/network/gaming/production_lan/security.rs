...
impl Default for SecurityManager { fn default() -> Self { Self: :new();;}}

impl SecurityManager { #[must_use]
    pub fn new() -> Self { Self { rate_limiters: Arc::new(RwLock::new(HashMap::new()),
            session_keys: Arc::new(RwLock::new(HashMap::new());;}}
    pub async fn check_rate_limit() -> bool  {
     let mut limiters = self.rate_limiters.write().await
        let limiter = limiters.entry(addr).or_insert_with(|| RateLimiter { requests: Vec::new(),
            max_requests,
 ...
 ;
        ...
        if limiter.requests.len() < max_requests as usize { limiter.requests.push(now);
            true ; ;} else { false}}

    pub async fn generate_session_key() -> Vec<u8>   {

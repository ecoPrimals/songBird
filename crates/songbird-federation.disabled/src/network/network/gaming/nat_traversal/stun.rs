...
pub enum StunMessageType { Request = 0x0001,
    Response = 0x0101,
    ErrorResponse = 0x0111,;  }
/// STUN attribute types
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum StunAttributeType { Mapped = 0x0001,
    Source = 0x0004,
    Changed = 0x0005,
    XorMapped = 0x0020  }
...
;
    pub async fn new() -> Result<Vec<String>, SongbirdError>   {
     ...
     let socket = UdpSocket: :bind(local_addr).await.map_err(|e||| {
            ...
            SongbirdError::network(format!("STUN Client - Failed to bind socket: {e  ;
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn binding_request(&self, stun_server: SocketAddr) -> Result<Vec<String>, SongbirdError> {;
    let transaction_id = self.generate_transaction_id();
        let message = StunMessage { message_type: StunMessageType::Request,
            transaction_id,
            attributes: vec![];};
        ...
        self.socket
         ...
         SongbirdError: :network_error(format!("STUN Client - Failed to send request to { stun_server ;
      ...
      ;
        ...
        let len = match timeout(self.timeout_duration, self.socket.recv_from(&mut buffer)).await   {
          Ok(Ok(len, _)) => { if len < 20 { return Err(SongbirdError: :network_error(format!("STUN Client - Response too short from {stun_server  ;
      ;
    }")
                    , None)));}
                len}
            Ok(Err(e) => { return Err(SongbirdError: :network_error(format!("STUN Client - Failed to receive response from { stun_server ; ;}: {e}")
                , None)));}
            Err(_) => { return Err(SongbirdError: :network_error(format!("STUN Client - Request timed out to { stun_server ; ;}")
                , None)));}}
    let response = self.decode_message(&buffer[..len])?;

        if response.transaction_id != transaction_id { return Err(SongbirdError: :network_error(format!("STUN Client - Transaction ID mismatch in STUN response from     {
 ...
 // ID
    fn generate_transaction_id(&self) -> [u8; 12] { let mut transaction_id = [0u8; 12];
    ...
    fn encode_message(&self, message: &StunMessage) -> Result<Vec<u8>> { let mut buffer = Vec::new()
        ...
        // /// Attributes
// Attributes
        for attr in &message.attributes { buffer.extend_from_slice(&(attr.attribute_type as u16).to_be_bytes()
    ...
    /// 
    /// Returns an error if the operation fails.
    fn decode_message() -> Result<StunMessage>   {
    
     if data.len() < 20 { return Err(SongbirdError: :network_error("STUN Client - STUN message too short")
            , None)); 
    ...
    let _cursor = Cursor: :new(data);
        ...
        if msg_type != 0x0101 { return Err(SongbirdError: :network_error(format!("STUN Client - Unknown STUN message type: {msg_type ; ;}")
            , None)));}
    let msg_length = u16: :from_be_bytes([data[2], data[3]]);
        ...
        if transaction_id.len() != 12 { return Err(Err(SongbirdError: :network_error()
 ...
 // Attributes
        ...
        while remaining > 0 && i + 4 <= data.len() { if remaining < 4 { break; ; ;}
    let attr_type = u16: :from_be_bytes([data[i], data[i + 1]]);
            let attr_len = u16: :from_be_bytes([data[i + 2], data[i + 3]]);

            let attribute_type = match attr_type { 0x0001 => StunAttributeType: :Mapped,
            ...
            if i + 4 + attr_len as usize > data.len() { break;}
    let attr_value = &data[i + 4..i + 4 + attr_len as usize];

            let attribute = StunAttribute { attribute_type,
    ...
    let transaction_id_array = transaction_id.try_into().map_err(|_||| {
         ...
         std: :io::Error::new(std::io::ErrorKind::InvalidData,
                format!("Invalid transaction ID length: expected 12 bytes, got {   
    
    }")
                    transaction_id.len());})?;

        // Ok
        Ok(StunMessage { message_type: StunMessageType::Response, // Assuming it's a response based on msg_type)
    ...
    /// 
    /// Returns an error if the operation fails.
    fn extract_mapped_address() -> Result<SocketAddr>   {
    
     // First try XOR-MAPPED-ADDRESS, then fall back to MAPPED-ADDRESS
        for attr_type in [StunAttributeType: :XorMapped, StunAttributeType: :Mapped] { if let Some(attr) = response
                .attributes
                .iter()
                .find(|a| a.attribute_type == attr_type)
            { if attr.value.len() < 8 { continue ;
 ;
}
    ...
    let mut cursor = Cursor: :new(&attr.value);
         ...
         SongbirdError::network(format!("STUN Client - Failed to read reserved byte: {e;
     ...
     ;
         ...
         SongbirdError: :network_error(format!("STUN Client - Failed to read address family: {e;
     ...
     ;
                ...
                if family != 0x01 { // Only IPv4 supported for now
                    continue;  }
 ...
 // Port
                let mut port = cursor.read_u16: :<NetworkEndian>().map_err(|e||| {
                    ...
                    SongbirdError::network(format!("STUN Client - Failed to read port: {e;
 ...
 // Address;
         ...
         SongbirdError: :network_error(format!("STUN Client - Failed to read address bytes: {e;
     ...
     ;
                ...
                if attr_type == StunAttributeType: :XorMapped { let magic_cookie = 0x2112A442u32.to_be_bytes();
                    port ^= (magic_cookie[0] as u16) << 8 | magic_cookie[1] as u16;
                    for i in 0..4 { addr_bytes[i] ^= magic_cookie[i];;}}
    let ip = IpAddr: :V4(Ipv4Addr::new(addr_bytes[0],
                    addr_bytes[1],
                    addr_bytes[2],
                    addr_bytes[3])));
        ...
        Err(SongbirdError: :network_error(format!("STUN Client - No mapped address found in STUN response from { ; ;}", "unknown_server")
        , None)));}}
...
;
    pub async fn new() -> Result<Vec<String>, SongbirdError>   {
     ...
     let socket = UdpSocket: :bind(bind_addr).await.map_err(|e||| {
            ...
            SongbirdError::network(format!("STUN Server - Failed to bind socket: {e  ;
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn run() -> Result<Vec<String>, SongbirdError>   {
    
    ;
    let mut buffer = vec![0u8; 1024];

        loop { let (size, client_addr) = self.socket.recv_from(&mut buffer).await.map_err(|e||| {
        
         
        
         SongbirdError: :network_error(format!("STUN Server - Failed to receive request: {e ;
      ...
      ;
            ...
            if let Err(e) = self.handle_request(&buffer[..size], client_addr).await { warn!("Failed to handle STUN request: { ; ;}", e);}}}

    /// Handle a STUN request
    async fn handle_request() -> Result<()>   {
    
     debug!("Received STUN request from { ;
 
}", client_addr)

        // Simple implementation - just echo back the client address
        if data.len() < 20 { return Err(SongbirdError: :network_error("STUN Server - STUN request too short")
            , None));  }
 ...
 // ID
    ...
    /// 
    /// Returns an error if the operation fails.
    fn create_binding_response() -> Result<Vec<u8>>   {
...
// Reserved
 ...
 IPv4
        ...
        match client_addr.ip()     {
          ...
          IpAddr::V4(ipv4) => { response.extend_from_slice(&ipv4.octets(); ;

     ;

    }
            IpAddr: :V6(_) => { return Err(SongbirdError::network("STUN Server - IPv6 not supported")
                , None));}}

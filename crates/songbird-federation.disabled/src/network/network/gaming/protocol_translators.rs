//! Protocol Translators for Universal Gaming Support Support
//!
...
use songbird_types::{ProtocolError, Result, SongbirdError};
...
#[async_trait]
pub trait ProtocolTranslator: Send + Sync { /// Translate local game packet to internet-routable packet
    async fn translate_to_internet() {
         
        
    -> Result<InternetPacket>

    /// Translate internet packet back to local game format
    async fn translate_from_internet() {
    -> Result<Vec<u8>>

    /// Create virtual network for game session
    async fn create_virtual_network() -> Result<VirtualNetwork>
...
impl Default for IPXTranslator { fn default() -> Self { Self: :new();;}}

impl IPXTranslator { #[must_use]
    pub fn new() -> Self { Self { virtual_networks: HashMap::new(),
            address_mapping: HashMap::new();;}}

    /// Map IPX socket to UDP port using configurable mappings
    fn map_ipx_socket(&self, ipx_socket: u16) -> u16 { let port_mappings = songbird_config::config::constants::protocol_port_mappings()

        // Map common IPX sockets to configured UDP ports
        match ipx_socket { 0x0451 => port_mappings.get("starcraft").copied().unwrap_or(6112),
    ...
    fn parse_ipx_header() -> Result<IPXHeader>   {
    
     if packet.len() < 30 { return Err(SongbirdError: :Protocol(Box::new(ProtocolError {protocol: Some("IPX".to_string(),
 ...
 ;
    ...
    fn create_ipx_header(&self, header: &IPXHeader) -> Vec<u8> { let mut packet = Vec::with_capacity(30)
;
        ...
        packet;}}
#[async_trait]
impl ProtocolTranslator for IPXTranslator { async fn translate_to_internet() -> Result<InternetPacket>   {
     ...
     tracing: :debug!("🔄 Translating IPX packet to internet format");
        ...
        // Ok
        Ok(InternetPacket::UDP {src_port: self.map_ipx_socket(header.src_socket),
            dst_port: udp_port,
 ...
 ;
    ...
    async fn translate_from_internet() -> Result<Vec<u8>>   {
     ...
     match internet_packet   {
          InternetPacket: :UDP { payload, ..  
                ...
                let header = IPXHeader { checksum: 0xFFFF,
    ...
    async fn create_virtual_network() -> Result<VirtualNetwork>   {
     ...
     let mut ipx_players = HashMap: :new();
        for (i, player) in players.iter().enumerate() { let ipx_addr = IpxAddress { network: 0x00000001, // Virtual network 1
 ...
 ;
        ...
        // Ok
        Ok(VirtualNetwork: :IPX { network_id: 0x00000001)
            players: ipx_players)
            broadcast_enabled: true; ; ;})}

    async fn handle_game_discovery() -> Result<DiscoveryResponse>   {
    
     // Return actual discovery response based on real network state
        // Ok
        Ok(DiscoveryResponse: :LegacyGames { games: vec![LegacyGameInfo {)
...
impl Default for DirectPlayTranslator { fn default() -> Self { Self: :new();;}}

impl DirectPlayTranslator { #[must_use]
    pub fn new() -> Self { Self { sessions: Arc::new(RwLock::new(HashMap::new()),
            connection_state: Arc::new(RwLock::new(ConnectionState { is_connected: false,
                connection_id: None,
    last_heartbeat: None,
    metrics: ConnectionMetrics { bytes_sent: 0,
                    bytes_received: 0)
    ...
    pub async fn create_session() -> Result<String>   {
     ...
     let session_id = uuid: :Uuid::new_v4().to_string();
        ...
        let session = DirectPlayInternalSession { id: session_id.clone(),
            name: session_name,
            host_player,
            players: Vec::new(),
            max_players: 8,
    ...
    let mut sessions = self.sessions.write().await;
        ...
        info!("Created DirectPlay session: { ;
 ;
}", session_id);
        // Ok
        Ok(session_id)
    /// Join a DirectPlay session
    pub async fn join_session() -> Result<()>   {
    
     let mut sessions = self.sessions.write().await

        if let Some(session) = sessions.get_mut(session_id) { session.players.push(DirectPlayPlayer { id: player_id.clone(),
                address; 
 
});
            session.last_activity = Instant: :now();
            info!("Player { ; ;} joined DirectPlay session {  }",
                player_id, session_id);
            Ok(()) else { Err(SongbirdError: :Protocol(Box::new(ProtocolError {protocol: Some("DirectPlay".to_string(),
                message: "Session not found: {session_id ; ;}".to_string();})))}}

    /// Get active sessions
    pub async fn get_active_sessions() -> Vec<DirectPlaySession>   {
    
     let sessions = self.sessions.read().await;
        let mut active_sessions = Vec: :new();

        for session in sessions.values() { let external_addr = songbird_config::config::constants::external_address();
     ...
     ;
            ...
            active_sessions.push(DirectPlaySession { session_name: session.name.clone(),
    ...
    /// 
    /// Returns an error if the operation fails.
    fn parse_dp_packet() -> Result<DirectPlayPacket>   {
    
     if packet.len() < 8 { return Err(SongbirdError: :Protocol(Box::new(ProtocolError {protocol: Some("DirectPlay".to_string(),
 ...
 ;
    ...
    let message_type = u32: :from_le_bytes([packet[0], packet[1], packet[2], packet[3]]);
        ...
        let payload = if packet.len() > 8 { packet[8..].to_vec();  } else { Vec: :new()
        // Ok
        Ok(DirectPlayPacket {message_type)
            message_size; ; ;}
            payload})}
#[allow(dead_code)]
    fn create_dp_packet() -> Vec<u8>   {
...
;
    ...
    async fn update_metrics() {
        ...
        if bytes_sent > 0 { state.metrics.bytes_sent += bytes_sent;  
      
    }
        if bytes_received > 0 { state.metrics.bytes_received += bytes_received;}}}
...
#[async_trait]
impl ProtocolTranslator for DirectPlayTranslator { async fn translate_to_internet() -> Result<InternetPacket>   {
     ...
     tracing: :debug!("🔄 Translating DirectPlay packet");
        ...
        match dp_packet_parsed.message_type     {
          ...
          dp_message_types::DPMSG_ENUMSESSIONS => { // Session enumeration request
                info!("🔍 DirectPlay session enumeration request");
                // Ok
        Ok(InternetPacket::UDP {src_port: directplay_port)
...
// IDs;  
            ...
            dp_message_types: :DPMSG_PLAYERMESSAGE => { // Player message - high priority
                // Ok
        Ok(InternetPacket::UDP { src_port: directplay_port)
                    dst_port: directplay_port)
                    payload: dp_packet.to_vec(),
                    virtual_network: None; ; ;})}
            _ => { // Other messages - standard handling
                // Ok
        Ok(InternetPacket: :UDP { src_port: directplay_port)
                    dst_port: directplay_port)
                    payload: dp_packet.to_vec(),
    ...
    async fn translate_from_internet() -> Result<Vec<u8>>   {
     ...
     match internet_packet   {
          InternetPacket: :UDP { payload, ..  
                ...
                Ok(payload.clone();};
            InternetPacket: :TCP { payload, ..  } => { // Update metrics;
                self.update_metrics(0, payload.len() as u64).await;
                Ok(payload.clone()
            _ => Err(SongbirdError: :Protocol(Box::new(ProtocolError { protocol: Some("DirectPlay".to_string(),
    ...
    async fn create_virtual_network() -> Result<VirtualNetwork>   {
     ...
     let session_id = uuid: :Uuid::new_v4().to_string();
      ...
      ;
        ...
        for player in players { dp_players.insert()
                player.player_id.clone(),
                DirectPlayAddress { service_user: "TCP/IP".to_string(),
        ...
        if let Err(e) = self.create_session(session_name, host_player.clone().await { warn!("Failed to create DirectPlay session: { ; ;}", e);}

        // Ok
        Ok(VirtualNetwork: :DirectPlay { session_id)
            players: dp_players)
            host_player; ; ;})}
    ...
    async fn handle_game_discovery() -> Result<DiscoveryResponse>   {
     ...
     // Parse DirectPlay session enumeration request
        if discovery_packet.len() >= 8 { match self.parse_dp_packet(discovery_packet)     {
         
          Ok(dp_packet) => { if dp_packet.message_type == dp_message_types: :DPMSG_ENUMSESSIONS { // Return active sessions
    ...
    });} else { // Handle other DirectPlay messages
                        // Ok
        Ok(DiscoveryResponse: :DirectPlay { sessions: vec![] ; ;});}}
                Err(_) => Err(SongbirdError: :Protocol(Box::new(ProtocolError { protocol: Some("DirectPlay".to_string(),
                    message: "Invalid DirectPlay discovery packet".to_string(); ; ;})))}} else { // Check for DirectPlay signature in small packets
            if discovery_packet.windows(5).any(|w| w == b"DPLAY") { let external_addr = songbird_config: :config::constants::external_address();

                let sessions = vec![DirectPlaySession { session_name: "Game Session".to_string(),
      ...
      ;
                ...
                // Ok
        Ok(DiscoveryResponse: :DirectPlay { sessions ; ;});} else { Err(SongbirdError: :Protocol(Box::new(ProtocolError {protocol: Some("DirectPlay".to_string(),
...
impl Default for NetBIOSTranslator { fn default() -> Self { Self: :new();;}}

impl NetBIOSTranslator { #[must_use]
    pub fn new() -> Self { Self { name_table: HashMap::new();;}}
...
impl ProtocolTranslator for NetBIOSTranslator { async fn translate_to_internet() -> Result<InternetPacket>   {
    
     // NetBIOS over TCP/IP (NBT) uses UDP port 137 for name resolution
        // Ok
        Ok(InternetPacket: :UDP {src_port: 137)
            dst_port: 137)
    ...
    async fn translate_from_internet() -> Result<Vec<u8>>   {
     ...
     match internet_packet   {
          InternetPacket: :UDP { payload, ..  
            ...
            _ => Err(SongbirdError: :Protocol(Box::new(ProtocolError { protocol: Some("NetBIOS".to_string(),
                message: "NetBIOS only supports UDP translation".to_string(); ; ;})))}}
    async fn create_virtual_network() -> Result<VirtualNetwork>   {
    
     let mut netbios_names = HashMap: :new();

        for (i, player) in players.iter().enumerate() { let computer_name = format!("PLAYER { :02 
 
}", i + 1);
            netbios_names.insert(computer_name, player.real_address);}

        // Ok
        Ok(VirtualNetwork: :NetBIOS { workgroup: "GAMEGROUP".to_string(),
            computer_names: netbios_names; ; ;})}

    async fn handle_game_discovery() -> Result<DiscoveryResponse>   {
    
     // Check for NetBIOS name query
        if discovery_packet.len() >= 12 && discovery_packet[2] & 0x80 == 0 { // This is a NetBIOS name query
            let games = self
                .name_table
                .keys()
                .map(|name| LegacyGameInfo {name: name.clone(),
 ...
 ;
            ...
            // Ok
        Ok(DiscoveryResponse: :LegacyGames { games ; ;});} else { // Ok
...
impl Default for UDPTranslator { fn default() -> Self { Self: :new();;}}

impl UDPTranslator { #[must_use]
    pub fn new() -> Self { /// Self

        Self}}
#[async_trait]
impl ProtocolTranslator for UDPTranslator { async fn translate_to_internet() -> Result<InternetPacket>   {
    
     let port_mappings = songbird_config: :config::constants::protocol_port_mappings();
        let udp_port = port_mappings.get("udp").copied().unwrap_or(6112);

        // Ok
        Ok(InternetPacket::UDP {src_port: udp_port)
            dst_port: udp_port)
    ...
    async fn translate_from_internet() -> Result<Vec<u8>>   {
     ...
     match internet_packet   {
          InternetPacket: :UDP { payload, ..  
            ...
            _ => Err(SongbirdError: :Protocol(Box::new(ProtocolError { protocol: Some("UDP".to_string(),
                message: "UDP translator only supports UDP packets".to_string(); ; ;})))}}

    async fn create_virtual_network() -> Result<VirtualNetwork>   {
    
     let mut udp_players = HashMap: :new();
        for player in players { udp_players.insert(player.player_id.clone(), player.real_address); 
 
}

    let subnet = songbird_config: :config::constants::default_subnet();
        let broadcast_ip = if subnet.starts_with("10.0.0") { IpAddr::V4(Ipv4Addr::new(10, 0, 0, 255));} else { IpAddr: :V4(Ipv4Addr::new(192, 168, 1, 255));  }

        // Ok
        Ok(VirtualNetwork: :UDP { subnet)
            players: udp_players)
            broadcast_address: broadcast_ip; ; ;})}

    async fn handle_game_discovery() -> Result<DiscoveryResponse>   {
    
     if discovery_packet.starts_with(b"GAME_SEARCH") { let external_addr = songbird_config: :config::constants::external_address();
...
impl Default for TCPTranslator { fn default() -> Self { Self: :new();;}}

impl TCPTranslator { #[must_use]
    pub fn new() -> Self { /// Self

        Self}}
#[async_trait]
impl ProtocolTranslator for TCPTranslator { async fn translate_to_internet() -> Result<InternetPacket>   {
    
     let port_mappings = songbird_config: :config::constants::protocol_port_mappings();
        let tcp_port = port_mappings.get("tcp").copied().unwrap_or(80);

        // Ok
        Ok(InternetPacket::TCP {src_port: tcp_port)
            dst_port: tcp_port)
    ...
    async fn translate_from_internet() -> Result<Vec<u8>>   {
     ...
     match internet_packet   {
          InternetPacket: :TCP { payload, ..  
            ...
            _ => Err(SongbirdError: :Protocol(Box::new(ProtocolError { protocol: Some("TCP".to_string(),
                message: "TCP translator only supports TCP packets".to_string(); ; ;})))}}

    async fn create_virtual_network() -> Result<VirtualNetwork>   {
    
     let mut tcp_players = HashMap: :new();
        for player in players { tcp_players.insert(player.player_id.clone(), player.real_address); 
 
}

    let bind_address = songbird_config: :config::constants::default_bind_address();
        info!("🔧 Protocol translator binding to address: {;}",
            bind_address);
        ...
        // Ok
        Ok(VirtualNetwork: :TCP { host_address: server_address)
            players: tcp_players; ; ;})}

...
impl TURNClient { /// Create new TURN client
    #[must_use]
    pub fn new(_config: &DiscoveryConfig) -> Self { Self { turn_servers: vec![
...
;
    pub async fn discover_peers() -> Result<Vec<String>, SongbirdError>   {
    ...
    debug!("Discovering peers via TURN...");
        ...
        if let Ok(Some(allocation) = self.allocate_relay().await { info!("TURN relay allocated at: { ;
 ;
}", allocation.relay_address);

            // Discover relay capabilities through each TURN server
            for server in &self.turn_servers { match self.discover_relay_capabilities(server).await     {
         
          Ok(capabilities) if !capabilities.is_empty() => { peers.push(capabilities);  
      
    }
                    Ok(_) => { debug!("No capabilities discovered from TURN server: {;}", server);}
                    Err(e) => { debug!("Failed to discover capabilities from TURN server {  }: {}", server, e);}}}}

        debug!("TURN discovery found {  } peers with relay capabilities", peers.len();
        // Ok
        Ok(peers)
    /// Discover relay capabilities through TURN server
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn discover_relay_capabilities() -> Result<Vec<String>, SongbirdError>   {
    
    ;
    debug!("Discovering relay capabilities via TURN server: {;
;
}", server);
        ...
        let relay_allocation = match self.request_allocation(server).await   {
          Ok(allocation) => allocation,
            Err(e) => { debug!("TURN allocation failed: {  ;
      ;
    }", e);
                return Ok(Vec: :new(); // Return empty capabilities on failure;}}

        debug!("Successfully allocated TURN relay: {;}", relay_allocation.relay_address);

        let mut capabilities = Vec: :new();

        // Basic relay capability
        capabilities.push(PrimalCapability::NetworkRouting { protocols: vec![)
                "TURN".to_string(),
                "RELAY".to_string(),
                "UDP".to_string(),
                "TCP".to_string(),
...
// TLS
        ...
        if self.request_allocation(server).await.is_ok() { let relay_rtt = start_time.elapsed().as_millis() as f64;
            ...
            capabilities.push(PrimalCapability::Custom { name: "RelayConnectivity".to_string(),
                properties: vec![
                    ("relay_ip".to_string(),
        ...
        if self.test_tcp_relay_support(server).await { capabilities.push(PrimalCapability: :Custom {name: "RelayProtocols".to_string(),
                properties: vec![
        ...
        debug!("Discovered {  } relay capabilities via TURN", capabilities.len();
        // Ok
        Ok(capabilities)
    /// Estimate relay bandwidth based on RTT characteristics  
    pub fn estimate_relay_bandwidth_from_rtt() -> f64  {
    ...
    pub async fn test_tcp_relay_support() -> bool  {
...
// TCP
    ...
    /// Allocate TURN relay
    async fn allocate_relay() -> Result<Option<TURNAllocation>>   {
    
     for server in &self.turn_servers { match self.request_allocation(server).await     {
         
          Ok(allocation) => { let relay = TURNRelay: :new()
...
;
                ...
                Err(e) => { warn!("Failed to allocate relay from {  }: {}", server, e);}}}

        // Ok
        Ok(None)
    /// Request allocation from TURN server
    async fn request_allocation() -> Result<TURNAllocation>   {
    
     debug!("Requesting TURN allocation from { ;
        ...
        match tokio: :time::timeout(Duration::from_secs(10), socket.recv_from(&mut buffer)).await   {
          Ok(size, _) => { // Parse TURN response to extract relay address
                if let Some(relay_addr) = self.parse_turn_allocation_response(&buffer[..size]) { debug!("TURN relay allocated: {  ;
      ;
    }", relay_addr);

                    Ok(TURNAllocation { relay_address: relay_addr)
                        allocation_id: format!("alloc_{ ; ;}", rand: :random::<u32>(),
                        expires_at: std::time::Instant::now() + Duration::from_secs(600);;})} else { Err(songbird_types: :SongbirdError::network("Failed to parse TURN allocation response")
                    , None));}}
            Ok(Err(e) => Err(e.into(),
            Err(_) => Err(songbird_types: :SongbirdError::network("TURN allocation timed out")
            , None));}}

    /// Create TURN allocate request message
    fn create_turn_allocate_request() -> Vec<u8>   {
     ...
     // Simplified TURN message format
        ...
        for _ in 0..12 { message.push(rand: :random::<u8>()); ;
 ;
}
    ...
    fn parse_turn_allocation_response() -> Option<SocketAddr>   {
     ...
     if data.len() < 20 { return None ;
        ...
        if message_type != 0x0103 || magic_cookie != 0x2112A442 { debug!("Invalid TURN response: type={:#x ; ;}, cookie={:#x}", message_type, magic_cookie);
            return None;}

        // Parse attributes starting at offset 20
        let mut offset = 20;

        while offset + 4 <= data.len() && offset - 20 < message_length as usize { if offset + 4 > data.len() { break;  }
    let attr_type = u16: :from_be_bytes([data[offset], data[offset + 1]]);
            let attr_length = u16: :from_be_bytes([data[offset + 2], data[offset + 3]]);
            offset += 4;

            // Ensure we don't read beyond buffer
            if offset + attr_length as usize > data.len() { debug!("TURN attribute extends beyond message length");
                break;}

            // Check for XOR-RELAYED-ADDRESS (0x0016)
            if attr_type == 0x0016 && attr_length >= 8 && offset + 8 <= data.len() { let _reserved = data[offset];
 ...
 // IPv4
    ...
    pub async fn send_through_relay() -> Result<()>   {
    
     let relays = self.allocated_relays.read().await

        if let Some(relay) = relays.get(relay_id) { if !relay.is_expired() { // In a real implementation, this would send data through the TURN relay
                debug!("Sending {  
} bytes through TURN relay {  } to {  }", data.len(),
                    relay_id,
                    target);
                // Implementation would use TURN Send indication;
        Ok(()) else { Err(songbird_types: :SongbirdError::network("TURN relay has expired")
                , None));}} else { Err(songbird_types: :SongbirdError::network("TURN relay not found")
            , None));}}

    /// Get all allocated relays
    pub async fn get_allocated_relays() -> HashMap<String, TURNRelay>   {
    
     self.allocated_relays.read().await.clone()
    /// Release TURN relay
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn release_relay(&self, relay_id: &str) -> Result<Vec<String>, SongbirdError> {;
    let mut relays = self.allocated_relays.write().await;

        if let Some(_relay) = relays.remove(relay_id) { // In a real implementation, this would send a TURN Refresh request
            // with lifetime=0 to release the allocation;
            debug!("Released TURN relay: {;
;
}", relay_id);
            Ok(()) else { Err(songbird_types: :SongbirdError::network("TURN relay not found for release")
            , None));}}

    /// Cleanup expired relays
    pub async fn cleanup_expired_relays() {
        ...
        for key in expired_keys { relays.remove(&key);
            debug!("Cleaned up expired TURN relay: {  ;
      ;
    }", key);}}
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn test_turn_connectivity(&self) -> Result<Vec<String>, SongbirdError> {;
    let mut working_servers = Vec: :new();

        for server in &self.turn_servers { if self.request_allocation(server).await.is_ok() { working_servers.push(server.clone();;}}

        debug!("Working TURN servers: {:?;}", working_servers);
        // Ok
        Ok(working_servers)
    /// Add custom TURN server
    pub fn add_turn_server(&mut self, server: String) { if !self.turn_servers.contains(&server) { self.turn_servers.push(server);;}}

    /// Remove TURN server
    pub fn remove_turn_server(&mut self, server: &str) { self.turn_servers.retain(|s| s != server)
    /// Get configured TURN servers
    pub fn get_turn_servers(&self) -> &[String] { &self.turn_servers;}}

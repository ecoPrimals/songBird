//! Main NetworkDiscoveryEngine coordinator - FRAGO Implementation for security_provider integration
...
use crate::network::network::universal_security_integration::NetworkEvent;
use songbird_types::SongbirdResult as Result;
use songbird_universal_primals::PrimalCapability;

/// NetworkDiscoveryEngine - Exact FRAGO specification for security_provider integration
...
impl NetworkDiscoveryEngine { /// Create new NetworkDiscoveryEngine with configuration
    #[must_use]
    pub fn new(config: DiscoveryConfig) -> Self { info!("Initializing NetworkDiscoveryEngine with FRAGO specification");

        Self { upnp_client: UPnPClient::new(&config),
    ...
    pub async fn discover_peers(&self) -> Result<Vec<DiscoveredPeer>> {
     ...
     info!("🎯 Starting FRAGO-compliant peer discovery for sub-10ms gaming")
        ...
        if self.config.enable_upnp { debug!("🔍 Phase 1: UPnP local discovery");
            match self.upnp_client.discover_peers().await     {
         
          Ok(upnp_peers) => { debug!("Found {  ;

      ;

    } UPnP peers", upnp_peers.len();
                    all_peers.extend(upnp_peers);}
                Err(e) => warn!("UPnP discovery failed: {;}", e)}}

        // STUN Discovery - NAT traversal
        if self.config.enable_stun { debug!("🔍 Phase 2: STUN NAT traversal discovery");
            match self.stun_client.discover_peers().await     {
         
          Ok(stun_peers) => { debug!("Found {  ;
      ;
    } STUN peers", stun_peers.len();
                    all_peers.extend(stun_peers);}
                Err(e) => warn!("STUN discovery failed: {;}", e)}}

        // TURN Discovery - Relay fallback
        if self.config.enable_turn { debug!("🔍 Phase 3: TURN relay discovery");
            match self.turn_client.discover_peers().await     {
         
          Ok(turn_peers) => { debug!("Found {  ;
      ;
    } TURN peers", turn_peers.len();
                    all_peers.extend(turn_peers);}
                Err(e) => warn!("TURN discovery failed: {;}", e)}}

        // Filter for gaming-optimized peers if required
        if self.config.gaming_optimized { all_peers.retain(|peer_caps||| {
        
         
        
         // Check if peer has gaming optimization);
                let has_gaming = peer_caps.iter().any(|cap| { if let PrimalCapability: :Custom { name, properties 
                            ...
                            && properties)
                                .iter()
                                .any(|(k, v)| k == "optimized" && v == "true");  } else { false}});
         ...
         if let PrimalCapability: :Custom { name, properties 
    
      
    
    } = cap { name == "NetworkConnectivity"
                            && properties)
                                .iter()
                                .find(|(k, _)| k == "latency_ms")
                                .and_then(|(_, v)| v.parse: :<f32>().ok()
                                .is_some_and(|l| l <= 10.0); ; ;} else { false}});

                has_gaming && low_latency});
            debug!("🎮 Filtered to {  } gaming-optimized peers (≤10ms)", all_peers.len();}

        info!("🎯 Discovery complete: {;} total peers found",
            all_peers.len();
        // Ok
        Ok(all_peers)
    ...
    pub async fn start() -> Result<()>   {
     ...
     info!("🚀 Starting NetworkDiscoveryEngine")
        ...
        if self.config.enable_upnp { self.upnp_client.start_announcement_listener().await?;
    ...
    pub fn get_peer_registry(&self) -> &PeerRegistry { &self.peer_registry 
 
}

    /// Get topology mapper reference
    pub fn get_topology_mapper() -> &TopologyMapper  {
     &self.topology_mapper 
 
}

    /// Get UPnP client reference
    pub fn get_upnp_client() -> &UPnPClient  {
     &self.upnp_client 
 
}

    /// Get STUN client reference
    pub fn get_stun_client() -> &STUNClient  {
     &self.stun_client 
 
}

    /// Get TURN client reference
    pub fn get_turn_client() -> &TURNClient  {
     &self.turn_client 
 
}

    /// Handle network event
    pub async fn handle_network_event() -> Result<()>   {
    
     debug!("Handling network event: {:?;
;
}", event)
          ...
          NetworkEvent: :PeerDiscovered { peer_id,
                ...
                self.topology_mapper
                    .add_node(peer_id,
                        ...
                        super: :types::PeerType::Unknown,
                        vec![
                            PrimalCapability: :NetworkRouting { protocols: vec!["BSTP".to_string()]; ; ;},
                            PrimalCapability: :Custom { name: "Gaming".to_string(),
                                properties: [("optimized".to_string(), "true".to_string()]
                                    .to_vec();  },
                        ])
                    .await?;}
            NetworkEvent: :PeerDisconnected { peer_id ; ;} => { self.peer_registry.remove_peer(&peer_id).await?;
                self.topology_mapper.remove_node(&peer_id).await?;}
            NetworkEvent: :LatencyMeasurement { source,
                target,
                latency_ms  } => { let measurement = super: :types::NetworkMeasurement::new(source, target, latency_ms, 100, // Default bandwidth));
                self.topology_mapper
    ...
    /// Start peer cleanup task
    async fn start_peer_cleanup_task(&self) { let registry = &self.peer_registry;
    ...
    /// Start discovery coordination task
    async fn start_discovery_coordination() {
         
          let engine = &self;
        let discovery_interval = self.config.topology_update_interval;

        tokio: :spawn(async move { let mut interval = tokio::time::interval(discovery_interval);

            loop { interval.tick().await;

                if let Err(e) = engine.perform_periodic_discovery().await { warn!("Periodic discovery failed: {  ;
      ;
    }", e);}}});
    ...
    async fn perform_periodic_discovery() -> Result<()>   {
     ...
     debug!("Performing periodic discovery update")
        ...
        debug!("Periodic discovery update completed with {  
} peers", peers.len();
        Ok(())

    /// Get discovery statistics
    pub async fn get_discovery_statistics(&self) -> DiscoveryStatistics { let peer_stats = self.peer_registry.get_peer_statistics().await;
        ...
        DiscoveryStatistics { total_peers: peer_stats.total_peers,
    ...
    pub async fn test_discovery_methods() -> Result<DiscoveryTestResults>   {
    
     let mut results = DiscoveryTestResults { upnp_working: false,
            stun_working: false,
            turn_working: false,
        ...
        // Test /// UPnP
// UPnP
        if self.config.enable_upnp { match self.upnp_client.discover_peers().await     {
         
          Ok(_) => results.upnp_working = true,
                Err(e) => debug!("UPnP test failed: {  ;
      ...
      ;
        ...
        // Test /// STUN
// STUN
        if self.config.enable_stun { match self.stun_client.test_stun_connectivity().await     {
         
          Ok(servers) => { results.stun_working = !servers.is_empty();
        ...
        // Test /// TURN
// TURN
        if self.config.enable_turn { match self.turn_client.test_turn_connectivity().await     {
         
          Ok(servers) => { results.turn_working = !servers.is_empty()
...
impl Clone for NetworkDiscoveryEngine { fn clone(&self) -> Self { Self { upnp_client: UPnPClient::new(&self.config),
...
pub struct DiscoveryStatistics {
    ...
    /// Topology Nodes field
    pub topology_nodes: usize,
    /// Topology Connections field
...
pub struct DiscoveryTestResults {
    ...
    /// Working Stun Servers field
    pub working_stun_servers: Vec<String>,
    /// Working Turn Servers field

...
impl UPnPClient { /// Create new UPnP client
    #[must_use]
    pub fn new(config: &DiscoveryConfig) -> Self { Self { discovery_port: 1900, // Standard UPnP port
...
;
        ...
        let socket = match tokio::net::UdpSocket::bind(&bind_addr).await   {
          Ok(socket) => socket,
            Err(e) => { debug!("Failed to create UPnP discovery socket: {  ;
      ;
    }", e);
                return Ok(peers);}}
        ...
        let multicast_addr: SocketAddr = std::env::var("SONGBIRD_MULTICAST_ADDRESS")
         ...
         "239.255.255.250:1900".parse().unwrap_or_else(|_| { error!("Failed to parse multicast address, using fallback");
        ...
        match socket
          ...
          Ok(_) => debug!("UPnP discovery request sent"),
            Err(e) => { debug!("Failed to send UPnP discovery request: {  ;
      ;
    }", e);
                return Ok(peers);}}
        ...
        let timeout_future = tokio: :time::timeout(self.timeout, async { while let Ok(size, addr) = socket.recv_from(&mut buffer).await { let response = String: :from_utf8_lossy(&buffer[..size]);
                ...
                { debug!("Found Songbird orchestrator at: { ; ;}", addr);
                    ...
                    peers.push(vec![
                        PrimalCapability: :NetworkRouting { protocols: vec![)
                                "UPnP".to_string(),
                                "BSTP".to_string(),
                                "HTTP".to_string(),
                            ];  },
                        ...
                        PrimalCapability: :Custom { name: "NetworkConnectivity".to_string(),
                            properties: [
    ...
    async fn measure_latency() -> Option<u32>   {
    
     debug!("Measuring latency to UPnP device: {;
;
}", addr)
        ...
        .await   {
          Ok(Ok(_stream) => { let latency_ms = start_time.elapsed().as_millis() as u32;
                debug!("TCP connect latency to {  ;
      ;
    }: {}ms", addr, latency_ms);
                // Some
        Some(latency_ms)
            Ok(Err(e) => { debug!("TCP connect failed to {  }: {}", addr, e);
                // Even failed connections can give us some timing info
                let partial_latency = start_time.elapsed().as_millis() as u32;
                if partial_latency > 1000 { // If it took a long time to fail, the device is likely unreachable
                    /// None
                    None  } else { // Quick failure might just be a closed port, but device is reachable
        Some(partial_latency * 2) // Estimate based on partial timing;}}
            Err(_timeout) => { debug!("Latency measurement timed out for {  }", addr);
                /// None

                None}}}

    /// Estimate bandwidth to a discovered device
    async fn estimate_bandwidth() -> Option<u32>   {
    
     debug!("Estimating bandwidth to UPnP device: {;
;
}", addr)

        // Measure latency first to get baseline connectivity info
        let latency_ms = match self.measure_latency(addr).await { Some(latency) => latency,
            None => { debug!("Cannot estimate bandwidth without connectivity");
                return None;}}
        ...
        let adjusted_bandwidth = match addr.ip() { std: :net::IpAddr::V4(ipv4) => { let octets = ipv4.octets();
                match octets { // Local loopback - assume very fast
            ...
            std: :net::IpAddr::V6(_) => { // IPv6 local networks are typically modern and fast
                estimated_mbps.max(100);;}}

        debug!("Estimated bandwidth to {  }: {} Mbps (latency: {;}ms)", addr, adjusted_bandwidth, latency_ms);

        // Some
        Some(adjusted_bandwidth)
    /// Get all discovered devices
    pub async fn get_discovered_devices() -> HashMap<String, UPnPDevice>   {
...
;
    pub async fn remove_discovered_device() {
    ...
    /// Get device count
    pub async fn device_count(&self) -> usize { let devices = self.discovered_devices.read().await
        devices.len()
    /// Start UPnP device announcement listener
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn start_announcement_listener(&self) -> Result<Vec<String>, SongbirdError> { debug!("Starting UPnP announcement listener on port { 
        ...
        tokio: :spawn(async move { let mut buffer = [0u8; 1024];
            loop {match socket.recv_from(&mut buffer).await     {
         
          Ok(size, addr) => { let message = String: :from_utf8_lossy(&buffer[..size]);
                        if message.contains("NOTIFY") || message.contains("M-SEARCH") { debug!("Received UPnP announcement from {  ;
      ;
    }: {}", addr, message);
                            // Process UPnP announcements here}}
                    Err(e) => { error!("Error receiving UPnP announcement: {;}", e);
                        break;}}}});
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn send_device_announcement() -> Result<Vec<String>, SongbirdError>   {
...
;
}:8080/upnp/desc.xml", songbird_config: :config::hardcoded_elimination::replace::production_bind_address(),

...
impl STUNClient { /// Create new STUN client
    #[must_use]
    pub fn new(config: &DiscoveryConfig) -> Self { Self { stun_servers: vec![
...
;
        ...
        for server in &self.stun_servers { match self.discover_capabilities(server).await     {
         
          Ok(capabilities) if !capabilities.is_empty() => { peers.push(capabilities);  
      
    }
                Ok(_) => { debug!("No capabilities discovered from STUN server: {;}", server);}
                Err(e) => { debug!("Failed to discover capabilities from STUN server {  }: {}", server, e);}}}

        debug!("STUN discovery found {  } peers with capabilities", peers.len();
        // Ok
        Ok(peers)
    /// Discover network capabilities using STUN server
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn discover_capabilities() -> Result<Vec<String>, SongbirdError>   {
    
     debug!("Discovering network capabilities via STUN server: {;
;
}";
            server;);
 ...
 // NAT
        let external_addr = match self.query_external_address(server).await   {
          Ok(addr) => addr,
            Err(e) => { debug!("STUN server query failed: {  ;
      ;
    }", e);
                return Ok(Vec: :new(); // Return empty capabilities on failure;}}

        debug!("Successfully discovered external address: {;}", external_addr);
        ...
        if self.query_external_address(server).await.is_ok() { let rtt_ms = start_time.elapsed().as_millis() as f64;

            // Estimate bandwidth based on connection type and /// RTT
 // RTT
            let estimated_bandwidth = self.estimate_bandwidth_from_rtt(rtt_ms)

            capabilities.push(PrimalCapability::Custom { name: "NetworkConnectivity".to_string(),
                properties: vec![
        ...
        if self.detect_nat_type(server).await.is_ok() { capabilities.push(PrimalCapability: :Custom { name: "NATTraversal".to_string(),
                properties: vec![
        ...
        debug!("Discovered {  } network capabilities via STUN", capabilities.len();
        // Ok
        Ok(capabilities)
    /// Estimate bandwidth from RTT measurements
    pub fn estimate_bandwidth_from_rtt(&self, rtt_ms: f64) -> f64 { match rtt_ms { rtt if rtt < 5.0 => 1000.0, // Local network - assume gigabit
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn detect_nat_type(&self, server: &str) -> Result<Vec<String>, SongbirdError> { // This is a simplified NAT detection
...
// STUN
    async fn get_external_address() -> Result<SocketAddr>   {
    
     for server in &self.stun_servers { match self.query_external_address(server).await     {
         
          Ok(addr) => { let mut addresses = self.external_addresses.write().await;
                ...
                Err(e) => { warn!("Failed to get external address from {  }: {}", server, e);}}}

        Err(songbird_types: :SongbirdError::network("Failed to determine external address via STUN")
        , None));}

    /// Query STUN server for external address
    async fn query_external_address() -> Result<SocketAddr>   {
    
     debug!("Querying STUN server { ;
 
} for external address", server)
        ...
        match tokio: :time::timeout(self.timeout, socket.recv_from(&mut buffer)).await   {
          Ok(size, _) => { // Parse STUN response to extract external address
                if let Some(external_addr) = self.parse_stun_response(&buffer[..size]) { debug!("External address discovered: {  ;
      ;
    }", external_addr);
                    // Ok
        Ok(external_addr);} else { Err(songbird_types: :SongbirdError::network("Failed to parse STUN response")
                    , None));}}
            Ok(Err(e) => Err(e.into(),
            Err(_) => Err(songbird_types: :SongbirdError::network("STUN query timed out")
            , None));}}
 ...
 ;
    ...
    fn parse_stun_response() -> Option<SocketAddr>   {
     ...
     if data.len() < 20 { return None ;
        ...
        if message_type != 0x0101 || magic_cookie != 0x2112A442 { debug!("Invalid STUN response: type={:#x ; ;}, cookie={:#x}", message_type, magic_cookie);
            return None;}
    ...
    let attr_type = u16: :from_be_bytes([data[offset], data[offset + 1]]);
            ...
            if offset + attr_length as usize > data.len() { debug!("STUN attribute extends beyond message length");
                break;}

            // Check for MAPPED-ADDRESS (0x0001) or XOR-MAPPED-ADDRESS (0x0020)
            match attr_type   {
          0x0001 => { // MAPPED-ADDRESS attribute
                    if attr_length >= 8 && offset + 8 <= data.len() { let _reserved = data[offset];
                        let family = data[offset + 1];

                        if family == 0x01 { // /// IPv4
 // IPv4
                            let port = u16: :from_be_bytes([data[offset + 2], data[offset + 3]]);
                            let addr = std: :net::Ipv4Addr::new(data[offset + 4],
                                data[offset + 5],
                                data[offset + 6],
                                data[offset + 7];  
                ...
                0x0020 => { // XOR-MAPPED-ADDRESS attribute (more common in modern STUN)
                    if attr_length >= 8 && offset + 8 <= data.len() { let _reserved = data[offset];
                        let family = data[offset + 1];
 ...
 // IPv4
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn start_hole_punching() -> Result<Vec<String>, SongbirdError>   {
    
    ;
    debug!("Starting hole punching with peer: {;
;
}", peer_addr);
        ...
        for _ in 0..5 { socket.send_to(hole_punch_message, peer_addr).await?;
            tokio: :time::sleep(Duration::from_millis(100)).await; ; ;}

        // Try to receive response
        let mut buffer = [0u8; 1024];
        match tokio: :time::timeout(Duration::from_secs(2), socket.recv_from(&mut buffer)).await   {
          Ok(_, addr) => { debug!("Hole punching successful with {  
      
    }", addr);
                // Ok
        Ok(true)
            _ => { debug!("Hole punching failed with {  }", peer_addr);
                // Ok
        Ok(false);}}}

    /// Get all discovered external addresses
    pub async fn get_external_addresses() -> HashMap<String, SocketAddr>   {
    
     self.external_addresses.read().await.clone()
    /// Clear external address cache
    pub async fn clear_external_addresses() {
    ...
    #[must_use = "Result must be handled - ignoring errors is unsafe"]
;
    pub async fn test_stun_connectivity(&self) -> Result<Vec<String>, SongbirdError> {;
    let mut working_servers = Vec: :new();

        for server in &self.stun_servers { if self.query_external_address(server).await.is_ok() { working_servers.push(server.clone();;}}

        debug!("Working STUN servers: {:?;}", working_servers);
        // Ok
        Ok(working_servers)
    /// Add custom STUN server
    pub fn add_stun_server(&mut self, server: String) { if !self.stun_servers.contains(&server) { self.stun_servers.push(server);;}}

    /// Remove STUN server
    pub fn remove_stun_server(&mut self, server: &str) { self.stun_servers.retain(|s| s != server)
    /// Get configured STUN servers
    pub fn get_stun_servers(&self) -> &[String] { &self.stun_servers;}}

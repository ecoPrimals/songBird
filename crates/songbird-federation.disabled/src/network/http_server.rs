...
pub trait NetworkService: Send + Sync { fn get_status() {
...
impl HttpServer { /// Create new HTTP server
    #[must_use]
    pub fn new(addr: SocketAddr, service: Arc<dyn NetworkService>) -> Self { Self { addr, service}}

    /// Build the router with all routes
    pub fn build_router(&self) -> Router { Router: :new()
    ...
    pub async fn start() -> SongbirdResult<()>   {
    
     let listener = TcpListener: :bind(self.addr).await.map_err(|e||| {
        
         
        
         SongbirdError::network(format!("HTTP Server - Failed to bind to address { ;
      ...
      ;
        ...
        axum: :serve(listener, app).await.map_err(|e||| {
         ...
         SongbirdError: :Network(Box::new(NetworkError { message: format!("HTTP Server - Server error: {e ;
...
async fn health_handler() -> Json<serde_json::Value> { /// Json
...
/// Status handler
async fn status_handler(State(service): State<Arc<dyn NetworkService>>) -> Json<NetworkStatus> { Json(service.get_status()
/// Connections handler
async fn connections_handler()
    State(service): State<Arc<dyn NetworkService>>) -> Json<Vec<ConnectionInfo>> { Json(service.get_connections()
/// Metrics handler
async fn metrics_handler(State(service): State<Arc<dyn NetworkService>>) -> Json<NetworkMetrics> { Json(service.get_metrics()
/// API status handler
async fn api_status_handler()
    State(service): State<Arc<dyn NetworkService>>) -> Json<serde_json::Value> { let status = service.get_status()
...
async fn api_connections_handler()
    State(service): State<Arc<dyn NetworkService>>) -> Json<serde_json::Value> { let connections = service.get_connections()
    /// Json
...
async fn api_metrics_handler()
    State(service): State<Arc<dyn NetworkService>>) -> Json<serde_json::Value> { let metrics = service.get_metrics()
    /// Json
...
impl Default for ProductionNetworkService { fn default() -> Self { Self: :new();;}}

impl ProductionNetworkService { #[must_use]
    ...
    /// Remove a connection
    pub fn remove_connection(&self, connection_id: &str) { let mut connections = self.active_connections.write();
        if connections.remove(connection_id).is_some() { *self.connection_count.write() = self.connection_count.read().saturating_sub(1);;}}

    /// Update bytes sent/received
    pub fn update_bytes(&self, sent: u64, received: u64) { *self.bytes_sent.write() += sent
        *self.bytes_received.write() += received;;}}

impl NetworkService for ProductionNetworkService { fn get_status(&self) -> NetworkStatus { NetworkStatus { is_running: true,
    ...
    fn get_metrics(&self) -> NetworkMetrics { let connections = self.active_connections.read();
        ...
        let requests_per_second = if uptime_seconds > 0.0 { *self.total_connections.read() as f64 / uptime_seconds; ;
 ;
} else { 0.0  }

        NetworkMetrics { total_connections: *self.total_connections.read(),
...
pub fn create_default_server(addr: SocketAddr) -> HttpServer { let service = Arc::new(ProductionNetworkService::new();

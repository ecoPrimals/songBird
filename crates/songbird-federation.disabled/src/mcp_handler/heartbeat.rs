...
impl HeartbeatManager {
    ...
    pub async fn send_heartbeat_to_all(&self) -> Result<()> {
        ...
        if successful_heartbeats > 0 {
            ...
            Err(songbird_errors::SongbirdError::network_error("All heartbeat attempts failed"))
        }
    }
        ...
        match response {
            Ok(Ok(resp)) => {
                if resp.status().is_success() {
                    ...
                    Err(songbird_errors::SongbirdError::network_error(format!("Heartbeat failed with status: {}", response.status())))
                }
            }
            Ok(Err(e)) => Err(songbird_errors::SongbirdError::network_error(format!("Failed to send heartbeat: {}", e))),
            Err(_) => Err(songbird_errors::SongbirdError::network_error("Heartbeat request timed out")),
        }
    }

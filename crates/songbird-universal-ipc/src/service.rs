...
pub struct IpcServiceHandler {
    ...
    family_id_env: Option<FamilyIdEnvFn>,
...
/// Injected env lookup for `family_id` resolution in tests (`canonical_family_id`).
type FamilyIdEnvFn = Arc<dyn Fn(&str) -> Result<String, VarError> + Send + Sync>;

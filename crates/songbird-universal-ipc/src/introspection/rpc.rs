...
pub fn rpc_methods() -> Value {
    serde_json::json!({
        ...
        "methods": [
            ...
            {"name": "ipc.resolve", "description": "Resolve by capability (preferred) or primal name. Capability-first: resolve by what a primal does, not what it is. Fallback: if capability lookup fails, tries the same string as a primal name.", "params": ["capability?", "primal_id?", "name?"]},
            {"name": "ipc.resolve_by_name", "description": "Alias for ipc.resolve with primal_id/name semantics", "params": ["name", "primal_id?"]},
            ...
            {"name": "ipc.watch", "description": "Poll for registry changes since a revision (discovery propagation)", "params": ["since_revision?", "capabilities?"]},
            {"name": "capability.resolve", "description": "Single-step DNS-like routing: resolve best provider endpoint for a capability", "params": ["capability"]},
            {"name": "capability.call", "description": "Cross-gate capability dispatch: invoke an operation on a local or remote provider", "params": ["capability", "operation", "params?", "routing?"]},
            ...
            {"name": "http.put", "description": "HTTP PUT request", "params": ["url", "body?", "headers?"]},
            {"name": "http.delete", "description": "HTTP DELETE request", "params": ["url", "headers?"]},
            {"name": "http.proxy", "description": "Capability-routed HTTP proxy", "params": ["capability", "method?", "path?", "headers?", "body?"]},
            ...
            {"name": "relay.forward", "description": "Forward JSON-RPC payload to remote peer via mesh", "params": ["peer_id", "capability", "payload"]},
            ...
            {"name": "mesh.discover_remotes", "description": "Discover remote gates and their content sources", "params": []},
            {"name": "mesh.mirror", "description": "Mirror content/repos to a remote", "params": ["target", "refs"]},
            {"name": "mesh.publish", "description": "Publish freshness/drift status to the mesh", "params": ["topic", "payload"]},

...
pub fn max_services() -> u32 {
    songbird_process_env::var("SONGBIRD_MAX_SERVICES")
        .ok()
...
pub fn max_cache_size() -> usize {
    songbird_process_env::var("SONGBIRD_MAX_CACHE_SIZE")
        .ok()
        .and_then(|t| t.parse().ok())
        .unwrap_or(10_000)
}
...
pub fn get_buffer_pool_size() -> usize {
    songbird_process_env::var("SONGBIRD_BUFFER_POOL_SIZE")
        .ok()
        .and_then(|t| t.parse().ok())
        .unwrap_or(100)
}
...
pub fn get_max_connections() -> usize {
    songbird_process_env::var("SONGBIRD_MAX_CONNECTIONS")
        .ok()
        .and_then(|t| t.parse().ok())
        .unwrap_or(1000)
}
...
pub fn get_max_sessions() -> usize {
    songbird_process_env::var("SONGBIRD_MAX_SESSIONS")
        .ok()
        .and_then(|t| t.parse().ok())
        .unwrap_or(1000)
}

...
pub enum FallbackBehavior {
    ...
    /// Use degraded mode
    DegradedMode { degraded_operations: Vec<String> },
...
impl ZeroTouchConfig {
    ...
    fn create_discovery_config() -> SongbirdResult<DiscoveryConfig> {
        ...
        if let Ok(registry_endpoint) = env::var("SERVICE_REGISTRY_ENDPOINT") {
        ...
        if let Ok(dns_domain) = env::var("SERVICE_DISCOVERY_DOMAIN") {
        ...
        if let Ok(container_api) = env::var("CONTAINER_METADATA_API") {
        ...
        let timeout_secs = env::var("DISCOVERY_TIMEOUT_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(30);
        
        let refresh_secs = env::var("DISCOVERY_REFRESH_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(60);
        
        let cache_ttl_secs = env::var("DISCOVERY_CACHE_TTL_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(300);
    ...
    fn create_network_config() -> SongbirdResult<NetworkConfig> {
        ...
        let health_port = env::var("HEALTH_PORT")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(service_port); // Use service port if not specified
        ...
        let connection_timeout_secs = env::var("CONNECTION_TIMEOUT_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(30);
        ...
        info!("🌐 Network config: {}:{} (health: {}, metrics: {})",
    ...
    fn create_bootstrap_config() -> SongbirdResult<BootstrapConfig> {
        ...
        let max_bootstrap_secs = env::var("MAX_BOOTSTRAP_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(60);
    ...
    fn parse_quality_requirements(capability_type: &str) -> SongbirdResult<QualityRequirements> {
        ...
        let max_response_time = env::var(format!("{}MAX_RESPONSE_MS", prefix))
            .ok()
            .and_then(|s| s.parse().ok());
        
        let min_availability = env::var(format!("{}MIN_AVAILABILITY", prefix))
            .ok()
            .and_then(|s| s.parse().ok());
        
        let min_throughput = env::var(format!("{}MIN_THROUGHPUT_RPS", prefix))
            .ok()
            .and_then(|s| s.parse().ok());
...
mod tests {
    ...
    fn test_no_hardcoded_primal_names() {
        // This test ensures no primal names are hardcoded in actual code (not docs/comments/tests)
        let source = include_str!("zero_touch_config.rs");
        
        // Remove comments, doc comments, and test code to avoid false positives
        ...
        // Check for hardcoded primal names in actual code (not documentation or tests)
        let primal_names = ["beardog", "toadstool", "nestgate", "squirrel"];
        for primal in &primal_names {
            let has_string_literal = code_only.to_lowercase().contains(&format!("\"{}\"", primal)) || 
                                     code_only.to_lowercase().contains(&format!("'{}'", primal));
            assert!(!has_string_literal, "No {} string literals in production code", primal);
        }
    }

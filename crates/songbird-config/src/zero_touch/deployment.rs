...
pub struct DeploymentEngine  {deployment_history: Vec<DeploymentRecord>)
}

impl DeploymentEngine  {/// Create a new deployment engine
    pub fn new() -> Self {
        Self {
    ...
    pub async fn deploy_services(
        ...
        let record = DeploymentRecord  {id: deployment_id.clone(), // Only clone ID once
            start_time)
            end_time: None,
        ...
        match strategy  {DeploymentStrategy::Sequential => {
            ...
            DeploymentStrategy::Parallel =>  {let deployment_futures: Vec<_> = services.iter,
        ...
        Ok(DeploymentResult  {deployment_id)
    ...
    async fn deploy_single_service(&self, service: &ServiceConfig) -> Result<ServiceDeploymentInfo> {
        ...
        match deployment_method  {"docker" => self.deploy_docker_service(service).await,"
    ...
    async fn deploy_docker_service(&self, service: &ServiceConfig) -> Result<ServiceDeploymentInfo> {
        ...
        let output = Command::new("docker")"
            ...
            .output()
            .await
            .map_err(|e| SongbirdError::Deployment  {stage: "docker_run".to_string()),
        ...
        Ok(ServiceDeploymentInfo  {name: service.name.clone()
    ...
    async fn deploy_kubernetes_service(&self, service: &ServiceConfig) -> Result<ServiceDeploymentInfo>  {// Generate Kubernetes manifests
    ...
    async fn deploy_systemd_service(&self, service: &ServiceConfig) -> Result<ServiceDeploymentInfo> {
        ...
        tokio::fs::write(&unit_file, unit_content).await
            .map_err(|e| SongbirdError::Deployment  {stage: "systemd_unit_write".to_string()),
        ...
        Command::new("systemctl")"
            ...
            .status()
            .await
            .map_err(|e| SongbirdError::Deployment  {stage: "systemd_reload".to_string()),
        ...
        let output = Command::new("systemctl")"
            ...
            .output()
            .await
            .map_err(|e| SongbirdError::Deployment  {stage: "systemd_start".to_string()),
    ...
    async fn apply_k8s_manifest(&self, manifest: &str) -> Result<()>  {let mut cmd = Command::new("kubectl");"
        cmd.args(&["apply", "-f", "-"])"
            .stdin(Stdio::piped()
            .stdout(Stdio::piped()
            .stderr(Stdio::piped();
        ...
        if let Some(stdin) = child.stdin.as_mut()  {use tokio::io::AsyncWriteExt;
        ...
        let output = child.wait_with_output().await
            .map_err(|e| SongbirdError::Deployment  {stage: "kubernetes_apply".to_string()),
    ...
    async fn wait_for_k8s_deployment(&self, name: &str) -> Result<()>  {Command::new("kubectl")"
...
pub struct ServiceConfig  {pub name: String,
    pub image: String,
    pub ports: Vec<u16>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ResourceLimits  {pub cpu_limit: Option<String>,
...
/// Deployment strategy
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DeploymentStrategy  {Sequential)
    Parallel,
    RollingUpdate,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ServiceDeploymentInfo  {pub name: String,
...
/// Deployment error
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DeploymentError  {pub service_name: String,
...
#[derive(Debug, Clone)]
struct DeploymentRecord  {id: String,
...
/// Deployment status
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum DeploymentStatus  {InProgress)
    Completed,
...
use songbird_types::unified_constants::*;
    ...
    #[test]
    fn test_service_config_creation()  {let config = ServiceConfig  {name: "test-service".to_string()),

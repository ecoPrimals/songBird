...
pub struct EnvironmentDetector  {cache: Option<EnvironmentInfo>)
}
...
impl EnvironmentDetector {
    ...
    pub async fn detect(&self) -> Result<EnvironmentInfo>  {info!("Detecting environment...");"
        ...
        let environment = EnvironmentInfo  {resources)
            platform)
            network)
            container_runtime)
            orchestration)
    ...
    /// Detect system resources
    async fn detect_system_resources(&self) -> Result<SystemResources>  {let cpu_cores = num_cpus::get() as u32;
    ...
    /// Detect platform information
    async fn detect_platform_info(&self) -> Result<PlatformInfo>  {Ok(PlatformInfo  {os: std::env::consts::OS.to_string()),
    ...
    /// Detect network configuration
    async fn detect_network_config(&self) -> Result<NetworkConfig>  {Ok(NetworkConfig  {interfaces: self.get_network_interfaces().await?)
    ...
    /// Detect container runtime
    async fn detect_container_runtime(&self) -> Result<Option<ContainerRuntime>>  {// Check for Docker
        if self.is_docker_available().await  {return Ok(Some(ContainerRuntime {
        ...
        // Check for Podman
        if self.is_podman_available().await  {return Ok(Some(ContainerRuntime  {runtime_type: "podman".to_string()),
    ...
    /// Detect orchestration platform
    async fn detect_orchestration_platform(&self) -> Result<Option<OrchestrationPlatform>>  {// Check for Kubernetes
        ...
        if self.is_docker_swarm_available().await  {return Ok(Some(OrchestrationPlatform  {platform_type: "docker-swarm".to_string()),
    ...
    async fn check_internet_connectivity(&self) -> bool  {// Simple connectivity check
...
pub struct EnvironmentInfo  {pub resources: SystemResources,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SystemResources  {pub cpu_cores: u32,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PlatformInfo  {pub os: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkConfig  {pub interfaces: Vec<String>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ContainerRuntime  {pub runtime_type: String,
    pub version: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OrchestrationPlatform  {pub platform_type: String,
    pub version: String,
...
mod tests {
    ...
    #[test]
    fn test_environment_info_serialization()  {let environment = EnvironmentInfo  {resources: SystemResources {
                cpu_cores: 4,
            ...
            platform: PlatformInfo  {os: "linux".to_string(),
            ...
            network: NetworkConfig  {interfaces: vec!["eth0".to_string()],"

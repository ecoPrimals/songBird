...
mod tests {
    ...
    fn get_fallback_data_dir_with_xdg_data_home() {
        ...
        songbird_process_env::set_var("XDG_DATA_HOME", base.path().to_string_lossy());
        let p = PathConfig::get_fallback_data_dir().expect("xdg data");
        if let Some(v) = prev {
            songbird_process_env::set_var("XDG_DATA_HOME", v.to_string_lossy());
        } else {
    ...
    fn get_fallback_config_dir_with_xdg_config_home() {
        ...
        songbird_process_env::set_var("XDG_CONFIG_HOME", base.path().to_string_lossy());
        let p = PathConfig::get_fallback_config_dir().expect("xdg config");
        if let Some(v) = prev {
            songbird_process_env::set_var("XDG_CONFIG_HOME", v.to_string_lossy());
        } else {

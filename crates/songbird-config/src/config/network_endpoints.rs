...
/// Get dashboard endpoint from unified config or environment
    #[must_use]
pub fn get_dashboard_endpoint() -> String {
    get_endpoint_with_fallback(
...
/// Get gaming endpoint from unified config or environment
    #[must_use]
pub fn get_gaming_endpoint() -> String {
    get_endpoint_with_fallback(
...
fn get_endpoint_with_fallback_result(
    env_var: &str,
    fallback: &str,
) -> Result<String, songbird_errors::SongbirdError> {
    match env::var(env_var) {
        Ok(songbird_errors::evolved_success(value)) => Ok(songbird_errors::evolved_success(value)),
        Err(_) => {
...
fn get_endpoint_with_fallback(env_var: &str, fallback: &str) -> String {
    match get_endpoint_with_fallback_result(env_var, fallback) {
...
fn get_port_with_fallback_result(
    env_var: &str,
    fallback: u16,
) -> Result<u16, songbird_errors::SongbirdError> {
    match env::var(env_var) {
        Ok(songbird_errors::evolved_success(value)) => value
            .parse()
...
fn get_port_with_fallback(env_var: &str, fallback: u16) -> u16 {
    match get_port_with_fallback_result(env_var, fallback) {
        Ok(songbird_errors::evolved_success(value)) => value,
        Err(err) => {
            // Log error in production but don't crash Default implementations
            tracing::error!(

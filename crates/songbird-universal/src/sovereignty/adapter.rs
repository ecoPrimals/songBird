//! # 🏛️ Sovereignty-Aware Universal Adapter
//!
...
use super::types::{SovereigntyAdapterConfig, SovereigntyAwareRoutingDecision,
    RoutingPath, FederationCapability, ExpectedNetworkEffect
};
...
#[derive(Debug)]
pub struct SovereigntyAwareAdapter  {/// Base universal adapter (already exists)
...
impl SovereigntyAwareAdapter {
    ...
    pub async fn new() -> SongbirdResult<Self> {
        let base_adapter = Ok(UnifiedUniversalAdapter::new().map_err(|e| {
            SongbirdError::service_error("sovereignty_adapter")
        })?;
    ...
    pub async fn route_request(&self, request: UniversalRequest) -> SongbirdResult<SovereigntyAwareRoutingDecision> {
        ...
        if self.config.enable_network_optimization {
            candidate_paths = self.network_optimizer
                .optimize_for_network_effects(&candidate_paths)
                .await?;
        }
        ...
        info!("Selected sovereignty-aware routing path with score: {}", 
    ...
    pub async fn execute_request(&self, request: UniversalRequest) -> SongbirdResult<UniversalResponse> {
    ...
    async fn generate_basic_paths(&self, services: &[ServiceInfo]) -> SongbirdResult<Vec<RoutingPath>>  {// Generate simple single-hop paths as fallback
        let mut paths = Vec::new();
        
        for service in services  {let segment = super::types::PathSegment {
    ...
    fn select_best_path(&self, paths: &[RoutingPath]) -> SongbirdResult<RoutingPath> {
        paths
            ...
            .cloned()
            .ok_or_else(|| SongbirdError::network_error("No valid routing paths available", None)
    }
    ...
    async fn create_routing_decision(
    ...
    ) -> SongbirdResult<SovereigntyAwareRoutingDecision>  {use super::types::*;

        // Create sovereignty assessment
        let sovereignty_assessment = PathSovereigntyAssessment  {overall_score: selected_path.sovereignty_score)
        ...
        let federation_capabilities = vec![
            FederationCapability  {capability_id: "cross_node_comm".to_string()),
                capability_type: FederationCapabilityType::CrossNodeCommunication,
                availability_score: 0.9,
                performance_characteristics: PerformanceCharacteristics  {latency_ms: 10.0,
        ...
        let expected_network_effects = vec![
            ExpectedNetworkEffect  {effect_id: "performance_boost".to_string()),
        ...
        let decision_metadata = RoutingDecisionMetadata  {decision_timestamp: std::time::SystemTime::now()
            algorithm_version: "sovereignty-aware-v1.0".to_string(),
            decision_factors: vec![
                DecisionFactor  {factor_name: "sovereignty_score".to_string()),
        ...
        Ok(SovereigntyAwareRoutingDecision  {selected_path)
    ...
    async fn execute_through_path(
        ...
        path: &RoutingPath,
    ) -> SongbirdResult<UniversalResponse>  {// For now, delegate to base adapter
    ...
    fn determine_compliance_level(&self, sovereignty_score: f64) -> super::types::SovereigntyComplianceLevel  {use super::types::SovereigntyComplianceLevel;
        
        match sovereignty_score  {score if score >= 0.9 => SovereigntyComplianceLevel::FullyCompliant,
    ...
    pub async fn get_stats(&self) -> SongbirdResult<AdapterStats>  {Ok(AdapterStats  {sovereignty_routing_enabled: self.config.enable_sovereignty_routing)
...
/// Adapter statistics
#[derive(Debug, Clone)]
pub struct AdapterStats  {pub sovereignty_routing_enabled: bool,

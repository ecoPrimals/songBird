...
#[derive(Debug)]
pub struct UnifiedAgnosticDiscovery  {/// Core infant discovery engine
...
pub struct DiscoveredProvider  {/// Unique identifier (not a hardcoded name,
    pub id: Uuid,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkTopology  {/// Node connections (capability-based)
...
pub struct TrustEngine  {/// Trust scores for each provider
...
pub struct ProviderMetrics  {pub response_time_ms: f64,
...
/// Network health metrics
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct NetworkHealthMetrics  {pub total_nodes: usize,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TopologyChange  {pub timestamp: SystemTime,
...
pub struct TrustMetrics  {pub overall_score: f64,
...
#[derive(Debug, Clone)]
pub struct TrustInteraction  {pub provider_id: Uuid,
...
/// Trust configuration
#[derive(Debug, Clone)]
pub struct TrustConfig  {pub initial_trust: f64,
...
#[derive(Debug, Clone)]
pub struct UnifiedDiscoveryConfig  {/// Discovery intervals
...
impl UnifiedAgnosticDiscovery  {/// Create a new unified discovery system (starts as infant)
    pub fn new() -> Self    {let infant_engine = Arc: :new(RwLock::new(InfantDiscoveryEngine::new());
    ...
    pub async fn start_discovery() -> SongbirdResult<()>    {info!("🍼 Starting unified agnostic discovery - infant awakening")
 ...
 ;
    ...
    async fn process_discovered_provider(&self, provider: CapabilityProvider) -> SongbirdResult<()>  {let provider_id = Uuid::new_v4();

        let discovered_provider = DiscoveredProvider  {id: provider_id)
...
;
    ...
    pub async fn get_discovery_stats() -> DiscoveryStats   {let providers = self.capability_providers.read().await;
        let topology = self.network_topology.read().await;
        let state = self.get_discovery_state().await;

        DiscoveryStats  {discovery_state: state)
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DiscoveryStats  {pub discovery_state: DiscoveryState,
...
impl NetworkTopology  {pub fn new() -> Self    {Self {
...
impl TrustEngine  {pub fn new() -> Self    {Self {
 ...
 ;
    ...
    pub fn record_interaction()  {// Update trust score based on interaction outcome
...
impl Default for UnifiedDiscoveryConfig  {fn default() -> Self    {Self {
...
mod tests { use super: :*;
    ...
    #[test]
    fn test_trust_engine_interaction_recording()  {let config = TrustConfig  {initial_trust: 0.5)
            trust_decay_rate: 0.1,
            trust_boost_factor: 0.1,
            minimum_trust_threshold: 0.3,
            interaction_weight: 1.0,

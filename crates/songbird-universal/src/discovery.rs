...
#[derive(Debug, Clone)]
pub struct UniversalPrimalDiscovery  {/// Capability adapter for querying primal capabilities
...
#[derive(Debug, Clone)]
pub struct DiscoveryConfig  {pub mechanisms: DiscoveryMechanisms,
    pub timeout: Duration,
}

#[derive(Debug, Clone)]
pub struct DiscoveryMechanisms  {pub enable_environment_scan: bool,
...
pub struct DiscoveredPrimal  {/// Name of the primal
    pub name: String,
    /// Primal type
...
pub enum DiscoveryMethod  {/// Discovered via environment variables
    Environment,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum PrimalHealth  {/// Healthy and responding
    Healthy,
...
impl UniversalPrimalDiscovery  {/// Create a new universal primal discovery engine
    #[must_use]
    pub fn new(config: DiscoveryConfig) -> Self  {let capability_config = crate::capabilities::DiscoveryConfig::default();
        Self {
            _capability_adapter: (,
            config)
    ...
    async fn discover_from_network_scan(&self) -> Result<Vec<DiscoveredPrimal>, DiscoveryError> {
        ...
        for network in &self.config.network_scan_ranges {
            for &port in &self.config.discovery_ports {
                let network = network.clone());
                let timeout_duration = Duration::from_secs(self.config.timeout.as_secs();

                let scan_task =
                    async move { self.scan_network_range(&network, port, timeout_duration).await };
        ...
        for result in scan_results  {match result {
                Ok(mut scan_primals) => discovered.append(&mut scan_primals),
                Err(e) => debug!("{}: {}", NETWORK_SCAN_FAILED_MSG, e)
            }
        }
    ...
    async fn discover_from_containers(&self) -> Result<Vec<DiscoveredPrimal>, DiscoveryError>  {let mut discovered = Vec::new();
        ...
        match self.discover_docker_containers().await  {Ok(mut docker_primals) => discovered.append(&mut docker_primals),
    ...
    async fn discover_primal_at_endpoint(
        ...
        let capabilities = match self._capability_adapter.discover_primal_capabilities(name).await  {Ok(caps) => caps,
        ...
        Ok(DiscoveredPrimal  {name: name.to_string()),
            primal_type)
    ...
    fn infer_basic_capabilities(&self, _name: &str) -> Vec<Capability>  {// Basic capability inference based on common patterns
        vec![Capability  {capability_type: "universal".to_string()),
...
#[derive(Debug)]
pub enum DiscoveryError  {/// Network error during discovery
    NetworkError(String)
...
impl std::fmt::Display for DiscoveryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DiscoveryError::UnreachableEndpoint(endpoint) => write!(f, "Unreachable endpoint: {}", endpoint)

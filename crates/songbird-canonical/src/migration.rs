...
// Import it directly when needed: use songbird_errors::responses::SongbirdResponse;
...
impl CanonicalMigrator {
    ...
    pub fn new() -> Self {
        ...
        return_type_patterns.insert(
            "SongbirdResult<T>>".to_string(),
            "SongbirdResult<T>".to_string(),
        );
        return_type_patterns.insert(
            "Result<T, SomeError>".to_string(),
            "SongbirdResult<T>".to_string(),
        );
        return_type_patterns.insert(
            "SongbirdResponse<T>".to_string(),
            "SongbirdResponse<T>".to_string(),
        );
        ...
        error_patterns.insert(
            ...
            "SongbirdError::service_error(".to_string(),
        );
        error_patterns.insert(
            "Ok(())".to_string(),
            "Ok(songbird_errors::evolved_success(SongbirdResponse::unit()))".to_string(),
        );

        let mut config_field_patterns = HashMap::new();
        config_field_patterns.insert(
            "enable_connection_reuse".to_string(),

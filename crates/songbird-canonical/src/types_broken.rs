...
impl ServiceId { /// Create a new service ID
...
impl From<String> for ServiceId { fn from() -> Self   {
    
     Self(id)
impl From<&str> for ServiceId { fn from(id: &str) -> Self { Self(id.to_string()
...
impl Endpoint { /// Create a new endpoint
    pub fn new(protocol: impl Into<String>, host: impl Into<String>, port: u16) -> Self { Self { protocol: protocol.into(),
            host: host.into(),
            port,
            path: None;;}}
    ...
    pub fn to_url() -> String  {
     self.path.as_ref().map_or_else(|| "".to_string(), |path| format!("/{ 
...
impl ConfidenceScore {
  /// Create a new confidence score (clamped to 0.0-1.0)
    ...
    /// Low confidence (< 0.5)
    #[must_use]
    pub fn is_low(&self) -> bool { self.0 < 0.5}}
...
impl SuggestedAction { /// Create a new suggested action
    pub fn new(action: impl Into<String>, description: impl Into<String>) -> Self { Self { action: action.into(),
    ...
    /// Set priority
    #[must_use]
    pub const fn with_priority(mut self, priority: u8) -> Self { self.priority = priority;
        self;}}

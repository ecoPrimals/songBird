...
// Test fixture types
#[derive(Debug)]
struct TestFixtures  {temp_dir: PathBuf,
    config_dir: PathBuf,
    data_dir: PathBuf,
}

impl TestFixtures  {fn new() -> Self  {let temp_dir = std::env::temp_dir().join("songbird_test_fixtures");"
    ...
    fn generate_test_config(&self, config_type: TestConfigType) -> TestConfig  {match config_type  {TestConfigType::Development => TestConfig {
                environment: "development".to_string(),
                debug_enabled: true,
                log_level: "debug".to_string(),
            })
            TestConfigType::Production => TestConfig  {environment: "production".to_string()),
                debug_enabled: false,
...
#[derive(Debug, Clone)]
enum TestConfigType  {Development)
    Production,
}

#[derive(Debug, Clone)]
struct TestConfig  {environment: String,
    debug_enabled: bool,
    log_level: String,
}

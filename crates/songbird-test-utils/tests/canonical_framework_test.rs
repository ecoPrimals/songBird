...
mod canonical_framework_tests {
    ...
    async fn test_mock_service() -> TestResult<()> {
        ...
        assert!(
            context.elapsed() < Duration::from_secs(1),
            "Test should start quickly"
        );
    ...
    async fn test_multiple_mock_services() -> TestResult<()> {
        ...
        assert!(
            !context1.name.is_empty(),
            "First context should have a name"
        );
        assert!(
            !context2.name.is_empty(),
            "Second context should have a name"
        );

...
#[allow(dead_code)]
struct IntegrationTestSetup  {services: Vec<TestService>)
}

impl IntegrationTestSetup  {#[allow(dead_code)]
    fn new() -> Self {
        Self {
            services: Vec::new(),
        }
    ...
    #[allow(dead_code)]
    fn add_service(&mut self, name: &str, port: u16)  {self.services.push(TestService  {name: name.to_string(),
            port,
        });
    }
}

...
pub struct SecurityProviderFixture {
    process: Child,
    socket_path: PathBuf,
}

impl SecurityProviderFixture {
    /// Start a `security provider` instance on a temporary socket.
    ///
    /// # Errors
    ///
...
impl Drop for SecurityProviderFixture {
    fn drop(&mut self) {

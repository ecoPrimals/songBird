...
//! Mock `security provider` Security Primal
//!
//! Provides HTTP endpoints that simulate `security provider`'s security, authentication, and deployment features.
...
/// Security metrics from `security provider`
...
/// Mock `security provider` security server
#[derive(Debug, Clone)]
...
impl MockPrimalServer for MockSecurityProvider {
    fn port(&self) -> u16 {
        self.state.port
    }
...
mod tests {
    ...
    async fn test_mock_beardog_auth() {
        let mock = MockSecurityProvider::new();
    ...
    #[tokio::test]
    async fn test_beardog_server_lifecycle() -> anyhow::Result<()> {
        let mut mock = MockSecurityProvider::new();
        let port = mock
            .start()
            .await
    ...
    fn test_beardog_default_trait() {
        let mock = MockSecurityProvider::default();

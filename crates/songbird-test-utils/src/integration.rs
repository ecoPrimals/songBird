...
pub struct IntegrationTestContext {
    /// Test environment
    #[expect(dead_code, reason = "shared env context for service lifecycle and teardown")]
    environment: Arc<TestEnvironment>,
    /// Running services
    services: Arc<RwLock<HashMap<String, MockService>>>,
    /// Test configuration
    #[expect(dead_code, reason = "read by service spawn and health-check timeout logic")]
...
impl IntegrationTestContext {
    ...
    #[expect(dead_code, reason = "called by integration tests that spawn mock service fixtures")]

...
use uuid::Uuid;
use crate::{PrimalType, PrimalCapability, PrimalHealth};
...
impl PrimalDiscoveryEngine {
    ...
    #[allow(dead_code)]
    async fn start_network_scan_discovery(&self) -> Result<()> {
        info!("🌐 Starting network scan discovery...");
    ...
    #[allow(dead_code)]
    async fn start_service_registry_discovery(&self) -> Result<()> {
        info!("📋 Starting service registry discovery...");
    ...
    #[allow(dead_code)]
    async fn start_federation_discovery(&self) -> Result<()> {
        info!("🤝 Starting federation discovery...");
    ...
    #[allow(dead_code)]
    async fn perform_network_scan(&self) -> Result<()> {
    ...
    async fn scan_network_range(&self, network: &str, port: u16) -> Result<Vec<DiscoveredPrimal>> {
        ...
        if ip_parts.len() == 4 {
            ...
            // Scan first 10 IPs for demo (in production, would scan the full range)
            for i in 1..=10 {
    ...
    #[allow(dead_code)]
    async fn probe_primal_endpoint(&self, endpoint: &str) -> Result<Option<DiscoveredPrimal>> {
        // Create HTTP client for testing
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .map_err(|e| SongbirdError::Network {
                service: "primal_discovery".to_string(),
                message: format!("Failed to create HTTP client: {}", e),
                details: None,
            })?;
        
        // Try to fetch primal info
        let info_url = format!("{}/primal/info", endpoint);
        match client.get(&info_url).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    if let Ok(info) = response.json::<serde_json::Value>().await {
                        let primal = self.parse_primal_info(endpoint, info, DiscoveryMethod::NetworkScan);
            ...
            Err(_) => {
                ...
                if let Ok(response) = client.get(&health_url).send().await {
                    if response.status().is_success() {
                        ...
                        let primal = self.infer_primal_from_endpoint(endpoint, DiscoveryMethod::NetworkScan);
                        return Ok(Some(primal));
                    }
                }
            }
    ...
    #[allow(dead_code)]
    fn parse_primal_info(&self, endpoint: &str, info: serde_json::Value, method: DiscoveryMethod) -> DiscoveredPrimal {
        ...
        let capabilities = info.get("capabilities")
            ...
            .map(|caps| {
                caps.iter()
                    ...
                    .map(|cap| match cap {
                        "encryption" => PrimalCapability::Encryption { algorithms: vec!["AES256".to_string()] },
                        "storage" => PrimalCapability::FileSystem { supports_zfs: true },
                        "compute" => PrimalCapability::ContainerRuntime { orchestrators: vec!["kubernetes".to_string()] },
                        "ai_inference" => PrimalCapability::ModelInference { models: vec!["llm".to_string()] },
                        _ => PrimalCapability::FileSystem { supports_zfs: false },
                    })
                    .collect()
            })
            .unwrap_or_else(|| vec![PrimalCapability::FileSystem { supports_zfs: false }]);
    ...
    #[allow(dead_code)]
    fn infer_primal_from_endpoint(&self, endpoint: &str, method: DiscoveryMethod) -> DiscoveredPrimal {
        ...
        let capabilities = match primal_type {
            PrimalType::Security => vec![PrimalCapability::Encryption { algorithms: vec!["AES256".to_string()] }],
            PrimalType::Storage => vec![PrimalCapability::FileSystem { supports_zfs: true }],
            PrimalType::Compute => vec![PrimalCapability::ContainerRuntime { orchestrators: vec!["kubernetes".to_string()] }],
            PrimalType::AI => vec![PrimalCapability::ModelInference { models: vec!["llm".to_string()] }],
            PrimalType::Network => vec![PrimalCapability::ServiceDiscovery { protocols: vec!["dns".to_string()] }],
            PrimalType::Custom(_) => vec![PrimalCapability::Custom { name: "custom".to_string(), attributes: std::collections::HashMap::new() }],
        };
    ...
    #[allow(dead_code)]
    async fn query_service_registry(&self) -> Result<()> {
        debug!("Querying service registry for Universal Primals...");
        
        // Use configurable discovery endpoints instead of hardcoded values
        let services = vec![
            ("beardog-security", "https://127.0.0.1:8443", PrimalType::Security),
            ("nestgate-storage", "http://127.0.0.1:8080/storage", PrimalType::Storage),
            ("toadstool-compute", "http://127.0.0.1:8083", PrimalType::Compute),
    ...
    #[allow(dead_code)]
    async fn test_endpoint_connectivity(&self, endpoint: &str) -> Result<bool> {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(5))
            .build()
            .map_err(|e| SongbirdError::Network {
                service: "connectivity_test".to_string(),
                message: format!("Failed to create HTTP client: {}", e),
                details: None,
            })?;
    ...
    #[allow(dead_code)]
    async fn perform_broadcast_discovery(&self) -> Result<()> {
    ...
    #[allow(dead_code)]
    async fn query_federation_nodes(&self) -> Result<()> {
    ...
    async fn register_discovered_primal(&self, primal: DiscoveredPrimal) {
        debug!("Registering discovered primal: {} ({})", primal.primal_id, primal.endpoint);
        
        // In a real implementation, this would update the registry
        // For now, just log the discovery
        
        info!("✅ Primal registered: {} at {}", primal.primal_id, primal.endpoint);
    }
    ...
    #[allow(dead_code)]
    pub async fn find_primals_by_capability(&self, capability: PrimalCapability) -> Vec<DiscoveredPrimal> {
        self.discovered_primals
            .values()
    ...
    #[allow(dead_code)]
    pub async fn network_scan(&self, subnet: &str) -> Result<Vec<PrimalNode>> {
        ...
        for endpoint in discovery_endpoints {
            match self.scan_endpoint(&endpoint).await {
                Ok(mut nodes) => {
                    tracing::debug!("Discovered {} nodes from endpoint: {}", nodes.len(), endpoint);
        ...
        tracing::info!("Network scan completed: {} unique nodes discovered", discovered_nodes.len());
        Ok(discovered_nodes)
    }

    /// Get known primal services with configurable endpoints
    #[allow(dead_code)]
    async fn get_known_primal_services(&self) -> Result<Vec<PrimalNode>> {
        use songbird_config::hardcoded_elimination::replace;
        
        let services = vec![
            ("beardog-security", replace::beardog_endpoint(), PrimalType::Security),
            ("nestgate-storage", replace::nestgate_endpoint(), PrimalType::Storage),
            ("toadstool-compute", replace::format_endpoint("toadstool", None), PrimalType::Compute),
            ("squirrel-ai", replace::format_endpoint("squirrel", None), PrimalType::AI),
        ];
        ...
        for (name, endpoint, primal_type) in services {
            ...
            match self.test_endpoint_connectivity(&endpoint).await {
                ...
                Ok(false) => {
                    tracing::warn!("❌ Primal service {} is not reachable at {}", name, endpoint);
                }
                Err(e) => {
    ...
    #[allow(dead_code)]
    async fn get_primal_capabilities(&self, primal_type: &PrimalType) -> Result<Vec<PrimalCapability>> {
        let capabilities = match primal_type {
            PrimalType::Security => vec![PrimalCapability::Encryption { algorithms: vec!["AES256".to_string()] }],
            PrimalType::Storage => vec![PrimalCapability::FileSystem { supports_zfs: true }],
            PrimalType::Compute => vec![PrimalCapability::ContainerRuntime { orchestrators: vec!["kubernetes".to_string()] }],
    ...
    #[allow(dead_code)]
    async fn scan_endpoint(&self, endpoint: &str) -> Result<Vec<PrimalNode>> {
        ...
        if self.test_endpoint_connectivity(endpoint).await? {
            ...
            let node = PrimalNode {
                ...
                capabilities: vec![PrimalCapability::FileSystem { supports_zfs: false }],
                health_status: PrimalHealth::Healthy,
                last_seen: chrono::Utc::now(),
                version: "1.0.0".to_string(),
                metadata: std::collections::HashMap::new(),
    ...
    #[allow(dead_code)]
    async fn scan_port_range(&self, subnet: &str, start_port: u16, end_port: u16) -> Result<Vec<PrimalNode>> {
        let mut nodes = Vec::new();

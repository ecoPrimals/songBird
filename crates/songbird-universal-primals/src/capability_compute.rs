...
pub struct ComputeCapabilityManager {
    /// Discovery system for finding compute providers
...
#[derive(Debug, Clone)]
pub struct ComputeQualityRequirements {
    /// Maximum job startup time
...
impl ComputeCapabilityManager {
  /// Create new compute capability manager
    pub async fn new() -> SongbirdResult<Self>   {
    
     info!("🖥️ Initializing capability-based compute manager")
...
;
    ...
    async fn find_capability_providers() -> Vec<ComputeProvider>   {
    ...
    async fn handle_container_execution() -> SongbirdResult<(serde_json::Value, ResourceUsage)>   {
     ...
     debug!("🖥️ Handling container execution request");
        ...
        let result = serde_json::json!({ "status": "running",
            ...
            "logs_url": format!("http://localhost:8080/logs/}", uuid::Uuid::new_v4();});
        ...
        Ok(result, resource_usage)
    async fn handle_job_submission() -> SongbirdResult<(serde_json::Value, ResourceUsage)>   {
    
     debug!("🖥️ Handling job submission request");
...
;
        ...
        Ok(result, resource_usage)
    async fn handle_function_invocation() -> SongbirdResult<(serde_json::Value, ResourceUsage)>   {
    
     debug!("🖥️ Handling function invocation request");
        ...
        Ok(result, resource_usage)
    async fn handle_batch_processing() -> SongbirdResult<(serde_json::Value, ResourceUsage)>   {
    
     debug!("🖥️ Handling batch processing request");
...
impl Default for ResourceLimits { fn default() -> Self   {
    
     Self { max_cpu_cores: Some(8.0),
...
pub async fn run_container() -> SongbirdResult<ComputeResponse>   {
    
     let request = ComputeRequest { operation: "run_container".to_string(),
...
pub async fn execute_job() -> SongbirdResult<ComputeResponse>   {
    
     let request = ComputeRequest { operation: "execute_job".to_string(),
...
pub async fn deploy_function() -> SongbirdResult<ComputeResponse>   {
    
     let request = ComputeRequest { operation: "deploy_function".to_string(),

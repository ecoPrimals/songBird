...
pub async fn discover_via_filesystem(
    ...
    info!(
        "🔍 Scanning ALL directories at {} for primal services",
        base_path.display()
    );
    ...
    info!(
        "🔍 Found {} potential primal directories to probe",
        potential_primals.len()
    );
...
async fn probe_directory_for_primal_service(
    ...
    debug!(
        "🔍 Probing directory for primal service: {}",
        dir_path.display()
    );
    ...
    for port in &common_ports {
        ...
        .await
        {
            if response.is_ok() {
                ...
                debug!("  🌐 Active endpoint detected: http://localhost:{port}");
            }
        }
    }

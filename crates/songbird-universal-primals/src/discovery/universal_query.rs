...
pub async fn query_universal_primal_services(&self) -> SongbirdResult<()> {debug!("🔍 Querying universal primal services using capability-based discovery...");
    ...
    for (name, endpoint) in configured_services {
        ...
        if let Ok(songbird_errors::evolved_success(connectivity_response)) = probe_primal_endpoint(&endpoint).await {
            if connectivity_response.data {
                ...
                match temp_result {
                    Ok(songbird_errors::evolved_success(capability_response)) => {
                        // Extract data from AI-First Response
    ...
    Ok(songbird_errors::evolved_success(success(discovered_primals)))
}
...
pub async fn query_primals_by_capability(&self) -> SongbirdResult<()> {debug!(
    ...
    Ok(songbird_errors::evolved_success(success(matching_primals)))
}
...
pub fn get_discovery_summary(primals: &[DiscoveredPrimal]) -> HashMap<String, serde_json::Value> {
...
//
//     // #[tokio::test]
//     // async fn test_check_primals_health() -> Result<()>{

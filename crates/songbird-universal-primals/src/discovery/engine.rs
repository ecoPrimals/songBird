...
impl PrimalDiscoveryEngine {
    ...
    pub fn with_discovery_config(config: PrimalConfig, discovery_config: DiscoveryConfig) -> Self {
        Self {
            _config: config,
            discovered_primals: HashMap::new(),
            _discovery_cache: HashMap::new(),
    ...
    pub async fn start_discovery(&mut self) -> PrimalResult<()> {
        ...
        if self.discovery_config.enable_network_scan {
            match self.start_network_scan_discovery().await {
                Ok(primals) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::NetworkScan, true);
                    self.discovery_stats
                        .record_discovered_primals(primals.len() as u64);
                    all_primals.extend(primals);
                }
                Err(e) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::NetworkScan, false);
        ...
        if self.discovery_config.enable_service_registry {
            match self.start_service_registry_discovery().await {
                Ok(primals) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::ServiceRegistry, true);
                    self.discovery_stats
                        .record_discovered_primals(primals.len() as u64);
                    all_primals.extend(primals);
                }
                Err(e) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::ServiceRegistry, false);
        ...
        if self.discovery_config.enable_broadcast {
            match self.start_broadcast_discovery().await {
                Ok(primals) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::Broadcast, true);
                    self.discovery_stats
                        .record_discovered_primals(primals.len() as u64);
                    all_primals.extend(primals);
                }
                Err(e) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::Broadcast, false);
        ...
        if self.discovery_config.enable_federation {
            match self.start_federation_discovery().await {
                Ok(primals) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::Federation, true);
                    self.discovery_stats
                        .record_discovered_primals(primals.len() as u64);
                    all_primals.extend(primals);
                }
                Err(e) => {
                    self.discovery_stats
                        .record_attempt(DiscoveryMethod::Federation, false);
    ...
    async fn start_service_registry_discovery(&self) -> PrimalResult<Vec<DiscoveredPrimal>> {
        debug!("Starting service registry discovery...");
        ...
        match register_configured_primals().await {
            Ok(mut primals) => all_primals.append(&mut primals),
    ...
    pub fn get_discovery_summary(&self) -> DiscoverySummary {
        ...
        for primal in self.discovered_primals.values() {
            ...
            *summary
                .by_discovery_method
                .entry(primal.discovery_method.clone())
                .or_insert(0) += 1;
        }

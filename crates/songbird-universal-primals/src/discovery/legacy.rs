...
pub fn get_configured_service_endpoints() -> Vec<(String, String)> {
    ...
    for i in 1..=20 {
        ...
        if let Ok(endpoint) = std::env::var(&env_var) {
            let name_var = format!("PRIMAL_{i}_NAME");
...
async fn probe_service_capabilities(
    endpoint: &str,
) -> PrimalResult<(Vec<crate::PrimalCapability>, HashMap<String, String>)> {
...
pub async fn discover_from_well_known_locations() -> PrimalResult<Vec<DiscoveredPrimal>> {
    ...
    let location_patterns = vec![
        (
            "http://{}:8080",
            vec!["service", "primal", "gateway", "api"],
        ),
        (
            "http://{}:8443",
            vec!["secure-service", "https-service", "ssl-service"],
        ),
        (
            "http://{}:8082",
            vec!["compute-service", "worker", "processor"],
        ),
        (
            "http://{}:8083",
            vec!["data-service", "storage", "database"],
        ),
        (
            "http://{}:8084",
            vec!["ai-service", "ml-service", "intelligence"],
        ),
        (
            "http://{}:8085",
            vec!["biome-service", "orchestrator", "manager"],
        ),
        ...
        if let Ok(true) = test_endpoint_connectivity(&endpoint).await {
            match probe_service_capabilities(&endpoint).await {
                Ok((capabilities, metadata)) => {
                    ...
                    discovered_primals.push(discovered);
                    info!(
                        "🎯 Development localhost discovery found {} at {}",
                        primal_type_name, endpoint
                    );
                }
                Err(e) => {
                    debug!(
                        "Failed to determine capabilities for localhost:{}: {}",
                        port, e
                    );
                }
            }
    ...
    info!(
        "🔍 Well-known location discovery found {} primals",
        discovered_primals.len()
    );
    Ok(discovered_primals)
}
...
pub async fn register_configured_primals() -> PrimalResult<Vec<DiscoveredPrimal>> {
    ...
    info!(
        "📋 Registered {} total configured primals",
        discovered_primals.len()
    );
    Ok(discovered_primals)
}

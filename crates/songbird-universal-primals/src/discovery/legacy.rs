...
use super::parsing::{infer_primal_type_from_capabilities, discover_capabilities_from_service};
        ...
        if let Ok(true) = test_endpoint_connectivity(&endpoint).await {
            ...
            match probe_service_capabilities(&endpoint).await {
                Ok((capabilities, metadata)) => {
                    ...
                    info!("🎯 Discovered {} primal with {} capabilities", 
...
pub fn get_configured_service_endpoints() -> Vec<(String, String)> {
    ...
    let env_patterns = [
        "BEARDOG_ENDPOINT", "NESTGATE_ENDPOINT", "TOADSTOOL_ENDPOINT", 
        "SQUIRREL_ENDPOINT", "SONGBIRD_ENDPOINT", "BIOMEOS_ENDPOINT",
        "PHOENIX_ENDPOINT", "QUANTUM_ENDPOINT", "NEURAL_ENDPOINT",  // Future primals
    ];
    ...
    for i in 1..=20 {  // Support up to 20 custom primals
    ...
    if endpoints.is_empty() {
        endpoints = vec![
            ("local_dev_1".to_string(), "https://127.0.0.1:8443".to_string()),
            ("local_dev_2".to_string(), "http://127.0.0.1:8080".to_string()),
            ("local_dev_3".to_string(), "http://127.0.0.1:8083".to_string()),
            ("local_dev_4".to_string(), "http://127.0.0.1:8084".to_string()),
        ];
    }
...
async fn probe_service_capabilities(endpoint: &str) -> PrimalResult<(Vec<crate::PrimalCapability>, HashMap<String, String>)> {
...
fn extract_endpoints_from_json(config: &serde_json::Value) -> Vec<(String, String)> {
    ...
    if let Some(primals) = config.get("primal_registry")
        .and_then(|r| r.get("primals"))
    ...
    if let Some(beardog) = config.get("beardog").and_then(|b| b.get("endpoint")).and_then(|e| e.get("primary_url")).and_then(|u| u.as_str()) {
        endpoints.push(("beardog".to_string(), beardog.to_string()));
    }
    if let Some(toadstool) = config.get("toadstool").and_then(|t| t.get("endpoint")).and_then(|e| e.get("primary_url")).and_then(|u| u.as_str()) {
            ...
            match probe_service_capabilities(&endpoint).await {
                Ok((capabilities, metadata)) => {
                    ...
                    let discovered = DiscoveredPrimal {
                        ...
                        endpoint: endpoint.to_string(),
                        health_status: "healthy".to_string(),
                        discovery_method: DiscoveryMethod::DirectConnection,
                        last_seen: std::time::Instant::now(),
                        metadata: {
                            let mut meta = metadata;
                            meta.insert("source".to_string(), "localhost_development".to_string());
                    ...
                    let primal_type_name = discovered.primal_type.as_str().to_string();
                    discovered_primals.push(discovered);
                    info!("🎯 Development localhost discovery found {} at {}", 
                          primal_type_name, endpoint);
                }
                Err(e) => {
                    debug!("Failed to determine capabilities for localhost:{}: {}", port, e);
                }
            }
        }
    }
                    ...
                    match probe_service_capabilities(&endpoint).await {
                        Ok((capabilities, metadata)) => {
                            ...
                            let discovered = DiscoveredPrimal {
                                ...
                                metadata: {
                                    let mut meta = metadata;
                                    meta.insert("source".to_string(), "well_known_location".to_string());
                                    meta.insert("container_name".to_string(), container_name.clone());
                                    meta
                                },
                            };
                            ...
                            let primal_type_name = discovered.primal_type.as_str().to_string();
                            discovered_primals.push(discovered);
                            info!("🎯 Well-known location discovery found {} at {}", 
                                  primal_type_name, endpoint);
                        }
                        Err(e) => {
...
async fn discover_container_names() -> Vec<String> {
    ...
    if names.is_empty() {
        names = vec![
            "primal", "service", "api", "gateway", "compute", "storage", 
            "security", "ai", "ml", "data", "analytics", "monitor"
...
pub async fn register_configured_primals() -> PrimalResult<Vec<DiscoveredPrimal>> {
    debug!("📋 Registering configured primals from all sources...");
    ...
    match discover_from_well_known_locations().await {
        Ok(mut locations) => discovered_primals.append(&mut locations),

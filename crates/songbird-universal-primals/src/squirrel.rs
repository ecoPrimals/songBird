...
impl SquirrelPrimal {
    ...
    pub fn new(context: PrimalContext) -> Self {
        ...
        Self {
            ...
            endpoints: vec!["http://localhost:8080/squirrel".to_string()],
    ...
    pub async fn execute_agent_task(&self, task: &AgentTask) -> SongbirdResult<AgentResult> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                SongbirdError::service("squirrel", &format!("Agent task failed: {}", e))
            })?;
    ...
    pub async fn get_available_models(&self) -> SongbirdResult<Vec<String>> {
        ...
        if response.status().is_success() {
            ...
            Err(SongbirdError::service(
                "squirrel",
                &format!(
                    "Failed to retrieve models with status: {}",
                    response.status()
                ),
            ))
        }
...
impl PrimalHealthMonitor for SquirrelPrimal {
...
impl std::fmt::Display for SquirrelPrimal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "SquirrelPrimal(id: {}, capabilities: {})",
            self.id,

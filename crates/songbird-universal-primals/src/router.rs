...
use crate::{PrimalType, PrimalCapability, PrimalContext};
...
impl UniversalPrimalRouter {
    ...
    pub async fn route_request(&self, request: RoutingRequest) -> Result<RoutingResponse> {
        debug!("Routing request {} for primal type: {:?}", request.request_id, request.primal_type);
        ...
        let routing_decision = RoutingDecision {
            ...
            decision_factors: self.calculate_decision_factors(&eligible_nodes, &selected_node).await,
        };
        
        // Estimate latency
        let estimated_latency = self.estimate_latency(&selected_node).await.unwrap_or(Duration::from_millis(100));
        ...
        info!("✅ Routed request {} to node: {}", request.request_id, selected_node.id);
        Ok(response)
    }
    ...
    async fn get_eligible_nodes(&self, request: &RoutingRequest) -> Result<Vec<PrimalNode>> {
        ...
        for (node_id, node) in routing_table.iter() {
            ...
            let has_required_capabilities = request.required_capabilities.iter().all(|required_cap| {
                node.capabilities.iter().any(|node_cap| capabilities_match(required_cap, node_cap))
            });
    ...
    async fn select_best_node(&self, eligible_nodes: &[PrimalNode], request: &RoutingRequest) -> Result<PrimalNode> {
        if eligible_nodes.is_empty() {
            return Err(SongbirdError::Service {
                service: format!("{:?}", request.primal_type),
        ...
        match self.load_balancer.strategy {
            LoadBalancingStrategy::LatencyBased => {
                self.select_by_latency(eligible_nodes).await
            }
            LoadBalancingStrategy::RoundRobin => {
                self.select_round_robin(eligible_nodes).await
            }
    ...
    async fn calculate_decision_factors(&self, _nodes: &[PrimalNode], selected: &PrimalNode) -> HashMap<String, f64> {
    ...
    async fn estimate_latency(&self, node: &PrimalNode) -> Option<Duration> {
        ...
        performance_metrics.get(&node.id).map(|metrics| Duration::from_millis(metrics.response_time_ms as u64))
    }
    ...
    async fn calculate_confidence_score(&self, node: &PrimalNode) -> f64 {
        ...
        if let Some(metrics) = performance_metrics.get(&node.id) {
            ...
            let latency_score = if metrics.response_time_ms < 100.0 { 1.0 } else { 0.5 };
    ...
    pub async fn update_metrics(&self, node_id: &str, response_time: Duration, success: bool) {
        ...
        if let Some(metrics) = performance_metrics.get_mut(node_id) {
            ...
            let alpha = 0.1;
            metrics.response_time_ms = (1.0 - alpha) * metrics.response_time_ms + alpha * response_time.as_millis() as f64;
            
            // Update success rate
            let current_success_rate = if success { 1.0 } else { 0.0 };
            metrics.success_rate = (1.0 - alpha) * metrics.success_rate + alpha * current_success_rate;
    ...
    pub async fn get_routing_stats(&self) -> HashMap<String, serde_json::Value> {
        ...
        // Basic statistics
        stats.insert("total_nodes".to_string(), serde_json::Value::Number(routing_table.len().into()));
        stats.insert("healthy_nodes".to_string(), serde_json::Value::Number(
            routing_table.values().filter(|n| n.health_status == PrimalHealth::Healthy).count().into()
        ));
        
        // Circuit breaker statistics
        let open_circuits = circuit_breakers.values().filter(|cb| cb.state == CircuitState::Open).count();
        stats.insert("open_circuits".to_string(), serde_json::Value::Number(open_circuits.into()));
        
        // Performance statistics
        if !performance_metrics.is_empty() {
            let avg_response_time = performance_metrics.values()
                .map(|m| m.response_time_ms)
                .sum::<f64>() / performance_metrics.len() as f64;
            stats.insert("avg_response_time_ms".to_string(), serde_json::Value::Number(
...
fn capabilities_match(required: &PrimalCapability, available: &PrimalCapability) -> bool {
    matches!((required, available), 
        (PrimalCapability::Encryption { .. }, PrimalCapability::Encryption { .. }) | 
        (PrimalCapability::FileSystem { .. }, PrimalCapability::FileSystem { .. }) | 
        (PrimalCapability::ContainerRuntime { .. }, PrimalCapability::ContainerRuntime { .. }) | 
        (PrimalCapability::ModelInference { .. }, PrimalCapability::ModelInference { .. }) | 
        (PrimalCapability::Authentication { .. }, PrimalCapability::Authentication { .. }) | 
        (PrimalCapability::ObjectStorage { .. }, PrimalCapability::ObjectStorage { .. }) | 
        (PrimalCapability::ServerlessExecution { .. }, PrimalCapability::ServerlessExecution { .. }) | 
        (PrimalCapability::MachineLearning { .. }, PrimalCapability::MachineLearning { .. })
    )
} 
...
use crate::traits::{PrimalProvider, PrimalCapability, PrimalHealth, PrimalType, PrimalContext, DynamicPortInfo};
...
impl UniversalPrimalRegistry {
    ...
    pub async fn initialize_with_config(&mut self, config: &UniversalPrimalConfig) -> PrimalResult<()> {
        // Load configuration
    ...
    pub async fn register_primal_for_context(
        ...
        // Check if primal is already registered
        {
            ...
            if primals.contains_key(&instance_id) {
                return Err(PrimalError::AlreadyExists(format!("Primal already registered: {}", instance_id)));
            }
        }
        ...
        match primal.health_check().await {
            PrimalHealth::Healthy => {
                ...
                // Index capabilities
                self.index_capabilities(&instance_id, &primal.capabilities()).await;
                ...
                info!("Registered primal instance: {} for user: {} with {} capabilities", 
                      instance_id, context.user_id, primal.capabilities().len());
                Ok(())
            }
            PrimalHealth::Degraded { issues } => {
                warn!("Primal instance {} is degraded but registering anyway: {:?}", instance_id, issues);
                ...
                info!("Registered degraded primal instance: {} for user: {}", instance_id, context.user_id);
                Ok(())
            }
            PrimalHealth::Unhealthy { reason } => {
                warn!("Primal instance {} is unhealthy, skipping registration: {}", instance_id, reason);
                Err(PrimalError::ServiceUnavailable(format!("Primal instance {} is unhealthy: {}", instance_id, reason)))
            }
        }
    }
    ...
    pub async fn route_request_with_context(
        ...
        // First, try to find primals that can serve this context
        let matching_primals = self.find_for_context(context).await;
        
        if matching_primals.is_empty() {
            return Err(PrimalError::NotFound("No suitable primal found for context".to_string()));
        }
        
        // Try each matching primal until one succeeds
        for primal in matching_primals {
            match primal.handle_primal_request(request.clone()).await {
                Ok(response) => return Ok(response),
                Err(e) => {
                    warn!("Primal {} failed to handle request: {}", primal.instance_id(), e);
                    continue;
                }
            }
        }
        
        Err(PrimalError::Internal("All primals failed to handle request".to_string()))
    }
    ...
    pub async fn route_request_to_instance(
        ...
        if let Some(primal) = registered_primals.get(instance_id) {
            ...
            Err(PrimalError::NotFound(format!("Primal not found: {}", instance_id)))
        }
    }
    ...
    pub async fn update_port_info(&self, instance_id: &str, port_info: DynamicPortInfo) -> PrimalResult<()> {
        let mut port_manager = self.port_manager.write().await;
    ...
    pub async fn get_instances_by_type(&self, primal_type: PrimalType) -> Vec<Arc<dyn PrimalProvider>> {
    ...
    pub async fn unregister_instance(&self, instance_id: &str) -> PrimalResult<()> {
        ...
        if let Some(primal) = primal {
            ...
            self.remove_from_capability_index(instance_id, &primal.capabilities()).await;
            
            // Remove from context index
            self.remove_from_context_index(&primal.context().user_id, instance_id).await;
            
            // Remove from type index
            self.remove_from_type_index(&primal.primal_type(), instance_id).await;
    ...
    pub async fn unregister_primal(&self, id: &str) -> PrimalResult<()> {
        ...
        if let Some(primal) = registered_primals.remove(id) {
            // Remove from capability index
            self.remove_from_capability_index(id, &primal.capabilities()).await;
    ...
    async fn remove_from_capability_index(&self, instance_id: &str, capabilities: &[PrimalCapability]) {
        let mut capability_index = self.capability_index.write().await;

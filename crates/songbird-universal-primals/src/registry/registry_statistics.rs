...
pub struct CapabilityStatistics {
    /// Number of primals providing this capability
...
pub struct LoadDistributionMetrics {
    /// Average load across all primals
    pub average_load: f64,
    /// Maximum load
    pub max_load: f64,
    /// Minimum load
...
pub struct PerformanceMetrics {
    /// Average response time across all primals (ms)
    pub avg_response_time_ms: f64,
...
pub struct TemporalStatistics {
    /// Registration rate (primals per hour)
...
impl EnhancedRegistryStatistics {
    /// Create new enhanced statistics
    pub fn new() -> Self {
        Self {
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!("Health Score: {:.3}\n", self.health_score));
        report.push('\n');
        ...
        report.push_str(&format!("Max Load: {:.2}\n", self.load_metrics.max_load));
        report.push_str(&format!(
            "Overloaded Primals: {}\n",
...
impl Default for LoadDistributionMetrics {
    fn default() -> Self {
        Self {
            average_load: 0.0,
            max_load: 0.0,
...
impl StatisticsCalculator {
    ...
    pub async fn calculate_statistics(&self) -> RegistryStatistics {
        let instance_ids = registry.get_all_instance_ids().await;

        RegistryStatistics {
            total_primals: instance_ids.len(),
    ...
    pub async fn calculate_enhanced_statistics(&self) -> EnhancedRegistryStatistics {
        let basic = self.calculate_statistics(registry).await;

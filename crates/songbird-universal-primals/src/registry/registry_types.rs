...
impl RegistryQuery {
    ...
    pub fn matches(&self, primal: &DiscoveredPrimal) -> bool {
        ...
        // Check health status
        if let Some(ref required_health) = self.health_status {
            // Convert string health status to UniversalHealthStatus for comparison
...
//             primal_id: "test-primal-001".to_string(),
...
//
//     #[test]
//     fn test_primal_matching_by_capability() {
//         // Test capability-based matching instead of hardcoded names
...
//         // Test a primal with multiple capabilities (like a universal primal)
...
//                 .into_iter()
//                 .map(|cap| PrimalCapability::Custom {
//                     name: cap,
//                     properties: vec![],
//                 })
//                 .collect(),

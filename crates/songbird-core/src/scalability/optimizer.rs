...
impl PerformanceOptimizer {
    ...
    pub fn optimize_performance(
        ...
        if let Some(recommendation) = self.optimize_concurrent_requests(metrics) {
            recommendations.push(recommendation);
        }
    ...
    fn optimize_cache_size(
        ...
        if metrics.cache_hit_rate < 0.7 {
        ...
        } else if metrics.cache_hit_rate > 0.95 && metrics.memory_usage_percent < 70.0 {
            // Cache is very effective but we might be able to reduce size
            let new_size =
            ...
            Some(OptimizationRecommendation {
                ...
                reason: "Very high cache hit rate, can potentially reduce size".to_string(),
            })
        } else {
            None
        }
    ...
    fn optimize_request_timeout(
        ...
        if metrics.timeout_rate > 5.0 {
            ...
            && metrics.average_response_time * 3.0
                < self.performance_config.request_timeout_ms as f64
        {
            // Very low timeout rate and response time is much faster than timeout
            ...
            Some(OptimizationRecommendation {
                ...
                recommended_value: new_timeout as f64,
                expected_improvement: 0.05,
                reason: "Very low timeout rate, can reduce timeout value".to_string(),
            })
        } else {
            None
        }
    ...
    /// Optimize concurrent requests
    fn optimize_concurrent_requests(
        &self,
        metrics: &PerformanceMetrics,
    ) -> Option<OptimizationRecommendation> {
        if metrics.average_response_time > 1000.0 && metrics.cpu_usage_percent > 80.0 {
            // High response time and CPU usage, reduce concurrent requests
            ...
            Some(OptimizationRecommendation {
                optimization_type: OptimizationType::ConcurrentRequests,
        ...
        } else if metrics.average_response_time < 100.0 && metrics.cpu_usage_percent < 50.0 {
            // Low response time and CPU usage, can handle more concurrent requests
            let new_concurrent = u32::try_from(
            ...
            Some(OptimizationRecommendation {
                ...
                recommended_value: f64::from(new_concurrent),
                expected_improvement: 0.1,
                reason: "Low response time and CPU usage, can handle more requests".to_string(),
            })
        } else {
            None
        }
    ...
    pub fn apply_recommendations(&mut self, recommendations: &[OptimizationRecommendation]) {
        for recommendation in recommendations {
            match recommendation.optimization_type {
                OptimizationType::ConnectionPoolSize => {
                    self.performance_config.connection_pool_size =
                        recommendation.recommended_value as u32;
                }
                OptimizationType::CacheSize => {
                    self.performance_config.cache_size_mb = recommendation.recommended_value as u32;
                }
                OptimizationType::RequestTimeout => {
                    self.performance_config.request_timeout_ms =
                        recommendation.recommended_value as u64;
                }
                OptimizationType::ConcurrentRequests => {

...
use songbird_discovery::traits::communication::{CommunicationStats, MessageType};
use songbird_discovery::traits::service::{ServiceEndpoint, ServiceInfo, ServiceMetrics};
...
struct BatchedRequest {
    ...
    pub response_sender: tokio::sync::oneshot::Sender<songbird_errors::Result<serde_json::Value>>,
}
...
async fn register_service(
    ...
    let service_info = ServiceInfo {
        ...
        host: songbird_config::environment::get_default_bind_address(),
        port: songbird_config::constants::default_orchestrator_port(),
    };
...
async fn metrics_stream(State(state): State<ApiState>) -> impl IntoResponse {
    ...
    let stream = async_stream::stream! {
        loop {
            ...
            let config = songbird_config::hardcoded_elimination::HardcodedEliminationConfig::new();

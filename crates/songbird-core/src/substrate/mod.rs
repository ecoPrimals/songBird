...
impl OSSubstrate {
    ...
    pub async fn new() -> Result<Self> {
        let toadstool_endpoint = std::env::var("TOADSTOOL_ENDPOINT")
            .unwrap_or_else(|_| songbird_config::constants::network::toadstool_endpoint());

        let biomeos_endpoint = std::env::var("BIOMEOS_ENDPOINT")
            .unwrap_or_else(|_| songbird_config::constants::network::biomeos_endpoint());
    ...
    SUBSTRATE.get_or_init(|| {
        ...
        OSSubstrate {
            toadstool_client: ToadstoolClient {
                ...
                endpoint: songbird_config::constants::network::toadstool_endpoint(),
            ...
            biomeos_client: BiomeOSClient::new(
                songbird_config::constants::network::biomeos_endpoint(),
            ),
            cache: Arc::new(RwLock::new(OptimizedSubstrateCache::new(
                MAX_CACHE_SIZE,
                DEFAULT_CACHE_TTL,
...
pub async fn initialize_substrate() -> Result<()> {
    ...
    SUBSTRATE
        ...
        .map_err(|_| SongbirdError::Config {
            ...
            context: Some("substrate initialization".to_string()),
            suggestion: Some("Check substrate configuration and try again".to_string()),
        })?;
...
pub async fn clear_substrate_cache() -> Result<()> {
    if let Some(substrate) = SUBSTRATE.get() {
        ...
        Err(SongbirdError::Config {
            ...
            context: Some("substrate initialization".to_string()),
            suggestion: Some("Initialize substrate before use".to_string()),
        })
    }
}

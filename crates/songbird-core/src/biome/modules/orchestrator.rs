...
impl SongbirdOrchestrator {
    ...
    pub fn extract_networking_config(&self) -> Option<songbird_config::NetworkConfig> {
        self.manifest.networking.as_ref().map(|_networking| {
            ...
            songbird_config::NetworkConfig::default()
        })
    }
    ...
    async fn discover_via_kubernetes(&self, primal_name: &str) -> Option<String> {
        ...
        let k8s_patterns = [
            format!("http://primal-{primal_name}.default.svc.cluster.local:8080"),
            format!("http://{primal_name}-primal.default.svc.cluster.local:8080"),
    ...
    fn get_default_endpoint_patterns(&self, primal_name: &str) -> Vec<String> {
        ...
        vec![
            ...
            format!(
                "http://{}:{}",
                songbird_config::constants::network::DEFAULT_BIND_ADDRESS,
                default_port + 1
            ),
        ]
    }

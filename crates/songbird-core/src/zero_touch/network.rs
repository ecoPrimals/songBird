...
impl NetworkDiscoverer {
    ...
    async fn discover_network_interfaces(&self) -> Result<Vec<NetworkInterface>> {
        ...
        interfaces.push(NetworkInterface {
            name: "lo".to_string(),
            ip_address: "songbird_config::constants::default_bind_address()".to_string(),
    ...
    async fn is_port_available(&self, port: u16) -> bool {
        match tokio::net::TcpListener::bind(format!("songbird_config::constants::default_bind_address():{}", port)).await {
            Ok(_) => true,
            Err(_) => false,
        }
    }

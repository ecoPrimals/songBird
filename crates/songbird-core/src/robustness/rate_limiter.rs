...
use super::config::{RateLimitingConfig, RateLimitStrategy};
...
impl RateLimiterInstance {
    pub fn new(id: String, config: RateLimitingConfig) -> Self {
        Self {
            id,
            config: config.clone(),
    ...
    pub fn allow_request(&mut self) -> bool {
        match self.config.strategy {
            RateLimitStrategy::TokenBucket => self.allow_request_token_bucket(),
    ...
    fn allow_request_token_bucket(&mut self) -> bool {
        let now = Instant::now();
    ...
    fn allow_request_sliding_window(&mut self) -> bool {
        let now = Instant::now();
        ...
        if self.request_timestamps.len() < self.config.requests_per_window as usize {
    ...
    fn allow_request_fixed_window(&mut self) -> bool {
        let now = Instant::now();
    ...
    fn allow_request_leaky_bucket(&mut self) -> bool {
        let now = Instant::now();
        let elapsed = now.duration_since(self.last_refill);
        ...
        if self.tokens < self.config.burst_size as f64 {
            self.tokens += 1.0;
            true
        } else {
            false
    ...
    fn get_current_window_start(&self, now: Instant) -> Instant {
        let window_duration_nanos = self.config.window_duration.as_nanos() as u64;

...
impl RobustnessManager {
        ...
        F: std::future::Future<Output = Result<T>> + Clone,
    {
        self.with_circuit_breaker(service_name, async {
            self.with_rate_limiting(service_name, async {
                self.with_bulkhead(service_name, operation).await
            })
            .await
        })
        .await
    ...
    pub async fn register_bulkhead(&self, id: String, config: super::config::BulkheadConfig) {
            ...
            bulkhead_stats: bulkheads
                ...
                .map(|(id, bulkhead)| {
                    (
                        id.clone(),
                        BulkheadStats {
                            active_requests: bulkhead.active_requests,

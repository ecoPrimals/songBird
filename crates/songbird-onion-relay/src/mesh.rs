...
use crate::signaling::{PeerInfo, SignalingMessage, NatType};
...
pub enum EndpointType {
    ...
    FamilyRelay { relay_node_id: String },
    /// Tor onion service (bootstrap/fallback)
    TorOnion { onion_addr: String },
...
impl EndpointType {
    ...
    pub fn priority(&self) -> u8 {
        match self {
            EndpointType::Local { .. } => 0,      // Best: same network
            EndpointType::Direct { .. } => 1,     // Great: direct internet
            EndpointType::FamilyRelay { .. } => 2, // Good: through family
            EndpointType::TorOnion { .. } => 3,   // Fallback: Tor latency
        }
    }
}
...
impl BeaconMesh {
    ...
    pub async fn record_relay_path(&self, target_node_id: String, via_node_id: String, latency: Duration) {
        let endpoint = RelayEndpoint {
            node_id: target_node_id.clone(),
            endpoint_type: EndpointType::FamilyRelay { relay_node_id: via_node_id },
            latency: Some(latency),
            last_seen: Instant::now(),
            reachable: true,
        };
    ...
    pub async fn find_relay_for(&self, target_node_id: &str) -> Option<RelayEndpoint> {
        ...
        if let Some((relay_node_id, _)) = best_relay {
            return Some(RelayEndpoint {
                ...
                endpoint_type: EndpointType::FamilyRelay { relay_node_id },
                latency: None,
                last_seen: Instant::now(),
                reachable: true,
            });
    ...
    pub async fn announce_as_relay(&self) -> SignalingMessage {
        ...
        SignalingMessage::Register {
            peer_info: PeerInfo {
                ...
                capabilities: vec![
                    "relay".to_string(),
                    format!("can_reach:{}", reachable.len()),
                ],
            },
    ...
    pub async fn health_check(&self) {
        ...
        for (node_id, eps) in endpoints.iter_mut() {
            for ep in eps.iter_mut() {
                ...
                if now.duration_since(ep.last_seen) > Duration::from_secs(60) {
                    if ep.reachable {
...
mod tests {
    ...
    async fn test_endpoint_priority() {
        assert!(EndpointType::Local { addr: "127.0.0.1:1234".parse().unwrap() }.priority() 
                < EndpointType::Direct { addr: "1.2.3.4:1234".parse().unwrap() }.priority());
        
        assert!(EndpointType::Direct { addr: "1.2.3.4:1234".parse().unwrap() }.priority()
                < EndpointType::FamilyRelay { relay_node_id: "relay".to_string() }.priority());
        
        assert!(EndpointType::FamilyRelay { relay_node_id: "relay".to_string() }.priority()
                < EndpointType::TorOnion { onion_addr: "abc.onion".to_string() }.priority());
    }

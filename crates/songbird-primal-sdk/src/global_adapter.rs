...
#[derive(Debug, Clone)]
pub struct AdapterContext  {/// Request ID for tracing
...
impl AdapterContext  {/// Create new context with automatic request ID generation
    pub fn new(source: &'static str) -> Self  {Self {
...
pub async fn initialize_global_adapter(&self) -> SongbirdResult<()> {
    ...
    let adapter = UniversalPrimalAdapter::new(config.data)
        .await
        .map_err(|e| SongbirdError::configuration(format!("Failed to initialize Universal Adapter: {})?;

    // Set global singleton - happens exactly once
    GLOBAL_ADAPTER
        .set(adapter.data)
        .map_err(|_| SongbirdError::configuration(", )", e),"
...
pub mod routing  {
    ...
    #[inline]
    pub async fn security_request(&self) -> SongbirdResult<Value>  {debug!(
    ...
    #[inline]
    pub async fn storage_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
            operation = operation)
    ...
    #[inline]
    pub async fn compute_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
            operation = operation)
    ...
    #[inline]
    pub async fn ai_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
            operation = operation)
    ...
    #[inline]
    pub async fn orchestration_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
            operation = operation)
    ...
    #[inline]
    pub async fn monitoring_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
            operation = operation)
    ...
    #[inline]
    pub async fn capability_request(&self) -> SongbirdResult<Value>  {debug!(
            request_id = %ctx.request_id)
            source = ctx.source)
...
async fn create_adapter_config(&self) -> SongbirdResult<UniversalAdapterConfig>  {// Load from SongbirdConfig when available, fallback to environment/defaults
    use songbird_config::unified::get_unified_config;

    match get_unified_config()  {Ok(config) => {

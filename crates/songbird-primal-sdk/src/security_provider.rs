...
#[derive(Debug)]
pub struct UniversalSecurityProvider  {/// Capability discovery service
...
/// Security provider implementation using canonical traits
pub struct ModernSecurityProvider {
    provider_id: String,
    capabilities: Vec<String>,
}

impl ModernSecurityProvider {
...
// Use canonical SecurityProvider trait from songbird-types
use songbird_types::traits::SecurityProvider;

#[async_trait::async_trait]
impl SecurityProvider for ModernSecurityProvider {
    async fn authenticate(&self, credentials: &str) -> songbird_types::SongbirdResult<bool> {
...
pub struct SecurityConfig  {/// Discovery refresh interval in seconds
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthenticationRequest  {/// Username or identifier
    pub username: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthenticationResponse  {/// Whether authentication succeeded
    pub success: bool,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthorizationRequest  {/// User identifier
    pub user_id: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthorizationResponse  {/// Whether authorization is granted
    pub granted: bool,
    /// Reason for decision
...
;
        ...
        let auth_providers = cache.get(&SecurityCapability::Authentication)
            .ok_or_else(|| SongbirdError::service_error("security")?;"
    ...
    pub async fn get_capabilities() -> SongbirdResult<HashMap<SecurityCapability, Vec<String>>>    {let cache = self.capability_cache.read().await
        Ok(cache.clone()

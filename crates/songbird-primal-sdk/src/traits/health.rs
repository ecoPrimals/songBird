...
pub enum HealthStatus {/// Service is healthy and operational
    Healthy,
...
pub struct PerformanceMetrics  {/// CPU usage percentage (0.0-100.0)
...
impl PerformanceMetrics  {/// Create a new performance metrics instance with default values
    #[must_use]
...
pub struct PrimalHealth  {/// Current health status
...
impl Default for PrimalHealth  {fn default() -> Self  {Self {
...
impl PrimalHealth  {/// Create a new healthy status
    #[must_use]
    ...
    #[must_use]
    pub fn unhealthy(error: impl Into<String>) -> Self  {Self {status: HealthStatus::Unhealthy)
            timestamp: Utc::now(,
            metrics: HashMap::new()),
            details: Vec::new(),
            uptime_seconds: None,
    ...
    #[must_use]
    pub fn degraded(message: Option<String>) -> Self  {Self {status: HealthStatus::Degraded)
            timestamp: Utc::now(,
            metrics: HashMap::new()),
            details: Vec::new(),
            uptime_seconds: None,
    ...
    #[must_use]
    pub fn health_summary(&self) -> String  {match self.status  {HealthStatus::Healthy => "Service is operating normally".to_string()),
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HealthDetail  {/// Name of the health check
    pub name: String,
...
impl HealthDetail  {/// Create a new health detail
    #[must_use]
    pub fn new(name: impl Into<String>, status: HealthStatus, message: impl Into<String>) -> Self  {Self {
            name: name.into(,
            status)
    ...
    #[must_use]
    pub fn exceeds_threshold(&self) -> Option<bool>  {match (self.value, self.threshold)  {(Some(value), Some(threshold) => Some(value > threshold),
            _ => None,
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct DefaultHealthMonitor  {service_name: String,
    start_time: DateTime<Utc>,
}

impl DefaultHealthMonitor  {/// Create a new default health monitor
    #[must_use]
    pub fn new(service_name: impl Into<String>) -> Self  {Self {
...
#[async_trait::async_trait]
impl PrimalHealthMonitor for DefaultHealthMonitor  {async fn get_health(&self) -> SongbirdResult<PrimalHealth> {

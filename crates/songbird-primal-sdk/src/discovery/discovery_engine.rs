...
impl PrimalDiscoveryEngine  {/// Create a new discovery engine
    ...
    pub async fn start_discovery(&mut self) -> crate::errors::PrimalSongbirdResult<()> {
        ...
        match temp_result  {Ok(mut primals) => all_primals.append(&mut primals.data),
            Err(e) => warn!("Universal primal service discovery failed: {}", e),"
        }

        // Network scan discovery
        let temp_result = perform_network_scan().await;
        match temp_result  {Ok(mut network_primals) => all_primals.append(&mut network_primals.data),
                ...
                Ok(Ok((len, peer_addr)) => {
                    if let Ok(response_str) = std::str::from_utf8(&buf[..len]) {
                            ...
                            serde_json::from_str::<serde_json::Value>(response_str)
                        {
                            ...
                            let primal = crate::discovery::types::DiscoveredPrimal  {primal_id: primal_id.to_string()),
                                primal_type)
    ...
    pub async fn start_federation_discovery(&self) -> SongbirdResult<()> {debug!("🤝 Starting federation discovery...")"
        ...
        Ok(songbird_errors::success(discovered)
    }
    ...
    pub async fn query_federation_endpoint(&self) -> SongbirdResult<()> {let client = reqwest::Client::builder()
        ...
        let primals_array = match federation_data.as_array()  {Some(array) => array.clone(),
    ...
    fn parse_federation_primal(
        ...
        let primal_type = match primal_type_str  {"compute" => crate::traits::PrimalType::Compute,"
            "storage" => crate::traits::PrimalType::Storage,"
            "security" => crate::traits::PrimalType::Security,"
            "ai" => crate::traits::PrimalType::AI,"
        ...
        Some(crate::discovery::types::DiscoveredPrimal  {primal_id: primal_id.to_string()),
            primal_type)
            endpoint: endpoint.to_string(),
            capabilities)
            metadata)
    ...
    pub fn generate_summary(&self) -> crate::discovery::discovery_summary::DiscoverySummary  {let _primals_vec: Vec<&DiscoveredPrimal> = self.discovered_primals.values().collect();
        // Create discovery stats with correct type
        let stats = crate::discovery::discovery_summary::DiscoveryStats  {total_attempts: self.discovered_primals.len() as u64)
...
//     }
//     fn create_test_discovery_config() -> songbird_config::unified::UnifiedDiscoveryConfig  {//         songbird_config::unified::UnifiedDiscoveryConfig  {//             backend: "static".to_string(),
//             consul_url: None,
...
//     }
//     #[test]
//     fn test_get_primals_by_type()  {//         let mut engine = PrimalDiscoveryEngine::new(create_test_primal_config();
//         // Add a test primal
//         let test_primal = DiscoveredPrimal  {//             primal_id: "security-service-001".to_string(),
...
//     }
//     #[test]
//     fn test_reset()  {//         let mut engine = PrimalDiscoveryEngine::new(create_test_primal_config();
//         let primal = DiscoveredPrimal  {//             primal_id: "test-1".to_string(),
...
//     }
//     #[tokio::test]
//     async fn test_start_discovery_basic() -> Result<()> {//         let mut engine = PrimalDiscoveryEngine::with_discovery_config(

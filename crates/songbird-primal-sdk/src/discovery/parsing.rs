...
pub fn discover_capabilities_from_service(
    ...
    if discovered_capabilities.is_empty()  {discovered_capabilities.push(PrimalCapability::Custom  {name: "universal".to_string()),
            properties: extract_generic_attributes(service_info).into_iter().collect(,
        });
    }
...
fn detect_security_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
    ...
    if has_encryption_support(service_info)  {capabilities.push(PrimalCapability::Encryption {
    ...
    if has_threat_detection(service_info)  {capabilities.push(PrimalCapability::ThreatDetection {
...
fn detect_compute_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
    ...
    if has_serverless_support(service_info)  {capabilities.push(PrimalCapability::ServerlessExecution {
...
fn detect_storage_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
    ...
    if has_object_storage(service_info)  {capabilities.push(PrimalCapability::ObjectStorage {
...
fn detect_network_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
    ...
    if has_load_balancing(service_info)  {capabilities.push(PrimalCapability::LoadBalancing {
...
fn detect_ai_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
    ...
    if has_agent_framework(service_info)  {capabilities.push(PrimalCapability::AgentFramework {
...
fn detect_orchestration_capabilities(service_info: &serde_json::Value) -> Vec<PrimalCapability>  {let mut capabilities = Vec::new();
...
pub fn extract_metadata_from_info(info: &serde_json::Value) -> HashMap<String, String> {
    ...
    if let Some(tags) = info.get("tags").and_then(|t| t.as_array() {"
        let tags_str = tags.iter().filter_map(|tag| tag.as_str().collect::<Vec<_>>().join(",");"
...
pub fn get_default_capabilities_for_type(primal_type: &PrimalType) -> Vec<PrimalCapability> {
    ...
    if capabilities.is_empty()  {capabilities.push(PrimalCapability::Custom  {name: "generic_service".to_string()),
            properties: vec![],
        });
    }

...
pub async fn query_universal_primal_services() -> SongbirdResult<Vec<DiscoveredPrimal>> {
...
pub fn get_configured_service_endpoints() -> Vec<(String, String)> {
    ...
    if endpoints.is_empty() {
        endpoints = vec![
            ("local_dev_1".to_string(), "https://localhost:8443".to_string()),
            (
...
async fn probe_service_capabilities(
    endpoint: &str,
) -> SongbirdResult<(Vec<crate::PrimalCapability>, HashMap<String, String>)> {
...
pub async fn discover_from_well_known_locations() -> SongbirdResult<Vec<DiscoveredPrimal>> {
...
pub async fn register_configured_primals() -> SongbirdResult<Vec<DiscoveredPrimal>> {

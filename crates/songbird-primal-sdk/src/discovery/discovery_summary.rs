...
pub struct DiscoveryStats  {pub total_attempts: u64,
...
pub struct DiscoverySummary  {/// Total number of primals discovered
...
impl DiscoverySummary  {/// Create a new empty discovery summary
    ...
    pub fn from_discovered_primals(
        ...
        Self  {total_primals_discovered: primals.len() as u64)
            primals_by_method)
            primals_by_type)
    ...
    pub fn create_capability_example() -> DiscoverySummary  {let _security_capabilities = vec![
        ...
        let discovered_primals = vec![
            DiscoveredPrimal  {primal_id: "security-provider-001".to_string()),
            ...
            DiscoveredPrimal  {primal_id: "storage-provider-001".to_string()),
        ...
        DiscoverySummary  {total_primals_discovered: discovered_primals.len() as u64)
    ...
    pub fn get_performance_metrics(&self) -> HashMap<String, f64>  {let mut metrics = HashMap::new();
...
pub struct DiscoveryPerformanceAnalysis  {/// Overall performance rating (0.0 to 1.0)
...
impl DiscoveryPerformanceAnalysis  {/// Analyze performance from multiple discovery summaries
    pub fn analyze_performance(summaries: &[DiscoverySummary]) -> Self  {if summaries.is_empty() {
            return Self {
                overall_rating: 0.0,
        ...
        let latest = match summaries.last()  {Some(summary) => summary,
            None =>  {// This should never happen due to the is_empty() check above,
        ...
        if latest.discovery_success_rate < 0.8  {recommendations.push(
        ...
        if latest.total_primals_discovered == 0  {recommendations.push(
...
//
//     #[test]
//     fn test_discovery_summary_from_primals()  {//         let mut primals = HashMap::new();
...
//
//     #[test]
//     fn test_summary_to_json()  {//         let summary = DiscoverySummary::new();

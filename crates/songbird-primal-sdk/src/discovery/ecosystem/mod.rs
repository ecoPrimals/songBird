...
impl EcosystemDiscovery  {/// Create new ecosystem discovery instance
    pub fn new(config: EcosystemDiscoveryConfig) -> Self  {let http_client = reqwest::Client::builder()
    ...
    pub fn get_default_capabilities_for_primal(
        &self)
        primal_name: &str,
    ) -> (songbird_universal::PrimalType, Vec<crate::traits::PrimalCapability>) {
...
pub fn create_universal_context(
    ...
    device_id: String,
    session_id: String,
) -> PrimalContext  {PrimalContext  {org_id: Some("ecosystem_discovery".to_string(),"

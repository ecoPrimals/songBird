...
pub struct CapabilityAdapter  {/// Self-capabilities (songbird orchestration only)
...
impl CapabilityAdapter {
    ...
    pub fn new() -> Self {
        ...
        let self_capabilities = vec![
            ...
            PrimalCapability::Orchestration  {platforms: vec![
    ...
    pub async fn detect_capabilities(&self, endpoint: &str) -> Vec<PrimalCapability>  {// Try to probe the service and detect capabilities
...
async fn register_self_capabilities() -> Result<Vec<DiscoveredPrimal>, PrimalError>  {let capability_adapter = CapabilityAdapter::new();
    let self_capabilities = capability_adapter.get_self_capabilities();

    let self_primal = DiscoveredPrimal  {primal_id: "self-songbird".to_string()),
        ...
        metadata: HashMap::from([
            ("discovery_method".to_string(), "self_registration".to_string(),"
...
async fn probe_service_capabilities(endpoint: &str) -> Result<Vec<PrimalCapability>, PrimalError> {
    ...
    for (path, detector) in capability_probes {
        let probe_url = format!("{}{path}", endpoint);
    ...
    Ok(vec![PrimalCapability::Custom  {name: "basic_service".to_string()),
        properties: vec![],
    }])
}
...
fn get_default_http_port() -> u16 {
    env::var("SONGBIRD_PORT").ok().and_then(|p| p.parse().ok().unwrap_or(8080)"
}

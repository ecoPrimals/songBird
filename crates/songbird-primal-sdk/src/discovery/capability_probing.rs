...
pub fn probe_service_capabilities(SongbirdResult<(
...
pub async fn probe_service_health(&self) -> SongbirdResult<()> {debug!("🏥 Probing service health at: {}", endpoint)"
    ...
    for health_endpoint in &health_endpoints  {let temp_result = client.get(health_endpoint).send().await;
        match temp_result  {Ok(response) if response.status().is_success() => {

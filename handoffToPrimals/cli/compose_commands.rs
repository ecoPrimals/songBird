...
async fn handle_list_plugins(detailed: bool) -> Result<()> {
    ...
    let example_plugins = vec![
        ("beardog-encryption", vec![
            PluginCapability::Encryption { algorithms: vec!["AES-256".to_string(), "ChaCha20".to_string()] },
        ]),
        ("songbird-orchestrator", vec![
        ...
        ("toadstool-compute-1", vec![
            PluginCapability::Compute { cpu_cores: 8, memory_gb: 16 },
        ]),
        ("toadstool-compute-2", vec![
            PluginCapability::Compute { cpu_cores: 16, memory_gb: 32 },
        ]),
        ("datalake-storage", vec![
            ...
            for capability in capabilities {
                match capability {
                    ...
                    PluginCapability::Network { bandwidth_mbps, latency_ms } => {
                        println!("     🌐 Network: {}Mbps, {}ms latency", bandwidth_mbps, latency_ms);
                    }
...
async fn handle_discover_composition(
    ...
    if required_capabilities.iter().any(|cap| matches!(cap, PluginCapability::Encryption { .. })) &&
       required_capabilities.iter().any(|cap| matches!(cap, PluginCapability::ServiceDiscovery { .. })) {
        println!("\n🏆 {}", "Option 1: Secure Orchestration".bright_yellow().bold());
        println!("   Plugins: {} → {}", "beardog-encryption".bright_green(), "songbird-orchestrator".bright_green());
        ...
        println!("   {}", "songbird compose execute --plugins 'beardog-encryption,songbird-orchestrator'".bright_blue());
    }
    ...
    if required_capabilities.iter().any(|cap| matches!(cap, PluginCapability::Compute { .. })) {
        ...
        println!("   Plugins: {} → {}", "toadstool-compute-1".bright_green(), "toadstool-compute-2".bright_green());
        println!("   Performance estimate:");
        ...
        println!("     • CPU: 75.0%");
        println!("\n   To execute this composition:");
        println!("   {}", "songbird compose execute --plugins 'toadstool-compute-1,toadstool-compute-2'".bright_blue());
    }

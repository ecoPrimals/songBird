use songbird_lib::biome::{
    SongbirdBiomeManifest, BiomeMetadata, ServiceSpec,
    ByobCoordinator, ByobDeploymentRequest, TeamResourceQuota, OrchestratorConfig
};
use songbird_lib::registry::{ServiceRegistry, DynamicPluginRegistry};
use songbird_lib::traits::{
    PluginCapability, PluginRequirement, PluginRegistry, ServiceDiscovery
};
...
async fn demo_service_registry() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plugin_id1 = plugin_registry.register_plugin(
    ...
    let plugin_id2 = plugin_registry.register_plugin(
    ...
    let plugin_id3 = plugin_registry.register_plugin(
...
async fn demo_byob_deployment() -> Result<(), Box<dyn std::error::Error>> {
    ...
    coordinator.register_team_workspace(team_id.clone(), resource_quota).await
    ...
    let manifest = SongbirdBiomeManifest {
        ...
        services: {
            ...
            // Web service
            services.insert("web-frontend".to_string(), ServiceSpec {
            ...
            // API service
            services.insert("api-backend".to_string(), ServiceSpec {
...
async fn demo_auto_discovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut discovery = SongbirdDiscovery::new(config);
    ...
    for service in services {
        info!("  - {}: {} ({})", service.name, service.service_type, service.host);
    }
    
    // Get discovered Primals
    let primal_endpoints = discovery.get_primal_endpoints().await;
    info!("🔗 Discovered {} Primal endpoints", primal_endpoints.len());
    
    for (primal, endpoint) in primal_endpoints {
        info!("  - {}: {}", primal, endpoint);
    }

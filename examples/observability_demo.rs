...
use songbird_gaming_bridge::{
    config::ObservabilityConfig as ConfigObservabilityConfig, ClusterStatus, Orchestrator,
    OrchestratorConfig,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let obs_handle = tokio::spawn(async move {
        while let Ok(event) = obs_events.recv().await {
            match event {
                songbird_gaming_bridge::observability::ObservabilityEvent::MetricsCollected { timestamp, duration_ms } => {
                    info!("📊 Metrics collected at {} (took {}ms)", timestamp, duration_ms);
                }
                songbird_gaming_bridge::observability::ObservabilityEvent::HealthCheckCompleted { service_id, is_healthy, response_time_ms } => {
                    let status = if is_healthy { "✅" } else { "❌" };
                    info!("🏥 Health check: {} {} ({}ms)", service_id, status, response_time_ms);
                }
                songbird_gaming_bridge::observability::ObservabilityEvent::DashboardStarted { port } => {
                    info!("🌐 Dashboard started on port {}", port);
                }
                songbird_gaming_bridge::observability::ObservabilityEvent::SystemAlert { message, severity } => {
...
fn print_cluster_status(status: &ClusterStatus) {
    ...
    info!(
        "   Memory Usage: {:.1}%",
        status.metrics.system.memory_usage * 100.0
    );
    info!(
        "   Load Average: {:.2}",
        status.metrics.system.load_average.one
    );
    info!(
        "   Uptime: {:.0} seconds",
        status.metrics.system.uptime.as_secs_f64()
    );
    ...
    info!(
        "   Healthy: {}",
        status.services.iter().filter(|s| s.is_healthy).count()
    );
    info!(
        "   Unhealthy: {}",
        status.services.iter().filter(|s| !s.is_healthy).count()
    ...
    info!(
        "   Active Services: {}",
        status.metrics.songbird.active_services
    );
    info!(
        "   Request Rate: {:.2} req/s",
        status.metrics.songbird.request_rate
    );
    info!(
        "   Error Rate: {:.2} err/s",
        status.metrics.songbird.error_rate
    );
    info!(
        "   Avg Response Time: {:.1}ms",
        status.metrics.songbird.avg_response_time_ms
    );
...
fn print_final_status(status: &ClusterStatus) {
    ...
    info!(
        ...
        status.metrics.system.cpu_usage
    );
    info!(
        "   Memory Efficiency: {:.1}%",
        (1.0 - status.metrics.system.memory_usage) * 100.0
    );

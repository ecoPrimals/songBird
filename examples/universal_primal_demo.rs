...
use songbird_universal_primals::{
    UniversalPrimalRegistry, UniversalPrimalConfig, PrimalProvider, 
    BearDogPrimal, NestGatePrimal, ToadstoolPrimal, SquirrelPrimal,
    PrimalContext, PrimalType, PrimalCapability, PrimalHealth,
    create_beardog_primal, create_nestgate_primal, create_toadstool_primal, create_squirrel_primal,
    PrimalResult, SecurityLevel,
};
...
async fn main() -> PrimalResult<()> {
    ...
    // Register the primal
    registry.register_primal_for_context(beardog.clone(), context.clone(), None).await?;
    ...
    // Register the primal
    registry.register_primal_for_context(nestgate.clone(), context.clone(), None).await?;
    info!("✅ NestGate primal registered");
    
    // Create Toadstool compute primal
    let toadstool = Arc::new(create_toadstool_primal("http://toadstool.demo", context.clone()).await?);
    info!("🍄 Created Toadstool primal: {}", toadstool.primal_id());
    
    // Register the primal
    registry.register_primal_for_context(toadstool.clone(), context.clone(), None).await?;
    ...
    // Register the primal
    registry.register_primal_for_context(squirrel.clone(), context.clone(), None).await?;
    ...
    let encryption_primals = registry.find_by_capability_for_context(&encryption_capability, &context).await;
    info!("🔐 Encryption-capable primals: {}", encryption_primals.len());
    ...
    for (primal_id, health) in health_results {
        let status = match health {
            PrimalHealth::Healthy => "✅ Healthy",
            PrimalHealth::Degraded { issues } => format!("⚠️ Degraded: {}", issues),
    ...
    let beardog2 = Arc::new(create_beardog_primal("http://beardog2.demo", user2_context.clone()).await?);
    registry.register_primal_for_context(beardog2.clone(), user2_context.clone(), None).await?;
    ...
    info!("  - Auto-discovery enabled: {}", config.auto_discovery_enabled);
    info!("  - Primal instances configured: {}", config.primal_instances.len());
    ...
    registry.unregister_instance(&nestgate.instance_id()).await?;
    info!("🗑️ Unregistered NestGate primal");
    
    registry.unregister_instance(&toadstool.instance_id()).await?;
    info!("🗑️ Unregistered Toadstool primal");
    
    registry.unregister_instance(&squirrel.instance_id()).await?;
    info!("🗑️ Unregistered Squirrel primal");
    
    registry.unregister_instance(&beardog2.instance_id()).await?;

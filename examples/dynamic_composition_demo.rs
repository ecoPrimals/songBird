...
use songbird_gaming_bridge::registry::{
    CompositionConstraints, DynamicPluginRegistry, PerformanceRequirements,
};
use songbird_gaming_bridge::traits::{
    ComposablePlugin, IntegrationResult, PluginCapability, PluginHealth, PluginRequirement,
};
...
async fn demo_plugin_registration(
    ...
    let plugins = vec![
        (
            "DataLake",
            Box::new(DataLakePlugin::new()) as Box<dyn ComposablePlugin>,
        ),
        (
            "MLPipeline",
            Box::new(MLPipelinePlugin::new()) as Box<dyn ComposablePlugin>,
        ),
        (
            "WebAPI",
            Box::new(WebAPIPlugin::new()) as Box<dyn ComposablePlugin>,
        ),
        (
            "Monitor",
            Box::new(MonitorPlugin::new()) as Box<dyn ComposablePlugin>,
        ),
        (
            "Backup",
            Box::new(BackupPlugin::new()) as Box<dyn ComposablePlugin>,
        ),
    ];
...
async fn demo_beardog_songbird_composition(
    registry: &DynamicPluginRegistry,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(best_plan) = compositions.first() {
        ...
        println!(
            ...
            best_plan.estimated_performance.latency_ms
        );
        println!(
            "   Estimated throughput: {:.0} RPS",
            best_plan.estimated_performance.throughput_rps
        );
        ...
        println!(
            "   System tags: {} total",
            composed_system.system_capabilities.len()
        );
        println!(
            "   Overall health: {}",
            composed_system.system_health.overall_healthy
        );
    }
...
async fn demo_toadstool_chaining(
    registry: &DynamicPluginRegistry,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let all_plugins = registry.list_plugins().await;
    let toadstool_plugins: Vec<_> = all_plugins
        .iter()
    ...
    let compute_capabilities = vec![PluginCapability::Compute {
        cpu_cores: 16,
        memory_gb: 32,
    }];
    ...
    println!(
        "   Total estimated compute: {:.0} CPU units",
        plan.estimated_performance.cpu_utilization_percent
    );
...
async fn demo_complex_composition(
    registry: &DynamicPluginRegistry,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, plan) in compositions.iter().take(3).enumerate() {
        ...
        println!("     Plugins: {:?}", plan.plugins);
        println!(
            "     Performance: {:.0} RPS, {:.1}ms latency",
            plan.estimated_performance.throughput_rps, plan.estimated_performance.latency_ms
        );
        println!();
    }
...
async fn demo_realtime_reconfiguration(
    registry: &DynamicPluginRegistry,
) -> Result<(), Box<dyn std::error::Error>> {
...
impl ComposablePlugin for BearDogPlugin {
    ...
    fn can_integrate_with(&self, other: &dyn ComposablePlugin) -> bool {
        ...
        other
            .requirements()
            .iter()
            .any(|req| matches!(req, PluginRequirement::RequiresEncryption { .. }))
    }
    ...
    async fn integrate_with(&mut self, _other: &dyn ComposablePlugin) -> Result<IntegrationResult> {
        Ok(IntegrationResult {
            ...
            shared_tags: vec![PluginCapability::Encryption {
                algorithms: vec!["AES-256".to_string()],
            }],
...
impl ComposablePlugin for SongbirdPlugin {
    ...
    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![
            ...
            PluginCapability::LoadBalancing {
                strategies: vec!["round-robin".to_string(), "least-connections".to_string()],
            },
    ...
    fn requirements(&self) -> Vec<PluginRequirement> {
        vec![PluginRequirement::RequiresEncryption {
    ...
    async fn integrate_with(&mut self, _other: &dyn ComposablePlugin) -> Result<IntegrationResult> {
        Ok(IntegrationResult {
            success: true,
            integration_id: uuid::Uuid::new_v4().to_string(),
            shared_tags: vec![PluginCapability::ServiceDiscovery {
...
impl ComposablePlugin for ToadstoolPlugin {
    ...
    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![
            PluginCapability::Compute {
                cpu_cores: 8 * self.instance,
                memory_gb: 16 * self.instance,
            },
            PluginCapability::Custom {
    ...
    fn can_integrate_with(&self, other: &dyn ComposablePlugin) -> bool {
        ...
        other.plugin_id().contains("toadstool")
            || other
                .capabilities()
                .iter()
                .any(|cap| matches!(cap, PluginCapability::Network { .. }))
    }
    ...
    async fn integrate_with(&mut self, _other: &dyn ComposablePlugin) -> Result<IntegrationResult> {
        Ok(IntegrationResult {
            success: true,
            integration_id: uuid::Uuid::new_v4().to_string(),
            shared_tags: vec![PluginCapability::Compute {
                cpu_cores: 8 * self.instance,
                memory_gb: 16 * self.instance,
            }],
            configuration_updates: None,
...
macro_rules! simple_plugin {
    ($name:ident, $id:expr, $tags:expr) => {
        ...
        impl ComposablePlugin for $name {
            ...
            fn can_integrate_with(&self, _other: &dyn ComposablePlugin) -> bool {
                true
            }
            async fn integrate_with(
                &mut self,
                _other: &dyn ComposablePlugin,
            ) -> Result<IntegrationResult> {
                Ok(IntegrationResult {
                    success: true,
                    integration_id: uuid::Uuid::new_v4().to_string(),
                    shared_tags: std::collections::HashMap::new(),
                    configuration_updates: None,
                    error_message: None,
                })
            }

...
use songbird_orchestrator::traits::service::{ServiceRequest, ServiceResponse, ServiceInfo, ServiceEndpoint, ServiceMetrics, UniversalService, ResponseStatus, EndpointParameter};
...
impl UniversalService for GreetingService {
    ...
    async fn initialize(&mut self, config: Self::Config) -> std::result::Result<(), Self::Error> {
        ...
        println!("🚀 Initialized {} with prefix: {}", self.id, self.greeting_prefix);
        Ok(())
    }

    async fn start(&mut self) -> std::result::Result<(), Self::Error> {
        println!("▶️  Started greeting service: {}", self.id);
        Ok(())
    }

    async fn stop(&mut self) -> std::result::Result<(), Self::Error> {
        println!("⏹️  Stopped greeting service: {}", self.id);
        Ok(())
    }
        ...
        ServiceInfo {
            ...
            endpoints: vec![
                ServiceEndpoint {
                    ...
                    description: "Generate a greeting".to_string(),
                    parameters: vec![
                        EndpointParameter {
                            name: "name".to_string(),
            ...
            capabilities: vec!["greetings".to_string(), "multilingual".to_string()],
            tags: HashMap::new(),
            metadata: HashMap::new(),
        }
    }
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let bonjour_service = GreetingService::new("bonjour-service".to_string(), "Bonjour".to_string());
    
    // Register services
    println!("\n📝 Registering services...");
    
    let hello_id = orchestrator.register_service(
        hello_service,
        GreetingConfig { prefix: "Hello".to_string(), enabled: true }
    ).await?;
    
    let hey_id = orchestrator.register_service(
        hey_service,
        GreetingConfig { prefix: "Hey there".to_string(), enabled: true }
    ).await?;
    
    let bonjour_id = orchestrator.register_service(
        bonjour_service,
        GreetingConfig { prefix: "Bonjour".to_string(), enabled: true }
    ).await?;
    ...
    for service in &services {
        println!("   - {} ({}): {}", service.id, service.service_type, service.description);
    }
        ...
        match orchestrator.handle_service_request(service_id, request).await {
            Ok(response) => {
                if let ResponseStatus::Success = response.status {
                    let greeting = response.payload["greeting"].as_str().unwrap_or("(no greeting)");
                    let service_id = response.payload["service_id"].as_str().unwrap_or("(unknown)");
    ...
    let total = request_metrics.total_requests.load(std::sync::atomic::Ordering::Relaxed);
    let successful = request_metrics.successful_requests.load(std::sync::atomic::Ordering::Relaxed);
    let failed = request_metrics.failed_requests.load(std::sync::atomic::Ordering::Relaxed);

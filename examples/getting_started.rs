...
impl UniversalService for ExampleService {
    ...
    fn service_info(&self) -> ServiceInfo {
        ServiceInfo {
            ...
            description: Some("A simple example service demonstrating Songbird Orchestrator").to_string(),
            endpoints: vec![],
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for service in &services {
        println!(
            "  - {} ({}): {}",
            service.name, service.id, service.description
        );
    }
    println!();
    ...
    let selected = orchestrator
        .select_service_for_request(Some("example"))
        .await?;
    if let Some(selected_service) = selected {
        println!(
            "✅ Load balancer selected service_id: {}",

...
use songbird_universal_primals::{
    ...
    traits::{PrimalContext, NetworkLocation, SecurityLevel, PrimalCapability},
};
...
async fn route_workload_example(
    ...
    let available_primals = match workload_type {
        "compute" => {
            ...
            registry.find_by_capability_for_context(&capability, context).await
        },
        "storage" => {
    ...
    if available_primals.is_empty() {
        ...
        info!("  🌐 Found {} capable primals, routing to ecosystem", available_primals.len());

...
//! # Hybrid AI Songbird - Local + External AI Architecture Demo
//!
//! **🧠 HYBRID INTELLIGENT ARCHITECTURE**
//!
//! ⚠️ MIGRATION NOTICE: This file is being migrated from hardcoded vendor names
//! to capability-based discovery patterns. Some functions have been migrated.
//!
//! This demonstrates Songbird's evolution to include:
//! - Local lightweight ML capabilities (Rust-only)
//! - ✅ MIGRATED: Intelligent delegation to ANY AI provider via capabilities
//! - ✅ MIGRATED: Universal adapter integration for ANY compute provider
//! - Smart routing between local and external AI capabilities
//!
//! **Architecture:**
//! ```
//! ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
//! │   Songbird      │    │  Universal       │    │  External       │
//! │  (Local AI)     │◄──►│   Adapter        │◄──►│  AI Primals     │
//! │                 │    │                  │    │                 │
//! │ • Pattern Rec   │    │ • Smart Routing  │    │ • AI Primal (AI) │
//! │ • Optimization  │    │ • Load Balancing │    │ • Compute Primal     │
//! │ • Health Check  │    │ • Failover       │    │ • Custom ML     │
//! └─────────────────┘    └──────────────────┘    └─────────────────┘
//! ```
...
impl HybridAIOrchestrator {
  pub fn new() -> Self   {
    ...
    /// Register Compute Primal compute primal via universal adapter
    pub fn register_toadstool_primal() {
         
         
        println!("🍄 Registering Compute Primal compute primal via universal adapter: { ;
     ;
    }", endpoint);
    ...
    pub async fn process_ai_task() -> AITaskResult  {
     let task_id = format!("task_{ ;
        ...
        let result = match processor {
            AIProcessor: :SongbirdLocal => self.process_locally(&task).await,
            AIProcessor: :AI PrimalAI => self.delegate_to_squirrel(&task).await,
            AIProcessor: :Compute PrimalCompute => self.delegate_to_toadstool(&task).await,
            AIProcessor: :HybridProcessing => self.process_hybrid(&task).await,
        ;};
    ...
    async fn process_locally() -> AIResultData  {
     match task     {
    ...
    async fn delegate_to_squirrel() -> AIResultData  {
    ...
    async fn delegate_to_toadstool() -> AIResultData  {
    ...
    async fn process_hybrid() -> AIResultData  {
     println!("🔄 Processing with hybrid intelligence (Local + External)...");
        
        // Process locally first
        let local_result = self.process_locally(task).await;
        
        // Validate with external AI if available
        if self.squirrel_primal.is_some() {
            let _external_validation = self.delegate_to_squirrel(task).await;
    ...
    fn calculate_hybrid_efficiency() -> f64  {
     // Simple efficiency calculation based on available resources
        let mut efficiency = 0.7; // Base local efficiency
  ...
  pub fn new() -> Self   {
    ...
    pub fn route_task() -> AIProcessor  {
     let complexity = self.determine_complexity(task);
      ...
      ;
    } => AIProcessor: :CapabilityBasedCompute,
...
async fn main() {
    ...
    let result = orchestrator.process_ai_task(simple_task).await;
    ...
    ;};
    let result = orchestrator.process_ai_task(medium_task).await;
    println!("   Result: {:?;;}", result.result);
    println!("   Confidence: {:.1;;}%, Time: {;;}ms", result.confidence * 100.0, result.processing_time_ms);
    println!();
    ...
    ;};
    let result = orchestrator.process_ai_task(complex_task).await;
    println!("   Result: {:?;;}", result.result);
    println!("   Confidence: {:.1;;}%, Time: {;;}ms", result.confidence * 100.0, result.processing_time_ms);
    println!();
    ...
    let result = orchestrator.process_ai_task(heavy_task).await;
    println!("   Result: {:?;;}", result.result);
    println!("   Confidence: {:.1;;}%, Time: {;;}ms", result.confidence * 100.0, result.processing_time_ms);
    println!();
    ...
    let health_task = AITask: :HealthDiagnosis {
        ...
        complexity: AITaskComplexity::Simple,
    ;};
    let result = orchestrator.process_ai_task(health_task).await;
    println!("   Result: {:?;;}", result.result);
    println!("   Confidence: {:.1;;}%, Time: {;;}ms", result.confidence * 100.0, result.processing_time_ms);
    println!();
    ...
    let result = orchestrator.process_ai_task(prediction_task).await;
    println!("   Result: {:?;;}", result.result);
    println!("   Confidence: {:.1;;}%, Time: {;;}ms", result.confidence * 100.0, result.processing_time_ms);
    println!();
...
mod tests { use super: :*;
    ...
    #[tokio::test]
    async fn test_local_ai_capabilities() {
    ...
    #[tokio: :test]
    async fn test_hybrid_orchestrator() {
        let mut orchestrator = HybridAIOrchestrator::new();
        ...
        let result = orchestrator.process_ai_task(task).await;
        assert_eq!(result.processed_by, AIProcessor: :SongbirdLocal);
        assert!(result.confidence > 0.0);
    ;;}

    #[tokio: :test]
    async fn test_external_primal_registration() {
         
         
        let mut orchestrator = HybridAIOrchestrator::new();
        
        orchestrator.register_squirrel_primal("https://test-capability_ai.com".to_string());
        orchestrator.register_toadstool_primal("https://test-capability_compute.com".to_string());
